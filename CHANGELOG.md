...
### Corrigé
- **UI — Cartes & tâches (écrans de moins de 1024 px)** : la grille passait en une colonne avec la zone tâches en `overflow: visible`, ce qui faisait défiler tout le document et masquait la carte. Grille **`auto` + `minmax(0,1fr)`**, défilement confiné à **`.desktop-split-scroll`**. **Suite** : un **`max-height`** seul sur la colonne carte + **`min-height: 0`** laissait la ligne du grid se replier sur la toolbar seule → **`map-view-canvas-outer`** à hauteur nulle et plan invisible ; hauteur **min = max** avec plancher **220px**. **`src/index.css`**, **`dist/`**.
