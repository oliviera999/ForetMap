...
### Corrigé
- **UI — Cartes & tâches (écrans de moins de 1024 px)** : grille **`auto` + `minmax(0,1fr)`**, défilement confiné à **`.desktop-split-scroll`**. Colonne carte : **`max-height`** (plafond) sans hauteur figée sur tout le volet ; **`min-height`** sur **`.map-view-canvas-outer`** (`clamp`) pour la mesure embedded ; toolbar en **`width: 100%`** pour éviter la troncature liée à **`--fm-map-canvas-w`**. **`src/index.css`**, **`dist/`**.
