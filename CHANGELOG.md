...
### Modifié
- **Carte & visite — zones** : plus d’**emoji d’en-tête** sur les zones (carte prof, mode visite) ; seul le **nom** (sans préfixe emoji) est affiché au centroïde. Création / édition / duplication : plus de champ ni de préfixe emoji dans le nom ; l’éditeur visite n’insère plus d’emoji dans le titre de zone. En-tête de fiche zone (hors zones spéciales) sans pictogramme catalogue. **`map-views.jsx`**, **`visit-views.jsx`**, **`docs/API.md`**.
