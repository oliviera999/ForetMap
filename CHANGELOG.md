...
### Corrigé
- **n3boss — barre d’onglets du haut (parchemin)** : hauteur **uniforme** quel que soit l’onglet ou le format d’écran, via variables `clamp()` (`--top-tabs-row-inner-h`, etc.) ; suppression des surcharges mobile/desktop contradictoires sur `.top-tab`.
