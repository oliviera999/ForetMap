...
- **Dev local & CI** : remplacement de MySQL 8 par **MariaDB 11.4.10** (`docker-compose.yml`, volume `foretmap_mariadb_data`, service `mariadb`, healthcheck `healthcheck.sh`) ; GitHub Actions (`.github/workflows/ci.yml`) aligné sur la même image ; documentation `docs/LOCAL_DEV.md`, `README.md`, `env.local.example`. Passage depuis l’ancien conteneur MySQL : `docker compose down -v` puis `up -d` (données binaires non réutilisables).
...
- **Cartes & tâches (carte interactive)** : le plan et son cadre épousent le **ratio de l’image** et remplissent au maximum le rectangle disponible **sans déborder** (requêtes de conteneur `cqw`/`cqh` + repli) ; centrage dans la zone sous la barre d’outils — vue seule et vue scindée.
