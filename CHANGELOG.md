...
- **Notifications (n3boss)** : une tâche **proposée** ne déclenchait plus une seule alerte — le suivi des propositions déjà notifiées était **réinitialisé** dès que la liste des tâches était vide (chargement / rafraîchissement), ce qui recréait une notification à chaque cycle. Le jeu d'identifiants notifiés est désormais **cumulatif** sur la session (une proposition = une notification).
