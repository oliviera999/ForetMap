...
async function getResourceCooldownState(
  ...
  return resourceCooldownStateFromRow(row, days);
}
 ...
 */
function resourceCooldownStateFromRow(row, retryDays) {
  const days = clampCooldownDays(retryDays, 0);
  if (days <= 0) {
    return {
      locked: false,
...
/**
 * Lignes de verrou de PLUSIEURS ressources en une requete (portee ressource, `question_code = ''`).
 *
 * Pendant groupe de `getResourceCooldownState`, pour `buildGatingSummary` qui en emettait une
 * par ressource (docs/AUDIT_CHARGE_BIODIVERSITE_2026-09.md, B4).
 *
...
async function loadResourceCooldownRows(
  ...
  if (isGlProduct(product)) {
    ...
    rows = await db.queryAll(
      `SELECT resource_ref, locked_until, wrong_attempts FROM gl_resource_gating_cooldowns
        WHERE reader_user_type = ? AND reader_user_id = ?
          AND resource_type = ? AND resource_ref IN (${placeholders}) AND question_code = ''
        ORDER BY locked_until DESC`,
    ...
    rows = await db.queryAll(
      `SELECT resource_ref, locked_until, wrong_attempts FROM resource_gating_cooldowns
  ...
  const byRef = new Map();
  // `ORDER BY locked_until DESC` + premier gagnant = le `LIMIT 1` du chemin unitaire.
    ...
    await db.execute(
      `INSERT INTO resource_gating_cooldowns
       ...
       ON DUPLICATE KEY UPDATE
         ...
         wrong_attempts = VALUES(wrong_attempts),
         updated_at = NOW()`,
      [String(userId), resourceType, resourceRef, keyCode, days, questionCode, wrongAttempts, days],
    );
  }
