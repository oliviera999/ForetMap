...
// gl_resource_gating_cooldowns (GL, cle lecteur). Table et colonnes de la cle sont decrites
// par le catalogue des produits (`lib/pedago/gatingProductCatalog.js`) : le SQL est ecrit une fois.
// Aucune dependance vers learningGatingAcknowledge (evite un require circulaire) : la
// verification du lien bloquant approuve est refaite ici par une petite requete dediee.
// =====================================================================
...
/**
 * Table des verrous du produit et clé du lecteur (`user_id`, ou couple GL). Seul « gl »
 * (casse ignorée) désigne GL, toute autre valeur ForetMap — lecture historique, inchangée.
 */
function cooldownStore(product) {
  const adapter = gatingProductOrDefault(product);
  return { table: adapter.tables.cooldowns, learner: adapter.learner };
}
...
async function getResourceCooldownState(
  ...
  const { table, learner } = cooldownStore(product);
  if (!learner.isKnown({ userId, reader })) return emptyLocked;
  const row = await db.queryOne(
    `SELECT locked_until, wrong_attempts FROM ${table}
      WHERE ${learner.where}
        AND resource_type = ? AND resource_ref = ? AND question_code IN (${placeholders})
      ORDER BY locked_until DESC
      LIMIT 1`,
    [...learner.values({ userId, reader }), resourceType, resourceRef, ...codes],
  );
...
async function loadCooldownRowsForRef(
  ...
  const { table, learner } = cooldownStore(product);
  if (!learner.isKnown({ userId, reader })) return groupCooldownRows([]);
  const rows = await db.queryAll(
    `SELECT question_code, locked_until, wrong_attempts FROM ${table}
      WHERE ${learner.where}
        AND resource_type = ? AND resource_ref = ?
      ORDER BY locked_until DESC`,
    [...learner.values({ userId, reader }), resourceType, resourceRef],
  );
...
async function loadResourceCooldownRows(
  ...
  const { table, learner } = cooldownStore(product);
  if (!learner.isKnown({ userId, reader })) return new Map();
  const rows = await db.queryAll(
    `SELECT resource_ref, question_code, locked_until, wrong_attempts
      ...
      ORDER BY locked_until DESC`,
    [...learner.values({ userId, reader }), resourceType, ...unique],
  );
...
async function isApprovedGatingLink(db, product, resourceType, resourceRef, questionCode) {
  ...
  const { tables } = gatingProductOrDefault(product);
  const row = await db.queryOne(
    `SELECT 1 AS ok FROM ${tables.links}
...
async function readWrongAttempts(
  ...
  try {
    // Pas de garde sur le lecteur ici (historique) : une clé incomplète ne trouve aucune ligne,
    // ou fait échouer la requête, et le compteur vaut alors 0.
    const { table, learner } = cooldownStore(product);
    const row = await db.queryOne(
      `SELECT wrong_attempts, locked_until FROM ${table}
        WHERE ${learner.where}
          AND resource_type = ? AND resource_ref = ? AND question_code = ? LIMIT 1`,
      [...learner.values({ userId, reader }), resourceType, resourceRef, keyCode],
    );
    if (!row) return 0;
...
async function touchWrongAttempts(
  ...
  const { table, learner } = cooldownStore(product);
  if (!learner.isKnown({ userId, reader })) return;
  await db.execute(
    `INSERT INTO ${table}
      (${learner.insertColumns}, resource_type, resource_ref, question_code,
       locked_until, wrong_question_code, wrong_attempts)
     VALUES (${learner.insertPlaceholders}, ?, ?, ?, ${COUNTING_LOCK_UNTIL_SQL}, ?, ?)
...
async function registerResourceCooldown(
  ...
  const { table, learner } = cooldownStore(product);
  if (!learner.isKnown({ userId, reader })) return null;
  await db.execute(
    `INSERT INTO ${table}
      (${learner.insertColumns}, resource_type, resource_ref, question_code,
       locked_until, wrong_question_code, wrong_attempts)
     VALUES (${learner.insertPlaceholders}, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? HOUR), ?, ?)
     ON DUPLICATE KEY UPDATE
