 ...
 *     compléments confidentiels (`restricted_note`).
 *
...
const ZONES_SQL = `SELECT z.id, z.map_id, z.name, z.emoji, z.points, z.color, z.description,
  ...
  z.visible_role_slugs, z.visible_group_ids, z.restricted_note,
  z.restricted_note_role_slugs, z.restricted_note_group_ids,
...
const MARKERS_SQL = `SELECT m.id, m.map_id, m.x_pct, m.y_pct, m.label, m.emoji, m.note,
  ...
  m.visible_role_slugs, m.visible_group_ids, m.restricted_note,
  m.restricted_note_role_slugs, m.restricted_note_group_ids,
...
async function buildPlanContent(map, settings, { surface = 'plan', auth = null } = {}) {
  ...
  const [zoneLinks, markerLinks] = await Promise.all([
