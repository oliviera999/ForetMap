...
const { parseCategoryIdsSetting, placeSurvivesHiddenCategories } = require('./categoryIdsSetting');
...
/** Liste d'identifiants séparés par `;` / `,` / espaces (réglages `*_category_ids`). */
function idListFromSetting(value) {
  return parseCategoryIdsSetting(value);
}
...
async function findPlanPlace({ mapId, kind, id, surface = 'plan', auth = null }) {
  ...
  // Même sémantique B que `buildPlanContent` : un lieu dont toutes les catégories sont
  // masquées par réglage n'existe pas pour ce lecteur.
  const settingsPrefix = surface === 'staff' ? 'ui.staff_plan.' : 'ui.plan.';
  const accessModes = surface === 'staff' ? ['disabled', 'code'] : ['public', 'code'];
  const settings = await loadPlanSettings({ prefix: settingsPrefix, accessModes });
  const hiddenCategoryIds = new Set(settings.hidden_category_ids.map(String));
  if (!placeSurvivesHiddenCategories(withCategories.category_ids, hiddenCategoryIds)) {
    return null;
  }
  return {
    kind: normalizedKind,
...
async function buildPlanContent(map, settings, { surface = 'plan', auth = null } = {}) {
  ...
  /** Ids des catégories de cette carte/surface avant masquage (pour purger les ids stale). */
  const servedCategoryIds = new Set(categoryRows.map((row) => String(row.id)));
  ...
  const zones = zoneRows
    ...
    // Sémantique B : lieux dont toutes les catégories sont masquées → absents du plan.
    .filter((row) => placeSurvivesHiddenCategories(row.category_ids, hiddenCategoryIds))
    .map((row) => {
  ...
  return {
    ...
    settings: {
      ...
      // Ids encore servis sur cette carte/surface (avant retrait du catalogue) — pas les
      // ids stale d'une catégorie supprimée.
      hidden_category_ids: settings.hidden_category_ids.filter((id) => servedCategoryIds.has(id)),
    },
    categories,
    zones,
    markers,
