...
async function assignPlayerToTeamTx(tx, { gameId, teamId, playerId }) {
  ...
  const player = await tx.queryOne(
    'SELECT id, class_id FROM gl_players WHERE id = ? LIMIT 1',
    [playerId]
  );
  ...
  await tx.execute(
    'UPDATE gl_players SET team_id = ?, updated_at = NOW() WHERE id = ?',
    [teamId, playerId]
  );
}
...
async function unassignPlayerFromGameTx(tx, { gameId, playerId }) {
  ...
  await tx.execute(
    'DELETE FROM gl_team_members WHERE game_id = ? AND player_id = ?',
    [gameId, playerId]
  );
  await tx.execute(
