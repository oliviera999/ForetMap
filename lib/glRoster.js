...
const GAME_ROSTER_SELECT = `SELECT p.id, tm.team_id, t.name AS team_name, t.type AS team_type,
...
function mapGameRosterRow(row, { vitalityEnabled = false } = {}) {
  const out = {
    ...
    // Peuple de l'équipe ('gnome' | 'unicorn') : porte la restriction de lancement
    // des sortilèges (`gl_spells.caster_kind`), cf. lib/glSpellCasterKind.js.
    teamType: row.team_type != null ? String(row.team_type) : null,
...
async function assignPlayerToTeamTx(tx, { gameId, teamId, playerId }) {
  ...
  // L'appartenance est portée par partie (`gl_team_members`) ; le pointeur global
  // `gl_players.team_id` n'est plus écrit (cf. lib/glPlayerMembership.js).
  await tx.execute(
