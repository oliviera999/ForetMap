 ...
 *
 * Deux notions, deux colonnes (migration 266) :
...
async function recomputeUserRole(userId) {
  ...
  if (!resolved.assigned) {
    ...
    // migration 266, écriture directe de `user_roles`) : ce profil devient le profil attribué,
...
async function setAssignedRole(userId, roleId) {
  ...
  if (nextRoleId == null) {
    ...
    // profil effectif courant, réservée aux données antérieures à la migration 266).
