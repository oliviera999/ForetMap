...
const { asTrimmedString, buildGlossaryLookupMap, matchGlossaryTermsForSpecies } = require('./glGlossaryMatch');
...
async function loadKnownCategorySlugs(deps) {
  const rows = await deps.queryAll('SELECT slug FROM quiz_categories');
...
async function syncSingleQuestionGlossaryLinks(deps, payload) {
  ...
  await execute('DELETE FROM quiz_question_glossary WHERE question_code = ?', [payload.question_code]);
  for (const term of matched) {
    await execute(
      'INSERT IGNORE INTO quiz_question_glossary (question_code, glossary_code) VALUES (?, ?)',
...
async function allocateNextQuizQuestionCode(deps) {
  const row = await deps.queryOne(
    `SELECT question_code FROM quiz_questions
      WHERE question_code REGEXP '^QF[0-9]+$'
      ORDER BY CAST(SUBSTRING(question_code, 3) AS UNSIGNED) DESC
      LIMIT 1`,
  );
  const current = row?.question_code ? Number(String(row.question_code).slice(2)) : 0;
  const next = Number.isFinite(current) && current > 0 ? current + 1 : 1;
  return `QF${String(next).padStart(4, '0')}`;
}
...
function buildListOrderClause(sort) {
  switch (sort) {
    ...
    case 'theme':
    default:
      return 'c.theme ASC, q.categorie_slug ASC, q.numero_dans_categorie ASC';
  }
}
...
async function listAdminQuestions(deps, options = {}) {
  ...
  const sort = VALID_SORTS.has(options.sort) ? options.sort : 'theme';
  ...
  if (q) {
    ...
    rows = rows.filter((row) => {
      const hay = `${row.question_code} ${row.question} ${row.tags || ''} ${row.categorie_slug}`.toLowerCase();
...
async function upsertQuizQuestion(deps, body, options = {}) {
  ...
  const errors = validateQuestionPayload(payload, 0, knownCategories);
  if (errors.length) {
    throw Object.assign(new Error(formatValidationError(errors)), { statusCode: 400, details: errors });
  }
