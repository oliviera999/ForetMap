 ...
 * Les projets `on_hold` restent inchangés : ce statut est une décision manuelle du professeur.
 *
...
async function syncTaskProjectCompletionForProject(projectId) {
  ...
  const project = await queryOne('SELECT id, status, map_id FROM task_projects WHERE id = ?', [pid]);
  ...
  const cur = String(project.status || 'active').trim().toLowerCase();
  if (cur === 'on_hold') return false;
