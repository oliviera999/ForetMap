...
const { tokenEpochMatches } = require('./tokenEpoch');
...
async function loadGlIdentity(claims, deps) {
  ...
  if (userType === 'gl_player') {
    // Depuis l'unification des identités (migration 211), l'état de compte du joueur —
    // actif, « doit changer son mot de passe », époque de jeton — vit sur le compte `users`
    // lié : une désactivation côté ForetMap coupe aussi le jeu, sans attendre l'expiration.
    const row = await queryOne(
      `SELECT p.id, p.class_id, p.is_active,
              u.is_active AS user_is_active, u.password_must_reset, u.token_epoch
         FROM gl_players p
    ...
    if (row.user_is_active != null && !Number(row.user_is_active)) return null;
    if (!tokenEpochMatches(claims, row.token_epoch)) return null;
    return {
      userType: 'gl_player',
  ...
  if (userType === 'gl_admin') {
    const row = await loadActiveGlStaff(userId, deps);
    if (!row) return null;
    if (!tokenEpochMatches(claims, row.token_epoch)) return null;
...
/**
 * Pendant une prise de contrôle, l'identité hydratée est celle du **joueur** ; l'acteur
 * (staff GL) doit rester valide à chaque requête, comme côté ForetMap
 * (`hydrateAuthFromTokenClaims` revérifie `admin.impersonate`). Sans ce contrôle, un MJ
 * désactivé — ou rétrogradé hors du staff — gardait le contrôle du compte joueur jusqu'à
 * l'expiration du jeton, alors que ses propres droits, eux, étaient bien révoqués.
 *
 * Les rôles admis sont ceux qu'exige la route d'ouverture (`POST /admin/impersonate`) :
 * un staff GL actif dont le rôle en base est `admin` ou `mj`.
 *
 * @returns {Promise<boolean>} `false` = la prise de contrôle n'est plus légitime.
 */
async function actorStillMayImpersonate(claims, deps) {
  if (!claims?.impersonating) return true;
  if (String(claims.actorUserType || '') !== 'gl_admin' || claims.actorUserId == null) {
    return false;
  }
  const actor = await loadActiveGlStaff(claims.actorUserId, deps);
  if (!actor) return false;
  const role = String(actor.role || '').toLowerCase();
  if (role !== 'admin' && role !== 'mj') return false;
  // Même contrat que ForetMap : un changement de mot de passe de l'enseignant lié
  // révoque la prise de contrôle (l'époque snapshotée à l'ouverture, `actorTokenEpoch`).
  if (actor.foretmap_user_id != null) {
...
/**
 * Ligne `gl_admins` d'un staff GL **encore légitime**, ou `null`.
 *
 * Le staff GL est un enseignant ForetMap (CDG-12) : quand la ligne est liée à un compte
 * `users`, celui-ci doit être un enseignant actif qui détient toujours `teacher.access`.
 * Retirer ce droit ou désactiver l'enseignant coupe donc la session MJ en cours, et une
 * prise de contrôle ouverte par lui. Une ligne sans compte lié (données antérieures à
 * l'exigence du compte enseignant à la connexion) reste acceptée : toute nouvelle connexion
 * la rattache (`resolveGlStaffLogin`), et la révocation s'applique dès lors.
 */
async function loadActiveGlStaff(adminId, deps) {
  const { queryOne } = deps;
  const row = await queryOne(
    `SELECT a.id, a.role, a.display_name, a.is_active, a.foretmap_user_id,
            u.is_active AS user_is_active, u.token_epoch,
            (SELECT 1
               FROM user_roles ur
               INNER JOIN role_permissions rp ON rp.role_id = ur.role_id
              WHERE ur.user_type = 'teacher' AND ur.user_id = u.id AND ur.is_primary = 1
                AND rp.permission_key = 'teacher.access'
              LIMIT 1) AS teacher_access
       FROM gl_admins a
       LEFT JOIN users u ON u.id = a.foretmap_user_id AND u.user_type = 'teacher'
      WHERE a.id = ? LIMIT 1`,
    [String(adminId)],
  );
  if (!row || !Number(row.is_active)) return null;
  if (row.foretmap_user_id != null) {
    if (row.user_is_active == null || !Number(row.user_is_active)) return null;
    if (!Number(row.teacher_access)) return null;
  }
  return row;
}
...
async function hydrateGlAuthFromClaims(claims, deps) {
  ...
  let membership = null;
  try {
    if (!(await actorStillMayImpersonate(claims, deps))) return null;
    ...
    if (identity.userType === 'gl_player') {
      // L'équipe suit la base, par partie (`gl_team_members`) : un joueur réaffecté pendant
      // sa session change d'équipe sans attendre un nouveau jeton. La partie portée par le
      // jeton est préférée ; à défaut d'appartenance en base, le claim du jeton fait foi.
      membership = await resolveGlPlayerActiveMembership(identity.userId, {
        preferredGameId: claims.gameId,
        preferredTeamId: identity.tokenTeamId,
