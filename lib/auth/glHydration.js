...
async function loadGlIdentity(claims, deps) {
  ...
  if (userType === 'gl_admin') {
    const row = await queryOne(
      `SELECT a.id, a.role, a.display_name, a.is_active,
              u.is_active AS user_is_active, u.token_epoch
         FROM gl_admins a
         LEFT JOIN users u ON u.id = a.foretmap_user_id
        WHERE a.id = ? LIMIT 1`,
      [userId],
    );
    if (!row || !Number(row.is_active)) return null;
    // Le staff GL délègue ses secrets à son compte enseignant ForetMap : compte désactivé ou
    // mot de passe changé → session GL révoquée elle aussi.
    if (row.user_is_active != null && !Number(row.user_is_active)) return null;
    if (!tokenEpochMatches(claims, row.token_epoch)) return null;
...
async function actorStillMayImpersonate(claims, deps) {
  ...
  const { queryOne } = deps;
  const actor = await queryOne('SELECT id, role, is_active FROM gl_admins WHERE id = ? LIMIT 1', [
    String(claims.actorUserId),
  ]);
  if (!actor || !Number(actor.is_active)) return false;
