...
async function loadGlIdentity(claims, deps) {
  ...
  if (userType === 'gl_player') {
    const row = await queryOne(
      'SELECT id, class_id, team_id, is_active FROM gl_players WHERE id = ? LIMIT 1',
      [userId],
    );
...
/**
 * Pendant une prise de contrôle, l'identité hydratée est celle du **joueur**, mais
 * l'acteur (MJ/admin) doit rester valide — même contrat que
 * `hydrateAuthFromTokenClaims` côté ForetMap (`admin.impersonate`). Sinon un staff
 * désactivé/supprimé conserve le contrôle du compte élève jusqu'à expiration du JWT.
...
async function actorStillMayImpersonate(claims, deps) {
  ...
  // Tout staff GL actif (admin ou MJ) peut impersonner — aligné sur canGlStaffImpersonate.
