...
async function ensureDefaultAssignments() {
  ...
  const teachers = await queryAll('SELECT id, email FROM teachers');
  const adminEmail = (process.env.TEACHER_ADMIN_EMAIL || '').trim().toLowerCase();
  for (const t of teachers) {
    const targetRoleId = adminEmail && String(t.email || '').trim().toLowerCase() === adminEmail
  ...
  const students = await queryAll('SELECT id FROM students');
...
async function buildAuthzPayload(userType, userId, elevated = false) {
  ...
  for (const row of rows) {
    ...
    if (!needsElevation || elevated) permissions.push(key);
