...
async function syncStudentPrimaryRoleFromProgress(
  ...
  if (isVisitorLikeSlug(currentSlug)) {
    const done =
      doneCount == null
        ? await countValidatedAssignmentsForStudent(studentId)
        : toPositiveInt(doneCount, 0);
...
async function ensureDefaultRolesAndPermissions() {
  ...
  // Semis matrice : uniquement si le profil n'a encore aucune permission.
  // Ainsi une révocation admin sur un profil système survit aux redémarrages ;
  // un profil neuf (ex. prof_classe après déploiement) reçoit toujours sa matrice.
