...
// Cache court de la resolution d'autorisation : buildAuthzPayload est appele sur CHAQUE
// requete authentifiee (3-5 SELECT). Cle = userType:userId:elevated. L'elevation PIN et
// l'impersonation changent les claims du token -> cle differente -> effet immediat.
// TTL court = filet de securite : un changement de role/permission rate par une invalidation
// explicite se resorbe en <= TTL secondes. Invalidation explicite sur les points de mutation.
const AUTHZ_CACHE_TTL_MS = 30000;
const authzCache = getNamedMemoryTtlCache('rbac-authz', {
  ttlMs: AUTHZ_CACHE_TTL_MS,
  maxEntries: 5000,
});

function authzCacheKey(userType, userId, elevated) {
  return `${String(userType || '').toLowerCase()}:${String(userId)}:${elevated ? 1 : 0}`;
}

/** Invalide les deux variantes (eleve/eleve+PIN) pour un utilisateur (changement de role). */
function invalidateAuthzCacheForUser(userType, userId) {
  authzCache.delete(authzCacheKey(userType, userId, false));
  authzCache.delete(authzCacheKey(userType, userId, true));
}

/** Invalide tout le cache (edition des permissions d'un role : impacte tous ses porteurs). */
function invalidateAllAuthzCache() {
  authzCache.clear();
}
...
const PERMISSIONS = [
  ...
  ['groups.manage', 'Gestion groupes utilisateurs', 'Créer/éditer/supprimer les groupes, membres et périmètres'],
  ['stats.read.all', 'Lecture stats globales', 'Consulter les stats de tous les n3beurs'],
  ['stats.read.group', 'Lecture stats par groupe', 'Consulter les stats dans le périmètre de ses groupes'],
  ...
  ['tasks.assign.group', 'Affectation tâches par groupe', 'Affecter des missions à un groupe/sous-groupe'],
  ...
  ['forum.group.moderate', 'Modération forum par groupe', 'Modérer les discussions dans son périmètre de groupe'],
  ...
  ['gl.action.request', 'Demande d’action joueur G&L', 'Un joueur peut proposer une action à valider par le MJ'],
];
...
const ROLE_PERMISSION_MATRIX = {
  admin: [
    ...
    ['observations.read.group', 0], ['forum.group.moderate', 0],
  ],
  prof: [
    ['teacher.access', 0], ['groups.read', 0], ['groups.manage', 1],
    ['stats.read.all', 0], ['stats.read.group', 0], ['stats.export', 1], ['students.import', 1], ['students.delete', 1],
    ['users.create', 1],
    ['tasks.manage', 1], ['tasks.assign.group', 1], ['tasks.validate', 1], ['tasks.propose', 0], ['tasks.assign_self', 0], ['tasks.unassign_self', 0],
  ...
  gl_admin: [
    ...
    ['gl.team.manage', 0],
    ['gl.event.emit', 0],
    ['gl.mascot.position', 0],
    ['gl.settings.manage', 0],
  ],
  gl_mj: [
    ['gl.read', 0],
    ['gl.content.manage', 0],
  ...
  gl_player: [['gl.read', 0], ['gl.action.request', 0]],
  gl_observateur: [['gl.read', 0]],
  visiteur: [],
};
...
const STAFF_ROLE_SLUGS = new Set(['admin', 'prof', 'visiteur']);

function isStaffRoleSlug(slug) {
  return STAFF_ROLE_SLUGS.has(String(slug || '').trim().toLowerCase());
}

function isGlRoleSlug(slug) {
  return String(slug || '').trim().toLowerCase().startsWith('gl_');
}
...
function resolveStudentRoleSlugFromValidatedCount(validatedCount, steps) {
  ...
  if (maxMin === 0) {
    return atMaxMin.reduce((best, step) => (
      !best || step.displayOrder < best.displayOrder
        || (step.displayOrder === best.displayOrder && String(step.roleSlug).localeCompare(String(best.roleSlug)) < 0)
        ? step
        : best
    )).roleSlug;
  }
...
async function syncStudentPrimaryRoleFromProgress(studentId, doneCount = null, progressionConfig = null, options = {}) {
  ...
  if (currentSlug !== targetSlug) {
    ...
    const overAssignedEleve = currentSlug.startsWith('eleve_') && done < currentMin;
    const promotion = targetRank > currentRank;
    const earnedStepUp = isCurrentProgressionRole
      && targetMin <= done
      && targetMin > currentMin;
    if (targetRole?.id && (promotion || overAssignedEleve || earnedStepUp)) {
...
async function buildAuthzPayload(userType, userId, elevated = false) {
  const key = authzCacheKey(userType, userId, elevated);
  const cached = authzCache.get(key);
  if (cached !== undefined) return cached;
  const payload = await computeAuthzPayload(userType, userId, elevated);
  authzCache.set(key, payload);
  return payload;
}

async function computeAuthzPayload(userType, userId, elevated = false) {
  await ensureRbacBootstrap();
...
async function ensurePrimaryRole(userType, userId, preferredRoleSlug) {
  ...
  invalidateAuthzCacheForUser(userType, userId);
...
module.exports = {
  ...
  ensureRbacBootstrap,
  buildAuthzPayload,
  invalidateAuthzCacheForUser,
  invalidateAllAuthzCache,
