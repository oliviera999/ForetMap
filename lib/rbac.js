...
/** Paliers n3beur : eleve_* ou profil perso. (rang < 400, hors staff / G&L). */
...
async function getStudentProgressionRoles() {
  const rows = await queryAll(
    `SELECT slug, display_name, emoji, min_done_tasks, display_order, \`rank\`
      ...
      WHERE slug LIKE 'eleve_%'
         OR (
           \`rank\` < 400
           AND slug NOT IN ('admin', 'prof', 'visiteur')
           AND slug NOT LIKE 'gl_%'
           AND min_done_tasks IS NOT NULL
         )
  ...
  const normalized = rows
    .map((row) => {
      ...
      if (!slug || !isStudentProgressionTierSlug(slug, row.rank)) return null;
