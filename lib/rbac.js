...
const ROLE_PERMISSION_MATRIX = {
  admin: [
    ...
    ['admin.settings.secrets.write', 1],
    ['stats.read.all', 0],
    ['stats.read.group', 0],
    ['stats.export', 1],
    ['students.import', 1],
    ...
    ['tasks.unassign_self', 0],
    ['tasks.done_self', 0],
    ['zones.manage', 1],
    ['map.manage_markers', 1],
    ['plants.manage', 1],
    ['tutorials.manage', 1],
    ['visit.manage', 1],
    ['audit.read', 1],
    ['observations.read.all', 1],
    ['observations.read.group', 0],
    ['observations.manage.all', 1],
    ['observations.manage.group', 0],
    ['forum.group.moderate', 0],
  ],
  prof: [
    ['teacher.access', 0],
    ['groups.read', 0],
...
async function ensureDefaultRolesAndPermissions() {
  ...
  for (const [roleSlug, entries] of Object.entries(ROLE_PERMISSION_MATRIX)) {
    ...
    await execute('INSERT IGNORE INTO role_pin_secrets (role_id, pin_hash) VALUES (?, ?)', [
      role.id,
      pinHash,
    ]);
  }
...
async function verifyRolePin(roleId, pin) {
  const row = await queryOne('SELECT pin_hash FROM role_pin_secrets WHERE role_id = ? LIMIT 1', [
    roleId,
  ]);
  if (!row) return false;
