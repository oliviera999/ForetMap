...
const { getForcedRoleGroupForStudent } = require('./groupDefaultRole');
...
const ADMIN_CANONICAL_LOGIN = String(process.env.ADMIN_CANONICAL_LOGIN || 'oliviera9')
  .trim()
  .toLowerCase();
...
const STAFF_ROLE_SLUGS = new Set(['admin', 'prof', 'visiteur', 'personnel']);
...
/**
 * Aligne le profil principal d'un n3beur sur son nombre de tâches validées.
 *
 ...
 * @param {{recordPromotionNotice?: boolean, manual?: boolean, includeVisitorLike?: boolean,
 *   allowDemotion?: boolean, allowOverAssignedCatchUp?: boolean, dryRun?: boolean}} [options]
 *   - `manual` : recalcul demandé explicitement par un n3boss (bouton « Recalculer »). Passe
 *     outre le réglage `rbac.progression_by_validated_tasks` **et** inclut les profils lecture
 *     seule — le périmètre est décidé en amont par l'appelant (cf.
 *     `lib/studentProgressionSync.js`). Ne touche jamais un profil staff (n3boss, admin, MJ,
 *     prof de classe) ni un profil hors échelle de progression.
 *   - `includeVisitorLike` : traite un profil visiteur/personnel (utilisé au rattachement à un
 *     groupe n3beur, où le compte quitte justement le profil visiteur) sans pour autant
 *     ignorer le réglage global.
 *   - `allowDemotion` : autorise l'alignement strict (le palier peut redescendre). Par défaut
 *     la montée seule s'applique : un profil attribué à la main n'est jamais rétrogradé.
 *   - `allowOverAssignedCatchUp` (défaut `true`) : conserve la règle historique du rattrapage —
 *     un palier `eleve_*` attribué au-dessus du compteur réel redescend au palier mérité.
 *     Le recalage déclenché par un rattachement de groupe le passe à `false` : ce chemin ne
 *     doit jamais faire perdre un palier, y compris un palier posé à la main par un n3boss.
 *   - `dryRun` : calcule la décision et la renvoie (`changed` + `dryRun: true`) sans écrire —
 *     alimente l'aperçu du recalcul en masse. Même chemin de décision que l'application
 *     réelle : l'aperçu ne peut pas diverger du résultat.
 */
async function syncStudentPrimaryRoleFromProgress(
  ...
  const includeVisitorLike = manual || !!options.includeVisitorLike;
  const allowOverAssignedCatchUp = options.allowOverAssignedCatchUp !== false;
  ...
  if (!config.autoProgressionEnabled && !manual) {
    ...
    return {
      changed: false,
      reason: 'auto_progression_disabled',
  ...
  // Profil imposé par un groupe (`groups.force_default_role`, migration 265) : le compte sort
  // de l'échelle automatique, y compris pour un recalcul manuel. C'est précisément ce que le
  // n3boss a demandé en cochant « imposer ce profil » — sinon la première tâche validée
  // reprendrait la main, et le réglage ne tiendrait pas une journée.
  const forcedGroup = await getForcedRoleGroupForStudent(studentId);
  if (forcedGroup) {
    const done =
      doneCount == null
        ? await countValidatedAssignmentsForStudent(studentId)
    ...
    return {
      ...
      forcedByGroupId: forcedGroup.groupId,
      forcedByGroupName: forcedGroup.groupName,
      currentRoleSlug: current?.slug || null,
      currentRoleDisplayName: current?.display_name || null,
      done,
      ...config,
  ...
  if (isVisitorLikeSlug(currentSlug) && !includeVisitorLike) {
    const done =
      doneCount == null
        ? await countValidatedAssignmentsForStudent(studentId)
        : toPositiveInt(doneCount, 0);
    return {
      changed: false,
      reason: 'visitor_like_skipped',
      currentRoleSlug: current?.slug || currentSlug || 'visiteur',
      currentRoleDisplayName: current?.display_name || null,
      done,
      ...config,
    };
  ...
  // Un profil hors échelle (n3boss, admin, MJ, prof de classe, profil maison…) n'est jamais
  // recalculé, même en manuel : seule l'échelle n3beur suit les tâches validées.
  if (current && !isCurrentProgressionRole && !isVisitorLikeSlug(currentSlug)) {
    return {
      changed: false,
      reason: 'role_out_of_ladder',
  ...
  if (currentSlug !== targetSlug) {
    ...
    const currentStep = (config.steps || []).find((s) => s.roleSlug === currentSlug);
    const targetStep = (config.steps || []).find((s) => s.roleSlug === targetSlug);
    const currentRank = Number(current?.rank ?? currentStep?.rank ?? 0);
    const targetRank = Number(targetRole?.rank ?? targetStep?.rank ?? 0);
    const currentMin = currentStep?.min ?? STUDENT_PROGRESS_DEFAULTS[currentSlug]?.min ?? 0;
    const targetMin = targetStep?.min ?? STUDENT_PROGRESS_DEFAULTS[targetSlug]?.min ?? 0;
    const overAssignedEleve =
      allowOverAssignedCatchUp && currentSlug.startsWith('eleve_') && done < currentMin;
    const promotion = targetRank > currentRank;
    const earnedStepUp = isCurrentProgressionRole && targetMin <= done && targetMin > currentMin;
    // `allowDemotion` : alignement strict demandé (case « aligner strictement »). Sinon on
    // conserve la règle historique — montée seule, plus le rattrapage d'un palier attribué
    // au-dessus du compteur réel (`overAssignedEleve`).
    if (targetRole?.id && (allowDemotion || promotion || overAssignedEleve || earnedStepUp)) {
      if (!dryRun) {
    ...
    return {
      changed: false,
      // Palier cible inférieur (ou profil inconnu) sans alignement strict demandé.
      reason: targetRole?.id ? 'demotion_not_allowed' : 'target_role_missing',
      currentRoleSlug: current?.slug || currentSlug || targetSlug,
      currentRoleDisplayName: current?.display_name || null,
...
async function ensureDefaultAssignments() {
  ...
  for (const t of teachers) {
    ...
    const targetRoleId =
      isConfiguredAdmin || isCanonicalAdmin
        ? adminRole
          ? adminRole.id
          : profRole.id
        : profRole.id;
    ...
    if (targetRoleId === (adminRole ? adminRole.id : null)) {
      await execute(
        'UPDATE user_roles SET is_primary = CASE WHEN role_id = ? THEN 1 ELSE 0 END WHERE user_type = ? AND user_id = ?',
        [targetRoleId, 'teacher', t.id],
      );
    }
  }
...
async function checkCriticalAdminAccount() {
  ...
  const adminEmail = String(process.env.TEACHER_ADMIN_EMAIL || '')
    .trim()
    .toLowerCase();
  ...
  return {
    ok: false,
    reason: 'admin_role_not_assigned',
