...
const { isN3beurTierSlug, defaultRoleSlugForUserType } = require('./shared/n3beurRolesCore');
...
const SYSTEM_ROLES = [
  ...
  /*
   * Personnel (agent, vie scolaire, AED…) : parcours Visite / Biodiversité, forum et
   * commentaires ouverts, plan des personnels — mais aucune gestion.
   *
   * Rang 320, au-dessus des paliers n3beur et sous « Prof de classe » (350) : c'est une
   * **qualité de personne**, pas une position sur l'échelle de progression. Au rang 50
   * d'origine, un personnel rattaché à un groupe classe se faisait conférer le profil du
   * groupe (`lib/effectiveRole.js`, « le plus élevé l'emporte ») et perdait sa qualité de
   * personnel — donc `staff_plan.access` et l'entrée sur proflyautey. Le rang ne lui donne
   * aucune capacité au passage : le seul seuil de rang du code est 400
   * (`N3BEUR_RANK_EXCLUSIVE_MAX`, vue globale comptes / groupes), et l'exclusion de
   * l'échelle n3beur se décide sur le slug (`NON_N3BEUR_SYSTEM_ROLE_SLUGS`), pas sur le rang.
   */
  { slug: 'personnel', display_name: 'Personnel', rank: 320, display_order: 61 },
];
...
const PERMISSIONS = [
  ...
  // Volontairement distincte de `plants.manage` : renseigner un danger et le *valider* ne
  // sont pas le même geste. Le pré-remplissage bibliographique de la migration 251 arrive
  // non validé, et la validation engage la sécurité des élèves en sortie — elle reste donc
  // à l'admin et au prof, pas au prof de classe (tuteur sans périmètre jardin).
  [
    'plants.hazards.validate',
    'Valider les dangers',
    'Confirmer la relecture des dangers et risques sanitaires d’une fiche espèce',
  ],
  [
    'id_keys.manage',
    'Clés d’identification',
    'Créer et éditer les clés dichotomiques d’identification',
  ],
  [
    'individuals.manage',
    'Suivi d’individus',
    'Créer et gérer les arbres suivis (individus) sur une carte',
  ],
  [
    'individuals.measure',
    'Mesures d’individus',
    'Saisir des mesures (circonférence, hauteur…) sur un individu suivi',
  ],
  ...
  // Réservée à l'admin : IP / User-Agent du journal `security_events` (RGPD).
  [
    'audit.security.read',
    'Lecture journal de sécurité',
    'Consulter IP et User-Agent des événements de sécurité',
  ],
...
const ROLE_PERMISSION_MATRIX = {
  ...
  /*
   * Tuteur de classe : élèves de ses groupes seulement ; pas de jardin ni de création de
   * comptes. **Sans `teacher.access`** depuis le réalignement du 22/09/2026 : ce profil a
   * une interface de type apprenant, pas la barre haute n3boss, et la porte d'entrée du
   * front est la session (`isTeacherAccount`, `src/App.jsx`), plus cette permission.
   *
   * Les quatre droits `tasks.*` sont ceux de l'apprenant. `task_assignments` est aujourd'hui
   * centrée sur l'élève (`student_id`, lectures filtrées sur `user_type = 'student'`) : ils
   * sont donc **inertes** pour un compte enseignant, et le sont volontairement — ils sont
   * inscrits ici pour que la matrice reflète la base de production, et non pour être
   * exploités en l'état.
   */
  prof_classe: [
    'groups.read',
  ...
  visiteur: [],
  // Staff non enseignant : aucune action de gestion, mais l'accès au plan des personnels —
  // c'est le profil visé par le sous-domaine proflyautey. La participation au forum et aux
  // commentaires de contexte n'est pas une permission RBAC : elle se décide sur le profil
  // (`lib/shared/visitorRoles.js`, `PARTICIPATION_EXCLUDED_ROLE_SLUGS`).
...
/**
 * Taxonomie des profils : source unique `lib/shared/n3beurRolesCore.js` (CDG-44). « Prof de
 * classe » est hors échelle : un seuil `min_done_tasks` posé sur ce profil ne le fait pas
 * entrer dans la progression automatique (CDG-09).
 */
const isStudentProgressionTierSlug = isN3beurTierSlug;
...
async function syncStudentPrimaryRoleFromProgress(
  ...
  // Entre deux paliers de l'échelle, c'est le **seuil** qui dit s'il y a montée : un palier
  // sur mesure garde souvent le rang par défaut de la console (150), si bien qu'atteindre
  // « expert » (40 tâches, rang 150) depuis « chevronné » (15 tâches, rang 300) passait pour
  // une rétrogradation, sans avis de promotion (audit du 25/09/2026, question 11). Hors
  // échelle (visiteur, profil conféré par un groupe), le rang reste la référence.
  const nextMin = stepOf(nextSlug)?.min;
  const currentMin = stepOf(currentSlug)?.min;
  const isPromotion =
    nextMin != null && currentMin != null ? nextMin > currentMin : nextRank > current.rank;
  if (!dryRun) {
    await setAssignedRole(studentId, targetRole.id);
    if (recordPromotionNotice && effectiveChanged && isPromotion) {
  ...
  return {
    changed: true,
    dryRun,
    reason: effectiveChanged ? (isPromotion ? 'promoted' : 'demoted') : 'assigned_updated',
    effectiveChanged,
