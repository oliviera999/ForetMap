...
const RESOLVERS = {
  ...
  ecosystem: {
    // Un écosystème est identifié par son biome (slug). Pas de table de biomes dédiée :
    // on accepte un slug bien formé présent au catalogue d'espèces (biome peuplé), sinon
    // on reste permissif sur le format (le contenu biotope/biocénose vit dans le chapitre).
    async exists(db, ref) {
      if (!SLUG_RE.test(ref)) return false;
      const row = await db.queryOne('SELECT 1 AS ok FROM gl_species WHERE biome_slug = ? LIMIT 1', [
        ref,
      ]);
      return !!row || SLUG_RE.test(ref);
    },
    async title() {
      // Pas de nom de biome en base : le client fournit le libellé (registre de biomes).
      return null;
    },
  },
};
