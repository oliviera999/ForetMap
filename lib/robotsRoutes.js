 ...
 * @param {object} deps
 * @param {(req: import('express').Request) => string} deps.resolveProductFromRequest résolveur
 *   de produit. `server.js` y passe `resolveSecureProductId` (`lib/surfaceAccess.js`) et non
 *   `resolveProductFromRequest` : la surcharge `X-Foretmap-Product` n'est lue qu'hors
 *   production (lot A), sinon une requête forgée suffirait à faire servir le `robots.txt`
 *   permissif sur une surface gardée.
