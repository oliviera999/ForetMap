...
async function presentFeuilletZone(
  ...
  // Contrôle rapide, hors transaction : il évite d'ouvrir une transaction pour rien dans le
  // cas courant. Il ne suffit pas — voir le re-contrôle sous verrou plus bas.
  ...
  await runTx(async (tx) => {
    // Le contrôle « déjà présentée » lit `gl_game_events` : deux requêtes simultanées le
    // passaient toutes deux, et l'équipe encaissait deux fois le gain de cœurs. Le verrou
    // sur la ligne d'équipe les sérialise ; la seconde relit alors un journal complet.
    await lockGlTeamRow(tx, teamId);
    const stillAvailable = await canPresentFeuilletZone(
