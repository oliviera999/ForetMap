...
async function presentFeuilletZone(
  ...
  // Fast-path hors TX (évite le travail si déjà présenté) ; la concurrence est
  // tranchée sous verrou équipe dans la transaction ci-dessous.
  const canPresentFast = await canPresentFeuilletZone(deps, { gameId, teamId, zoneId });
  if (!canPresentFast) {
  ...
  let presentEvent = null;
  try {
    await runTx(async (tx) => {
      await lockGlTeamRow(tx, { gameId, teamId });
      const canPresent = await canPresentFeuilletZone(
        { queryAll: tx.queryAll.bind(tx) },
        { gameId, teamId, zoneId },
      );
      if (!canPresent) {
        const err = new Error('ZONE_ALREADY_PRESENTED');
        err.status = 409;
        throw err;
      }
      ...
      presentEvent = await insertGameEvent(tx, {
        gameId,
        teamId,
        actorType,
        actorId: String(actorId),
        eventType: PRESENT_EVENT_TYPE,
        payload: {
    ...
    if (err?.message === 'ZONE_ALREADY_PRESENTED') {
      return { error: { status: 409, message: 'Zone feuillet déjà présentée pour cette équipe' } };
    }
    if (err?.status === 404 && err?.message === 'TEAM_NOT_FOUND') {
      return { error: { status: 404, message: 'Équipe introuvable dans cette partie' } };
    }
    throw err;
  }
