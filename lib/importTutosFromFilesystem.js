...
/**
 * Rapprochement de dernier recours entre le nom de fichier (`fiche-compost-punk.html` →
 * `compost`) et le slug d'un tutoriel déjà en base (`compostage`). Il ne joue qu'après les
 * critères sûrs (chemin, empreinte du contenu, slug, titre).
 *
 * DÉLIBÉRÉMENT ÉTROIT : la règle historique acceptait n'importe quelle inclusion, dans les
 * deux sens et jusqu'aux slugs privés de tirets. Elle déclarait « déjà en base » des fiches
 * réellement nouvelles — `fiche-plantes-punk.html` capté par `associations-plantes`,
 * `fiche-semences-locales-punk.html` par `semences` — et l'import n'avait alors plus rien à
 * proposer. On exige désormais que le radical soit le DÉBUT du slug, et qu'il soit assez
 * long pour ne pas rapprocher deux sujets sans rapport.
 */
...
function slugMatchesFilenameStem(slug, stem) {
  ...
  if (!s || !t) return false;
  if (s === t) return true;
  if (t.length < MIN_STEM_MATCH_LENGTH) return false;
