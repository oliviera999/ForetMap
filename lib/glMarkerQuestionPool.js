...
async function drawQuestionFromMarker(deps, marker, chapterBiomeSlugs, excludeCodes = []) {
  ...
  if (questionCfg.mode === 'fixed') {
    ...
    if (!isPresentableQuestionRow(row)) {
      return { error: presentableQuestionError(code), questionCode: null };
    }
  ...
  if (items.length === 0) {
    return { error: 'Aucune question présentable dans le pool (vérifiez les choix QCM)', questionCode: null };
  }
