...
function resolveFetchTimeoutMs(options = {}) {
  ...
  return Math.min(
    5200,
    Math.max(200, Number(options.timeoutMs) > 0 ? Number(options.timeoutMs) : 6000),
  );
}
...
function mapTrefleRow(row) {
  ...
  const spec =
    row.specifications && typeof row.specifications === 'object' ? row.specifications : null;
  if (spec) {
  ...
  if (Array.isArray(row.edible_part) && row.edible_part.length) {
    fields.harvest_part = row.edible_part
      .map((x) => String(x))
      .join(', ')
      .slice(0, 200);
  }
...
async function fetchTrefleSpeciesTraits(scientificName, options = {}) {
  ...
  try {
    ...
    if (!res.ok || json?.error) {
      logger.warn(
        { msg: 'trefle_autofill_http', status: res.status },
        'Pré-saisie Trefle : réponse en échec',
      );
      return null;
    }
    ...
    logger.warn(
      { msg: 'trefle_autofill_err', err: String(err?.message || err) },
      'Pré-saisie Trefle : erreur',
    );
    return null;
  } finally {
    clearTimeout(timer);
