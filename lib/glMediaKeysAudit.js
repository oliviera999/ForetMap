...
const INTRO_IMAGE_KEYS = [
  'intro_01_la-boite',
  'intro_02_le-copiste',
  'intro_03_le-carnet-de-selene',
  'intro_04_le-miroir-passage',
  'intro_05_selene-au-seuil',
  'intro_06_le-corbeau-messager',
  'intro_07_salle-de-classe',
  'intro_08_le-carnet-dans-la-savane',
  'intro_09_la-traversee-des-biomes',
];
...
function feuilletPrefix(code) {
  return `recit_feuillet-action_${String(code || '')
    .trim()
    .toLowerCase()}_`;
}
...
function auditGlMediaKeys(keyIndex, options = {}) {
  ...
  const resolvePlateauBoardSlug =
    options.resolvePlateauBoardSlug ||
  ...
  const wiredSlugs = new Set(
    expected.filter((e) => e.slug && !e.slug.endsWith('*')).map((e) => e.slug),
  );
  ...
  for (const row of expected) {
    ...
    const pathOk =
      entry?.relativePath &&
      (isAudioRow
        ? isAudioRelativePath(entry.relativePath)
        : isImageRelativePath(entry.relativePath));
