...
/**
 * Même règle, appliquée **en mémoire** à une ligne déjà chargée (liste d'assignations
 * d'une tâche). Le contrôle « déjà assigné » de la route d'inscription lisait encore la
 * condition historique : deux homonymes s'y reconnaissaient, et un nom envoyé par le
 * client suffisait à se voir refuser — ou attribuer — la place d'un tiers.
 *
 * @param {{ student_id?: any, student_first_name?: any, student_last_name?: any }} row
 * @param {{ studentId?: any, firstName?: any, lastName?: any }} actor
 */
function assignmentRowMatchesStudent(row, { studentId, firstName, lastName } = {}) {
  if (!row) return false;
  const rowId = row.student_id;
  const hasRowId = rowId != null && String(rowId).trim() !== '';
  const hasActorId = studentId != null && String(studentId).trim() !== '';
  // Une ligne qui porte un identifiant n'est reconnue que par cet identifiant.
  if (hasRowId) return hasActorId && String(rowId) === String(studentId);
  // Ligne héritée : le nom reste la seule clé disponible.
...
module.exports = { assignmentIdentityMatch, assignmentRowMatchesStudent };
