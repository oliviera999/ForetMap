...
const { buildWorkbookBuffer, jsonRowsToAoa } = require('../spreadsheet');
...
async function executeTasksProjectsImport({
  ...
  for (const project of projectRows) {
    ...
    await execute(
      ...
      [id, project.mapId, project.title, project.description || null, nowDbTimestamp()],
    );
  ...
  for (const task of taskRowsResolved) {
    ...
    const existingProject = projectKey
      ...
      : null;
    const id = uuidv4();
    const importedStatus = task.status || 'available';
    const seriesId = task.recurrence ? uuidv4() : null;
    await execute(
      'INSERT INTO tasks (id, title, description, map_id, project_id, zone_id, marker_id, start_date, due_date, required_students, status, recurrence, recurrence_series_id, danger_level, difficulty_level, importance_level, validated_at, created_at) VALUES (?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        id,
        task.title,
        task.description || '',
        ...
        // Une tâche importée déjà « validée » doit porter sa date de validation : c'est la
        // seule référence de délai de l'archivage automatique. Sans elle (`validated_at`
        // NULL), le job quotidien ignore la ligne et la tâche encombre les listes à vie.
        importedStatus === 'validated' ? new Date() : null,
        nowDbTimestamp(),
      ],
    );
