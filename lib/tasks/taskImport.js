...
async function executeTasksProjectsImport({
  ...
  for (const task of taskRowsResolved) {
    ...
    await execute(
      'INSERT INTO tasks (id, title, description, map_id, project_id, zone_id, marker_id, start_date, due_date, required_students, status, recurrence, danger_level, difficulty_level, importance_level, validated_at, created_at) VALUES (?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        id,
        task.title,
        task.description || '',
