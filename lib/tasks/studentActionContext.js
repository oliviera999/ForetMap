...
async function ensureStudentPermission({ studentId, permissionKey }) {
  ...
  if (!base.permissions.includes(permissionKey)) {
    ...
    // `group_forced` : même origine (le profil par défaut du groupe), simplement imposé —
    // le rattrapage doit s'appliquer aussi, sinon un groupe imposant priverait ses membres
    // de cette réparation.
    if (resolved?.roleId && (resolved.source === 'group' || resolved.source === 'group_forced')) {
