...
async function ensureStudentPermission({ studentId, permissionKey }) {
  // Le profil effectif est recalculé (profil attribué ⊕ groupes) avant de lire les droits :
  // un membre ajouté hors connexion agit avec le profil que son groupe lui confère.
  await recomputeUserRole(studentId);
  const base = await buildAuthzPayload('student', studentId);
