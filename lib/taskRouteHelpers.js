...
/**
 * Clause `ORDER BY` de tri des tâches (épinglées par sort_order d'abord, puis
 * par niveau d'importance décroissant, puis échéance). Factorisée pour rester
 * identique entre la liste (`routes/tasks.js`, préfixe `t.`) et la réordonnance
 * de projet (préfixe vide).
 * @param {string} [prefix] préfixe de colonne (ex. `'t.'`), '' par défaut.
 * @returns {string} corps de la clause ORDER BY, sans le mot-clé `ORDER BY`.
 */
function taskImportanceOrderBySql(prefix = '') {
  const c = prefix;
  return `CASE WHEN COALESCE(${c}sort_order, 0) > 0 THEN 0 ELSE 1 END ASC,
    CASE WHEN COALESCE(${c}sort_order, 0) > 0 THEN ${c}sort_order ELSE NULL END ASC,
    CASE WHEN COALESCE(NULLIF(TRIM(${c}importance_level), ''), '') = '' THEN 1 ELSE 0 END ASC,
    CASE LOWER(TRIM(${c}importance_level))
...
const { attachSpeciesToEntity, livingBeingsListFromSpecies } = require('./speciesJunction');

/**
 * @param {object} task
 * @param {object[]|undefined} speciesRows
 * @param {{ includeSpeciesObjects?: boolean }} [options] — liste polling : omettre `species[]`
 *   (garder `living_beings_list` + `species_ids`).
 */
function attachTaskLivingBeingsApiFields(task, speciesRows, options = {}) {
  if (!task) return;
  const includeSpeciesObjects = options.includeSpeciesObjects !== false;
  if (Array.isArray(speciesRows)) {
    const updated = attachSpeciesToEntity(task, speciesRows, { legacySingleName: '' });
    task.living_beings_list = updated.living_beings_list;
    task.species_ids = updated.species_ids;
    if (includeSpeciesObjects) task.species = updated.species;
    else delete task.species;
    delete task.living_beings;
    return;
  }
  task.living_beings_list = livingBeingsListFromSpecies([], task.living_beings, '');
  delete task.living_beings;
  if (!includeSpeciesObjects) delete task.species;
}
...
/**
 * Dates de tâche à l’écriture : `AAAA-MM-JJ` strict, ou vide/null.
 * Partagé par `POST/PUT /api/tasks` et `POST /api/tasks/proposals`. Les colonnes sont des
 * `DATE` depuis la migration 254 : sans ce contrôle, l’arbitrage d’une valeur mal formée
 * revient au `sql_mode` de MariaDB, que l’application ne fixe pas (rejet brut ou
 * troncature selon le serveur). Ici, c’est un 400 explicite et identique partout.
...
/**
 * Cohérence du couple : une tâche ne peut pas être due avant d’avoir commencé (elle
 * s’afficherait « en attente » et « en retard » à la fois, et la récurrence retombe
 * alors sur son ancrage de repli).
 * @returns {{error: string}|null}
 */
function validateTaskDateRange(startDate, dueDate) {
  if (!startDate || !dueDate) return null;
  if (dueDate < startDate) {
    return { error: "La date d'échéance ne peut pas précéder la date de début" };
  }
  return null;
}
...
function enrichTaskRow(task, zonesLinked, markersLinked, tutorialsLinked, referentsLinked) {
  ...
  task.tutorials_linked = tl.map((x) => {
    ...
    // Présents seulement si le loader les a fournis (détail / liste avec sources).
    if (Object.prototype.hasOwnProperty.call(x, 'source_url')) {
      linked.source_url = x.source_url || null;
    }
    if (Object.prototype.hasOwnProperty.call(x, 'source_file_path')) {
      linked.source_file_path = x.source_file_path || null;
    }
    return linked;
  });
  ...
  // F5 : les liens N-N (task_zones/task_markers) sont l'unique source de vérité ;
  // les colonnes tasks.zone_id/marker_id ne sont qu'une copie du premier lien
  // (syncLegacyLocationColumns) et ne servent plus de repli en lecture.
  task.zone_map_id = zl[0]?.map_id ?? null;
  task.marker_map_id = ml[0]?.map_id ?? null;
  task.zone_name = zl[0]?.name ?? null;
  task.marker_label = ml[0]?.label ?? null;
}
...
/**
 * Normalise un paramètre de requête d'archivage en portée de liste.
 * - 'active'  : seulement les entités non archivées (`archived_at IS NULL`) — défaut.
 * - 'archived': seulement les entités archivées (`archived_at IS NOT NULL`).
 * - 'all'     : archivées + actives (aucun filtre).
 * Accepte les alias courants (fr/en, 1/0, true/false) pour rester tolérant côté client.
 */
function normalizeArchivedFilter(value) {
  ...
  if (['all', 'tous', 'toutes', 'include', 'include_archived', 'with_archived'].includes(raw)) {
    return 'all';
  }
  if (
    ['1', 'true', 'yes', 'oui', 'only', 'archived', 'archive', 'archives', 'archivees'].includes(
      raw,
    )
  ) {
    return 'archived';
...
/** Fragment SQL WHERE pour un filtre d'archivage (retourne '' pour la portée 'all'). */
function archivedFilterSql(scope, columnRef = 'archived_at') {
  if (scope === 'archived') return `${columnRef} IS NOT NULL`;
  if (scope === 'all') return '';
