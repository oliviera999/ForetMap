...
// Vue propre à ForetMap : tables, réglages du site et identifiant de produit viennent de
// l'adaptateur (`lib/pedago/gatingProducts.js`).
const { GATING_PRODUCTS } = require('./pedago/gatingProducts');
...
async function getFmResourceProgressSummary(db, { resourceType, resourceRef } = {}) {
  ...
  const site = await FM.loadSiteSettings();
  const perResource = await db.queryOne(
    `SELECT * FROM ${FM.tables.policy} WHERE resource_type = ? AND resource_ref = ? LIMIT 1`,
  ...
  const links = await loadApprovedGatingLinks(db, FM.id, resourceType, resourceRef);
