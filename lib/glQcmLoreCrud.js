...
async function syncSingleQuestionGlossaryLinks(deps, payload) {
  ...
  // question_dataset='qcm_lore', resource_type='lore_glossary'. DELETE scopé à status='approved'
  // pour ne PAS toucher les liens 'suggested' d'autres pipelines.
  await execute(
    `DELETE FROM gl_resource_question_links
      WHERE question_dataset = 'qcm_lore' AND resource_type = 'lore_glossary'
        AND status = 'approved' AND question_code = ?`,
