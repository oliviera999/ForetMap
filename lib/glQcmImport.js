...
function buildQuestionPayload(row = {}) {
  ...
  return {
    ...
    biome_slug: asTrimmedString(mapped.biome_slug).toLowerCase(),
...
function parseQcmWorkbook(buffer) {
...
const QUESTION_UPSERT_SQL = `
  INSERT INTO gl_qcm_questions (
    ...
    wikipedia_title, wikipedia_url, photo_method, statut, created_at, updated_at
  ) VALUES (
    ?, ?, ?, ?, ?,
    ?, ?, ?, ?, ?,
    ?, ?, ?, ?, ?,
...
function buildQcmTemplateWorkbook() {
...
function buildQcmExportWorkbook({ categories, questions }) {
  const wb = XLSX.utils.book_new();
...
async function loadQcmExportRows(deps, options = {}) {
  ...
  const categorySlugsInExport = new Set(questions.map((q) => String(q.categorie_slug)));
  const filteredCategories = hasQuestionFilter
    ? categories.filter((c) => categorySlugsInExport.has(String(c.slug)))
    : categories;
