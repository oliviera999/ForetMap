...
/** Format des clés de dialogue personnalisées (kebab/snake-case minuscule). */
export const DIALOG_CUSTOM_KEY_RE = /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/;

/**
 * Une clé de dialogue est acceptable si c'est un événement connu (après
 * normalisation legacy) **ou** une clé personnalisée bien formée (ex. clé d'un
 * déclencheur personnalisé `customTriggers`).
 * @param {string} key
 * @returns {boolean}
 */
export function isAcceptableDialogKey(key) {
  const stable = normalizeDialogEventKey(key);
  if (VISIT_MASCOT_DIALOG_EVENT_KEYS.includes(stable)) return true;
  return DIALOG_CUSTOM_KEY_RE.test(stable);
}

/**
 * Schéma data-driven : événements connus **ou** clés personnalisées (validées par
 * format), chaque valeur = lignes de bulle. Remplace l'ancienne énumération figée
 * (`.strict()`), pour des comportements personnalisés extensibles.
 */
export function buildDialogProfileSchema() {
  return z.record(z.string(), dialogLinesSchema).superRefine((obj, ctx) => {
    for (const key of Object.keys(obj || {})) {
      if (!isAcceptableDialogKey(key)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [key],
          message: `Clé de dialogue invalide « ${key} » (événement connu ou clé personnalisée a-z0-9_-).`,
        });
      }
    }
  });
...
export function sanitizeDialogProfile(profile) {
  ...
  for (const [key, lines] of Object.entries(profile)) {
    ...
    // Accepte les événements connus (clé normalisée) ET les clés personnalisées valides
    // (ex. déclencheurs personnalisés). Rejette le reste.
    if (!isAcceptableDialogKey(eventKey)) continue;
