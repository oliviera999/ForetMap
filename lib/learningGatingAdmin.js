...
const { GATING_PRODUCT_CATALOG, exactGatingProduct } = require('./pedago/gatingProductCatalog');

const MAX_ROWS = 200;
const FM_LOCKS_TABLE = GATING_PRODUCT_CATALOG.fm.tables.cooldowns;
const GL_LOCKS_TABLE = GATING_PRODUCT_CATALOG.gl.tables.cooldowns;
...
function toLockRow(row, product) {
  ...
  // ForetMap identifie par compte ; GL par couple (type de lecteur, id) — un invite ou un MJ
  // n'a pas de compte utilisateur. « gl » EXACTEMENT designe GL (lecture historique).
  const descriptor = exactGatingProduct(product) || GATING_PRODUCT_CATALOG.fm;
  return {
    product,
    learner: {
      ...descriptor.lockLearner(row),
    ...
    remaining_hours: duration.remainingHours(remainingMs),
    remaining_days: duration.remainingDays(remainingMs),
    remaining_label: duration.formatRemainingLabel(remainingMs),
