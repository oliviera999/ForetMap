...
const { loadGameRosterForState } = require('../glRoster');
...
/**
 * Colonnes de quota par tour, sur liste fermée : jamais d'identifiant venu de la requête
 * dans le SQL (voir `claimTeamRoundAction`).
 */
const ROUND_ACTION_COLUMNS = {
  move: 'last_move_round_number',
  dice: 'last_dice_round_number',
};

/**
 * Réserve, pour une équipe, l'action « une seule fois par tour » (déplacement ou jet de dés).
 *
 * Les routes lisaient `last_*_round_number` puis, plus tard, l'écrivaient : entre les deux,
 * une seconde requête lisait la même valeur périmée. Deux clics simultanés — ou deux membres
 * de la même équipe — obtenaient donc deux déplacements ou deux jets pour le même tour.
 * L'UPDATE conditionnel ci-dessous fait lecture et écriture en une seule opération atomique :
 * la seconde ne touche aucune ligne, et l'appelant annule sa transaction.
 *
 * @param {'move'|'dice'} action
 * @returns {Promise<boolean>} `true` si le tour vient d'être consommé, `false` s'il l'était déjà.
 */
async function claimTeamRoundAction(tx, { gameId, teamId, roundNumber, action }) {
  const column = ROUND_ACTION_COLUMNS[action];
  if (!column) throw new Error(`Action de tour inconnue: ${action}`);
  const round = Number(roundNumber);
  if (!Number.isFinite(round) || round <= 0) return true;
  const result = await tx.execute(
    ...
    [round, teamId, gameId, round],
  );
  return Number(result?.affectedRows || 0) > 0;
}

/**
 * Applique le déplacement d'une mascotte (UPDATE gl_teams) dans une transaction.
 * Partagé par /events (MJ) et la route joueur /teams/:teamId/move pour garantir la
 * même validation (repère existant) et la même écriture (position + consommation du tour).
 *
 * @param {{ markerId?: number|null, xp?: number|null, yp?: number|null, roundNumber?: number|null }} move
 *   `markerId` prioritaire ; sinon `xp`/`yp` (déplacement libre). `roundNumber` (>0) marque
 *   le tour comme consommé pour l'équipe (last_move_round_number).
 * @throws {Error} status 404 message MARKER_NOT_FOUND si le repère est introuvable.
 */
async function applyTeamMoveTx(
  tx,
  { gameId, teamId, markerId = null, xp = null, yp = null, roundNumber = null },
) {
  const consumesRound = Number.isFinite(Number(roundNumber)) && Number(roundNumber) > 0;
  if (markerId != null) {
    ...
    await tx.execute(
      `UPDATE gl_teams
          SET position_marker_id = ?,
              position_x_pct = ?,
              position_y_pct = ?,
              ${consumesRound ? 'last_move_round_number = ?,' : ''}
              updated_at = NOW()
      ...
      consumesRound
        ...
        : [markerId, Number(marker.x_pct), Number(marker.y_pct), teamId, gameId],
    );
    return;
  }
  await tx.execute(
    `UPDATE gl_teams
        SET position_marker_id = NULL,
            ...
            updated_at = NOW()
      WHERE id = ? AND game_id = ?`,
    consumesRound ? [xp, yp, Number(roundNumber), teamId, gameId] : [xp, yp, teamId, gameId],
  );
}
...
async function readGameState(gameId) {
  const game = await queryOne(
    `SELECT g.id, g.class_id, g.chapter_id, g.name, g.status, g.current_team_id,
            g.current_round_number, g.current_round_started_at,
            ...
            g.board_movement_mode, g.board_path_start_index,
            ...
            ch.map_markers_visible AS chapter_map_markers_visible,
            ch.map_zones_visible AS chapter_map_zones_visible,
  ...
  const teams = await queryAll(
    `SELECT t.id, t.game_id, t.name, t.type, t.mascot_id, t.position_marker_id, t.color, t.created_at, t.updated_at,
            t.last_move_round_number, t.last_dice_round_number,
  ...
  // Mode classique : une équipe a « déjà bougé » ce tour si son dernier déplacement
  // correspond au tour courant (et qu'un tour est lancé).
  const currentRoundNumber = Number(game.current_round_number) || 0;
  for (const team of teams) {
    team.hasMovedThisRound =
      currentRoundNumber > 0 && Number(team.last_move_round_number || 0) >= currentRoundNumber;
    team.hasRolledDiceThisRound =
      currentRoundNumber > 0 && Number(team.last_dice_round_number || 0) >= currentRoundNumber;
  }
  ...
  const roster = await loadGameRosterForState(queryAll, gameId, {
    vitalityEnabled: settings.vitalityEnabled,
  });
  return {
    game,
    teams,
