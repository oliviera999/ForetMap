...
const { getPasswordMinLength } = require('../passwordReset');
...
async function importPlayersFromRows(parsedRows, { dryRun = false } = {}) {
  ...
  const errors = [];
  const validRows = [];
  // Plancher réglable (`security.password_min_length`), le même que ForetMap (CDG-47).
  const passwordMinLength = await getPasswordMinLength();
  for (let i = 0; i < payloads.length; i += 1) {
    const rowNumber = i + 2;
    const payload = payloads[i];
    const rowErrors = validatePlayerImportPayload(payload, rowNumber, { passwordMinLength });
