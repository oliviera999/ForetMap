...
/**
 * Type gnome / licorne déduit du premier mot du nom (section 2.4 / 10.2).
 * « gnomes sylvestres » → gnome, « licornes des eaux » → unicorn.
 */
function typeFromTeamName(name) {
  const first = String(name || '')
    ...
    .replace(/\p{M}/gu, '')
    .toLowerCase();
  if (first.startsWith('licorne')) return 'unicorn';
  if (first.startsWith('gnome')) return 'gnome';
  return null;
}
