...
/** Politiques d'équipes d'une classe (`gl_classes.team_policy`, migration 217, § 6). */
const TEAM_POLICIES = Object.freeze(['carry_over', 'reshuffle_each', 'reshuffle_per_plateau']);
...
/**
 * Réparation des verrous « ensemble » avant la recherche locale : ramène `b` dans l'équipe de
 * `a` (échange avec un membre libre — ni épinglé, ni lui-même verrouillé « ensemble » avec un
 * membre de cette équipe). La recherche locale ne fait ensuite que préserver l'acquis : un
 * échange qui séparerait la paire coûte `LOCK_VIOLATION_PENALTY`.
 */
function applyTogetherLocks(slots, locks, pinnedSet) {
  const together = Array.isArray(locks?.together) ? locks.together : [];
  ...
  const lockedWith = new Map();
  for (const [a, b] of together) {
    const x = Number(a);
    const y = Number(b);
    if (!lockedWith.has(x)) lockedWith.set(x, new Set());
    if (!lockedWith.has(y)) lockedWith.set(y, new Set());
    lockedWith.get(x).add(y);
    lockedWith.get(y).add(x);
  }
  for (const [a, b] of together) {
    const pa = locate(Number(a));
    const pb = locate(Number(b));
    if (!pa || !pb || pa.s === pb.s) continue;
    // Le membre épinglé reste ; sinon on déplace `b` vers `a` (ordre du verrou = ordre stable).
    const stay = pinnedSet.has(Number(b)) && !pinnedSet.has(Number(a)) ? pb : pa;
    const move = stay === pa ? pb : pa;
    if (pinnedSet.has(slots[move.s][move.i])) continue;
    const teamSet = new Set(slots[stay.s].map(Number));
    const swapIdx = slots[stay.s].findIndex((other) => {
      const o = Number(other);
      if (pinnedSet.has(o)) return false;
      const partners = lockedWith.get(o);
      if (!partners) return true;
      for (const p of partners) if (teamSet.has(p)) return false;
      return true;
    });
    const moving = slots[move.s][move.i];
    if (swapIdx >= 0) {
      slots[move.s][move.i] = slots[stay.s][swapIdx];
      slots[stay.s][swapIdx] = moving;
    } else {
      slots[move.s].splice(move.i, 1);
      slots[stay.s].push(moving);
    }
  }
}

/** Sépare une paire `apart` si elle est dans la même équipe (contrainte dure §10.3).
 * Échange (pas un déplacement) pour conserver les effectifs. */
function applyApartLocks(slots, locks, pinnedSet) {
  const apart = Array.isArray(locks?.apart) ? locks.apart : [];
  if (!apart.length || slots.length < 2) return;
  const together = Array.isArray(locks?.together) ? locks.together : [];
  const lockedWith = new Map();
  for (const [x, y] of together) {
    const a = Number(x);
    const b = Number(y);
    if (!lockedWith.has(a)) lockedWith.set(a, new Set());
    if (!lockedWith.has(b)) lockedWith.set(b, new Set());
    lockedWith.get(a).add(b);
    lockedWith.get(b).add(a);
  }
  const locate = (pid) => {
    for (let s = 0; s < slots.length; s += 1) {
      const i = slots[s].indexOf(pid);
  ...
  const tiedHere = (pid, slotIndex) => {
    const partners = lockedWith.get(Number(pid));
    if (!partners) return false;
    const team = new Set(slots[slotIndex].map(Number));
    for (const p of partners) if (team.has(Number(p))) return true;
    return false;
  };
  for (const [a, b] of apart) {
    ...
    const pb = locate(Number(b));
    if (!pa || !pb || pa.s !== pb.s) continue;
    const preferB = !pinnedSet.has(Number(b)) && !tiedHere(b, pb.s);
    const move = preferB ? pb : pa;
    const movingId = Number(slots[move.s][move.i]);
    if (pinnedSet.has(movingId)) continue;
    let dest = 0;
    let destSize = Infinity;
    for (let s = 0; s < slots.length; s += 1) {
      if (s === move.s) continue;
      if (slots[s].length < destSize) {
        destSize = slots[s].length;
        dest = s;
      }
    }
    const swapIdx = slots[dest].findIndex((other) => {
      const o = Number(other);
      if (pinnedSet.has(o)) return false;
      if (tiedHere(o, dest)) return false;
      return true;
    });
    if (swapIdx < 0) continue;
    const tmp = slots[move.s][move.i];
    slots[move.s][move.i] = slots[dest][swapIdx];
    slots[dest][swapIdx] = tmp;
  }
}

/**
 * Compose les équipes : départ équilibré, épingles, réparation des verrous, puis recherche
 * locale par échange.
 *
 * @param {object} params
...
function computeComposition({
  ...
  applyTogetherLocks(slots, locks, pinnedSet);
  applyApartLocks(slots, locks, pinnedSet);
...
/**
 * Séries de peuple par joueur d'après l'historique (le plus récent d'abord) : `type` du
 * dernier peuple joué et `streak` = nombre de parties consécutives récentes dans ce peuple.
 * Pur (lot v3, rotation des peuples § 4.3).
 *
 * @param {Array<{ teams: Array<{ type: string, memberIds: number[] }> }>} history
 * @returns {Map<number, { type: string, streak: number }>}
 */
function buildPeopleStreaks(history) {
  const out = new Map();
  const closed = new Set();
  for (const game of Array.isArray(history) ? history : []) {
    for (const team of game.teams || []) {
      const type = String(team.type || '').toLowerCase();
      if (!type) continue;
      for (const raw of team.memberIds || []) {
        const pid = Number(raw);
        if (closed.has(pid)) continue;
        const cur = out.get(pid);
        if (!cur) out.set(pid, { type, streak: 1 });
        else if (cur.type === type) cur.streak += 1;
        else closed.add(pid);
      }
    }
  }
...
/**
 * Choisit le peuple de la première équipe (les suivantes alternent) pour limiter les joueurs
 * qui joueraient un 3ᵉ tour consécutif dans le même peuple. À égalité, `gnome`.
 *
 * @returns {{ startWith: 'gnome'|'unicorn', thirdStreaks: number, alternative: number }}
 */
function choosePeopleStart({ slots, streaks, minStreak = 2 }) {
  const count = (startWith) => {
    let n = 0;
    (Array.isArray(slots) ? slots : []).forEach((team, idx) => {
      const type = idx % 2 === 0 ? startWith : startWith === 'gnome' ? 'unicorn' : 'gnome';
      for (const pid of team) {
        const s = streaks?.get(Number(pid));
        if (s && s.type === type && s.streak >= minStreak) n += 1;
      }
    });
    return n;
  };
  const gnomeFirst = count('gnome');
  const unicornFirst = count('unicorn');
  return unicornFirst < gnomeFirst
    ? { startWith: 'unicorn', thirdStreaks: unicornFirst, alternative: gnomeFirst }
    : { startWith: 'gnome', thirdStreaks: gnomeFirst, alternative: unicornFirst };
}
...
/**
 * API de composition alignée sur la spec Moodle §10.3 (réutilise le moteur existant).
 * `keepApart` est dur : si une paire reste ensemble, `ok` est faux.
 */
function composeTeams({ players, teams, history = [], options = {} }) {
  const teamCount = Array.isArray(teams) ? teams.length : 0;
  const seed = options.seed != null ? String(options.seed) : 'moodle-default';
  const rng = createSeededRng(seedFromLabel(seed));
  const windowN = Math.max(0, Math.trunc(Number(options.avoidRepeatWindow ?? 2)));
  const hist = Array.isArray(history) ? history : [];
  const sliced = windowN > 0 ? hist.slice(0, windowN) : [];
  const pairHistory = new Map();
  sliced.forEach((game, rank) => {
    const weight = 0.8 ** rank;
    for (const team of game.teams || []) {
      const ids = team.memberIds || [];
      for (let i = 0; i < ids.length; i += 1) {
        for (let j = i + 1; j < ids.length; j += 1) {
          const key = pairKey(ids[i], ids[j]);
          pairHistory.set(key, (pairHistory.get(key) || 0) + weight);
        }
      }
    }
  });
  const locks = {
    together: options.keepTogether || [],
    apart: options.keepApart || [],
  };
  const pool = (Array.isArray(players) ? players : []).map((p) => ({
    playerId: Number(p?.playerId ?? p?.id ?? p),
  }));
  ...
  const warnings = [];
  const slotOf = new Map();
  out.slots.forEach((slot, idx) => slot.forEach((pid) => slotOf.set(Number(pid), idx)));
  for (const [a, b] of locks.apart) {
    if (slotOf.get(Number(a)) != null && slotOf.get(Number(a)) === slotOf.get(Number(b))) {
      warnings.push({ code: 'keepApart', a, b });
    }
  }
  const sizes = out.slots.map((s) => s.length);
  const delta = sizes.length ? Math.max(...sizes) - Math.min(...sizes) : 0;
  const maxSizeDelta = Number.isFinite(Number(options.maxSizeDelta))
    ? Number(options.maxSizeDelta)
    : 1;
  if (delta > maxSizeDelta) warnings.push({ code: 'maxSizeDelta', delta, maxSizeDelta });
  const assignments = out.slots.map((memberIds, i) => ({
    team: teams[i] || { id: i },
    memberIds,
  }));
  return { ok: warnings.length === 0, assignments, warnings, slots: out.slots };
}
