...
async function handleQuestionDraw(
  ...
  req,
  res,
  { table, scopeColumn, scopeSlugs, emptyScopeError, excludeCodes = [] },
) {
  ...
  const exclude = excludeRaw
    ? excludeRaw
        ...
        .filter(Boolean)
    : [];
  // Questions réservées à la validation d'une fiche (sévérité `strict`) : jamais tirées au
  // hasard dans l'entraînement libre — sinon l'élève y verrait la bonne réponse sans enjeu.
  for (const code of Array.isArray(excludeCodes) ? excludeCodes : []) {
    if (code && !exclude.includes(code)) exclude.push(code);
  }
