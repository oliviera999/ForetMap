...
async function getEffectiveMaxActiveTaskAssignments(studentId) {
  ...
  const globalVal = Math.min(MAX_CONCURRENT_TASKS_CAP, Math.max(0, parseInt(globalRaw, 10) || 0));
  ...
  if (row && row.mct !== null && row.mct !== undefined) {
    const parsed = parseInt(row.mct, 10);
    if (!Number.isNaN(parsed)) {
      return Math.min(MAX_CONCURRENT_TASKS_CAP, Math.max(0, parsed));
    }
  }
  return globalVal;
}
...
async function countStudentActiveTaskAssignments(studentId, firstName, lastName) {
  ...
  // Deux homonymes ne partagent pas leur plafond : chaque ligne est rattachée par
  // identifiant dès qu'elle en porte un (cf. assignmentIdentityMatch).
  ...
  const row = await queryOne(
    `SELECT COUNT(DISTINCT ta.task_id) AS c
      ...
      WHERE LOWER(t.status) <> 'validated'
        -- Une tâche archivée ne demande plus rien à personne : elle ne doit pas
        -- consommer le quota. Sans ce filtre, un enseignant qui range ses vieilles
        -- tâches non validées laissait ses élèves bloqués, sans moyen de comprendre
        -- pourquoi — les tâches en cause ayant disparu de leur écran.
