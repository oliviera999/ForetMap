...
function applyDerivedGroup4IfEmpty(payload) {
  ...
  if (trimStr(payload.taxon_genus)) return;

  const kingdom = trimStr(payload.taxon_kingdom).toLowerCase();
  if (kingdom.includes('végétal')) {
    const family = trimStr(payload.taxon_family);
    if (family) payload.taxon_genus = family;
    return;
  }
  if (kingdom.includes('animal')) {
    const genus = extractGenusFromScientificName(payload.scientific_name);
    if (genus) payload.taxon_genus = genus;
  }
}
