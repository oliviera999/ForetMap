 ...
 *
 * Le libellé du catalogue — « Accès interface n3boss » — décrit un usage et non sa
 * portée réelle : décocher la case sur **Prof de classe** paraît cohérent (ce profil
 * n'a justement pas la barre haute n3boss) et coupait pourtant l'accès à tous les
 * tuteurs de classe. Depuis la migration `241`, une révocation est durable : elle
 * n'était plus rattrapée au redémarrage.
 *
 ...
 */
const TEACHER_ACCESS_LOCKED_ROLE_SLUGS = new Set(['admin', 'prof', 'prof_classe']);
...
function reservedRoleSlugError(slug) {
  ...
  return (
    'Ce slug est réservé au système (admin, n3boss, prof de classe, visiteur, personnel ou palier n3beur d’origine). ' +
...
function isStaffRoleSlug(slug) {
  const s = String(slug || '')
    .trim()
    .toLowerCase();
  return s === 'admin' || s === 'prof' || s === 'visiteur' || s === 'personnel';
}
