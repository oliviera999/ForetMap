...
} = require('./speciesAutofillPlantnet');
...
async function runSpeciesAutofillProviderSelfTest(options = {}) {
  ...
  const timeoutMs = Math.min(
    8000,
    Math.max(2000, Number(options.timeoutMs) > 0 ? Number(options.timeoutMs) : 4500),
  );
  ...
  const ok = (!plantnet.tested || plantnet.ok === true) && (!openai.tested || openai.ok === true);
