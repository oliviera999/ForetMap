...
/**
 * Le feuillet appartient-il au pool du chapitre (ou au lot d'ouverture) ?
 * Sert de garde à la présentation directe : un joueur ne peut faire apparaître
 * qu'un feuillet que son chapitre est censé proposer.
 * @returns {Promise<boolean>}
 */
async function isFeuilletInChapterPool(deps, { chapterId, feuilletCode }) {
  const code = String(feuilletCode || '').trim();
  if (!code) return false;
  const offert = await deps.queryOne(
    'SELECT 1 AS ok FROM gl_lore_feuillets WHERE feuillet_code = ? AND offert_ouverture = 1 LIMIT 1',
    [code],
  );
  ...
  const row = await deps.queryOne(
    `SELECT 1 AS ok FROM gl_lore_feuillets f
      WHERE f.feuillet_code = ? AND ${built.clause} LIMIT 1`,
    [code, ...built.params],
  );
  return !!row;
}
