...
function canViewRestrictedNote(entity, auth, { publicSurface = false } = {}) {
  ...
  const explicit = parseRoleSlugList(entity?.restricted_note_role_slugs);
  const audience = explicit.length === 0 ? RESTRICTED_NOTE_DEFAULT_ROLE_SLUGS : explicit;
  const role = resolveViewerRoleSlug(auth, { publicSurface });
  return role != null && audience.includes(role);
}
...
function canViewLocationLink(link, auth, { publicSurface = false } = {}) {
  if (isLocationManager(auth)) return true;
  const audience = parseRoleSlugList(link?.audience_role_slugs);
  if (audience.length === 0) return true;
  const role = resolveViewerRoleSlug(auth, { publicSurface });
  return role != null && audience.includes(role);
}
...
function projectLocationLinksForViewer(links, auth, { publicSurface = false } = {}) {
  ...
  for (const link of links) {
    ...
    const { audience_role_slugs: _audience, ...rest } = link;
    out.push(rest);
  }
  return out;
}
