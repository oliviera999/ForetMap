 ...
 *   slugs vides pour le complément = audience par défaut
 *   (`RESTRICTED_NOTE_DEFAULT_ROLE_SLUGS` : administrateur, n3boss, prof de classe),
 *   en plus des gestionnaires du jardin.
...
/**
 * Audience par défaut du complément réservé (aucune case cochée) : l'encadrement.
 ...
 */
const RESTRICTED_NOTE_DEFAULT_ROLE_SLUGS = Object.freeze(['prof_classe', 'prof', 'admin']);
...
function readAudienceWriteFields(body = {}) {
  ...
  const noteGroups = normalizeGroupIdInput(body.restricted_note_group_ids, {
    field: 'restricted_note_group_ids',
  });
  if (!noteGroups.ok) return noteGroups;
  return {
    ok: true,
    ...
    restricted_note_group_ids: noteGroups.value,
  };
}
...
async function assertAudienceGroupsExist(db, audienceInput) {
  for (const field of ['visible_group_ids', 'restricted_note_group_ids']) {
...
function canViewRestrictedNote(entity, auth, { publicSurface = false } = {}) {
  ...
  const audience = {
    roleSlugs: parseRoleSlugList(entity?.restricted_note_role_slugs),
    groupIds: parseGroupIdList(entity?.restricted_note_group_ids),
  };
  // Rien de coché = encadrement (comportement historique du complément réservé) : un
  // complément est confidentiel par nature, il ne s'ouvre pas tout seul.
  if (isEmptyAudience(audience)) {
    const role = resolveViewerRoleSlug(auth, { publicSurface });
    return role != null && RESTRICTED_NOTE_DEFAULT_ROLE_SLUGS.includes(role);
  }
  return viewerMatchesAudience(audience, auth, { publicSurface });
}
...
function withLocationAudienceFields(row) {
  ...
  return {
    ...
    restricted_note_group_ids: parseGroupIdList(row.restricted_note_group_ids),
  };
}
...
function projectLocationAudienceForViewer(row, auth, { publicSurface = false } = {}) {
  ...
  if (!canViewRestrictedNote(base, auth, { publicSurface })) {
    ...
    delete out.restricted_note_group_ids;
  } else {
    delete out.restricted_note_role_slugs;
    delete out.restricted_note_group_ids;
  }
