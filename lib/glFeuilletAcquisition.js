...
const { lockGlTeamRow } = require('./glVitality');
...
/**
 * Le corpus nomme les canaux en français (`espece`, `glossaire`…), les ressources
 * consultables en anglais (`species`, `glossary`…). Sans cette table, la recherche
 * du « feuillet dédié à cet élément » ne pouvait jamais aboutir et l'on retombait
 * toujours sur le premier feuillet du pool.
 */
const SOURCE_CANAL_ALIASES = Object.freeze({
  species: ['species', 'espece'],
  glossary: ['glossary', 'glossaire'],
  lore_glossary: ['lore_glossary', 'glossaire_lore'],
  tutorial: ['tutorial', 'tutoriel'],
  content_page: ['content_page', 'page', 'page_contenu'],
  ecosystem: ['ecosystem', 'ecosysteme'],
});

function canalCandidatesForSource(source) {
  const key = String(source || '').trim();
  return SOURCE_CANAL_ALIASES[key] || (key ? [key] : []);
}
...
async function pickFeuilletForConsultation(deps, { gameId, teamId, chapterId, source, sourceRef }) {
  ...
  const canaux = canalCandidatesForSource(source);
  if (canaux.length && sourceRef) {
    const linked = await deps.queryOne(
      `SELECT ${FEUILLET_SELECT}
         FROM gl_lore_feuillets f
        WHERE f.statut = 'actif'
          AND f.lien_canal IN (${canaux.map(() => '?').join(', ')})
          AND f.lien_ref = ?
        LIMIT 1`,
      [...canaux, sourceRef],
    );
...
module.exports = {
  SOURCE_CANAL_ALIASES,
  canalCandidatesForSource,
  resolveLoreSettings,
  loadTeamFoundCodes,
