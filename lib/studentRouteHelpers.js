...
function buildRoleAliasesFromDbRows(roleRows) {
  ...
  for (const row of roleRows || []) {
    ...
    if (!IMPORT_ROLE_SLUGS.has(slug)) continue;
...
/**
 * Un n3boss ne peut pas modifier un administrateur existant (mot de passe, profil).
 * Miroir de `PUT /api/rbac/users/:id/role`.
 */
function canActorMutateImportedAdmin(auth, existingRoleSlug) {
  if (!isAdminRoleSlug(existingRoleSlug)) return true;
  return (
 ...
 * multi-groupes, chemin `Parent>Enfant`, absence de groupe, affiliation vide, ligne
...
function buildTemplateWorkbookRows() {
  ...
  const row = (role, firstName, lastName, password, affiliation, groups, pseudo, email, note) => ({
    [col[0]]: role,
    [col[1]]: firstName,
    [col[2]]: lastName,
    [col[3]]: password,
  ...
  return [
    ...
    row(
      ...
      'Même profil écrit avec son nom affiché : slug et libellé sont acceptés indifféremment. Remplacer ou supprimer.',
    ),
    row(
      'eleve_novice',
      'Nina',
      ...
      'Deux classes séparées par « | » (le « ; » marche aussi). Affiliation n3 = plan N3 seulement. Remplacer ou supprimer.',
    ),
    row(
      'n3beur novice',
      'Noé',
      ...
      '',
      '',
      'Ligne minimale : nom affiché du profil, affiliation vide (= both), pseudo / e-mail / description laissés vides. Remplacer ou supprimer.',
    ),
    row(
      'Élève avancé',
      'Ava',
    ...
    row(
      ...
      'Sans groupe : le rattachement peut se faire plus tard. Affiliation : n3, foret, both — ou l’identifiant d’une carte existante pour n’ouvrir que celle-ci. Remplacer ou supprimer.',
    ),
    row(
      'eleve_avance',
      'Dora',
