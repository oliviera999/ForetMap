...
} = require('./legacyZoneShapeConvert');
...
function readSqliteGardenRows(sqlite, options = {}) {
  ...
  for (const marker of markers) {
    ...
    else skippedMarkers.push({ id: marker.id, label: marker.label, reason: 'coordonnees_invalides' });
  }
  return { mapId, zones: normalizedZones, markers: normalizedMarkers, skippedZones, skippedMarkers };
}
...
function buildGardenImportSql(sqlite, options = {}) {
  const { mapId, zones, markers, skippedZones, skippedMarkers } = readSqliteGardenRows(sqlite, options);
