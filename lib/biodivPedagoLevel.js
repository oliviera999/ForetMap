 ...
 *
 * `curriculum_niveau` suit l'échelle unique de l'apprenant depuis la migration 301 :
 * `universite` y est accepté (et donne l'étape Université). Le résolveur de l'apprenant
 * (lib/pedago/learnerLevel.js) fait primer ce niveau de classe sur `level`, qui n'est plus
 * qu'un repli.
 *
...
function resolveGroupPedagoProfile(groupId, groupsById) {
  ...
  while (cursor && !visited.has(cursor) && groupsById.has(cursor)) {
    ...
    const ownNiveau = normalizeLearnerNiveau(row.curriculum_niveau);
    if (!curriculumNiveau && ownNiveau) curriculumNiveau = ownNiveau;
    if (!level) level = normalizePedagoLevel(row.pedago_level) || etapeForLearnerNiveau(ownNiveau);
...
async function loadUserGroupPedagoProfile(userId) {
  ...
  return { levels, curriculumNiveaux: sortLearnerNiveaux([...curriculumNiveaux]) };
}
