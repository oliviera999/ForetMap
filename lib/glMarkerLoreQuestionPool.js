...
async function drawLoreQuestionFromMarker(deps, marker, chapterPlateauNumber, excludeCodes = []) {
  ...
  if (questionCfg.mode === 'fixed') {
    ...
    if (!row) return { error: 'Question fixe introuvable ou inactive', questionCode: null, qcmSet: 'lore' };
  ...
  if (items.length === 0) {
    return { error: 'Aucune question lore présentable dans le pool', questionCode: null, qcmSet: 'lore' };
  }
