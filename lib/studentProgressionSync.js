 ...
 *
 * Périmètre : uniquement les comptes élèves actifs dont le **profil effectif** est un palier
 * n3beur. Les profils hors échelle (n3boss, admin, MJ, prof de classe, visiteur, profil
 * maison) ne sont jamais touchés — c'est `syncStudentPrimaryRoleFromProgress` qui refuse, et
 * le compte est reporté en « ignoré » avec son motif.
 */
...
/** Élèves actifs dont le profil effectif est un palier de l'échelle n3beur. */
const N3BEUR_MEMBERS_SQL = `
  SELECT DISTINCT u.id, u.first_name, u.last_name, r.slug AS role_slug,
   ...
   WHERE u.user_type = 'student'
     AND u.is_active = 1
     AND (r.slug LIKE 'eleve\\_%' OR r.min_done_tasks IS NOT NULL)`;
...
async function listProgressionCandidates({ scope, groupId = null, userId = null }) {
  if (scope === 'group') {
    return queryAll(
      `${N3BEUR_MEMBERS_SQL}
         AND EXISTS (SELECT 1 FROM group_members gm WHERE gm.user_id = u.id AND gm.group_id = ?)
