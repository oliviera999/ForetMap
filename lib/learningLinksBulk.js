'use strict';

// =====================================================================
// Rattachements question <-> ressource : approbation EN LOT et garde-fou de type.
// Commun aux deux produits (tables `resource_question_links` et `gl_resource_question_links`).
//
// Deux problemes constates a l'usage :
//
//  1. Le rattachement automatique insere en `status = 'suggested'`, alors que le
//     conditionnement n'accepte que `'approved'`. Sur quarante propositions, il fallait
//     donc quarante changements de liste deroulante, un par ligne. Personne n'allait au
//     bout : l'ecran produisait des liens que rien n'activait jamais.
//
//  2. Un lien peut viser un type que le produit ne sait pas VALIDER : un lien bloquant y
//     est accepte sans un mot et reste inerte pour toujours, pendant que le professeur
//     croit avoir conditionne la fiche. C'etait le cas du glossaire ForetMap, qui n'avait
//     aucun bouton « marquer ».
//
//     ETAT ACTUEL : depuis que le glossaire ForetMap se valide (migration 201), TOUS les
//     types liables sont validables des deux cotes — la garde ne se declenche donc plus
//     aujourd'hui. Elle est conservee a dessein : c'est un controle d'INVARIANT, pas un
//     reglage. Elle se declenchera le jour ou un type liable sera ajoute sans geste de
//     validation, c'est-a-dire exactement la situation qu'on vient de corriger. Sa logique
//     reste couverte par les tests unitaires de ce module.
// =====================================================================

const { gatingProductOrDefault } = require('./pedago/gatingProductCatalog');

/** Plafond d'une approbation en lot : garde-fou, et invite a filtrer plutot qu'a tout prendre. */
const BULK_MAX = 200;

// Table des liens et types validables : portés par le catalogue des produits (seul « gl »,
// casse ignorée, désigne GL — lecture historique, inchangée).
function tableFor(product) {
  return gatingProductOrDefault(product).tables.links;
}

function markableFor(product) {
  return gatingProductOrDefault(product).markable;
}

/**
 * Le produit sait-il VALIDER ce type de ressource ? Un lien bloquant sur un type non
 * validable ne conditionnera jamais rien.
 */
function isMarkableResourceType(product, resourceType) {
  return markableFor(product).has(String(resourceType || ''));
}

/**
 * Message expliquant pourquoi un lien bloquant est refuse sur ce type — il doit dire ce
 * qui manque, pas seulement que c'est interdit.
 */
function nonMarkableGatingError(product, resourceType) {
  const known = [...markableFor(product)].join(', ');
  return (
    `Le type « ${resourceType} » n'a pas de validation de lecture dans cette application : ` +
    `un lien bloquant y resterait sans effet. Types validables : ${known}. ` +
    `Le lien reste possible en non bloquant (valeur documentaire).`
  );
}

/**
 * Approbation / rejet en lot des rattachements, pour les deux produits.
 *
 * Deux formes, et une seule route par produit (`POST .../review`) :
 *   - par `ids` : agit sur les lignes designees, quel que soit leur statut — c'est la
 *     forme historique, inchangee ;
 *   - par ressource : agit sur les seules propositions (`status = 'suggested'`) de cette
 *     ressource. C'est celle qui manquait, et sans laquelle le rattachement automatique
 *     ne debouchait sur rien.
 *
 * Ne touche QUE `status` : le caractere bloquant (`is_gating`) reste la decision explicite
 * du professeur, ligne par ligne. Approuver n'est pas conditionner.
 *
 * @param {object} db objet portant `execute`
 * @param {object} params
 * @param {'fm'|'gl'} params.product
 * @param {'approved'|'rejected'} params.status
 * @param {Array<number|string>} [params.ids]
 * @param {string} [params.resourceType]
 * @param {string} [params.resourceRef]
 * @returns {Promise<{ updated: number }>}
 */
async function reviewSuggestedLinks(db, { product, status, ids, resourceType, resourceRef } = {}) {
  const table = tableFor(product);
  if (status !== 'approved' && status !== 'rejected') return { updated: 0 };
  // Approuver ou rejeter change l'ensemble des questions réservées au tirage libre.
  require('./learningGatingLockMode').invalidateStrictCodesCache();

  const wanted = (Array.isArray(ids) ? ids : [])
    .map((id) => Number(id))
    .filter((id) => Number.isFinite(id) && id > 0)
    .slice(0, BULK_MAX);

  if (wanted.length > 0) {
    const placeholders = wanted.map(() => '?').join(', ');
    const res = await db.execute(
      `UPDATE ${table} SET status = ?, updated_at = NOW() WHERE id IN (${placeholders})`,
      [status, ...wanted],
    );
    return { updated: res?.affectedRows || 0 };
  }

  if (!resourceType || !resourceRef) return { updated: 0 };
  const res = await db.execute(
    `UPDATE ${table} SET status = ?, updated_at = NOW()
      WHERE status = 'suggested' AND resource_type = ? AND resource_ref = ?`,
    [status, resourceType, resourceRef],
  );
  return { updated: res?.affectedRows || 0 };
}

/**
 * Rend bloquantes (ou non) des questions rattachées, en lot — le geste explicite qui manquait
 * une fois qu'approuver ne conditionne plus rien (docs/AUDIT_VALIDATION_QUIZ_2026-09.md, lot 4).
 *
 * Deux formes, comme `reviewSuggestedLinks` :
 *   - par `ids` : les lignes désignées, quel que soit leur statut ;
 *   - par ressource : les seuls liens APPROUVÉS de cette ressource (un lien proposé ou
 *     rejeté reste inerte de toute façon).
 * Rendre bloquant un type que le produit ne sait pas valider est refusé (`ok: false`).
 *
 * @returns {Promise<{ ok: boolean, updated: number, error?: string }>}
 */
async function setLinksGating(db, { product, isGating, ids, resourceType, resourceRef } = {}) {
  const table = tableFor(product);
  const flag = isGating ? 1 : 0;
  if (flag && resourceType && !isMarkableResourceType(product, resourceType)) {
    return { ok: false, updated: 0, error: nonMarkableGatingError(product, resourceType) };
  }
  require('./learningGatingLockMode').invalidateStrictCodesCache();

  const wanted = (Array.isArray(ids) ? ids : [])
    .map((id) => Number(id))
    .filter((id) => Number.isFinite(id) && id > 0)
    .slice(0, BULK_MAX);

  if (wanted.length > 0) {
    const placeholders = wanted.map(() => '?').join(', ');
    // Par identifiants : le type validable est vérifié ligne par ligne, en SQL, pour ne pas
    // rendre bloquant un lien d'un type inerte glissé dans la liste.
    const markable = [...markableFor(product)];
    const typeGuard = flag ? ` AND resource_type IN (${markable.map(() => '?').join(', ')})` : '';
    const res = await db.execute(
      `UPDATE ${table} SET is_gating = ?, updated_at = NOW()
        WHERE id IN (${placeholders})${typeGuard}`,
      [flag, ...wanted, ...(flag ? markable : [])],
    );
    return { ok: true, updated: res?.affectedRows || 0 };
  }

  if (!resourceType || !resourceRef) return { ok: true, updated: 0 };
  const res = await db.execute(
    `UPDATE ${table} SET is_gating = ?, updated_at = NOW()
      WHERE status = 'approved' AND resource_type = ? AND resource_ref = ? AND is_gating <> ?`,
    [flag, resourceType, resourceRef, flag],
  );
  return { ok: true, updated: res?.affectedRows || 0 };
}

module.exports = {
  BULK_MAX,
  isMarkableResourceType,
  nonMarkableGatingError,
  reviewSuggestedLinks,
  setLinksGating,
};
