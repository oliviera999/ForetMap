...
function computeTaskStatusFromProgress({
  currentStatus,
  completionMode,
  assignedCount,
  doneCount,
...
async function recalculateTaskStatusWithConn(conn, taskLike) {
  const task =
    typeof taskLike === 'string'
      ? await conn.queryOne('SELECT id, status, completion_mode FROM tasks WHERE id = ?', [
          taskLike,
        ])
      : taskLike;
  ...
  if (nextStatus !== currentStatus) {
    // Écriture conditionnée à l'état lu (compare-and-set). `task` vient souvent d'une
    // lecture antérieure, hors transaction : entre cette lecture et l'écriture, une
    // validation par un enseignant peut avoir eu lieu. Un `UPDATE` inconditionnel la
    // rétrogradait alors silencieusement — `computeTaskStatusFromProgress` protège bien
    // `validated`, mais sur une valeur devenue périmée. La clause `AND status = ?`
    // referme la fenêtre : si l'état a bougé, l'écriture ne s'applique simplement pas,
    // et le recalcul suivant repartira de la valeur à jour.
    // `<=>` et non `=` : une tâche héritée peut avoir `status` à NULL, et `= NULL` ne
    // correspond jamais — l'écriture serait alors perdue au lieu d'être conditionnée.
    await conn.execute('UPDATE tasks SET status = ? WHERE id = ? AND status <=> ?', [
      nextStatus,
      task.id,
      task.status ?? null,
    ]);
