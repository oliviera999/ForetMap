 ...
 * @param {object|string|number} taskLike Ligne `tasks` avec id (status/completion_mode ignorés
 *   s'ils sont fournis) ou id seul. Le statut courant est **toujours** relu en base pour
 *   éviter d'écraser `validated` / `on_hold` / `proposed` avec un objet périmé (course avec
 *   POST /validate pendant assign / done / unassign / assign-group).
 */
async function recalculateTaskStatusWithConn(conn, taskLike) {
  const taskId =
    typeof taskLike === 'string' || typeof taskLike === 'number'
      ? taskLike
      : taskLike && taskLike.id != null
        ? taskLike.id
        : null;
  if (taskId == null || taskId === '') return null;
  // Toujours relire : l'appelant peut passer un snapshot lu avant une validation concurrente.
  const task = await conn.queryOne('SELECT id, status, completion_mode FROM tasks WHERE id = ?', [
    taskId,
  ]);
