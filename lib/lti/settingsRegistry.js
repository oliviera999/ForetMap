...
// « queue » (file d'attente des inconnus) n'a jamais été branché : l'accepter en réglage
// promettait un comportement inexistant (CDG-53). Seul « refuse » est câblé.
const UNKNOWN_USER_VALUES = Object.freeze(['refuse']);
...
function validateUnknownUser(value) {
  ...
  if (!UNKNOWN_USER_VALUES.includes(v))
    return 'unknown_user doit valoir refuse (seule valeur câblée)';
  return null;
}
