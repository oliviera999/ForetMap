...
async function runLtiCheck({ env = readLtiEnv(), fetchImpl = nodeHttpFetch } = {}) {
  const report = {
    ...
    toolKeySource: env.toolPrivateKeySource || null,
    jwksOk: false,
    jwksKeys: 0,
    toolJwkOk: false,
    errors: [],
  };
  if (env.toolPrivateKeyError) {
    report.errors.push({ step: 'tool_key', message: env.toolPrivateKeyError });
  }
  if (!env.configured) {
    report.errors.push({
      step: 'env',
      message:
        'Secrets LTI absents — sur cPanel préférer LTI_TOOL_PRIVATE_KEY_FILE (fichier PEM) ou LTI_TOOL_PRIVATE_KEY_B64',
    });
    return report;
  }
  try {
