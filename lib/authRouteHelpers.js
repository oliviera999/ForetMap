 ...
 *   type. Le profil « Personnel » peut être porté par un compte `student` (comptes créés
 *   avant le réalignement du 22/09/2026) comme par un compte `teacher` (le défaut depuis) :
 *   le refuser au motif qu'il n'est pas enseignant fermait proflyautey à toute la vie
 *   scolaire ;
...
/**
 * Redirection d'échec OAuth.
 *
 * `roleLabel` (facultatif) nomme le profil que le serveur a vu sur le compte. Un refus qui ne
 * dit pas *quel* profil a été refusé n'apprend rien à la personne ni à l'administrateur
 * qu'elle va voir : c'est le profil de son propre compte, rien d'autre n'est révélé.
 */
function buildOAuthFrontendErrorRedirect(frontendOrigin, code, mode, roleLabel) {
  const base = String(frontendOrigin || '').replace(/\/+$/, '');
  const label = String(roleLabel || '').trim();
  const role = label ? `&role=${encodeURIComponent(label.slice(0, 80))}` : '';
  return `${base}/#oauth_error=${encodeURIComponent(code)}&mode=${encodeURIComponent(normalizeOAuthMode(mode))}${role}`;
}
