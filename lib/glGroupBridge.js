...
async function resolveInsertPseudo(pseudo, glPlayerId) {
  const base = String(pseudo || '').trim();
  if (!base) {
    return glPlayerId ? `gl-player-${glPlayerId}` : null;
  }
  const conflict = await queryOne(
    'SELECT id, user_type FROM users WHERE LOWER(pseudo) = LOWER(?) LIMIT 1',
    [base],
  );
  if (!conflict) return base;
  if (conflict.user_type === 'student') return base;
  ...
  if (!user) {
    ...
    try {
      ...
      if (err?.errno === 1062 || err?.code === 'ER_DUP_ENTRY') {
        ...
        if (!user && insertPseudo !== normalizedPseudo) {
          user = await findStudentUser({
            linkedId: null,
            email: null,
            pseudo: insertPseudo,
    ...
    await execute(
      ...
      [
        String(firstName || '').trim(),
        String(lastName || '').trim(),
        displayName || normalizedPseudo || user.display_name,
