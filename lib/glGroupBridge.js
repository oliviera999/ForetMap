...
async function ensureForetmapGroupForGlClass(classRow, options = {}) {
  ...
  const slugBase = slugify(`gl-class-${classId}-${classRow?.name || 'classe'}`);
  const slug = slugBase || `gl-class-${classId}`;
  // Sans profil demandé : « Visiteur », c'est-à-dire aucun effet (le profil attribué de
  // l'élève reste le plus élevé). Un profil de palier n3beur ouvre la carte de travail.
  const visitorRole = await getRoleBySlug('visiteur');
  let defaultRoleId = options.defaultRoleId != null ? Number(options.defaultRoleId) : null;
  if (defaultRoleId == null && options.grantsN3beurAccess) {
    defaultRoleId = (await getRoleBySlug('eleve_novice'))?.id ?? null;
  }
  if (defaultRoleId == null) defaultRoleId = visitorRole?.id ?? null;
  ...
  await execute(
    ...
    [groupId, slug, name, `Groupe lié à la classe GL #${classId}`, defaultRoleId],
  );
...
async function upsertForetmapUserForGlPlayer({
    ...
    try {
      await execute(
        ...
        (id, user_type, legacy_user_id, email, pseudo, first_name, last_name, display_name,
         description, avatar_path, password_hash, auth_provider, password_must_reset,
         is_active, last_seen, created_at, updated_at)
       VALUES (?, 'student', NULL, ?, ?, ?, ?, ?, NULL, NULL, ?, 'gl_bridge', ?, 1, NOW(), NOW(), NOW())`,
        [
          userId,
          insertEmail,
          insertPseudo || null,
      ...
      if (err?.errno === 1062 || err?.code === 'ER_DUP_ENTRY') {
        ...
        await execute(
          ...
          (id, user_type, legacy_user_id, email, pseudo, first_name, last_name, display_name,
           description, avatar_path, password_hash, auth_provider, password_must_reset,
           is_active, last_seen, created_at, updated_at)
         VALUES (?, 'student', NULL, NULL, ?, ?, ?, ?, NULL, NULL, ?, 'gl_bridge', ?, 1, NOW(), NOW(), NOW())`,
          [
            userId,
            retryPseudo || null,
            String(firstName || '').trim(),
  ...
  await execute(
    `INSERT INTO group_members (group_id, user_id, user_type)
     VALUES (?, ?, 'student')
     ON DUPLICATE KEY UPDATE user_type = 'student'`,
    [group.id, user.id],
  );

  await pruneOtherGlClassGroupMemberships(user.id, group.id);

  await recomputeUserRole(user.id);
