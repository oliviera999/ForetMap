...
const { createWriteVersionCache } = require('./shared/writeVersionCache');
    ...
    for (const rc of refColumns) {
      for (const ref of extractMediaUrlRefs(row[rc.column])) {
        addUsage(
          ...
          lookup,
          { kind: 'url', value: ref },
          { ...locBase, field: rc.label || rc.column },
        );
      }
    }
      ...
      if (isIntro) {
        for (const slugRef of extractIntroSlugRefs(value)) {
          addUsage(
            usageMap,
            lookup,
            { kind: 'slug', value: slugRef.slug },
            { ...locBase, field: slugRef.field },
          );
        }
      }
...
const GL_SOURCES = [
  ...
  },
  {
    ...
    label: 'Article de carnet joueur',
  ...
  for (const item of itemList) {
    for (const location of conventionLocationsForItem(item)) {
      addUsage(
        usageMap,
        resolvedLookup,
        { kind: 'slug', value: item.stableKey },
        {
          app: 'gl',
          table: 'convention',
          id: item.stableKey,
...
/**
 * Cache de l'usage des médias, invalidé par la **version d'écriture globale**.
 *
 * Pourquoi : `collectMediaLibraryUsage` balaie 15 tables sources et exécute, pour chacune,
 * un `SHOW COLUMNS` puis un `SELECT`. Soit une trentaine de requêtes SQL et autant de
 * balayages à chaque ouverture de l'onglet Médiathèque — un coût fixe, payé intégralement,
 * qui croît avec la taille des tables et non avec ce que l'écran affiche
 * (cf. docs/AUDIT_CHARGE_BIODIVERSITE_2026-09.md, T1).
 *
 * Pourquoi la version d'écriture et non un simple TTL : l'usage sert à prévenir avant une
 * suppression (« ce média est utilisé à N endroits »). Un résultat périmé de quelques
 * minutes ferait supprimer un média venant d'être référencé. Ici, toute écriture passée par
 * les helpers de `database.js` périme le cache instantanément — dont l'import et la
 * suppression de médias eux-mêmes, qui écrivent au journal d'audit. Le TTL du module partagé
 * ne reste qu'un garde-fou pour les écritures hors process.
 *
 * @param {object} options
 * @param {() => number} options.writeVersion Version d'écriture globale (invalidation).
 */
function createMediaLibraryUsageCache(options = {}) {
  return createWriteVersionCache({ ...options, name: 'createMediaLibraryUsageCache' });
}
 ...
 *
 * @param {object} deps `{ queryAll }`.
 * @param {object} [options]
 * @param {string} [options.app] Produit ciblé (`foretmap` | `gl`).
 * @param {{get: Function, set: Function}} [options.cache] Cache d'usage (voir
 *   `createMediaLibraryUsageCache`). Absent — dans les tests, notamment — le scan a lieu à
 *   chaque appel. La charge utile rendue depuis le cache est partagée entre appelants : elle
 *   est sérialisée telle quelle par les routes et ne doit pas être mutée.
 */
async function collectMediaLibraryUsage(deps, { app, cache = null } = {}) {
  const queryAll = deps?.queryAll;
  if (typeof queryAll !== 'function') {
    throw new Error('collectMediaLibraryUsage requiert deps.queryAll');
  }
  const cacheKey = String(app || 'all');
  ...
  for (const source of sourcesForApp(app)) {
    try {
      ...
      const rows = await queryAll(
        `SELECT ${selectSql} FROM \`${source.table}\` LIMIT ${ROW_LIMIT}`,
      );
