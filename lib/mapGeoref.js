...
function withMapGeoref(row) {
  ...
  const {
    geo_anchors_json: rawAnchors,
    gps_enabled,
    heading_up_enabled,
    scale_compass_enabled: rawScaleCompass,
    ...rest
  } = row;
  const georef = parseAnchors(rawAnchors);
  const gpsOn = !!gps_enabled && !!georef;
  /** Échelle + rose des vents : dès qu'un calage valide existe (indépendant de gps_enabled). */
  const scaleRaw = rawScaleCompass == null ? 1 : rawScaleCompass;
  return {
    ...rest,
    georef,
    gps_enabled: gpsOn,
    /** Orientation boussole : autorisée seulement si le suivi GPS l'est aussi. */
    heading_up_enabled: gpsOn && !!Number(heading_up_enabled),
    scale_compass_enabled: !!georef && !!Number(scaleRaw),
  };
}
