...
function parseHttpLogMode() {
  const raw = String(process.env.FORETMAP_HTTP_LOG || '')
    .trim()
    .toLowerCase();
...
function shouldSkipHttpMetricsAndLog(req) {
  ...
  if (
    ...
    p === '/api/health/db' ||
    p === '/api/version' ||
    // Sonde de polling différentiel : appelée chaque minute par chaque client.
    p === '/api/sync-state'
  ) {
    return true;
  }
...
function createHttpRequestLogMiddleware() {
  // Réglages résolus une seule fois à la création du middleware (au démarrage du
  // serveur), pas à chaque requête : `FORETMAP_HTTP_LOG` / `FORETMAP_HTTP_SLOW_MS`
  // ne changent pas en cours d'exécution.
