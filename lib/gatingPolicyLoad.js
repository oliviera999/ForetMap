...
// La table vient du catalogue des produits (`lib/pedago/gatingProductCatalog.js`).

const { GATING_PRODUCT_CATALOG, exactGatingProduct } = require('./pedago/gatingProductCatalog');

/**
 * Table des politiques : « gl » EXACTEMENT désigne GL, toute autre valeur ForetMap (lecture
 * historique de ce module, inchangée).
 */
function policyTable(product) {
  return (exactGatingProduct(product) || GATING_PRODUCT_CATALOG.fm).tables.policy;
}

async function loadResourcePolicy(db, product, resourceType, resourceRef) {
  const table = policyTable(product);
  try {
