...
function validatePayload(payload, rowNumber) {
  const errors = [];
  if (!payload.lore_code) errors.push({ row: rowNumber, field: 'lore_code', error: 'Code lore requis' });
  if (!payload.terme) errors.push({ row: rowNumber, field: 'terme', error: 'terme requis' });
  if (!payload.categorie) errors.push({ row: rowNumber, field: 'categorie', error: 'categorie invalide' });
  return errors;
}

function resolveLoreGlossaryImportBody(body = {}) {
...
function parseLoreGlossaryWorkbook(buffer, options = {}) {
...
function buildLoreGlossaryTemplateWorkbook() {
  const wb = XLSX.utils.book_new();
  const headers = ['id', 'terme', 'variantes', 'categorie', 'niveau', 'definition_courte', 'definition_complete', 'role_recit', 'correspondance_reelle', 'chapitre', 'termes_lies', 'source', 'statut'];
  XLSX.utils.book_append_sheet(
    wb,
    XLSX.utils.aoa_to_sheet([headers, ['LR0001', 'la Trame', 'Trame', 'cosmologie', 'cle', 'Définition courte', '', '', '', 'tous', '', '', 'actif']]),
    GLOSSARY_SHEET
  );
...
function buildLoreGlossaryExportWorkbook(rows) {
  const wb = XLSX.utils.book_new();
  const headers = ['id', 'terme', 'variantes', 'categorie', 'niveau', 'definition_courte', 'definition_complete', 'role_recit', 'correspondance_reelle', 'chapitre', 'termes_lies', 'source', 'statut'];
  const data = [headers];
  for (const row of rows || []) {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(data), GLOSSARY_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}
