...
function getMascotPackValidatorCandidates() {
  const seen = new Set();
  const out = [];
  const hasLibMirror = (root) =>
    fs.existsSync(path.join(root, 'lib', 'visit-pack', 'mascotPack.js'))
    && fs.existsSync(path.join(root, 'lib', 'visit-pack', 'visitMascotState.js'))
    && fs.existsSync(path.join(root, 'lib', 'visit-pack', 'visitMascotInteractionEvents.js'))
    && fs.existsSync(path.join(root, 'lib', 'visit-pack', 'visitMascotDialogEvents.js'))
    && fs.existsSync(path.join(root, 'lib', 'visit-pack', 'visitMascotDialogApply.js'))
    && fs.existsSync(path.join(root, 'lib', 'visit-pack', 'visitMascotCatalog.js'));
  for (const root of projectRoots()) {
    const srcPack = path.join(root, 'src', 'utils', 'mascotPack.js');
    const libPack = hasLibMirror(root) ? path.join(root, 'lib', 'visit-pack', 'mascotPack.js') : null;
...
function getMascotPackLibProbe() {
  ...
  const libMirrorOk = roots.some(
    (r) =>
      ...
      && fs.existsSync(path.join(r, 'lib', 'visit-pack', 'visitMascotInteractionEvents.js'))
      && fs.existsSync(path.join(r, 'lib', 'visit-pack', 'visitMascotDialogEvents.js'))
      && fs.existsSync(path.join(r, 'lib', 'visit-pack', 'visitMascotDialogApply.js'))
      && fs.existsSync(path.join(r, 'lib', 'visit-pack', 'visitMascotCatalog.js')),
  );
  return {
    roots,
