...
const DEFAULT_GAMEPLAY = {
  ...
  // Composition automatique des équipes : recettes fondées sur les six axes de profil
  // (mixte, complémentarité, groupes de besoin). Un établissement peut les couper d'un geste ;
  // les recettes aléatoires / reconduction restent toujours disponibles.
...
const DEFAULT_MODULES = {
  ...
  introEnabled: true,
  /** Présence « en ligne » (pastilles MJ/admin) — cœur partagé `presenceCore`. */
...
function camelKeyFor(settingKey) {
  switch (settingKey) {
    ...
    case 'gameplay.team_composition_profile_recipes_enabled':
      return 'teamCompositionProfileRecipesEnabled';
    default:
      return null;
  }
}
...
const GL_GAMEPLAY_REGISTRY = {
  ...
  'gameplay.team_composition_profile_recipes_enabled': glBoolean(
...
const GL_SETTINGS_REGISTRY = Object.freeze({
  ...
  'gl.classes.team_templates': {
    group: 'platform',
    type: 'json',
    shape: 'object',
    default: {},
    validate: (v) => require('./gl/teamTemplates').normalizeTeamTemplates(v).error || null,
    normalize: (v) => require('./gl/teamTemplates').normalizeTeamTemplates(v).templates,
    errorMessage: 'Gabarits d’équipes invalides',
  },
});
