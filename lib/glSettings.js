...
const DEFAULT_GATING = {
  ...
  // Delai (jours) avant de reessayer de valider une ressource apres une erreur au QCM
  // de validation (0 = desactive).
  retryCooldownDays: 3,
};
...
function clampGatingCooldownDays(value, fallback = 3) {
  const n = Number(value);
  if (!Number.isFinite(n)) return fallback;
  ...
  for (const row of rows) {
    switch (row.key) {
      ...
      case 'gating.default_required_correct':
        ...
        break;
      case 'gating.retry_cooldown_days':
        out.retryCooldownDays = clampGatingCooldownDays(
          safeJsonParse(row.value_json, DEFAULT_GATING.retryCooldownDays),
          DEFAULT_GATING.retryCooldownDays,
        );
        break;
      default:
        break;
...
async function setGlGatingSetting(key, value, updatedBy = null) {
  ...
  if (key === 'gating.enabled' || key === 'gating.auto_mark_on_correct') {
    ...
    normalized = v;
  } else if (key === 'gating.retry_cooldown_days') {
    const n = Number(value);
    if (!Number.isFinite(n)) return { ok: false, error: 'Valeur numerique attendue' };
    normalized = clampGatingCooldownDays(n, DEFAULT_GATING.retryCooldownDays);
  } else {
    const n = Number(value);
    if (!Number.isFinite(n)) return { ok: false, error: 'Valeur numerique attendue' };
    normalized = clampGatingRequired(n, DEFAULT_GATING.defaultRequiredCorrect);
  }
  await upsertGlSetting(key, normalized, updatedBy == null ? null : String(updatedBy));
