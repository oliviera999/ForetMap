...
function registerUploadedMedia(relativePath, originalName, mimeType, options = {}) {
  ...
  if (stableKey) {
    const registration = registerMediaStableKey(stableKey, relativePath, originalName, app ? { app } : {});
...
function enrichItemWithStableKey(item, keyIndex) {
  const match = Object.entries(keyIndex).find(([, entry]) => entry.relativePath === item.relativePath);
