...
function deleteMediaLibraryItem(relativePath) {
  ...
  const mediaRootWithSep = MEDIA_LIBRARY_DIR + path.sep;
  if (!absolutePath.startsWith(mediaRootWithSep)) {
    const err = new Error('Chemin média invalide');
    err.status = 400;
    throw err;
  }
  const safeRel = path.relative(UPLOADS_DIR, absolutePath).replace(/\\/g, '/');
  deleteFile(safeRel);
  return { ok: true };
}
