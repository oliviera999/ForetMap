...
function listMediaLibraryItemsWithKeys(limit = 300, options = {}) {
  return listMediaLibraryItems(limit, options);
}
...
/**
 * Médiathèque d'appartenance d'un chemin `media-library/...` (hérité sans
 * étiquette → G&L). Valide aussi que le chemin reste sous le dossier médiathèque.
 */
function resolveMediaLibraryItemAppFromPath(relativePath) {
  resolveMediaLibraryItemPath(relativePath);
  const rel = String(relativePath || '')
    .replace(/^\/+/, '')
    .replace(/\\/g, '/');
  const keyIndex = loadMediaKeyIndex();
  const match = Object.values(keyIndex).find((entry) => entry && entry.relativePath === rel);
  return resolveMediaItemApp(match?.app);
}

function assertMediaLibraryItemApp(relativePath, appFilter) {
  const filter = normalizeMediaApp(appFilter);
  if (!filter) {
    throw mediaLibraryError('Médiathèque (app) requise');
  }
  const itemApp = resolveMediaLibraryItemAppFromPath(relativePath);
  if (itemApp !== filter) {
    throw mediaLibraryError('Média hors périmètre de cette médiathèque', 403);
  }
  return itemApp;
}

function clearMediaLibraryItems(options = {}) {
  const app = normalizeMediaApp(options.app);
  if (!app) {
    // Filet de sécurité : sans filtre app, on refusait autrefois de lister côté
    // routes mais `clear_all` vidait quand même les deux médiathèques.
    throw mediaLibraryError('Médiathèque (app) requise');
  }
  const items = listMediaLibraryItems(MAX_BULK_DELETE_COUNT, { app });
  if (items.length === 0) {
    syncAssetManifests();
...
function executeMediaLibraryDeleteRequest(body = {}, options = {}) {
  ...
  if (parsed.mode === 'clear_all') {
    return { ok: true, ...clearMediaLibraryItems({ app }) };
  }
  if (parsed.mode === 'bulk') {
    // Vérifier tout le lot avant toute suppression : un chemin hors périmètre
    // ne doit ni passer, ni laisser un half-delete de l'autre médiathèque.
    for (const relativePath of parsed.paths) {
      assertMediaLibraryItemApp(relativePath, app);
    }
    return { ok: true, ...deleteMediaLibraryItems(parsed.paths) };
  }
  assertMediaLibraryItemApp(parsed.relativePath, app);
...
module.exports = {
  ...
  resolveMediaLibraryItemAppFromPath,
  assertMediaLibraryItemApp,
  normalizeMediaApp,
  resolveMediaItemApp,
  mediaItemMatchesApp,
  MEDIA_LIBRARY_APPS,
