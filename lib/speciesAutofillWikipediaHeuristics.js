...
function extractTraitsFromWikipediaExtract(text) {
  ...
  if (m) {
    ...
    m = raw.match(
      /\b(?:entre|de)\s+(\d{1,3})\s*(?:°\s*C|°C|°)?\s+(?:et|à|a)\s+(\d{1,3})\s*°?\s*C\b/i,
    );
  ...
  if (
    /\b(?:reproduction|se\s+reproduit)\s+(?:par|au moyen de|grâce à)\s+([^.;]{3,90})/i.test(raw)
  ) {
    const mm = raw.match(
      /\b(?:reproduction|se\s+reproduit)\s+(?:par|au moyen de|grâce à)\s+([^.;]{3,90})/i,
    );
    ...
    /\b(?:graines?|semences?)\b/i.test(head) &&
    /\b(?:dispers|dissémin|germination)\b/i.test(raw.slice(0, 400))
  ) {
    fields.reproduction = 'Reproduction par graines (indice résumé Wikipedia)';
  }

  if (
    /\b(?:fruits?|feuilles|fleurs|racines|tubercules|bulbes)\s+comestibles?\b/i.test(
      raw.slice(0, 500),
    )
  ) {
    const mfc = raw.match(
      /\b(fruits?|feuilles|fleurs|racines|tubercules|bulbes)\s+comestibles?\b/i,
    );
    if (mfc) {
  ...
  if (Object.keys(fields).length > 0) {
    warnings.push(
      'Résumé Wikipedia : indices automatiques (température, pH, taille, longévité, culture, etc.) — à vérifier.',
    );
  }
