      ...
      await execute(
        `UPDATE gl_admins
            SET display_name = ?, google_sub = COALESCE(?, google_sub), role = ?, is_active = 1,
                last_seen = NOW(), updated_at = NOW()
          WHERE id = ?`,
        [displayName || admin.display_name || normalizedEmail, googleSub || null, safeRole, admin.id]
      );
    }
...
async function findGlAdminByForetmapUserId(teacherId) {
  ...
  try {
    return await queryOne(
      'SELECT * FROM gl_admins WHERE foretmap_user_id = ? LIMIT 1',
      [String(teacherId)]
    );
  } catch (err) {
...
async function findGlAdminByLoginIdentifier(loginIdentifier) {
  const key = String(loginIdentifier || '').trim().toLowerCase();
  if (!key) return null;
  return queryOne(
...
async function resolveGlStaffLogin({ email, displayName, googleSub, teacherId = null, loginIdentifier = null }) {
  const normalizedEmail = String(email || '').trim().toLowerCase();
  const emailIsValid = isValidEmail(normalizedEmail);
  const teacherKey = teacherId != null ? String(teacherId) : null;
  const loginKey = String(loginIdentifier || '').trim().toLowerCase();
  ...
  if (!emailIsValid) {
    const byLogin = await findGlAdminByLoginIdentifier(loginKey);
    if (byLogin && Number(byLogin.is_active)) {
      const glRole = String(byLogin.role || 'mj').toLowerCase() === 'admin' ? 'admin' : 'mj';
      return { ok: true, admin: byLogin, glRole };
    }
    return {
      ok: false,
      status: 403,
  ...
  if ((!existing || !Number(existing.is_active)) && loginKey && loginKey !== normalizedEmail) {
    const byLogin = await findGlAdminByLoginIdentifier(loginKey);
    if (byLogin && Number(byLogin.is_active)) existing = byLogin;
  }
        ...
        await execute(
          `UPDATE gl_admins
              SET display_name = COALESCE(?, display_name),
                  ...
                  updated_at = NOW()
            WHERE id = ?`,
          [displayName || null, googleSub || null, teacherKey, existing.id]
        );
      } catch (err) {
        if (!isMissingColumnError(err)) throw err;
        await execute(
          `UPDATE gl_admins
              SET display_name = COALESCE(?, display_name),
                  ...
                  updated_at = NOW()
            WHERE id = ?`,
          [displayName || null, googleSub || null, existing.id]
        );
      }
    }
