...
async function ensureGlAdminRecord({
  ...
  if (!admin) {
    ...
    try {
      await execute(
        `UPDATE gl_admins
            SET display_name = ?, google_sub = COALESCE(?, google_sub), role = ?, foretmap_user_id = COALESCE(?, foretmap_user_id),
      ...
      await execute(
        `UPDATE gl_admins
            SET display_name = ?, google_sub = COALESCE(?, google_sub), role = ?,
                last_seen = NOW(), updated_at = NOW()
          WHERE id = ?`,
        [
          displayName || admin.display_name || normalizedEmail,
...
async function resolveGlStaffLogin({
  ...
  // Le staff G&L est un enseignant ForetMap, quelle que soit la voie de connexion (mot de
  // passe **ou** Google) : sans compte `users` enseignant, pas de session MJ — une ligne
  // `gl_admins` seule ne porte ni `is_active`, ni époque de jeton, ni `teacher.access`
  // (CDG-12). Le compte se crée depuis ForetMap ; la ligne `gl_admins` s'y rattache ensuite.
  if (!teacherKey) {
    return {
      ok: false,
      status: 403,
      error:
        'Accès réservé aux enseignants ForetMap : connectez-vous avec un compte enseignant (ou demandez sa création).',
    };
  }
  ...
  if (teacherKey) {
    ...
    if (isForetmapAdminForGl(authz)) {
      ...
      // Une ligne staff désactivée le reste, même pour un administrateur ForetMap : la
      // réactivation est un geste explicite dans l'administration du jeu (CDG-47).
      if (!Number(admin?.is_active)) {
        return { ok: false, status: 403, error: 'Accès maître du jeu désactivé pour ce compte.' };
      }
