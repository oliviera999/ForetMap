...
const BIOMES_STATS_TEMPLATE_HEADERS = ['biome_slug', 'biome_nom', 'faune', 'flore', 'total'];

const BIOMES_STATS_TEMPLATE_SAMPLE_ROW = ['foret_temperee', 'Forêt tempérée', '12', '18', '30'];
...
const HEADER_ALIASES = new Map([
  ...
  ['taxon_source_url', 'taxon_source_url'],
  ['groupe', 'groupe'],
  ['famille', 'famille'],
...
function normalizeTaxonRank(value) {
  const s = asTrimmedString(value).toLowerCase();
  if (['species', 'genus', 'family', 'clade'].includes(s)) return s;
  return null;
}
...
function buildSpeciesPayload(row = {}) {
  ...
  return {
    ...
    taxon_rank: normalizeTaxonRank(mapped.taxon_rank),
    taxon_source: normalizeOptionalString(mapped.taxon_source),
    taxon_source_url: normalizeOptionalString(normalizeHttpsUrl(mapped.taxon_source_url)),
 ...
 */
async function parseSpeciesWorkbook(buffer, options = {}) {
...
const SPECIES_UPSERT_SQL = `
  INSERT INTO gl_species (
    species_code, biome_slug, type, nom_commun, nom_scientifique, taxon_rank, taxon_source,
    taxon_source_url, groupe, famille,
  ...
  ON DUPLICATE KEY UPDATE
    ...
    taxon_rank = COALESCE(VALUES(taxon_rank), taxon_rank),
    taxon_source = COALESCE(VALUES(taxon_source), taxon_source),
    taxon_source_url = COALESCE(VALUES(taxon_source_url), taxon_source_url),
...
async function allocateNextSpeciesCode(queryAll) {
  const rows = await queryAll(
    `SELECT species_code FROM gl_species WHERE species_code REGEXP '^SP[0-9]+$'`,
  );
  let maxNum = 0;
  for (const row of rows) {
    const match = String(row.species_code || '').match(/^SP(\d+)$/i);
...
/**
 * UPSERT d'une seule espèce catalogue.
 * @returns {{ created: boolean, payload: object }}
 */
async function upsertSpeciesRow(deps, body, options = {}) {
  const { queryAll, execute } = deps;
  const forceCode = options.species_code ? asTrimmedString(options.species_code) : null;
  const payload = buildSpeciesPayload(body);
  if (!payload.species_code && !forceCode) {
    payload.species_code = await allocateNextSpeciesCode(queryAll);
  } else if (forceCode) {
    payload.species_code = forceCode;
  }

  const rowErrors = validateSpeciesPayload(payload, 1);
  if (rowErrors.length) {
    const err = new Error('Données espèce invalides');
    err.statusCode = 400;
  ...
  if (!created && options.requireNew) {
    const err = new Error('Ce code espèce existe déjà');
    err.statusCode = 409;
    throw err;
  }
  ...
  await ensureBiomeExists(payload, knownBiomes, { execute, dryRun: false });
...
async function applySpeciesImport(deps, speciesRows, options = {}) {
  ...
  // Biomes inconnus créés d'abord (rare : le Set les court-circuite presque toujours),
  // puis upserts espèces par lots de 100 (G4) — comptage par ligne inchangé.
  for (const { payload } of validRows) {
    await ensureBiomeExists(payload, knownBiomes, { execute, dryRun });
  }
  await executeQuestionUpserts({ execute }, validRows, {
    sql: SPECIES_UPSERT_SQL,
    buildParams: buildSpeciesUpsertParams,
    existingCodes,
    totals: report.totals,
    codeOf: (payload) => payload.species_code,
  });
...
async function buildSpeciesTemplateWorkbook() {
  return buildWorkbookBuffer([
    { name: SPECIES_SHEET, aoa: [SPECIES_TEMPLATE_HEADERS, SPECIES_TEMPLATE_SAMPLE_ROW] },
    {
      name: BIOMES_STATS_SHEET,
      aoa: [BIOMES_STATS_TEMPLATE_HEADERS, BIOMES_STATS_TEMPLATE_SAMPLE_ROW],
    },
  ]);
}

async function buildSpeciesExportWorkbook({ speciesRows, biomeStatsRows }) {
  const speciesData = [SPECIES_TEMPLATE_HEADERS, ...speciesRows.map(speciesRowToExportArray)];
  const biomeData = [
    BIOMES_STATS_TEMPLATE_HEADERS,
    ...biomeStatsRows.map(biomeStatsRowToExportArray),
  ];
  return buildWorkbookBuffer([
    { name: SPECIES_SHEET, aoa: speciesData },
    { name: BIOMES_STATS_SHEET, aoa: biomeData },
  ]);
}
...
const resolveImportRows = (body = {}) => resolveWorkbookImportRows(body, parseSpeciesWorkbook);
