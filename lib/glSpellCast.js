...
function resolveSpellCastError(err) {
  const map = {
    ...
    SPELL_CAST_SCHEMA_OUTDATED: {
      status: 503,
      error:
        'Schéma sortilèges incomplet (migrations 113, 139 et 173 requises). ' +
...
function isSpellCastSchemaError(err) {
  ...
  if (errno === 1054 || code === 'ER_BAD_FIELD_ERROR') {
    return /roster_scope|gl_spell_cast|approval_mode|cast_scope|caster_kind|approval_required/i.test(
      msg,
    );
  }
  return false;
...
async function resolveDraftApproval({ gameId, draftId, decision, auth }) {
  ...
  assertCastersCasterKindAllowed(
    spell,
    contributions.filter((c) => c.gems > 0 || c.hearts > 0),
    roster,
  );
  let result;
  try {
