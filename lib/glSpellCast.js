...
function resolveSpellCastError(err) {
  const map = {
    ...
    PLAYER_NOT_ON_ROSTER: { status: 400, error: 'Ce joueur n’est pas dans l’équipe sélectionnée' },
...
async function loadTeamRoster(gameId, teamId) {
  const rows = await queryAll(
    `SELECT p.id, p.pseudo, p.first_name, p.last_name, p.health_points, p.power_points
...
async function findCollectingDraft(gameId, teamId, spellCode) {
  return queryOne(
...
async function createOrGetDraft({
  ...
  if (!canSelectTeam(auth, { teamId, config, playerTeamId })) {
    throw makeHttpError('TEAM_FORBIDDEN', 403);
  }
  assertTurnAllowsTeam(game, teamId, config);

  const code = spell.spellCode;
  let draft = await findCollectingDraft(gameId, teamId, code);
  if (!draft) {
    const insert = await execute(
      `INSERT INTO gl_spell_cast_drafts
        (game_id, team_id, spell_code, status, created_by_player_id,
         created_by_actor_type, created_by_actor_id, created_at, updated_at)
       VALUES (?, ?, ?, 'collecting', ?, ?, ?, NOW(), NOW())`,
      [gameId, teamId, code, actorPlayerId, actor.actorType, actor.actorId]
    );
  ...
  const roster = await loadTeamRoster(gameId, teamId);
...
async function getDraftById(draftId, gameId) {
  ...
  const roster = await loadTeamRoster(gameId, draft.team_id);
  return formatDraftPayload(draft, spell, roster);
}
...
async function updateDraftContributions({
  ...
  for (const item of list) {
    ...
    if (!rosterIds.has(playerId)) throw makeHttpError('PLAYER_NOT_ON_ROSTER', 400);
