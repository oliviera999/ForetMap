...
function resolveSpellCastError(err) {
  const map = {
    ...
    DRAFT_ALREADY_CAST: {
      status: 409,
      error: 'Ce sortilège a déjà été lancé',
    },
    DRAFT_NOT_LAUNCHABLE: {
      status: 409,
      error: 'Ce brouillon ne peut plus être lancé',
    },
    DRAFT_NOT_READY: {
      status: 409,
...
async function finalizeCastTx(tx, { gameId, draftId, draft, spell, contributions, roster, actor }) {
  ...
  // Verrouiller le brouillon EN PREMIER : deux launch/accept concurrents avec
  // soldes suffisants pour payer deux fois ne doivent pas double-débiter ni
  // émettre deux événements spell_cast.
  ...
  if (!castUpdate.affectedRows) {
    throw makeHttpError('DRAFT_ALREADY_CAST', 409);
  }
...
async function launchDraft({ gameId, draftId, auth, config }) {
  ...
  if (spellRequiresApproval(config, spell) && !isStaff(auth)) {
    ...
    if (!submitted.affectedRows) {
      throw makeHttpError('DRAFT_NOT_COLLECTING', 409);
    }
