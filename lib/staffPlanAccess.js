...
async function resolveAllowedRoleSlugs() {
  ...
  return parseAllowedRoleSlugs(raw);
}

/**
 * Un compte authentifié peut-il ouvrir le plan des personnels ?
 *
 * 1. Son profil figure dans `ui.staff_plan.allowed_role_slugs` (réglage admin) → oui.
 * 2. Sinon, profil **hors** catalogue d'audience (profil maison) portant
 *    `staff_plan.access` → oui (échappatoire RBAC pour les rôles créés sur mesure).
 * 3. Sinon → non. Un profil du catalogue décoché dans les réglages est refusé même s'il
 *    a encore la permission RBAC : le curseur des réglages fait foi.
 *
...
function accountMayAccessStaffPlan(auth, allowedSlugs) {
  ...
  if (roleSlug && allowed.includes(roleSlug)) return true;
  if (
    roleSlug &&
    !isKnownAudienceRoleSlug(roleSlug) &&
    hasPermission(auth, STAFF_PLAN_PERMISSION)
  ) {
    return true;
  }
  return false;
