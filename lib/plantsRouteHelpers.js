...
const { detectImageExtensionFromDataUrl } = require('./shared/dataUrlImage');
const { applyDerivedGroup4IfEmpty } = require('./plantGroup4');
const { syncNormalizedAndLegacyPlantFields, LEGACY_IMPORT_FIELDS } = require('./plantPayloadSync');
const { parseWorkbookRowsFromBuffer, createImportReport } = require('./importRows');
...
/**
 * Danger (migration 251) — gravité, voies d'exposition, conduite à tenir. Groupés à part des
 * champs de détermination : la détermination répond à « qu'est-ce que c'est », le danger à
 * « qu'est-ce que ça peut me faire », et une espèce parfaitement identifiée peut rester
 * dangereuse.
 *
 * Le drapeau `hazard_reviewed` n'y figure plus : écrit par le formulaire et l'import avec la
 * seule permission `plants.manage`, il posait « relu et validé » sans relecteur ni date — la
 * cause des fiches validées sans relecteur (audit du 25/09/2026, § 1.3.6, migration 294).
 * Seule `POST /:id/validate-hazard` (permission `plants.hazards.validate`) l'écrit.
 */
const PLANT_HAZARD_FIELDS = ['toxicity_level', 'hazard_exposure', 'hazard_notes'];

/**
 * Risque sanitaire (migration 271) — ce que l'espèce peut *transmettre*, par opposition à la
 * toxicité qui décrit ce qu'elle fait à qui la touche ou la mange. Deux listes séparées :
 * une relecture d'enseignant peut porter sur l'un sans l'autre, et l'affichage les distingue.
 *
 * `hazard_reviewed_by` / `hazard_reviewed_at` n'y figurent PAS : ce sont des colonnes de
 * traçabilité, écrites par la seule route de validation (`POST /:id/validate-hazard`) et
 * jamais par le formulaire ni par un import.
 */
const PLANT_HEALTH_FIELDS = ['health_risk', 'health_notes'];

/** Attribution de la photo principale (migration 252) — mêmes noms que `quiz_questions`. */
const PLANT_PHOTO_CREDIT_FIELDS = ['photo_credit', 'photo_licence'];
...
const PLANT_EXTRA_FIELDS = [
  ...
  'preferred_nutrients',
  // Détermination : neutres vis-à-vis du règne (végétal, animal, champignon, micro-organisme).
  'identification_criteria',
  'lookalike_species',
  'identification_period',
  // Classification emboîtée (migration 274) — groupe le plus précis.
  'clade_id',
  ...PLANT_HAZARD_FIELDS,
  ...PLANT_HEALTH_FIELDS,
  ...PLANT_PHOTO_CREDIT_FIELDS,
];
...
const HEADER_ALIASES = new Map([
  ...
  ['rang_taxonomique', 'taxon_rank'],
  ['groupe_1', 'taxon_kingdom'],
  ['groupe_2', 'taxon_group'],
  ['groupe_3', 'taxon_family'],
  ['groupe_4', 'taxon_genus'],
  ['regne', 'taxon_kingdom'],
  ...
  ['type_habitat', 'habitat_type'],
  ['trophic_role', 'trophic_role'],
  ['role_trophique', 'trophic_role'],
  ['is_edible', 'is_edible'],
  ['comestible', 'is_edible'],
  ['origin_status', 'origin_status'],
  ['statut_biogeographique', 'origin_status'],
  ['statut_biogeo', 'origin_status'],
  ['statut_origine', 'origin_status'],
  ['statut_indigene', 'origin_status'],
  ['photo_credit', 'photo_credit'],
  ['credit_photo', 'photo_credit'],
  ['auteur_photo', 'photo_credit'],
  ['photo_licence', 'photo_licence'],
  ['licence_photo', 'photo_licence'],
  ['photo_license', 'photo_licence'],
  ['toxicity_level', 'toxicity_level'],
  ['toxicite', 'toxicity_level'],
  ['danger', 'toxicity_level'],
  ['niveau_de_danger', 'toxicity_level'],
  ['gravite', 'toxicity_level'],
  ['hazard_exposure', 'hazard_exposure'],
  ['voie_d_exposition', 'hazard_exposure'],
  ['voies_d_exposition', 'hazard_exposure'],
  ['exposition', 'hazard_exposure'],
  ['hazard_notes', 'hazard_notes'],
  ['note_de_danger', 'hazard_notes'],
  ['precautions', 'hazard_notes'],
  ['health_risk', 'health_risk'],
  ['risque_sanitaire', 'health_risk'],
  ['risques_sanitaires', 'health_risk'],
  ['zoonose', 'health_risk'],
  ['zoonoses', 'health_risk'],
  ['health_notes', 'health_notes'],
  ['note_sanitaire', 'health_notes'],
  ['notes_sanitaires', 'health_notes'],
  ['iucn_status', 'iucn_status'],
  ['statut_iucn', 'iucn_status'],
  ['statut_uicn', 'iucn_status'],
  ['uicn', 'iucn_status'],
  ['iucn', 'iucn_status'],
  ['liste_rouge', 'iucn_status'],
  ['life_cycle', 'life_cycle'],
  ['cycle_vie', 'life_cycle'],
  ...
  // Détermination : les en-têtes sont saisis à la main dans un tableur, donc on accepte
  // les tournures naturelles autant que les clés canoniques (`normalizeHeader` réduit
  // accents, apostrophes et espaces à des `_`).
  ['determination', 'identification_criteria'],
  ['criteres_determination', 'identification_criteria'],
  ['criteres_de_determination', 'identification_criteria'],
  ['criteres_identification', 'identification_criteria'],
  ['criteres_d_identification', 'identification_criteria'],
  ['confusions', 'lookalike_species'],
  ['confusions_possibles', 'lookalike_species'],
  ['especes_ressemblantes', 'lookalike_species'],
  ['risques_de_confusion', 'lookalike_species'],
  ['periode_observation', 'identification_period'],
  ['periode_d_observation', 'identification_period'],
  ['periode_determination', 'identification_period'],
  ['quand_l_observer', 'identification_period'],
    ...
    if (
      ...
      payload[field] =
        sourceValue == null || sourceValue === ''
          ? null
          : Number(String(sourceValue).replace(',', '.'));
      if (payload[field] != null && !Number.isFinite(payload[field])) payload[field] = null;
      if (
        [
          'temp_min_c',
          'temp_max_c',
        ...
        payload[field] = Math.trunc(payload[field]);
      }
      continue;
    }
...
/** Rapport d'import : forme commune (`lib/importRows.js`) + stratégie et `skipped_existing`. */
function buildImportReportBase(strategy, dryRun, sourceType, rowsCount) {
  return createImportReport({
    ...
    sourceType,
    received: rowsCount,
    extraTotals: { skipped_existing: 0 },
  });
}
...
function validateImportPayloadRow(row, rowNumber) {
  ...
  const tempErr =
    payload.temp_min_c != null || payload.temp_max_c != null
      ? (() => {
          const lo = payload.temp_min_c ?? payload.temp_max_c;
          const hi = payload.temp_max_c ?? payload.temp_min_c;
          if (lo != null && (lo < -20 || lo > 80)) return 'Hors plage (-20 à 80 °C)';
          if (hi != null && (hi < -20 || hi > 80)) return 'Hors plage (-20 à 80 °C)';
          return null;
        })()
      : validateRangeText(mapped.ideal_temperature_c ?? payload.ideal_temperature_c, -20, 80);
  if (tempErr) errors.push({ row: rowNumber, field: 'temp_min_c', error: tempErr });
  const phErr =
    payload.ph_min != null || payload.ph_max != null
      ? (() => {
          const lo = payload.ph_min ?? payload.ph_max;
          const hi = payload.ph_max ?? payload.ph_min;
          if (lo != null && (lo < 0 || lo > 14)) return 'Hors plage (0 à 14)';
          if (hi != null && (hi < 0 || hi > 14)) return 'Hors plage (0 à 14)';
          return null;
        })()
      : validateRangeText(mapped.optimal_ph ?? payload.optimal_ph, 0, 14);
  if (phErr) errors.push({ row: rowNumber, field: 'ph_min', error: phErr });
