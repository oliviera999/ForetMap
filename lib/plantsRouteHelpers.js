...
function buildPlantPayload(body, fallback = {}) {
  ...
  for (const field of PLANT_EXTRA_FIELDS) {
    ...
    if (
      ...
      payload[field] =
        sourceValue == null || sourceValue === '' ? null : Number(String(sourceValue).replace(',', '.'));
      if (payload[field] != null && !Number.isFinite(payload[field])) payload[field] = null;
      if (['temp_min_c', 'temp_max_c', 'gbif_key', 'is_edible', 'is_ornamental'].includes(field) && payload[field] != null) {
...
function validateImportPayloadRow(row, rowNumber) {
  ...
  const tempErr =
    payload.temp_min_c != null || payload.temp_max_c != null
      ...
      : validateRangeText(
          mapped.ideal_temperature_c ?? payload.ideal_temperature_c,
          -20,
          80,
        );
