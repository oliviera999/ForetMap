...
const { executeQuestionUpserts, readSheetRows } = require('./shared/xlsxImportCore');
...
const { resolveWorkbookImportRows } = require('./importRows');
const { createImportReport } = require('./importRows');
...
/**
 * Options ENUM du sort. Séparées de `SPELL_FIELD_KEYS` parce qu'elles obéissent à une
 * règle d'écriture différente : une colonne **absente** de la feuille laisse la valeur
 * en base inchangée (cf. SPELL_UPSERT_SQL) au lieu de la remettre au défaut. Écraser
 * silencieusement une restriction de lancement lors d'un ré-import d'un vieux fichier
 * serait une levée de garde invisible.
 */
const SPELL_OPTION_KEYS = ['caster_kind', 'approval_mode', 'cast_scope'];
...
const HEADER_ALIASES = new Map([
  ...
  ['cree_le', 'cree_le'],
  ['lanceurs', 'caster_kind'],
  ['peuple', 'caster_kind'],
  ['caster_kind', 'caster_kind'],
  ['validation_mj', 'approval_mode'],
  ['approval_mode', 'approval_mode'],
  ['portee_lancement', 'cast_scope'],
  ['cast_scope', 'cast_scope'],
]);
...
/**
 * Options ENUM fournies mais non reconnues : signalées ligne par ligne plutôt
 * qu'écrasées silencieusement (une restriction mal orthographiée doit se voir).
 * @returns {Array<{field: string, error: string}>}
 */
function collectOptionErrors(mapped) {
  const checks = [
    ['lanceurs', mapped.caster_kind, normalizeCasterKindOrNull, 'gnomes | licornes | tous'],
    ['validation_mj', mapped.approval_mode, normalizeApprovalModeOrNull, 'auto | mj_required'],
    ['portee_lancement', mapped.cast_scope, normalizeCastScopeOrNull, 'solo | collective | any'],
  ];
  const errors = [];
  for (const [label, raw, normalize, expected] of checks) {
    if (asTrimmedString(raw) !== '' && normalize(raw) == null) {
      errors.push({ field: label, error: `Valeur attendue : ${expected}` });
    }
  }
  return errors;
}
...
function buildSpellPayload(row = {}) {
  ...
  return {
    option_errors: collectOptionErrors(mapped),
    ...
    // `null` = colonne absente ou vide => on conserve la valeur en base (cf. SPELL_OPTION_KEYS).
    caster_kind: normalizeCasterKindOrNull(mapped.caster_kind),
    approval_mode: normalizeApprovalModeOrNull(mapped.approval_mode),
    cast_scope: normalizeCastScopeOrNull(mapped.cast_scope),
  };
}
...
function validateSpellPayload(payload, rowNumber) {
  ...
  for (const item of payload.option_errors || []) {
    errors.push({ row: rowNumber, field: item.field, error: item.error });
  }
  return errors;
}
...
/** Rapport d'import : forme commune (`lib/importRows.js`), compteurs propres ci-dessous. */
function buildImportReportBase(dryRun, sourceType, rowsCount) {
  return createImportReport({
    dryRun,
    sourceType,
    received: rowsCount,
    extraTotals: { categories_synced: 0 },
  });
}
...
// Les trois options ENUM (`caster_kind`, `approval_mode`, `cast_scope`) sont NOT NULL :
// à la création un paramètre nul retombe sur le défaut via COALESCE, à la mise à jour
// il laisse la colonne intacte. Les paramètres correspondants sont donc passés deux
// fois (INSERT puis ON DUPLICATE KEY UPDATE), cf. buildSpellUpsertParams.
const SPELL_UPSERT_SQL = `
  INSERT INTO gl_spells (
    ...
    caster_kind, approval_mode, cast_scope, created_at, updated_at
  ) VALUES (
    ?, ?, ?, ?, ?, ?, ?,
    ?, ?, ?, ?, ?, ?, ?,
    ?, ?, ?, ?,
    COALESCE(?, 'any'), COALESCE(?, 'auto'), COALESCE(?, 'any'), NOW(), NOW()
  )
  ON DUPLICATE KEY UPDATE
    caster_kind = COALESCE(?, caster_kind),
    approval_mode = COALESCE(?, approval_mode),
    cast_scope = COALESCE(?, cast_scope),
...
function buildSpellUpsertParams(payload) {
  ...
  const base = SPELL_FIELD_KEYS.map((key) => row[key]);
  const options = SPELL_OPTION_KEYS.map((key) => payload[key] ?? null);
  // Une fois pour le COALESCE de l'INSERT, une fois pour celui de l'UPDATE.
...
async function applySpellsImport(deps, spellRows, options = {}) {
  ...
  // Catégories inconnues créées d'abord (rare : le Set les court-circuite presque
  // toujours), puis upserts sorts par lots de 100 (G4) — comptage par ligne inchangé.
  for (const { payload } of validRows) {
    await ensureCategoryExists(payload, knownCategories, { execute, dryRun });
  }
  await executeQuestionUpserts({ execute }, validRows, {
    sql: SPELL_UPSERT_SQL,
    buildParams: buildSpellUpsertParams,
    existingCodes,
    totals: report.totals,
    codeOf: (payload) => payload.spell_code,
  });
...
async function loadSpellsExportRows(deps, options = {}) {
  ...
  const spellDb = await queryAll(
    `SELECT s.spell_code, s.category_slug, c.nom AS category_nom, s.nom, s.emoji,
            ...
            s.caster_kind, s.approval_mode, s.cast_scope
       FROM gl_spells s
  ...
  const spellRows = spellDb.map((row) => ({
    ...
    lanceurs: row.caster_kind || 'any',
    validation_mj: row.approval_mode || 'auto',
    portee_lancement: row.cast_scope || 'any',
  }));
...
/** Lignes du classeur d'import (décodage base64 commun, parseur métier local). */
const resolveImportRows = (body = {}) => resolveWorkbookImportRows(body, parseSpellsWorkbook);
