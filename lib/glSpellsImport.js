...
const CATEGORIES_STATS_TEMPLATE_SAMPLE_ROW = [
  'mouvement',
  '6',
  '4',
  '2',
 ...
 * @param {Buffer} buffer
 */
function parseSpellsWorkbook(buffer, options = {}) {
...
async function applySpellsImport(deps, spellRows, options = {}) {
  ...
  if (syncCategories && categoryRows.length > 0) {
    report.totals.categories_synced = await syncCategoriesFromRows(categoryRows, { execute, dryRun });
  }
...
function buildSpellsTemplateWorkbook() {
  const wb = XLSX.utils.book_new();
  const wsSpells = XLSX.utils.aoa_to_sheet([SPELL_TEMPLATE_HEADERS, SPELL_TEMPLATE_SAMPLE_ROW]);
  const wsCats = XLSX.utils.aoa_to_sheet([
    CATEGORIES_STATS_TEMPLATE_HEADERS,
    CATEGORIES_STATS_TEMPLATE_SAMPLE_ROW,
  ]);
  XLSX.utils.book_append_sheet(wb, wsSpells, SPELLS_SHEET);
  XLSX.utils.book_append_sheet(wb, wsCats, CATEGORIES_STATS_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}

function buildSpellsExportWorkbook({ spellRows, categoryStatsRows }) {
  const wb = XLSX.utils.book_new();
  const spellData = [SPELL_TEMPLATE_HEADERS, ...spellRows.map(spellRowToExportArray)];
  const catData = [CATEGORIES_STATS_TEMPLATE_HEADERS, ...categoryStatsRows.map(categoryStatsRowToExportArray)];
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(spellData), SPELLS_SHEET);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(catData), CATEGORIES_STATS_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}
