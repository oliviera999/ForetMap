...
function resolveVisitAssetAbsolute(ref, ctx = {}) {
  ...
  const packPrefix =
    packId && /^[0-9a-f-]{36}$/i.test(packId)
...
function rewriteVisitPackForServerImport(pack, packUuid) {
  ...
  for (const spec of Object.values(stateFrames)) {
    if (!spec || typeof spec !== 'object') continue;
    const names = Array.isArray(spec.files) ? spec.files.map(String) : [];
    spec.files = names.map((name) => {
      const trimmed = String(name || '').trim();
      if (trimmed.startsWith(PORTABLE_FRAMES_BASE)) {
        const base = path.basename(trimmed);
        return base ? `${prefix}${base}` : trimmed;
      }
      const base = path.basename(trimmed);
      return base && !trimmed.includes('/') ? `${prefix}${base}` : trimmed;
    });
    delete spec.srcs;
  }
  next.stateFrames = stateFrames;
...
function buildVisitExportArchive({ packRow, packJson, mapId }) {
  ...
  return { manifest, pack: portablePack, assetFiles: collected.files };
}
