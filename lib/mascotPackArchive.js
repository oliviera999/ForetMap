...
// Chargé au premier import/export de pack (audit charge serveur, piste 1 : rien au boot).
let AdmZipLazy = null;
function getAdmZip() {
...
} = require('./visitMascotSpriteLibraryFiles');
...
function resolveVisitAssetAbsolute(ref, ctx = {}) {
  ...
  const libPrefix = VISIT_MASCOT_SPRITE_LIBRARY_API_ROOT;
  ...
  // Bibliothèque : URL canonique ou historique par carte — seul le nom de fichier compte.
  if (raw.startsWith(libPrefix)) {
    return resolveVisitMascotSpriteLibraryAbsolutePath(
      visitMascotSpriteLibraryFilenameFromUrl(raw),
    );
  }
  ...
  if (base && !/^https?:\/\//i.test(base)) {
    ...
    if (filename && normBase.startsWith(libPrefix)) {
      return resolveVisitMascotSpriteLibraryAbsolutePath(filename);
    }
 ...
 */
function buildVisitExportArchive({ packRow, packJson, unified = false }) {
...
/**
 * Archive d'un **modèle catalogue** (mascotte livrée avec l'application), sans passer par un
 * pack serveur. Ses trames vivent sous `public/assets/` : `resolveVisitAssetAbsolute` sait déjà
 * les lire, il n'y a donc rien de nouveau à résoudre — seulement à ne pas exiger de `packRow`.
 *
 * `modelInfo.hasRealAnimation` à `false` **n'empêche pas** l'export : un modèle à image fixe reste
 * un point de départ valable pour construire un pack. Mais l'avertissement part avec l'archive,
 * pour que personne ne croie exporter une animation qui n'existe pas.
 *
 * @param {{ catalogId: string, pack: Record<string, unknown>,
 *   modelInfo?: { label?: string, hasRealAnimation?: boolean, frameCount?: number } }} args
 */
function buildCatalogExportArchive({ catalogId, pack, modelInfo = {} }) {
  const collected = collectVisitPackAssets(pack, {});
  const portablePack = buildPortableVisitPack(pack, collected.fileNameBySource);
  const warnings = [...collected.warnings];
  if (modelInfo.hasRealAnimation === false) {
    warnings.push(
      `Modèle « ${modelInfo.label || catalogId} » sans animation propre : ses états partagent une image unique.`,
    );
  }
  const manifest = {
    format: ARCHIVE_FORMAT,
    ...
    source: {
      ...
      label: modelInfo.label || pack?.label || catalogId,
      origin: 'catalog-model',
      has_real_animation: modelInfo.hasRealAnimation !== false,
      frame_count: collected.files.length,
    },
    warnings,
  };
  return { manifest, pack: portablePack, assetFiles: collected.files, warnings };
}
