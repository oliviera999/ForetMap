 ...
 * pour consultation via GET /api/admin/logs et GET /api/admin/diagnostics (secret DEPLOY_SECRET).
 * `appendForcedLogLine` alimente aussi ce tampon hors filtre de niveau Pino.
 */
'use strict';
...
/**
 * Enregistre une ligne dans le tampon mémoire (GET /api/admin/logs) sans passer par le filtre de niveau Pino.
 * Utile si LOG_LEVEL=error ou si stdout n’est pas visible sur l’hébergeur.
 * @param {Record<string, unknown>|string} obj
 */
function appendForcedLogLine(obj) {
  try {
    const line = typeof obj === 'string' ? obj : JSON.stringify(obj);
    pushCompleteLines(`${line}\n`);
  } catch {
    /* ignore */
  }
}
