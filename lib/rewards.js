 ...
 *
 * L'attribution ne dépend PAS de l'interrupteur `ui.modules.rewards_enabled` : module éteint,
 * les badges mérités sont enregistrés en silence et apparaissent tous au rallumage (décision
 * du 25/09/2026). Seule l'annonce est coupée, par `announceableRewards`. Aucun doublon
 * possible : clé primaire (user_id, reward_key) + `INSERT IGNORE`.
 * @param {string} userId
...
/**
 * Badges à annoncer à l'élève (fenêtre de fin de séance) : tous si le module récompenses est
 * allumé, aucun sinon — ils sont pourtant enregistrés, et « Mes badges » les montrera au
 * rallumage.
 * @param {object[]} awarded badges nouvellement obtenus (`evaluateSessionRewards`)
 */
async function announceableRewards(awarded) {
  if (!Array.isArray(awarded) || !awarded.length) return [];
  return (await isModuleEnabled('foret', 'rewards')) ? awarded : [];
}
