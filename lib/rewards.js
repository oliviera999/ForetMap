 ...
 * Module `ui.modules.rewards_enabled` éteint → rien n'est attribué (et rien n'est rattrapé au
 * rallumage : un badge se gagne à la fin de séance qui le mérite).
 * @param {string} userId
...
async function evaluateSessionRewards(userId, { run, level } = {}) {
  ...
  if (!(await isModuleEnabled('foret', 'rewards'))) return [];
