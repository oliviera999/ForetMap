 ...
 *   `src/utils/visitMascotCatalog.js`, servi côté serveur par son miroir ESM
 *   `lib/visit-pack/visitMascotCatalog.js` (sous-package `type: module`, sync `sync:visit-pack-lib`) ;
...
// Sens unique : le semis ne requiert le registre que **paresseusement**, dans son corps de
// fonction. Ce `require` de haut niveau ne referme donc aucun cycle.
const { listDeletedBuiltinCatalogIds } = require('./visitMascotBuiltinSeed');
...
async function listVisitMascotRegistry() {
  const [staticEntries, packs, connues, supprimees] = await Promise.all([
  ...
  //
  // **Une mascotte livrée supprimée est aussi sans ligne** — et le repli la ramènerait donc,
  // exactement comme il ramenait les dépubliées. La suppression aurait alors l'air de réussir au
  // studio sans rien changer pour les visiteurs. Ce qui distingue les deux absences n'est pas
  // lisible dans `visit_mascot_packs` : c'est la trace laissée en la supprimant.
  const repli = connues
    ? staticEntries.filter((entry) => {
        const id = String(entry.id).trim();
        return !connues.has(id) && !supprimees.has(id);
      })
    : staticEntries.filter((entry) => !supprimees.has(String(entry.id).trim()));
