...
const { addUserToGroup: addMemberToGroup } = require('./groupMembers');
const { validateGroupDefaultRole } = require('./groupDefaultRolePolicy');
...
const GROUP_IMPORT_HEADER_ALIASES = new Map([
  ...
  ['profil_par_defaut', 'defaultRole'],
  ['profil_par_defaut_slug_ou_nom_optionnel', 'defaultRole'],
  ['profil', 'defaultRole'],
  ['role', 'defaultRole'],
  ['default_role', 'defaultRole'],
  ['role_par_defaut', 'defaultRole'],
]);
...
function parseGroupRefsCell(raw) {
  ...
  return s
    ...
    .filter(Boolean)
    .map((token) => ({
      // Seul « > » sépare parent et enfant : un nom de groupe peut contenir « / »
      // (« 6A/6B » n'est pas un chemin, CDG-34).
      path: token
        .split(/\s*>\s*/)
        .map((p) => p.trim())
        .filter(Boolean),
  ...
  return [
    {
      ...
      [GROUP_TEMPLATE_COLUMNS[4]]:
        'Classe principale, profil « n3beur novice » conféré à ses membres — remplacer ou supprimer.',
      [GROUP_TEMPLATE_COLUMNS[5]]: 'eleve_novice',
    },
    {
    ...
    },
    {
      ...
      [GROUP_TEMPLATE_COLUMNS[3]]: '',
      [GROUP_TEMPLATE_COLUMNS[4]]: 'Exemple club (type club), profil écrit avec son nom affiché.',
      [GROUP_TEMPLATE_COLUMNS[5]]: 'n3beur novice',
    },
  ];
}
...
async function loadGroupsIndex() {
  const rows = await queryAll(
    'SELECT id, slug, name, kind, parent_group_id, description, default_role_id, is_active FROM `groups`',
  );
  const byId = new Map();
...
/**
 * Groupe par slug ou nom. Avec `parentId`, la recherche est **bornée aux enfants de ce
 * parent** : « 6ème A > Atelier sciences » puis « 6ème B > Atelier sciences » désignent deux
 * ateliers distincts, chacun sous sa classe (CDG-24).
 */
function findGroupInIndex(index, nameOrSlug, parentId = null) {
  ...
  if (parentId) {
    ...
    return (
      index.rows.find(
        (row) =>
          String(row.parent_group_id || '') === parent &&
...
/** Vrai si `candidateParentId` est `groupId` lui-même ou l'un de ses descendants (cycle). */
function wouldCreateCycle(index, groupId, candidateParentId) {
  const target = String(groupId);
  let cursor = candidateParentId ? String(candidateParentId) : null;
  const seen = new Set();
  while (cursor && !seen.has(cursor)) {
    if (cursor === target) return true;
    seen.add(cursor);
    const row = index.byId.get(cursor);
    cursor = row?.parent_group_id ? String(row.parent_group_id) : null;
  }
  return false;
}
...
async function createGroupRecord({
  ...
  await execute(
    `INSERT INTO \`groups\` (id, slug, name, description, kind, parent_group_id, default_role_id, is_active, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)`,
    [
      id,
      finalSlug,
...
async function resolveGroupPath(
  ...
  pathParts,
  index,
  { createMissing = true, dryRun = false } = {},
) {
  const created = [];
  const wouldCreate = [];
  let parentId = null;
  ...
  for (let i = 0; i < pathParts.length; i += 1) {
    ...
    // En aperçu, un groupe « à créer » plus haut dans le chemin porte un identifiant fictif :
    // ses enfants sont alors eux aussi « à créer », sans rien écrire.
    const existing = String(parentId || '').startsWith('dry:')
      ? null
      : findGroupInIndex(index, label, parentId);
    if (existing) {
    ...
    if (parentId && !String(parentId).startsWith('dry:')) {
    ...
    if (dryRun) {
      wouldCreate.push(pathParts.slice(0, i + 1).join(' > '));
      current = { id: `dry:${pathParts.slice(0, i + 1).join('>')}`, name: label };
      parentId = current.id;
      continue;
    }
  ...
  return { ok: true, group: current, created, wouldCreate };
}

/**
 * Aperçu (dry-run) des références de groupes d'un import : quels groupes seraient créés,
 * quelles références seraient refusées — sans rien écrire (CDG-51).
 * @returns {Promise<{ toCreate: string[], errors: string[] }>}
 */
async function previewGroupRefs(auth, groupRefs, index) {
  const toCreate = new Set();
  const errors = [];
  for (const ref of groupRefs || []) {
    const resolved = await resolveGroupPath(auth, ref.path, index, { dryRun: true });
    if (!resolved.ok) {
      errors.push(resolved.error);
      continue;
    }
    for (const path of resolved.wouldCreate || []) toCreate.add(path);
  }
  return { toCreate: [...toCreate], errors };
}

/** Rattachement d'un compte (élève ou enseignant) — même chemin que la console (`lib/groupMembers.js`). */
async function addUserToGroup(userId, userType, groupId) {
...
function mapGroupImportRow(row = {}) {
  ...
  return {
    ...
    // Type absent = inchangé à la mise à jour (`undefined`), `class` à la création.
    kind: normalizeOptionalString(mapped.kind) ? normalizeGroupKind(mapped.kind) : undefined,
    parent: normalizeOptionalString(mapped.parent),
    description: normalizeOptionalString(mapped.description),
    defaultRole: normalizeOptionalString(mapped.defaultRole),
  };
}
...
function mergeGroupImportPayloads(prev, next) {
  return {
    ...
    defaultRole: pickLastNonEmptyGroupField(prev.defaultRole, next.defaultRole),
  };
}
...
/**
 * Mise à jour d'un groupe existant : **une cellule vide laisse la valeur en place** (parent,
 * type, description, profil) — un fichier de rentrée incomplet ne détache plus tous les
 * sous-groupes ni ne remet une équipe en classe (CDG-25). Le parent se retire avec la
 * valeur `aucun` (ou `-`).
 * @param {{ name?: string, kind?: string|null, parentId?: string|null|undefined,
 *   description?: string|null, defaultRoleId?: number|null|undefined }} changes
 *   `undefined` = inchangé.
 */
async function updateGroupRecord(existing, changes, index) {
  const { name, kind, parentId, description, defaultRoleId } = changes;
  const finalKind = kind ? normalizeGroupKind(kind) || existing.kind : existing.kind || 'class';
  const finalName = String(name || existing.name || '').trim();
  const finalParentId = parentId === undefined ? (existing.parent_group_id ?? null) : parentId;
  const finalDescription =
    description === undefined || description === null
      ? (existing.description ?? null)
      : description;
  const finalRoleId =
    defaultRoleId === undefined ? (existing.default_role_id ?? null) : (defaultRoleId ?? null);
  await execute(
    `UPDATE \`groups\`
        SET name = ?, description = ?, kind = ?, parent_group_id = ?, default_role_id = ?,
            is_active = 1, updated_at = NOW()
      WHERE id = ?`,
    [finalName, finalDescription, finalKind, finalParentId, finalRoleId, existing.id],
  );
  const row = {
    ...existing,
    name: finalName,
    kind: finalKind,
    parent_group_id: finalParentId,
    description: finalDescription,
    default_role_id: finalRoleId,
    is_active: 1,
  };
  rememberGroupInIndex(index, row);
  return row;
...
async function importGroupsFromRows(auth, rawRows, { dryRun = false } = {}) {
  ...
  while (pending.length > 0 && guard < payloads.length + 2) {
    ...
    for (const item of pending) {
      ...
      // `undefined` = parent inchangé (cellule vide) ; `null` = détaché (« aucun » / « - »).
      let parentId;
      const parentCell = String(payload.parent || '')
        .trim()
        .toLowerCase();
      if (['aucun', 'aucune', 'none', '-'].includes(parentCell)) {
        parentId = null;
      ...
      if (payload.defaultRole) {
        const roleCheck = await validateGroupDefaultRole(auth, payload.defaultRole);
        if (!roleCheck.ok) {
          report.totals.skipped_invalid += 1;
          report.errors.push({ row: rowNumber, field: 'default_role', error: roleCheck.error });
          continue;
        }
      ...
      if (existing) {
        if (parentId && wouldCreateCycle(index, existing.id, parentId)) {
          report.totals.skipped_invalid += 1;
          report.errors.push({
            row: rowNumber,
            field: 'parent',
            error: 'Parenté circulaire interdite (le groupe serait son propre ancêtre)',
          });
          continue;
        }
        if (!(await isGroupInManageScope(auth, existing.id)) && !canBypassGroupScope(auth)) {
        ...
        if (!dryRun) {
          ...
          if (defaultRoleId !== undefined) {
            const { recomputeGroupMembersRoles } = require('./effectiveRole');
            await recomputeGroupMembersRoles(existing.id);
          }
        }
