...
async function revealFeuilletForSpeciesStudy(
  ...
  let skippedAsAlreadyFound = false;
  await withTransaction(async (tx) => {
    await lockGlTeamRow(tx, { gameId, teamId });
    const existing = await tx.queryOne(
      `SELECT status, discovered_at FROM gl_game_feuillet_states
        WHERE game_id = ? AND team_id = ? AND feuillet_code = ? LIMIT 1`,
      [gameId, teamId, feuillet.feuillet_code],
    );
    if (existing && (existing.discovered_at || String(existing.status || '') !== 'locked')) {
      skippedAsAlreadyFound = true;
      return;
    }
  ...
  if (skippedAsAlreadyFound) return null;
