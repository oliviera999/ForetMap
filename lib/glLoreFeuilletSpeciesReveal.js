...
const { BIOME_TO_PAYS, biomeToPays } = require('./glBiomePays');
...
/** Premier feuillet du pool du chapitre que l'équipe n'a pas encore trouvé. */
async function findNextChapterPoolFeuillet(deps, gameId, teamId, chapterId) {
  if (!chapterId) return null;
  const pool = await resolveChapterFeuilletPool(deps, { chapterId });
  for (const row of pool) {
    if (!(await isFeuilletRevealedForTeam(deps, gameId, teamId, row.feuillet_code))) return row;
  }
  return null;
}

/**
 * Feuillet à révéler pour l'étude d'une espèce, du plus spécifique au plus large :
 *   1. le feuillet dédié à cette espèce (`lien_canal = 'espece'`) ;
 *   2. le prochain feuillet du pays de l'espèce (`lien_canal = 'espece_pays'`) ;
 *   3. **repli** : le premier feuillet non trouvé du pool du chapitre.
 *
 * Sans (3), l'étude d'espèce plafonnait au nombre de feuillets `espece_pays` du pays —
 * 3 pour le pays 1 contre 15 pour le pays 4, alors que chaque chapitre propose une
 * cinquantaine d'espèces — et ne rapportait **rien** pour les biomes hors voyage
 * (mangrove, prairie/steppe), qui n'ont pas de pays.
 */
async function pickFeuilletForSpeciesStudy(
  deps,
  { gameId, teamId, chapterId = null, speciesCode, biomeSlug },
) {
  const direct = await findSpeciesLinkedFeuillet(deps, gameId, teamId, speciesCode);
  if (direct) return direct;
  const pays = biomeToPays(biomeSlug);
  if (pays != null) {
    const next = await findNextPaysFeuillet(deps, gameId, teamId, pays);
    if (next) return next;
  }
  return findNextChapterPoolFeuillet(deps, gameId, teamId, chapterId);
}

async function revealFeuilletForSpeciesStudy(
  deps,
  { gameId, teamId, speciesCode, biomeSlug, actorType, actorId, actorName = null, isMj = false },
) {
  // La partie est chargée d'abord : le repli du picking a besoin du chapitre.
  ...
  await withTransaction(async (tx) => {
    ...
    await upsertFeuilletState(tx, {
      ...
      unlockedVia: 'espece',
      kingdomZoneId: null,
      discoveredByPlayerId: actorId != null ? String(actorId) : null,
      discoveredByName: actorName || null,
      discoveredSource: 'espece',
    });
  });
