...
function buildImportReportBase(dryRun, rowsCount) {
  return createImportReport({
    dryRun,
    received: rowsCount,
    extraTotals: { relations_synced: 0, biome_links_synced: 0 },
  });
}
...
async function applyGlossaryImport(deps, glossaryRows, options = {}) {
  ...
  // Upserts des termes par lots de 100 (G4) — comptage created/updated inchangé.
  await executeQuestionUpserts({ execute }, validRows, {
    sql: GLOSSARY_UPSERT_SQL,
    buildParams: buildGlossaryUpsertParams,
    existingCodes,
    totals: report.totals,
    codeOf: (payload) => payload.glossary_code,
  });

  // Biomes inconnus créés d'abord (rare), puis liens terme↔biome par lots.
  const biomeLinkPairs = [];
  for (const { payload } of validRows) {
    if (payload.all_biomes) continue;
    for (const slug of payload.biome_slugs) {
      if (!knownBiomes.has(slug)) {
        await execute(
          `INSERT INTO gl_biomes (slug, nom, order_index, created_at, updated_at)
      ...
      biomeLinkPairs.push([payload.glossary_code, slug]);
    }
  }
  for (const chunk of chunkRows(biomeLinkPairs)) {
    await execute(
      expandMultiRowInsertSql(
        `INSERT IGNORE INTO gl_glossary_term_biomes (glossary_code, biome_slug) VALUES (?, ?)`,
        chunk.length,
  ...
  report.totals.biome_links_synced += biomeLinkPairs.length;
  ...
  for (const { payload } of validRows) {
    ...
    for (const toCode of related) {
      if (toCode === payload.glossary_code) continue;
      relationPairs.push([payload.glossary_code, toCode]);
      report.totals.relations_synced += 1;
    }
  }
  for (const chunk of chunkRows(relationPairs)) {
    await execute(
      expandMultiRowInsertSql(
        `INSERT IGNORE INTO gl_glossary_term_relations (from_code, to_code) VALUES (?, ?)`,
        chunk.length,
...
const resolveImportRows = (body = {}) => resolveWorkbookImportRows(body, parseGlossaryWorkbook);
