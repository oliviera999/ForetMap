...
function buildGlossaryPayloadFromBody(body = {}) {
  ...
  if (Array.isArray(body.termes_lies)) {
    termesLies = body.termes_lies.map((s) => asTrimmedString(s)).filter(Boolean).join(', ');
  } else {
...
async function loadGlossaryTermToCodeMap(queryAll) {
  const rows = await queryAll(
    `SELECT glossary_code, terme, variantes FROM gl_glossary_terms`
  );
  return buildTermToCodeMap(rows);
}

async function syncGlossaryBiomesForTerm(payload, knownBiomes, { execute }) {
  await execute('DELETE FROM gl_glossary_term_biomes WHERE glossary_code = ?', [payload.glossary_code]);
...
async function syncGlossaryRelationsForTerm(payload, termToCode, { execute }) {
  await execute('DELETE FROM gl_glossary_term_relations WHERE from_code = ?', [payload.glossary_code]);
...
async function buildGlossaryTemplateWorkbook() {
  return buildWorkbookBuffer([{ name: GLOSSARY_SHEET, aoa: [GLOSSARY_TEMPLATE_HEADERS, GLOSSARY_TEMPLATE_SAMPLE_ROW] }]);
}
