'use strict';

const crypto = require('crypto');
const { execute, queryOne } = require('../database');
const { getSettingValue } = require('./settings');

const PASSWORD_RESET_MIN_LEN = 4;
const PASSWORD_RESET_TTL_MINUTES = 60;
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

function hashResetToken(token) {
  return crypto.createHash('sha256').update(token).digest('hex');
}

function getPasswordResetBaseUrl() {
  return (
    process.env.PASSWORD_RESET_BASE_URL || process.env.FRONTEND_ORIGIN || 'http://localhost:3000'
  );
}

/** Ensemble des hôtes autorisés pour les liens de réinitialisation (issus de l'env). */
function collectAllowedResetHosts() {
  const hosts = new Set();
  const sources = [
    process.env.GL_PASSWORD_RESET_BASE_URL,
    process.env.GL_FRONTEND_ORIGIN,
    process.env.PASSWORD_RESET_BASE_URL,
    process.env.FRONTEND_ORIGINS,
    process.env.FRONTEND_ORIGIN,
  ];
  for (const entry of sources) {
    String(entry || '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean)
      .forEach((origin) => {
        try {
          hosts.add(new URL(origin).host.toLowerCase());
        } catch (_) {
          hosts.add(
            origin
              .replace(/^https?:\/\//i, '')
              .replace(/\/.*$/, '')
              .toLowerCase(),
          );
        }
      });
  }
  return hosts;
}

function getGlPasswordResetBaseUrl(req) {
  const fromEnv = String(
    process.env.GL_PASSWORD_RESET_BASE_URL || process.env.GL_FRONTEND_ORIGIN || '',
  ).trim();
  if (fromEnv) return fromEnv.replace(/\/$/, '');
  // Anti password-reset poisoning : ne pas faire confiance à l'en-tête Host brut.
  // Il n'est accepté que s'il correspond à une origine configurée (ou localhost hors prod).
  if (req?.protocol && req?.get) {
    const host = String(req.get('host') || '').toLowerCase();
    const isLocal = /^(localhost|127\.0\.0\.1|\[::1\])(:\d+)?$/.test(host);
    const allowed = collectAllowedResetHosts();
    if (host && (allowed.has(host) || (process.env.NODE_ENV !== 'production' && isLocal))) {
      return `${req.protocol}://${req.get('host')}`.replace(/\/$/, '');
    }
  }
  return getPasswordResetBaseUrl().replace(/\/$/, '');
}

function makeResetUrl(type, token, { product = 'foret' } = {}, req = null) {
  const base = (
    product === 'gl' ? getGlPasswordResetBaseUrl(req) : getPasswordResetBaseUrl()
  ).replace(/\/$/, '');
  if (product === 'gl') {
    return `${base}/#resetType=${encodeURIComponent(type)}&resetToken=${encodeURIComponent(token)}`;
  }
  return `${base}/?resetType=${encodeURIComponent(type)}&resetToken=${encodeURIComponent(token)}`;
}

async function getPasswordMinLength() {
  const n = await getSettingValue('security.password_min_length', PASSWORD_RESET_MIN_LEN);
  const parsed = parseInt(n, 10);
  if (!Number.isFinite(parsed)) return PASSWORD_RESET_MIN_LEN;
  return Math.min(Math.max(parsed, 4), 32);
}

/**
 * Plancher renforcé pour les comptes à privilèges.
 *
 * `security.password_min_length` descend à 4 et vaut 4 par défaut — un compromis assumé pour
 * des élèves de sixième, qui saisissent leur mot de passe en classe. Il ne l'est pas pour un
 * compte **professeur ou administrateur** : ceux-là portent `admin.impersonate`, c'est-à-dire
 * la prise de contrôle de n'importe quel compte. Quatre caractères y sont hors de proportion
 * avec ce que la clé ouvre.
 *
 * Le réglage garde donc son sens — il pilote le plancher **élève** — et les comptes à
 * privilèges reçoivent un plancher propre, jamais inférieur. Rien n'invalide les mots de
 * passe existants : la règle ne s'applique qu'à une création ou à un changement.
 *
 * Le motif n'est pas nouveau dans le dépôt : `routes/gl/auth.js` relève déjà à 8 pour le
 * staff Gnomes & Licornes (`Math.max(8, …)`). Cette fonction généralise cette idée côté
 * ForetMap plutôt que de la réécrire à chaque site d'appel.
 */
const PRIVILEGED_PASSWORD_MIN_LEN = 12;

async function getPasswordMinLengthFor(userType) {
  const base = await getPasswordMinLength();
  if (String(userType || '').trim() === 'teacher') {
    return Math.max(base, PRIVILEGED_PASSWORD_MIN_LEN);
  }
  return base;
}

/**
 * Plafond de demandes « mot de passe oublié » **par adresse visée** (CDG-52) : le limiteur
 * d'IP laissait passer 60 courriels vers une même boîte en 15 minutes. Au-delà de
 * FORGOT_PASSWORD_MAX_PER_WINDOW demandes pour une adresse dans la fenêtre, la route répond
 * toujours de façon neutre mais n'envoie rien. État en mémoire (un processus), comme le
 * throttle de connexion.
 */
const FORGOT_PASSWORD_WINDOW_MS = 15 * 60 * 1000;
const FORGOT_PASSWORD_MAX_PER_WINDOW = 3;
const forgotPasswordHits = new Map();

function forgotPasswordAllowed(email, now = Date.now()) {
  const key = String(email || '')
    .trim()
    .toLowerCase();
  if (!key) return false;
  const recent = (forgotPasswordHits.get(key) || []).filter(
    (t) => now - t < FORGOT_PASSWORD_WINDOW_MS,
  );
  if (recent.length >= FORGOT_PASSWORD_MAX_PER_WINDOW) {
    forgotPasswordHits.set(key, recent);
    return false;
  }
  recent.push(now);
  forgotPasswordHits.set(key, recent);
  if (forgotPasswordHits.size > 10000) {
    for (const [k, hits] of forgotPasswordHits) {
      if (!hits.some((t) => now - t < FORGOT_PASSWORD_WINDOW_MS)) forgotPasswordHits.delete(k);
    }
  }
  return true;
}

function resetForgotPasswordLimiterForTests() {
  forgotPasswordHits.clear();
}

async function createPasswordResetToken(userType, userId) {
  const token = crypto.randomBytes(32).toString('hex');
  const tokenHash = hashResetToken(token);
  const ttlMs = PASSWORD_RESET_TTL_MINUTES * 60 * 1000;
  const expiresAt = new Date(Date.now() + ttlMs);
  await execute(
    'INSERT INTO password_reset_tokens (id, user_type, user_id, token_hash, expires_at, used_at) VALUES (?, ?, ?, ?, ?, NULL)',
    [crypto.randomUUID(), userType, String(userId), tokenHash, expiresAt],
  );
  return token;
}

async function consumePasswordResetToken(userType, token) {
  const tokenHash = hashResetToken(token);
  const row = await queryOne(
    `SELECT id, user_id
       FROM password_reset_tokens
      WHERE user_type = ?
        AND token_hash = ?
        AND used_at IS NULL
        AND expires_at > NOW()
      LIMIT 1`,
    [userType, tokenHash],
  );
  if (!row) return null;

  const result = await execute(
    'UPDATE password_reset_tokens SET used_at = NOW() WHERE id = ? AND used_at IS NULL',
    [row.id],
  );
  if (!result.affectedRows) return null;
  return row.user_id;
}

module.exports = {
  FORGOT_PASSWORD_MAX_PER_WINDOW,
  forgotPasswordAllowed,
  resetForgotPasswordLimiterForTests,
  EMAIL_RE,
  PASSWORD_RESET_MIN_LEN,
  PASSWORD_RESET_TTL_MINUTES,
  hashResetToken,
  getPasswordResetBaseUrl,
  getGlPasswordResetBaseUrl,
  collectAllowedResetHosts,
  makeResetUrl,
  getPasswordMinLength,
  getPasswordMinLengthFor,
  PRIVILEGED_PASSWORD_MIN_LEN,
  createPasswordResetToken,
  consumePasswordResetToken,
};
