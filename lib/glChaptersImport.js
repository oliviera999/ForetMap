...
async function buildChaptersWorkbookFromData(data, scope) {
  ...
  if (scope === 'content_markers' || scope === 'full') {
    sheets.push({ name: MARKERS_SHEET, aoa: [MARKERS_TEMPLATE_HEADERS, ...data.markers.map((m) => m.row)] });
  }

  if (scope === 'full') {
    sheets.push({ name: ZONES_SHEET, aoa: [ZONES_TEMPLATE_HEADERS, ...data.zones.map((z) => z.row)] });
...
async function buildChaptersTemplateWorkbook(scopeInput) {
  ...
  if (scope === 'content_markers' || scope === 'full') {
    sheets.push({ name: MARKERS_SHEET, aoa: [MARKERS_TEMPLATE_HEADERS, ...MARKERS_TEMPLATE_SAMPLE_ROWS] });
  }
