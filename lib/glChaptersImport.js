...
const { chunkRows, expandMultiRowInsertSql, readSheetRows } = require('./shared/xlsxImportCore');
...
const { lowerTrim } = require('./shared/slug');
...
function buildChapterPayload(row = {}) {
  ...
  const slug = lowerTrim(mapped.slug);
  ...
  const storyMarkdown = repairSupplementaryPlaneEmojiMojibake(
    asTrimmedString(mapped.story_markdown),
  );
  const biotopeMarkdown = repairSupplementaryPlaneEmojiMojibake(
    asTrimmedString(mapped.biotope_markdown),
  );
  const biocenoseMarkdown = repairSupplementaryPlaneEmojiMojibake(
    asTrimmedString(mapped.biocenose_markdown),
  );
  const sortilegesMarkdown = repairSupplementaryPlaneEmojiMojibake(
    asTrimmedString(mapped.sortileges_markdown),
  );
...
function buildMarkerPayload(row = {}) {
  ...
  const chapterSlug = lowerTrim(mapped.chapter_slug);
  ...
  const emojiRaw = asTrimmedString(mapped.emoji);
  const emoji = emojiRaw ? repairSupplementaryPlaneEmojiMojibake(emojiRaw) : null;
...
async function loadChaptersExportData(deps, options = {}) {
  ...
  const slugFilter = lowerTrim(options.slug);
  ...
  for (const [chapterSlug, items] of byChapter.entries()) {
    ...
    for (const { payload } of items) {
      ...
      if (targetId) {
        ...
        // Les nouveaux repères ne rejoignent pas `keptIds` : `toDelete` ne compare
        // qu'aux repères PRÉEXISTANTS, jamais aux insertions de ce même import.
        insertParams.push([
          chapterId,
          payload.xPct,
          payload.yPct,
    ...
    // G4 : insertions par lots de 100 au lieu d'un aller-retour MySQL par repère.
    for (const chunk of chunkRows(insertParams)) {
      await execute(
  ...
  for (const [chapterSlug, items] of byChapter.entries()) {
    ...
    for (const { payload } of items) {
      ...
      if (targetId) {
        ...
        // Comme pour les repères : `toDelete` ne compare qu'aux zones préexistantes.
        const musicUrlsJson = payload.musicUrls?.length ? JSON.stringify(payload.musicUrls) : null;
        insertParams.push([
          chapterId,
          payload.label,
          payload.description,
    ...
    // G4 : insertions par lots de 100 au lieu d'un aller-retour MySQL par zone.
    for (const chunk of chunkRows(insertParams)) {
      await execute(
        expandMultiRowInsertSql(
