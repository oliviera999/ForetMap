...
function buildMarkerPayload(row = {}) {
  ...
  const eventType = asTrimmedString(mapped.event_type).toLowerCase() || null;
...
function validateMarkerPayload(payload, rowNumber, knownChapterSlugs) {
  ...
  if (payload.eventType && !MARKER_EVENT_TYPES.has(payload.eventType)) {
    errors.push({ row: rowNumber, field: 'type_evenement', error: `type_evenement invalide : ${payload.eventType}` });
  }
  if (payload.hasEventConfig && !payload.eventConfig) {
    errors.push({ row: rowNumber, field: 'event_config_json', error: 'event_config_json invalide' });
  }
...
function validateZonePayload(payload, rowNumber, knownChapterSlugs) {
  ...
  if (!payload.hasPoints || !validateZonePoints(payload.points)) {
    errors.push({ row: rowNumber, field: 'points_json', error: 'points_json invalide (3-200 points {x,y} en %)' });
  }
...
function parseChaptersWorkbook(buffer) {
...
function resolveChaptersImportRows(body = {}) {
...
function markerRowToExport(marker, chapterSlug) {
  const eventConfigJson = marker.event_config_json
    ? String(marker.event_config_json)
    : (marker.event_config ? serializeEventConfig(marker.event_config) : '');
  return [
    chapterSlug,
    marker.id ?? '',
    marker.label ?? '',
...
async function loadChaptersExportData(deps, options = {}) {
  ...
  const chapterRows = slugFilter
    ? await queryAll(
      `SELECT id, slug, title, biome, map_image_url, story_markdown, biotope_markdown,
              biocenose_markdown, sortileges_markdown, order_index
         FROM gl_chapters
        WHERE slug = ?
  ...
  if (scope === 'full' && chapterIds.length > 0) {
    ...
    const zoneRows = await queryAll(
      `SELECT id, chapter_id, label, description, points_json, color, music_url, music_volume
...
function buildChaptersWorkbookFromData(data, scope) {
  const wb = XLSX.utils.book_new();
  const chapterData = [CHAPTERS_TEMPLATE_HEADERS, ...data.chapters.map((c) => c.row)];
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(chapterData), CHAPTERS_SHEET);

  if (scope === 'content_markers' || scope === 'full') {
    const markerData = [MARKERS_TEMPLATE_HEADERS, ...data.markers.map((m) => m.row)];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(markerData), MARKERS_SHEET);
  }

  if (scope === 'full') {
    const zoneData = [ZONES_TEMPLATE_HEADERS, ...data.zones.map((z) => z.row)];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(zoneData), ZONES_SHEET);
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(charteAoA), CHARTE_SHEET);
  }

  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}

function buildChaptersTemplateWorkbook(scopeInput) {
  const scope = normalizeExportScope(scopeInput);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(
    wb,
    XLSX.utils.aoa_to_sheet([CHAPTERS_TEMPLATE_HEADERS, CHAPTERS_TEMPLATE_SAMPLE_ROW]),
    CHAPTERS_SHEET
  );
  ...
  if (scope === 'content_markers' || scope === 'full') {
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([MARKERS_TEMPLATE_HEADERS, ...MARKERS_TEMPLATE_SAMPLE_ROWS]),
      MARKERS_SHEET
    );
  }
  ...
  if (scope === 'full') {
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([ZONES_TEMPLATE_HEADERS, ZONES_TEMPLATE_SAMPLE_ROW]),
      ZONES_SHEET
    );
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.aoa_to_sheet([CHARTE_TEMPLATE_HEADERS, CHARTE_TEMPLATE_SAMPLE_ROW]),
      CHARTE_SHEET
    );
  }
...
async function applyMarkersImport(deps, validMarkers, options, report, chapterIdBySlug) {
  ...
  for (const [chapterSlug, items] of byChapter.entries()) {
    ...
    if (syncReperes) {
      const toDelete = existingMarkers
        .map((m) => Number(m.id))
          ...
          await execute(
            `UPDATE gl_kingdom_zones
                SET label = ?, description = ?, points_json = ?, color = ?,
                    music_url = ?, music_volume = ?, updated_at = NOW()
              WHERE id = ? AND chapter_id = ?`,
            [
              payload.label,
              payload.description,
...
async function applyChaptersImport(deps, parsed, options = {}) {
  ...
  const existingRows = await queryAll(
    'SELECT id, slug, title, biome, map_image_url, story_markdown, biotope_markdown, biocenose_markdown, sortileges_markdown, order_index FROM gl_chapters'
  );
  ...
  if (dryRun) {
    ...
    if (parsed.hasCharteSheet && charteRows.length > 0) {
      const charteReport = await applyChapterCharteImport({ queryAll, execute }, charteRows, { dryRun: true });
      report.totals.charte_updated = (charteReport.totals.updated || 0) + (charteReport.totals.created || 0);
  ...
  for (const { payload, existing } of validChapters) {
    if (existing) {
      ...
      const inserted = await queryAll('SELECT id FROM gl_chapters WHERE slug = ? LIMIT 1', [payload.slug]);
  ...
  if (parsed.hasCharteSheet && charteRows.length > 0) {
    const charteReport = await applyChapterCharteImport({ queryAll, execute }, charteRows, { dryRun: false });
    report.totals.charte_updated = (charteReport.totals.updated || 0) + (charteReport.totals.created || 0);
    report.errors.push(...(charteReport.errors || []));
  }
