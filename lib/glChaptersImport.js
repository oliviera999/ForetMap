...
function buildMarkerPayload(row = {}) {
  ...
  const sousBiomeSlug = asTrimmedString(mapped.sous_biome_slug) || null;
...
function parseChaptersWorkbook(buffer, options = {}) {
  ...
  const chapterRows = slugFilter
    ? await queryAll(
      `SELECT id, slug, title, biome, map_image_url, story_markdown, biotope_markdown,
              biocenose_markdown, sortileges_markdown, souffle_face, order_index
         FROM gl_chapters
        WHERE slug = ?
...
async function applyChaptersImport(deps, parsed, options = {}) {
  ...
  const existingRows = await queryAll(
    'SELECT id, slug, title, biome, map_image_url, story_markdown, biotope_markdown, biocenose_markdown, sortileges_markdown, souffle_face, order_index FROM gl_chapters'
  );
    ...
    if (existing) {
      ...
      await execute(
        `INSERT INTO gl_chapters (slug, title, biome, map_image_url, story_markdown,
                                   ...
                                   souffle_face, order_index, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          payload.slug,
          payload.title,
