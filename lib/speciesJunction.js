...
function attachSpeciesToEntity(entity, speciesRows) {
...
async function runSpeciesSync(db, { table, entityColumn, entityId, plantIds, livingBeingsNames, livingBeingsTable, livingBeingsColumn }) {
  ...
  const sync = async (tx) => {
    ...
    await tx.execute(
      `UPDATE ${livingBeingsTable} SET living_beings = ? WHERE ${livingBeingsColumn} = ?`,
      [serializeLivingBeingsJson(names), entityId],
    );
  };
...
async function syncZoneSpecies(db, zoneId, plantIds, livingBeingsNames) {
  return runSpeciesSync(db, {
    ...
    plantIds,
    livingBeingsNames,
    livingBeingsTable: 'zones',
    livingBeingsColumn: 'id',
  });
}
