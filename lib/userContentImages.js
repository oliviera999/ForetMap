...
/** Taille max par image après décodage base64 (évite de saturer le corps JSON ~10 Mo). */
const MAX_USER_CONTENT_IMAGE_BYTES = Math.floor(1.5 * 1024 * 1024);
...
/** Décode une data URL / base64 ; vérifie taille et signature (JPEG, PNG, WebP). */
function decodeUserContentImageBuffer(imageData) {
  ...
  if (buf.length > MAX_USER_CONTENT_IMAGE_BYTES) {
    return { error: `Image trop volumineuse (max ${Math.round(MAX_USER_CONTENT_IMAGE_BYTES / (1024 * 1024) * 10) / 10} Mo après décodage)` };
  }
