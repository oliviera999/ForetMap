...
function journalAssetFileUrl(assetId) {
  return `/api/user-journal/assets/${Number(assetId)}/file`;
}
...
function isAllowedJournalImageUrl(url, userId) {
  ...
  // Route API authentifiée (illustrations privées, y compris après réécriture)
  if (/^\/api\/user-journal\/assets\/\d+\/file$/.test(s)) return true;
  return false;
}

/** Remplace les URL `/uploads/…` d'illustrations par la route API autorisée. */
function rewriteJournalImageUrls(bodyMarkdown, assets) {
  let out = String(bodyMarkdown || '');
  if (!out || !Array.isArray(assets)) return out;
  for (const asset of assets) {
    const path = String(asset.asset_path || '');
    if (!path || path.includes('..')) continue;
    const publicUrl = `/uploads/${path}`;
    if (!out.includes(publicUrl)) continue;
    out = out.split(publicUrl).join(journalAssetFileUrl(asset.id));
  }
  return out;
}
...
/**
 * Résout titres + cartes enrichies pour les encarts journal (batch par type).
...
async function resolveJournalEmbedCards(embeds) {
  ...
  if (plantIds.length) {
    ...
    const rows = await queryAll(
      `SELECT id, name, description, habitat, photo, photo_species
         FROM plants WHERE id IN (${placeholders})`,
      plantIds,
    );
    for (const r of rows) {
      const card = makeEmbedCard({
        title: r.name,
        label: EMBED_LABELS.plant,
        imageUrl: pickImageUrl(r.photo, r.photo_species),
        excerpt: truncatePlainText(r.description) || truncatePlainText(r.habitat),
      });
      if (card) cards[`plant|${r.id}`] = card;
    }
  }
  ...
  if (glossaryCodes.length) {
    ...
    const rows = await queryAll(
      `SELECT glossary_code, terme, definition_courte, definition_complete
         FROM glossary_terms WHERE glossary_code IN (${placeholders})`,
      glossaryCodes,
    );
    for (const r of rows) {
      const card = makeEmbedCard({
  ...
  if (tutorialIds.length) {
    ...
    const rows = await queryAll(
      `SELECT id, title, summary, html_content, cover_image_url
         FROM tutorials WHERE id IN (${placeholders})`,
      tutorialIds,
    );
    for (const r of rows) {
      const card = makeEmbedCard({
        title: r.title,
        label: EMBED_LABELS.tutorial,
        imageUrl: pickImageUrl(r.cover_image_url),
        excerpt: truncatePlainText(r.summary) || truncatePlainText(r.html_content),
      });
      if (card) cards[`tutorial|${r.id}`] = card;
    }
  }
...
function serializeArticle(row, assets, zoneName = null) {
  const rawBody = row?.body_markdown != null ? String(row.body_markdown) : '';
  const bodyMarkdown = rewriteJournalImageUrls(rawBody, assets);
  return {
    id: Number(row.id),
...
/**
 * `observation_logs.created_at` est un `VARCHAR(32)` (héritage du portage SQLite → MySQL,
 * cf. `lib/legacyTimestampNormalization.js`) qui porte de l'ISO-8601 UTC — `2026-09-13T12:37:27.071Z`.
 * La cible `user_journal_articles.created_at` est un vrai `DATETIME` : passer la chaîne telle
 * quelle fait échouer l'INSERT en `ER_TRUNCATED_WRONG_VALUE`, et la reprise renvoyait un 500
 * sur `GET /api/user-journal/me` dès qu'une observation restait à migrer.
 *
 * On repasse donc par un `Date`, que `mysql2` sérialise comme le repli `new Date()` juste à
 * côté. Les deux écritures héritées s'y retrouvent : le suffixe `Z` fixe le fuseau, et la
 * forme `2026-04-05 18:04:00` — écrite en heure locale — est relue en heure locale.
 * Une valeur illisible retombe sur maintenant plutôt que de bloquer toute la reprise.
 */
function legacyTimestampToDate(value) {
      ...
      for (const o of pending) {
        const result = await execute(
          ...
          [
            ...
            o.zone_id || null,
            legacyTimestampToDate(o.created_at),
            legacyTimestampToDate(o.created_at),
          ],
        );
        ...
        if (o.image_path) {
          await execute(
            `INSERT INTO user_journal_article_assets
             ...
             VALUES (?, ?, ?, 'image/jpeg', 0, ?)`,
            [articleId, o.student_id, o.image_path, legacyTimestampToDate(o.created_at)],
          );
        }
      }
...
async function searchJournalEmbeds(type, q, limit = 20) {
  ...
  if (t === 'plant') {
    const rows = await queryAll(
      'SELECT id AS ref, name AS title FROM plants WHERE name LIKE ? OR scientific_name LIKE ? OR second_name LIKE ? ORDER BY name ASC LIMIT ' +
        lim,
      [like, like, like],
    );
    return rows.map((r) => ({ type: 'plant', ref: String(r.ref), title: String(r.title || '') }));
  }
  if (t === 'glossary') {
    const rows = await queryAll(
      "SELECT glossary_code AS ref, terme AS title FROM glossary_terms WHERE statut = 'actif' AND (terme LIKE ? OR glossary_code LIKE ?) ORDER BY terme ASC LIMIT " +
        lim,
      [like, like],
    );
    return rows.map((r) => ({
  ...
  if (t === 'tutorial') {
    const rows = await queryAll(
      'SELECT id AS ref, title FROM tutorials WHERE is_active = 1 AND title LIKE ? ORDER BY title ASC LIMIT ' +
        lim,
      [like],
    );
    return rows.map((r) => ({
