...
/**
 * `observation_logs.created_at` est un `DATETIME(3)` depuis la migration 249 ; il remonte
 * donc déjà en objet `Date`, que cette fonction laisse passer tel quel.
 *
 * Elle reste nécessaire pour les reprises : un export antérieur à la migration porte encore
 * la chaîne ISO-8601 UTC (`2026-09-13T12:37:27.071Z`) ou, plus rarement, la forme locale
 * `2026-04-05 18:04:00`. Passer l'une ou l'autre telle quelle à l'INSERT vers
 * `user_journal_articles.created_at` échouait en `ER_TRUNCATED_WRONG_VALUE`, et la reprise
 * renvoyait un 500 sur `GET /api/user-journal/me` dès qu'une observation restait à migrer.
 *
 * On repasse donc par un `Date`, que `mysql2` sérialise comme le repli `new Date()` juste à
 * côté. Une valeur illisible retombe sur maintenant plutôt que de bloquer toute la reprise.
 */
