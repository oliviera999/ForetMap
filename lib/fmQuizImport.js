...
async function syncGlossaryLinks(validRows, deps, dryRun) {
  ...
  // Source de vérité unifiée : resource_question_links (cf. migration 144). Le rapprochement
  // par mots-clés ne purge **que ses propres liens** (origin='keyword', migration 293) : la
  // curation manuelle (migration 227, stockée sous origin='import' et bloquante) et les liens
  // des scripts d'enrichissement ('generated'/'auto') survivent à un import (audit du
  // 25/09/2026, § 1.3.3). resource_type='glossary' obligatoire (modèle polymorphe).
  if (!dryRun) {
    await execute(
