...
const {
  ...
  asTrimmedString,
} = require('./glossaryMatch');
const { CHOICE_LETTERS } = require('./qcmChoices');
...
function buildQuestionPayload(row = {}) {
  ...
  return {
    ...
    // Cellule vide : `null`, résolu à l'application de l'import (une question existante garde
    // son statut, une nouvelle est `actif`). Un fichier sans colonne statut ne réactive donc
    // plus les questions désactivées (lot B, audit du 25/09/2026, § 1.5).
    statut: normalizeOptionalString(mapped.statut)?.toLowerCase() ?? null,
...
/** Statuts admis : une valeur libre (« Actif », « brouillon ») activait ou masquait au hasard. */
const QUESTION_STATUTS = new Set(['actif', 'inactif']);
...
function validateQuestionPayload(payload, rowNumber, knownCategories) {
  ...
  if (payload.statut != null && !QUESTION_STATUTS.has(payload.statut)) {
    errors.push({
      row: rowNumber,
      field: 'statut',
      error: `statut invalide : ${payload.statut} (attendu : actif ou inactif)`,
    });
  }
...
async function syncGlossaryLinks(validRows, deps, dryRun) {
  const glossaryRows = await deps.queryAll(
  ...
  for (const { payload } of validRows) {
    ...
    for (const term of matched) {
      links.push({ glossaryCode: term.glossary_code, questionCode: payload.question_code });
    }
  }

  // Service unique des liens (`lib/pedago/learningLinks.js`) : le rapprochement par
  // mots-clés ne purge **que ses propres liens** (origin='keyword', lot P0) ; la curation
  // relue (migration 227, stockée sous origin='import' et bloquante), les saisies, les
  // reprises éditoriales (migration 300) et les liens des scripts survivent à un import
  // (audit du 25/09/2026, § 1.3.3). Purge globale : les liens par mots-clés sont recalculés
  // pour les lignes importées. Même transaction que l'upsert des questions (route d'import).
  if (!dryRun) {
    await learningLinks.replaceKeywordGlossaryLinks(deps, { links });
  }
  return links.length;
}

/**
 * Statut des lignes dont la cellule est vide : celui de la question existante, sinon `actif`.
 * Muté sur place (les payloads sont propres à cet import).
 */
async function resolveMissingStatuts(deps, validRows) {
  const missing = validRows.filter(({ payload }) => payload.statut == null);
  if (missing.length === 0) return;
  const rows = await deps.queryAll('SELECT question_code, statut FROM quiz_questions');
  const current = new Map(
    (Array.isArray(rows) ? rows : []).map((r) => [String(r.question_code), r.statut]),
  );
  for (const { payload } of missing) {
    payload.statut = current.get(String(payload.question_code)) || 'actif';
  }
}
...
async function applyFmQuizImport(deps, categoryRows, questionRows, options = {}) {
  ...
  const existingCodes = await loadExistingQuestionCodes(deps, EXISTING_CODES_SQL);
  await resolveMissingStatuts(deps, validRows);
