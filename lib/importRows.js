...
/**
 * Rapport d'import commun à tous les importeurs (élèves, joueurs, tâches, plantes, espèces,
 * sorts, glossaires, chapitres, questions) : une seule forme, les compteurs propres à chaque
 * importeur s'ajoutent via `extraTotals`. Sept fabriques locales coexistaient
 * (audit 2026-09-13, §4.2).
 *
 * @param {object} params
 * @param {boolean} params.dryRun
 * @param {number} params.received nombre de lignes reçues
 * @param {string} [params.sourceType='xlsx']
 * @param {string} [params.strategy] stratégie d'import (plantes : `upsert` / `skip_existing`…)
 * @param {object} [params.extraTotals] compteurs supplémentaires initialisés à 0 (ou à la valeur donnée)
 */
function createImportReport({ dryRun, received, sourceType = 'xlsx', strategy, extraTotals = {} }) {
  const report = {
    dryRun: !!dryRun,
    sourceType,
    totals: {
  ...
  if (strategy !== undefined) return { strategy, ...report };
  return report;
}
