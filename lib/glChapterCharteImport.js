...
function parseChapterCharteWorkbook(buffer, options = {}) {
...
function buildChapterCharteExportWorkbook(rows) {
  const wb = XLSX.utils.book_new();
  const data = [CHARTE_TEMPLATE_HEADERS, ...rows.map(fixExportColorOrder)];
  const ws = XLSX.utils.aoa_to_sheet(data);
  XLSX.utils.book_append_sheet(wb, ws, CHARTE_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}

function buildChapterCharteTemplateWorkbook() {
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet([CHARTE_TEMPLATE_HEADERS, CHARTE_TEMPLATE_SAMPLE_ROW]);
  XLSX.utils.book_append_sheet(wb, ws, CHARTE_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}
...
async function applyChapterCharteImport(deps, rawRows, options = {}) {
  ...
  const existingBySlug = new Map(
    existingRows.map((r) => [String(r.slug), r])
  );
  ...
  for (const { payload, existing } of validRows) {
    if (existing) {
      ...
      if (updates.length > 0) {
        ...
        await execute(
          `UPDATE gl_chapters SET ${updates.join(', ')} WHERE id = ?`,
          params
        );
