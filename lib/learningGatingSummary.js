...
const { exactGatingProduct } = require('./pedago/gatingProducts');
...
function serializeChallenge(state) {
  return {
    ...
    retry_cooldown_hours: state.retry_cooldown_hours,
    retry_cooldown_label: state.retry_cooldown_label,
    retry_cooldown_days: state.retry_cooldown_days,
    lock_mode: state.lock_mode,
    effective_sources: state.effective_sources,
    satisfied: state.satisfied,
    cooldown: state.cooldown,
    // ForetMap seulement (absent côté GL) : repli « tous niveaux » signalé.
    ...(state.level_fallback ? { level_fallback: state.level_fallback } : {}),
  };
}
...
function serializeSummaryItem(resourceRef, state) {
  return {
    ...
    remaining_hours: state.cooldown?.remaining_hours || 0,
    remaining_label: state.cooldown?.remaining_label || '',
    retry_days: state.cooldown?.retry_days || 0,
    retry_hours: state.cooldown?.retry_hours || 0,
    retry_label: state.cooldown?.retry_label || '',
    allowed_wrong_attempts: state.allowed_wrong_attempts || 0,
    wrong_attempts: state.cooldown?.wrong_attempts || 0,
  };
}
...
async function buildGatingSummary(
  ...
  // Côté GL, les réponses de l'ÉQUIPE sont préchargées À PART : `getChallengeState` ne les
  // fusionne que si la granularité effective de la ressource est `team` — les fusionner ici
  // faisait dire « acquis » au résumé là où le challenge posait la question (audit A3).
  let correctCodesPreload = null;
  let teamCodesPreload = null;
  // Produit lu à l'identique de l'historique : « fm » ou « gl » exactement, sinon aucun
  // préchargement (chaque ressource retombe alors sur le chemin unitaire).
  const adapter = exactGatingProduct(product);
  const learner = adapter ? adapter.resolveLearner({ userId, glAuth }) : { ok: false };
  if (learner.ok) {
    correctCodesPreload = new Set(await adapter.listCorrectCodes(db, learner));
    if (adapter.capabilities.teamAnswers) {
      teamCodesPreload = new Set(await adapter.listTeamCorrectCodes(db, glAuth));
    }
  }
  ...
  // Clé de lecteur GL pour le chargement groupé des verrous (ForetMap : aucune, la clé est
  // le compte).
  const cooldownReader = adapter ? adapter.readerKey(glAuth) : null;
