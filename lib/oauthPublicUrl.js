...
/**
 * Origine (schéma + hôte) d'une URL absolue, `''` si elle n'en est pas une.
 *
 * L'hôte passe par `normalizeOAuthPublicHost`, exactement comme celui de la requête dans
 * `resolveOAuthPublicOrigin` : les deux origines doivent être normalisées **de la même façon**
 * avant d'être comparées. Sans cela, une `redirect_uri` écrite avec `www.` ne coïnciderait
 * jamais avec l'origine calculée depuis la requête, et le rebond de `/google/start` se
 * relancerait indéfiniment.
 */
function originOfUrl(value) {
  try {
    const url = new URL(String(value || ''));
    if (url.protocol !== 'https:' && url.protocol !== 'http:') return '';
    return `${url.protocol}//${normalizeOAuthPublicHost(url.host)}`;
  } catch (_) {
    return '';
  }
}
