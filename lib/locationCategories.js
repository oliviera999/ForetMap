...
const { parseGroupIdList, parseRoleSlugList } = require('./locationAudience');
...
function serializeCategoryRow(row) {
  return {
    ...
    // Lot 5 : lieux affichés seulement quand la carte est zoomée (désencombrement).
    zoom_only: !!row.zoom_only,
    // Audience héritée (migration 262) : les lieux de la catégorie qui ne déclarent aucune
    // audience propre prennent celle-ci. Vide = catégorie neutre. Ces deux champs suivent
    // l'entité (`attachCategoriesToEntity`) jusqu'à `resolveEffectiveLocationAudience`, et
    // sont retirés par la projection pour un lecteur ordinaire.
...
const CATEGORY_SELECT = `SELECT id, map_id, slug, label, emoji, color, description,
  applies_to, is_infrastructure, sort_order, is_active, surfaces, zoom_only,
...
/**
 * Retire l'audience héritée d'une catégorie sérialisée (migration 262).
 *
 * Le **catalogue** de catégories est servi largement — jusqu'au visiteur anonyme de la Visite
 * et du Plan. Y laisser `visible_role_slugs` / `visible_group_ids` révélerait à qui n'y a
 * aucun droit la cartographie des rôles et des classes visés par chaque catégorie. Ces deux
 * champs ne sortent donc que par `GET /api/map-categories/manage` (permission `zones.manage`),
 * et sur les catégories attachées à un lieu pour un gestionnaire
 * (`projectLocationAudienceForViewer`).
 */
function stripCategoryAudience(category) {
 ...
 * @param {object} db
 * @param {{ mapId?: string, kind?: 'zone'|'marker', includeInactive?: boolean, surface?: string,
 *   includeAudience?: boolean }} [options]
 *   `surface` : ne garde que les catégories qui apparaissent sur cette surface (lot 4).
 *   `includeAudience` : conserve l'audience héritée — réservé aux écrans de gestion.
 */
async function listCategories(db, options = {}) {
  ...
  const where = [];
  const params = [];
  // `surface` accepte une surface ou **plusieurs** : le filtrage par surface est une
  // intersection depuis que la surface est décidée par le serveur — une catégorie doit
  // apparaître sur toutes celles demandées (`lib/shared/surfaceCore.js`).
  for (const value of Array.isArray(surface) ? surface : [surface]) {
  ...
  const serialized = rows.map(serializeCategoryRow);
  return includeAudience ? serialized : serialized.map(stripCategoryAudience);
}
...
async function loadCategoriesMap(db, kind, entityIds) {
  ...
  const rows = await db.queryAll(
    `SELECT j.${column} AS entity_id, c.id, c.map_id, c.slug, c.label, c.emoji, c.color,
            ...
            c.surfaces, c.zoom_only, c.visible_role_slugs, c.visible_group_ids
       FROM ${table} j
