 ...
 * @param {object} db
 * @param {{ mapId?: string, kind?: 'zone'|'marker', includeInactive?: boolean }} [options]
 */
async function listCategories(db, options = {}) {
  const { mapId = '', kind = '', includeInactive = false } = options;
  const where = [];
  const params = [];
  if (mapId) {
...
/**
 * Ajoute `categories`, `category_ids` et `is_infrastructure` à une entité sérialisée.
