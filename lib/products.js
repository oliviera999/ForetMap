 ...
 * @property {boolean} indexable Vrai si le produit peut être référencé par les moteurs.
 *   Faux ⇒ `robots.txt` interdit tout et chaque réponse porte `X-Robots-Tag: noindex`
 *   (`lib/robotsRoutes.js`).
...
const PRODUCTS = Object.freeze({
  ...
  plan: Object.freeze({
    ...
    apiPrefix: '/api/plan',
    authRateLimitPaths: Object.freeze([]),
    // Le plan porte des lieux sensibles au titre du PPMS (entrées, loge, infirmerie) :
    // il est gardé par un code, et il n'a aucune raison d'être référencé. Constat **S9**
    // de `docs/AUDIT_SECURITE_2026-09-22.md`.
    indexable: false,
    swFile: 'sw-plan.js',
  ...
  staff: Object.freeze({
    ...
    // Surface réservée aux personnels : jamais référencée (constat **S9**).
    indexable: false,
    swFile: 'sw-staff.js',
