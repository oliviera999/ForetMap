 ...
 * @returns {Promise<{ ok: true, studentId: string, displayName: string, affectedTaskIds: string[] } | { ok: false, reason: string }>}
 */
async function deleteStudentById(studentId) {
  ...
  const summary = await withTransaction(async (tx) => {
    ...
    const affectedTaskIds = await deleteTaskAssignmentsAndRecalcStatuses(tx, s);
