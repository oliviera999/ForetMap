...
async function deleteTaskAssignmentsAndRecalcStatuses(tx, s) {
  ...
  // Appariement nominal réservé aux lignes héritées (student_id NULL).
  // Sans cette borne, supprimer « Marie Dupont » effaçait aussi les inscriptions
  // et journaux d’un homonyme encore présent en base (même prénom+nom, autre id).
  const matchSql =
    'student_id = ? OR (student_id IS NULL AND student_first_name = ? AND student_last_name = ?)';
  const matchParams = [id, first_name, last_name];
  const affectedRows = await tx.queryAll(
    `SELECT DISTINCT task_id FROM task_assignments WHERE ${matchSql}`,
    matchParams,
  );
  const affectedTaskIds = affectedRows.map((r) => r.task_id);
  const affectedMapIds = new Set();

  await tx.execute(`DELETE FROM task_assignments WHERE ${matchSql}`, matchParams);
  await tx.execute(`DELETE FROM task_logs WHERE ${matchSql}`, matchParams);
