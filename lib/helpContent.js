...
// Divergence assumée : le « normalizeOptionalString » historique de ce module renvoyait
// toujours une chaîne ('' pour null/undefined/vide), jamais null — soit exactement le contrat
// d'`asTrimmedString` (et PAS celui du `normalizeOptionalString` canonique de
// lib/shared/httpHelpers, qui renvoie null). On aliase donc vers la source partagée
// iso-comportement pour supprimer la copie locale sans changer le rendu.
const { asTrimmedString: normalizeOptionalString } = require('./shared/stringHelpers');
...
const loadDefaultHelpConfig = createDefaultsLoader(DEFAULT_CONFIG_PATH);
...
async function getHelpConfigFromDb() {
  ...
  return resolveStoredConfig(row?.value_json, {
    loadDefaults: loadDefaultHelpConfig,
    normalize: normalizeHelpConfig,
  });
}

/**
 * Défauts **sous forme normalisée** — la seule référence valable pour calculer
 * une surcharge.
 *
 * `data/help.default.json` n'est pas déjà normalisé : la normalisation rogne les
 * espaces (`'Astuce : '` → `'Astuce :'`), matérialise `text`/`textTeacher` pour
 * chaque infobulle et injecte un item vide dans un panneau qui n'en a pas.
 * Comparer au fichier brut ferait donc apparaître des « écarts » que personne
 * n'a saisis, et la surcharge ne serait jamais vide. C'est cette forme-là que
 * la lecture sert quand aucune ligne n'existe : c'est donc à elle qu'il faut se
 * comparer.
 */
function loadNormalizedDefaults() {
  return normalizeHelpConfig(loadDefaultHelpConfig());
}

/**
 * Surcharge à persister pour une configuration donnée : uniquement ce qui
 * s'écarte des défauts versionnés.
 *
 * Exposé pour le script de compactage (`scripts/compact-help-registry.js`) et
 * pour les tests — la règle de réduction, elle, vit dans le noyau partagé.
 */
function buildHelpOverride(config) {
  return buildStoredOverride(normalizeHelpConfig(config), loadNormalizedDefaults());
}

/**
 * Enregistre le registre d'aide en ne stockant **que la surcharge**.
 *
 * L'API ne change pas : la valeur retournée (et donc la réponse des routes)
 * reste la configuration dense normalisée. Seule la représentation en base
 * change — et avec elle la propriété qui manquait : une amélioration des
 * défauts versionnés reste visible partout où un prof n'a rien réécrit.
 */
async function saveHelpConfigToDb(config, actor = {}) {
  const normalized = normalizeHelpConfig(config);
  const override = buildStoredOverride(normalized, loadNormalizedDefaults());
  await execute(
    `INSERT INTO app_settings
     ...
     ON DUPLICATE KEY UPDATE
      ...
      updated_at = NOW()`,
    [HELP_REGISTRY_KEY, JSON.stringify(override), actor.userType || null, actor.userId || null],
  );
  return normalized;
}
