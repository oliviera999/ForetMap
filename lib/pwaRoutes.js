 ...
 *   - `GET /offline.html` : page de repli hors ligne (précachée par chaque service worker),
 *     jetons `%BRAND_*%` substitués pour le produit résolu (`lib/brandHtml.js`), `no-cache`.
 *
...
const { brandHtmlTokens, renderBrandHtml } = require('./brandHtml');
...
/** Page de repli hors ligne : `public/offline.html`, copiée telle quelle dans `dist/` par Vite. */
const OFFLINE_PAGE_PATH = '/offline.html';
/**
 * Revalidée à chaque demande : le service worker la précache à l'installation, et un
 * changement de marque (variables `FORETMAP_BRAND_*`) doit l'atteindre au déploiement suivant.
 */
const OFFLINE_CACHE_CONTROL = 'no-cache';
...
/**
 * Monte `GET /sw.js`, `GET /manifest.json` et `GET /offline.html`.
...
function registerPwaRoutes(
  ...
  app.get('/sw.js', (req, res) => {
    ...
    return res.sendFile(resolved.path, { dotfiles: 'allow' });
  });
  ...
  app.get('/manifest.json', (req, res) => {
    ...
    if (resolved.kind === 'file') return res.sendFile(resolved.path, { dotfiles: 'allow' });
  ...
  // Page hors ligne : `public/` n'est pas transformé par le plugin Vite des entrées HTML,
  // la marque y est donc posée ici, au service, et selon le produit du host — G&L, le Plan
  // et le plan des personnels précachent la même page et y lisaient « ForêtMap » en dur.
  app.get(OFFLINE_PAGE_PATH, async (req, res, next) => {
    let template;
    try {
      template = await fs.promises.readFile(path.join(root, 'offline.html'), 'utf8');
    } catch {
      return next();
    }
    const product = resolveProduct(resolveProductFromRequest(req));
    ...
    res.type('html');
    return res.send(renderBrandHtml(template, brandHtmlTokens(product)));
  });
}
