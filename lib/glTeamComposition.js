...
const RECIPES = Object.freeze(['random', 'random_memory', 'carry_over']);
...
async function buildCompositionProposal(params, deps = {}) {
  ...
  if (!teamsDraft) {
    const weights = RECIPE_WEIGHTS[recipe] || RECIPE_WEIGHTS.random;
  ...
  const explain = buildExplain({ recipe, teams, pairs, history, pool });
...
function buildExplain({ recipe, teams, pairs, history, pool }) {
  const lines = [];
