    ...
    socket.on('subscribe:gl-game', async (payload = {}) => {
      ...
      try {
        if (!(await canAccessGlGame(socket.data?.auth, gameId))) {
          socket.emit('gl:game:subscription-refused', { gameId, error: 'Accès refusé à cette partie' });
          return;
        }
        ...
        logger.warn({ err, gameId, socketId: socket.id }, 'Socket.IO GL : vérification d’accès à la partie en échec');
        socket.emit('gl:game:subscription-refused', { gameId, error: 'Accès refusé à cette partie' });
      }
    });
    ...
    socket.on('subscribe:gl-class', async (payload = {}) => {
      ...
      try {
        if (!(await canAccessGlClass(socket.data?.auth, classId))) {
          socket.emit('gl:class:subscription-refused', { classId, error: 'Accès refusé à cette classe' });
          return;
        }
        ...
        logger.warn({ err, classId, socketId: socket.id }, 'Socket.IO GL : vérification d’accès à la classe en échec');
        socket.emit('gl:class:subscription-refused', { classId, error: 'Accès refusé à cette classe' });
      }
    });
