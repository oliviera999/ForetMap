...
const { getGlPlayerGameMembership, normalizeGlId } = require('./glGameAccess');
...
function initRealtime(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe:gl-game', async (payload = {}, ack) => {
      const gameId = normalizeGlId(payload.gameId);
      if (!gameId) return;
      const auth = socket.data?.auth || {};
      if (String(auth.product || '').toLowerCase() !== 'gl') {
        if (typeof ack === 'function') ack({ ok: false, error: 'forbidden' });
        return;
      }
      const permissions = Array.isArray(auth.permissions) ? auth.permissions : [];
      if (!permissions.includes('gl.read')) {
        if (typeof ack === 'function') ack({ ok: false, error: 'forbidden' });
        return;
      }
      if (String(auth.userType || '').toLowerCase() === 'gl_player') {
        try {
          const membership = await getGlPlayerGameMembership(gameId, auth.userId);
          if (!membership) {
            if (typeof ack === 'function') ack({ ok: false, error: 'forbidden' });
            return;
          }
        } catch (err) {
          logger.warn({ err, socketId: socket.id, gameId, msg: 'socket_gl_game_acl_error' }, 'Socket.IO GL : contrôle d’accès impossible');
          if (typeof ack === 'function') ack({ ok: false, error: 'forbidden' });
          return;
        }
      }
