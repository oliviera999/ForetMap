...
/** Forum G&L : un seul fil commun à tout le royaume, rejoint par chaque socket G&L. */
const GL_FORUM_ROOM = 'gl:forum';
...
/** Snapshot admin : toutes les clés refcount > 0 (`product:userId`). */
function listOnlinePresenceEntries() {
  const out = [];
  for (const [key, count] of presenceRefcounts) {
    if (!(count > 0)) continue;
    const idx = key.indexOf(':');
    if (idx <= 0) continue;
    out.push({ product: key.slice(0, idx), userId: key.slice(idx + 1) });
  }
  return out;
}
...
async function registerPresenceSocket(socket) {
  ...
  if (prev === 0) {
    ...
    // Passage / activité légère (throttle 1 h) — ne doit pas bloquer la présence.
    try {
...
/** Quitte la salle carte courante, s'il y en a une. */
function leaveCurrentMapRoom(socket) {
...
/**
 * Abonne une socket aux signaux d'une carte, si elle existe **et** si le compte y a droit.
 *
 * Lot H de `docs/AUDIT_SECURITE_2026-09-22.md` (§4.3). Avant ce lot, la seule condition était
 * l'existence de la carte : n'importe quel compte ForêtMap authentifié pouvait demander
 * `subscribe:map` sur une carte hors de son périmètre de groupe — y compris une carte réservée
 * au plan — et recevoir ensuite tous ses signaux de mutation. La charge utile ne porte pas de
 * contenu (un motif, des identifiants), mais elle trahit l'activité d'une carte qu'on n'a pas
 * le droit de lire : qui édite quoi, et quand. Les routes HTTP appliquaient déjà ce périmètre
 * (`requireMapAccess`) ; la socket, bien plus durable qu'une requête, ne l'appliquait pas.
 *
 * Un refus **quitte** la salle courante plutôt que de la conserver : si le périmètre change
 * pendant la session, changer de carte ne doit pas laisser le flux précédent ouvert.
 */
async function joinMapRoomIfExists(socket, mapId) {
  ...
  try {
    ...
    if (!(await canAccessMapId(socket.data?.auth || null, normalized))) {
      logger.warn(
        { msg: 'socket_map_scope_denied', mapId: normalized, socketId: socket.id },
        'Socket.IO : carte hors du périmètre du compte',
      );
      leaveCurrentMapRoom(socket);
      return;
    }
  } catch (err) {
    logger.warn(
...
function initRealtime(httpServer) {
  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      if (err instanceof AuthRevokedError) {
        // Compte supprimé, désactivé ou mot de passe changé : même refus que sur HTTP.
        logger.warn(
          { msg: 'socket_auth_revoked', socketId: socket.id, reason: err.reason || err.message },
          'Socket.IO : session ForetMap révoquée',
        );
        return next(new Error('unauthorized'));
      }
      logger.error(
  ...
  io.on('connection', (socket) => {
    ...
    if (isForetProduct) {
      ...
      const ownUserId = String(
        socket.data?.auth?.canonicalUserId || socket.data?.auth?.userId || '',
      ).trim();
      if (ownUserId) socket.join(userRoomName(ownUserId));
    } else {
      socket.join(GL_FORUM_ROOM);
    }
    socket.data.mapRoom = null;
...
/**
 * Nouvelle notification pour des comptes précis. Passe par `io.to` directement : `safeEmit`
 * enverrait au salon de la carte dès qu'un `mapId` est présent, donc à tout le monde.
 */
function emitNotificationToUsers(userIds, payload = {}) {
  if (!io) return;
  const ids = [...new Set((userIds || []).map((id) => String(id || '').trim()).filter(Boolean))];
  if (!ids.length) return;
  try {
    const body = { ts: Date.now(), ...payload };
    for (const id of ids) io.to(userRoomName(id)).emit('notifications:new', body);
  } catch (err) {
    logger.warn({ err }, 'Émission Socket.IO notification en échec');
  }
}
...
/** Après mutation des sujets, messages, réactions ou signalements du forum G&L. */
function emitGlForumChanged(payload = {}) {
  if (!io) return;
  try {
    io.to(GL_FORUM_ROOM).emit('gl:forum:changed', { ts: Date.now(), ...payload });
  } catch (err) {
    logger.warn({ err }, 'Émission Socket.IO forum GL en échec');
  }
}
