...
const { hydrateGlAuthFromClaims, GlAuthInfraError } = require('./auth/glHydration');
...
function initRealtime(httpServer) {
  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      // Audit B6 : pour GL, les droits sont relus en base (révocation immédiate), comme en HTTP.
      if (product === 'gl') {
        let glAuth;
        try {
          ...
          if (err instanceof GlAuthInfraError) {
            logger.error({ err: err.cause, msg: 'socket_gl_auth_hydration_failed' }, 'Socket.IO GL');
            return next(new Error('unauthorized'));
          }
          throw err;
        }
        if (!glAuth) {
          logger.warn(
            { msg: 'socket_gl_auth_revoked', socketId: socket.id },
            'Socket.IO GL : session révoquée ou compte désactivé',
          );
          return next(new Error('unauthorized'));
        }
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe:gl-game', async (payload = {}) => {
      ...
      if (!gameId) return;
      try {
        // Re-hydratation à chaque abonnement : un staff désactivé après la connexion
        // socket ne doit pas rejoindre (ni rester dans) la room live.
        if (socket.data?.product === 'gl' && socket.data?.claims) {
          const fresh = await hydrateGlAuthFromClaims(socket.data.claims, { queryOne });
          if (!fresh) {
            socket.emit('gl:game:subscription-refused', {
              gameId,
              error: 'Session expirée ou compte désactivé',
            });
            return;
          }
          socket.data.auth = fresh;
        }
