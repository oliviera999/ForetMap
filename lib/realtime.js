...
function hasSocketGlPermission(auth, permission) {
  const permissions = Array.isArray(auth?.permissions) ? auth.permissions : [];
  return permissions.includes(permission);
}

async function canSubscribeGlGame(auth, gameId) {
  if (String(auth?.product || '').toLowerCase() !== 'gl') return false;
  const userType = String(auth?.userType || '').toLowerCase();
  if (userType === 'gl_admin') {
    return hasSocketGlPermission(auth, 'gl.read') || hasSocketGlPermission(auth, 'gl.game.manage');
  }
  if (userType !== 'gl_player' || !auth?.userId) return false;
...
function initRealtime(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe:gl-game', async (payload = {}) => {
      const gameId = parsePositiveId(payload.gameId);
      if (!gameId) return;
      try {
        const allowed = await canSubscribeGlGame(socket.data?.auth, gameId);
        if (!allowed) {
          socket.emit('gl:subscription:error', { error: 'Accès refusé à cette partie', gameId });
          logger.warn(
            {
              socketId: socket.id,
              gameId,
              product: socket.data?.auth?.product || null,
              userType: socket.data?.auth?.userType || null,
              msg: 'gl_socket_subscription_denied',
            },
            'Socket.IO GL : abonnement refusé'
          );
          return;
        }
        ...
        logger.warn({ err, socketId: socket.id, gameId, msg: 'gl_socket_subscription_error' }, 'Socket.IO GL : erreur abonnement');
        socket.emit('gl:subscription:error', { error: 'Abonnement impossible', gameId });
      }
    });
