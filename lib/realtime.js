...
const { canAccessGlGame, parsePositiveId } = require('./glAccess');
    ...
    socket.on('subscribe:gl-game', async (payload = {}) => {
      ...
      try {
        if (!(await canAccessGlGame(socket.data?.auth, gameId))) {
          socket.emit('gl:game:subscription-error', { gameId, error: 'Accès refusé à cette partie' });
          return;
        }
        ...
        logger.warn(
          { err, socketId: socket.id, gameId, msg: 'gl_socket_subscribe_failed' },
          'Socket.IO GL : abonnement partie refusé'
        );
        socket.emit('gl:game:subscription-error', { gameId, error: 'Abonnement impossible' });
      }
    });
