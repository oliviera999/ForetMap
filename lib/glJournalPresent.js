...
/** Recettes de composition automatique (docs/GL_EQUIPES_AUTO_CONCEPTION.md § 4.2). */
const TEAM_RECIPE_LABELS = {
  random: 'aléatoire',
  random_memory: 'aléatoire à mémoire',
  carry_over: 'reconduction',
  mixed: 'mixte',
  roles: 'complémentarité',
  homogeneous: 'groupes de besoin',
};
  ...
  switch (eventType) {
    case 'narration': {
      ...
      return {
        ...base,
        kind: 'narration',
        title: forPlayer ? 'Annonce du maître du jeu' : 'Narration du MJ',
        body: text || (forPlayer ? 'Le maître du jeu a partagé une annonce.' : ''),
        imageUrl,
        actorLabel: forPlayer ? 'Maître du jeu' : actorLabel,
      };
    }
    case 'move': {
      ...
      const place = markerLabel
        ? `le repère « ${markerLabel} »`
        : forPlayer
      ...
      return {
        ...base,
        kind: 'move',
        title: forPlayer ? 'Déplacement sur la carte' : 'Déplacement',
        body: forPlayer
          ? `${teamPhrase(teamLabel)} se déplace vers ${place}.`
          : teamLabel
            ? `${teamLabel} — ${place}`
            : place,
      };
    ...
    case 'turn_change': {
      ...
      return {
        ...base,
        kind: 'turn_change',
        title: forPlayer ? 'Changement de tour' : 'Tour de jeu',
        body: forPlayer
          ? `C’est au tour de l’équipe ${who}.`
          : `C’est au tour de ${nextLabel || 'équipe inconnue'}.`,
        teamLabel: nextLabel || teamLabel,
      };
    }
    case 'game_status': {
      const statusKey = String(payload.status || '').toLowerCase();
      const statusLabel =
        GAME_STATUS_LABELS[statusKey] || (forPlayer ? 'mise à jour' : statusKey || 'inconnu');
      return {
        ...base,
        kind: 'game_status',
        title: forPlayer ? 'État de la partie' : 'Statut de partie',
        body: forPlayer
          ? `La partie est maintenant ${statusLabel}.`
          : `La partie passe en « ${statusLabel} ».`,
        actorLabel: forPlayer ? 'Maître du jeu' : actorLabel,
      };
    }
    case 'teams_composed': {
      // Composition automatique (lib/glTeamComposition.js) : factuel, jamais de score individuel.
      const count = Number(payload.teamCount);
      const recipeLabel = TEAM_RECIPE_LABELS[String(payload.recipe || '').toLowerCase()] || null;
      ...
      return {
        ...base,
        kind: 'teams_composed',
        title: forPlayer ? 'Équipes formées' : 'Composition des équipes',
        body: forPlayer
          ? `Le maître du jeu a formé ${countStr}.`
          : `Le MJ a composé ${countStr}${recipeLabel ? ` (recette « ${recipeLabel} »)` : ''}${payload.replaced ? ', en remplacement des équipes précédentes' : ''}.`,
        actorLabel: 'Maître du jeu',
      };
    }
    case 'score': {
      ...
      const pts =
        Number.isFinite(delta) && delta !== 0
          ? delta > 0
            ? `gagne ${Math.abs(delta)} point${Math.abs(delta) > 1 ? 's' : ''}`
            : `perd ${Math.abs(delta)} point${Math.abs(delta) > 1 ? 's' : ''}`
          : 'aucun changement de points';
      const reasonTxt = reason ? (forPlayer ? ` Motif : ${reason}.` : ` (${reason})`) : '';
      return {
        ...base,
        kind: 'score',
        title: forPlayer ? 'Points' : 'Score',
        body: `${teamPhrase(teamLabel)} ${pts}.${reasonTxt}`.replace(/\.\./g, '.'),
      };
    }
    case 'vitality_change': {
      ...
      if (Number.isFinite(hd) && hd !== 0)
        vitParts.push(
          forPlayer
            ? `${hd > 0 ? 'gagne' : 'perd'} ${Math.abs(hd)} cœur${Math.abs(hd) > 1 ? 's' : ''}`
            : `cœurs ${h}`,
        );
      if (Number.isFinite(pd) && pd !== 0)
        vitParts.push(
          forPlayer
            ? `${pd > 0 ? 'gagne' : 'perd'} ${Math.abs(pd)} gemme${Math.abs(pd) > 1 ? 's' : ''}`
            : `gemmes ${p}`,
        );
      const vitText = vitParts.length
        ? vitParts.join(forPlayer ? ', ' : ', ')
        : forPlayer
          ? 'aucun changement de vitalité'
          : 'aucun changement';
      return {
        ...base,
        kind: 'vitality_change',
        title: forPlayer ? 'Cœurs et gemmes' : 'Vitalité',
        body: `${teamPhrase(teamLabel)} ${vitText}${reasonPart}`.trim(),
      };
    }
    case 'action_request': {
      const actionKey = String(payload.actionType || '').toLowerCase();
      const actionLabel =
        ACTION_TYPE_LABELS[actionKey] ||
        (forPlayer ? 'faire une action sur la carte' : actionKey || 'action');
      return {
        ...base,
        kind: 'action_request',
        title: forPlayer ? 'Demande des joueurs' : 'Demande d’action',
        body: forPlayer
          ? `${teamPhrase(teamLabel)} souhaite ${actionLabel}.`
          : `${teamPhrase(teamLabel)} demande : ${actionLabel}`,
      };
    }
    case 'action_resolved': {
      ...
      const verdict =
        accepted === true
          ? forPlayer
            ? 'acceptée par le maître du jeu'
            : 'acceptée'
          : accepted === false
            ? forPlayer
              ? 'refusée par le maître du jeu'
              : 'refusée'
            : forPlayer
              ? 'traitée par le maître du jeu'
              : 'traitée';
      return {
        ...base,
        kind: 'action_resolved',
        title: forPlayer ? 'Réponse du maître du jeu' : 'Action traitée',
        body: `${teamPhrase(teamLabel)} : demande ${verdict}.`,
        actorLabel: forPlayer ? 'Maître du jeu' : actorLabel,
      };
    }
    case 'qcm_answer': {
      ...
      return {
        ...base,
        kind: 'qcm_answer',
        title: forPlayer ? 'Question du plateau' : 'Réponse QCM',
        body: forPlayer
          ? `${teamPhrase(teamLabel)} a donné une ${correct ? 'bonne' : 'mauvaise'} réponse.`
          : `${teamPhrase(teamLabel)} : ${correct ? 'bonne réponse' : 'réponse incorrecte'}`,
      };
    }
    case 'marker_question_presented': {
      const markerId = payload.markerId != null ? Number(payload.markerId) : null;
      const markerLabel = normalizeOptionalString(payload.markerLabel);
      const target = markerLabel
        ? `« ${markerLabel} »`
        : forPlayer
          ? 'un repère de la carte'
          : markerId != null
            ? `repère #${markerId}`
            : 'un repère';
      return {
        ...base,
        kind: 'marker_question_presented',
        title: forPlayer ? 'Question sur la carte' : 'Question présentée',
        body: forPlayer
          ? `${teamPhrase(teamLabel)} découvre une question à ${target}.`
          : `${teamPhrase(teamLabel)} — question à ${target}`,
      };
    }
    case 'spell_cast': {
      ...
      const label = name
        ...
        : forPlayer
          ? 'un sortilège'
          : 'sort';
      // Audit S14 — pot commun ouvert par le MJ sur tout le plateau : les contributeurs
      // peuvent venir de plusieurs équipes, la ligne n'attribue donc plus le sort à une
      // seule d'entre elles.
      const wholeGame = String(payload.rosterScope || 'team') === 'game';
      return {
        ...base,
        kind: 'spell_cast',
        title: forPlayer ? 'Sortilège' : 'Sortilège lancé',
        body: wholeGame
          ? `Toute la partie lance ${label}.`
          : `${teamPhrase(teamLabel)} lance ${label}.`,
      };
    }
    // G11 — l'effet d'un sort est appliqué à la main par le MJ : le journal en garde la
    // trace, pour qu'un sort payé dont l'effet a été oublié se repère après coup.
    case 'spell_effect_applied': {
      const name = normalizeOptionalString(payload.spellName);
      const emoji = normalizeOptionalString(payload.spellEmoji);
      const label = name ? (emoji ? `${emoji} ${name}` : name) : 'un sortilège';
      return {
        ...base,
        kind: 'spell_effect_applied',
        title: forPlayer ? 'Effet du sortilège' : 'Effet appliqué',
        body: forPlayer
          ? `Le maître du jeu a appliqué l’effet de ${label}.`
          : `Effet de ${label} appliqué.`,
      };
    }
    case 'marker_arrival': {
    ...
    case 'marker_effect': {
      ...
      if (Number.isFinite(moveDelta) && moveDelta !== 0) {
        const autoApplied = payload.autoMoveApplied === true;
        parts.push(`cases ${formatDelta(moveDelta)}${autoApplied ? ' (auto)' : ' (manuel)'}`);
      }
    ...
    case 'feuillet_discovered': {
      const titre =
        normalizeOptionalString(payload.titre) ||
        normalizeOptionalString(payload.feuilletCode) ||
        'un feuillet';
      return {
        ...base,
    ...
    case 'feuillet_read': {
      ...
      return {
        ...
        body: forPlayer
          ? `${teamPhrase(teamLabel)} lit ${code}.`
          : `${teamPhrase(teamLabel)} — lu : ${code}`,
      };
    }
    case 'feuillet_held': {
      const code = normalizeOptionalString(payload.feuilletCode) || 'feuillet';
    ...
    default:
      return {
        ...base,
        kind: forPlayer ? 'other' : eventType || 'unknown',
        title: forPlayer ? 'Évènement' : eventType ? `Type : ${eventType}` : base.title,
        body: forPlayer
          ? teamLabel
            ? `Un évènement concerne ${teamPhrase(teamLabel).toLowerCase()}.`
            : 'Un évènement s’est produit dans la partie.'
          : teamLabel
            ? `${teamPhrase(teamLabel)}`
            : '',
      };
  }
}
