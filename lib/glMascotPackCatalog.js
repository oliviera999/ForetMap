...
function loadGlMascotPackToVisitModule() {
  if (!glMascotPackToVisitPromise) {
    const absolute = path.join(__dirname, '..', 'src', 'gl', 'utils', 'glMascotPackToVisit.js');
...
async function loadGlMascotPackCatalogEntries() {
  ...
  for (const row of rows || []) {
    ...
    if (
      data.renderer === 'sprite_cut' &&
      toVisitMod &&
      typeof toVisitMod.glMascotPackSpriteCutToVisitValidation === 'function'
    ) {
      const mapped = toVisitMod.glMascotPackSpriteCutToVisitValidation(data, {
        relaxAssetPrefix: true,
      });
      if (mapped.ok) {
        entry.spriteCut = mapped.spriteCut;
        entry.fallbackSilhouette =
          mapped.visitPack?.fallbackSilhouette || data.fallbackSilhouette || 'gnome';
