...
function replayGameEvents(events, initialState = {}) {
  const state = {
    ...
    currentTeamId: initialState.currentTeamId != null ? Number(initialState.currentTeamId) : null,
    currentRoundNumber:
      initialState.currentRoundNumber != null ? Number(initialState.currentRoundNumber) : 0,
  ...
  for (const evt of events || []) {
    ...
    switch (event.eventType) {
      ...
      case 'round_start': {
        const roundNumber =
          event.payload?.roundNumber != null ? Number(event.payload.roundNumber) : null;
        if (Number.isFinite(roundNumber)) state.currentRoundNumber = roundNumber;
        break;
      }
      case 'narration': {
...
const GAME_EVENT_SELECT_BY_ID =
  'SELECT id, game_id, team_id, actor_type, actor_id, event_type, payload_json, created_at FROM gl_game_events WHERE id = ?';

/**
 * Insère un événement de partie puis relit la ligne par son insertId.
 * Remplace le motif « INSERT puis re-SELECT ORDER BY id DESC LIMIT 1 » qui,
 * sous concurrence, pouvait relire (et émettre) l'événement d'une autre requête.
 * `db` : exécuteur ({ execute, queryOne }) — le module database ou une transaction.
 * Retourne la ligne normalisée (normalizeEventRow), prête pour emitGlGameEvent.
 */
async function insertGameEvent(
  db,
  { gameId, teamId = null, actorType, actorId = null, eventType, payload },
) {
  ...
  const row = await db.queryOne(GAME_EVENT_SELECT_BY_ID, [result.insertId]);
