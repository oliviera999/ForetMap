...
const MAX_ARCHIVE_BYTES = parseEnvBytes(process.env.FORETMAP_CONTENT_LIBRARY_MAX_ARCHIVE_BYTES, 50 * 1024 * 1024);
const MAX_FILE_BYTES = parseEnvBytes(process.env.FORETMAP_CONTENT_LIBRARY_MAX_FILE_BYTES, 32 * 1024 * 1024);
...
function mapMulterFile(file) {
  ...
  return {
    fileName: asTrimmedString(file.originalname) || 'fichier',
    buffer: file.buffer,
  };
}
...
function readAnalyzeUploadPayload(req) {
  if (isMultipartRequest(req)) {
    ...
    if (uploadedFiles.length === 0) throw uploadError('Aucun fichier reçu (champs archive ou files attendus)');
