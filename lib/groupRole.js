...
async function applyGroupDefaultRoleForStudent(userId, options = {}) {
  ...
  // `visiteur` et `personnel` ont le **même rang** : la règle automatique n'a aucune raison
  // de préférer l'un à l'autre. `personnel` est un profil *choisi* (staff non enseignant),
  // pas un défaut dérivé de l'appartenance aux groupes — or la résolution retombe sur
  // `visiteur` dès qu'aucun groupe n3beur n'est trouvé. Sans cette garde, un compte importé
  // en « Personnel » redevenait « Visiteur » à la première synchronisation, c'est-à-dire dès
  // son rattachement à une classe. Le geste explicite (`force`) et un groupe imposant son
  // profil restent prioritaires, et la promotion vers un profil n3beur passe toujours
  // (`resolvedSlug` n'est alors pas « type visiteur »).
  if (!forced && isVisitorLikeSlug(currentSlug) && isVisitorLikeSlug(resolvedSlug)) {
    return {
      changed: false,
      reason: 'visitor_like_preserved',
      currentRoleSlug: currentSlug,
      forced,
      forcedByGroupId,
    };
