...
function resolvePlantTaxonomy(row) {
  ...
  return {
    kingdom: trimStr(source.taxon_kingdom) || trimStr(source.group_1) || null,
    group: trimStr(source.taxon_group) || trimStr(source.group_2) || null,
    family: trimStr(source.taxon_family) || trimStr(source.group_3) || null,
    genus: trimStr(source.taxon_genus) || trimStr(source.group_4) || null,
...
function resolvePlantPhRange(row) {
  ...
  const parsed = parsePhRangeText(source.optimal_ph);
  if (parsed) return { ...parsed, source: 'optimal_ph' };
  return null;
}
...
function resolvePlantTempRange(row) {
  ...
  const parsed = parseTempRangeText(source.ideal_temperature_c);
  if (parsed) return { ...parsed, source: 'ideal_temperature_c' };
  return null;
}
