...
function resolveFetchTimeoutMs(options = {}) {
  ...
  return Math.min(
    20000,
    Math.max(200, Number(options.timeoutMs) > 0 ? Number(options.timeoutMs) : 14000),
  );
}
...
function normalizePlantnetIdentifyResponse(data) {
  ...
  for (const row of rawList) {
    ...
    const fullSci =
      asOptionalText(sp.scientificName) ||
...
async function plantnetIdentifyFromImages(params = {}) {
  if (!isPlantnetAutofillEnabled()) {
    return {
      ok: false,
      error: 'Pl@ntNet désactivé (SPECIES_AUTOFILL_PLANTNET=1 et PLANTNET_API_KEY requis).',
    };
  }
  const apiKey = asTrimmedString(process.env.PLANTNET_API_KEY);
  ...
  const project =
    asTrimmedString(params.project) || asTrimmedString(process.env.PLANTNET_PROJECT) || 'all';
  ...
  try {
    ...
    if (!res.ok) {
      const msg =
        asOptionalText(json?.message) || asOptionalText(json?.error) || `HTTP ${res.status}`;
      logger.warn(
        { msg: 'plantnet_identify_http', status: res.status },
        'Pl@ntNet identify : HTTP en échec',
      );
    ...
    logger.warn(
      { msg: 'plantnet_identify_err', err: String(err?.message || err) },
      'Pl@ntNet identify : erreur',
    );
