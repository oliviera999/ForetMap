...
async function fetchPlantnetSpeciesTraits(scientificName, options = {}) {
  ...
  try {
    ...
    const alignUrl = `${alignBase}?api-key=${encodeURIComponent(apiKey)}&name=${encodeURIComponent(name)}&authorship=${authorship}&synonyms=true&lang=${encodeURIComponent(lang)}`;
