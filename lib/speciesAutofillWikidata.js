 ...
 * - P9714 « taxon range » → geographic_origin (concaténé avec P183, dédoublonné).
 *
 * Candidats phase ultérieure (non implémentés ici) : habitat WD peu standardisé, température/pH claims rares.
 */

const TRAIT_PROPERTY_IDS = ['P366', 'P183', 'P9714'];
...
async function fetchWikidataLabelsForIds(ids, fetchJson, options = {}) {
  const map = new Map();
  const unique = [
    ...new Set((ids || []).map((id) => asTrimmedString(id)).filter((id) => /^Q\d+$/i.test(id))),
  ];
...
function extractWikidataTraitFields(claims, labelMap) {
  ...
  if (useLabels.length > 0) {
    ...
    fields.human_utility =
      joined.length > MAX_HUMAN_UTILITY_LEN
  ...
  const rangeLabels = [];
  const stmts9714 = claims?.P9714;
  if (Array.isArray(stmts9714)) {
    for (const st of stmts9714) {
      const v = st?.mainsnak?.datavalue?.value;
      const qid = v && typeof v === 'object' ? asTrimmedString(v.id) : '';
      if (!/^Q\d+$/i.test(qid)) continue;
      const norm = qid.toUpperCase().replace(/^q/, 'Q');
      const lbl = labelForId(labelMap, norm);
      if (lbl) rangeLabels.push(lbl);
    }
  }
  geoLabels.push(...rangeLabels);

  if (geoLabels.length > 0) {
    const joined = [...new Set(geoLabels)].join(' · ');
    fields.geographic_origin =
      joined.length > MAX_GEOGRAPHIC_ORIGIN_LEN
        ? `${joined.slice(0, MAX_GEOGRAPHIC_ORIGIN_LEN - 1)}…`
        : joined;
  }

  if (fields.human_utility || fields.geographic_origin) {
    warnings.push('Wikidata : usages / zone (P366, P183, P9714) — à valider avant publication.');
  }
