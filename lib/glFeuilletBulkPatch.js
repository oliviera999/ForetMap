...
const BOOL_FIELDS = new Set(['offert_ouverture']);
...
function coerceField(field, rawValue) {
  if (BOOL_FIELDS.has(field)) {
    ...
    if (['1', 'true', 'oui', 'yes'].includes(raw)) return { value: 1 };
    if (['0', 'false', 'non', 'no'].includes(raw)) return { value: 0 };
    return { error: `${field} doit valoir 0/1 (ou oui/non)` };
  }
