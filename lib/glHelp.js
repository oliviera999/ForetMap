...
/**
 * `bodyMj` est **optionnel** : c'est la variante lue par un MJ ou un admin là où
 * l'écran est partagé avec les joueurs (la carte, le carnet, le marché…). Son absence
 * fait retomber sur `body`, jamais sur du vide — même contrat que le `textTeacher` de
 * ForetMap, dont il est le pendant (`src/shared/help/roleText.js`).
 */
const helpEntrySchema = z.object({
  title: z.string().max(80),
  body: z.string().max(2000),
  bodyMj: z.string().max(2000).optional(),
});
...
function normalizeHelpEntry(raw, fallback = {}) {
  ...
  // Champ optionnel : on ne l'inscrit que s'il porte un texte, pour qu'une entrée sans
  // variante MJ reste strictement identique à ce qu'elle était (et que la surcharge
  // stockée ne se peuple pas d'une chaîne vide à chaque enregistrement).
  const bodyMj = String(raw?.bodyMj ?? fallback.bodyMj ?? '');
  if (bodyMj.trim()) entry.bodyMj = bodyMj;
  return entry;
}
