...
/**
 * Contexte ressource grave dans le jeton (lot 2 de docs/AUDIT_VALIDATION_QUIZ_2026-09.md) :
 * une question presentee « pour la fiche X » ne peut plus etre repondue hors de ce contexte,
 * et la reponse lit le contexte DANS le jeton, jamais dans le corps (que le client controle).
 * Absent pour une presentation libre (Quiz d'entrainement).
 */
function normalizeResourceClaim(resource) {
  if (!resource) return null;
  const type = String(resource.resourceType ?? resource.type ?? '').trim();
  const ref = String(resource.resourceRef ?? resource.ref ?? '').trim();
...
function presentQuestion(questionRow, glossaryTerms = [], options = {}) {
  ...
  const resource = normalizeResourceClaim(options?.resource);
  ...
  const presentationToken = signJwtToken(
    {
      ...
      // Contexte de validation, lu par la route de réponse (jamais le corps de la requête).
      ...(resource ? { resource } : {}),
    },
    JWT_SECRET,
  ...
  return {
    ...
    // Le client sait ainsi pour quelle fiche la présentation vaut (affichage, pas décision).
...
function verifyPresentationAnswer(presentationToken, questionCode, choiceId, options = {}) {
  ...
  return {
    ...
    selectedLetter,
    jti,
    // Contexte ressource porté par le jeton (null pour une présentation libre).
    resource: normalizeResourceClaim(claims.resource),
  };
}
