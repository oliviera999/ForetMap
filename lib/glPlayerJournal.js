...
async function getPlayerJournalLimits() {
  ...
  return {
    maxChars: Number(settings.playerJournalMaxChars) || 20000,
    maxAssets: Number(settings.playerJournalMaxAssets) || 30,
  };
}
...
async function validateJournalEmbeds(embeds) {
  for (const embed of embeds) {
    ...
    if (embed.type === 'spell') {
      const row = await queryOne('SELECT spell_code FROM gl_spells WHERE spell_code = ? LIMIT 1', [embed.ref]);
      if (!row) return { error: `Sortilège introuvable : ${embed.ref}` };
      continue;
    }
    if (embed.type === 'species') {
      const row = await queryOne('SELECT species_code FROM gl_species WHERE species_code = ? LIMIT 1', [embed.ref]);
      if (!row) return { error: `Espèce introuvable : ${embed.ref}` };
      continue;
    }
    if (embed.type === 'glossary') {
      const row = await queryOne('SELECT glossary_code FROM gl_glossary_terms WHERE glossary_code = ? LIMIT 1', [embed.ref]);
