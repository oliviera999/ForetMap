...
const { resolveMarkerAppearance } = require('./glMarkerAppearance');
...
function formatMarkerRow(row) {
  ...
  const formatted = {
    ...
    sous_biome_slug: row.sous_biome_slug ?? null,
    effet_mecanique: row.effet_mecanique ?? null,
    qcm_categorie_slug: row.qcm_categorie_slug,
    qcm_question_code: row.qcm_question_code,
    display_mode: row.display_mode ?? null,
    emoji: row.emoji ?? null,
    icon_url: row.icon_url ?? null,
    order_index: Number(row.order_index || 0),
    event_config: eventConfig,
  };
  formatted.appearance = resolveMarkerAppearance(formatted);
  return formatted;
}
...
function buildMarkerWriteFields({
  eventType, description, orderIndex, eventConfig, legacy, appearance,
  sousBiomeSlug, effetMecanique,
}) {
  ...
  return {
    ...
    displayMode: appearance?.displayMode ?? null,
    emoji: appearance?.emoji ?? null,
    iconUrl: appearance?.iconUrl ?? null,
    sousBiomeSlug: sousBiomeSlug ?? null,
    effetMecanique: effetMecanique ?? null,
  };
}
