...
async function getChallengeState(
  ...
  const cooldown = preload?.cooldowns
    ? resourceCooldownStateFromRow(preload.cooldowns.get(ref), retryHours)
...
async function assertGatingSatisfiedForAcknowledge(
  ...
  if (state.cooldown?.locked) {
    const remaining = state.cooldown.remaining_label || '1 h';
    return {
      ok: false,
      status: 403,
      error: `Une erreur a été commise : réessaie dans ${remaining} pour valider cette ressource.`,
      missing_question_codes: [],
