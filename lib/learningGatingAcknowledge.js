...
// Adaptateur de produit : tables, lecteur, bonnes réponses, filtre de niveau, granularité de
// chapitre… Le moteur ne teste plus `product === 'gl'` ; il interroge l'adaptateur. Les
// fonctions propres à un produit restent réexportées ici (API inchangée pour les appelants).
const {
...
/** `fm` / `gl`, ou `null` si le produit est inconnu (lecture historique, inchangée). */
function normalizeProduct(value) {
  return resolveGatingProduct(value)?.id ?? null;
}

/**
 * Adaptateur, type et référence d'une ressource, ou `null` si l'un manque ou si le type n'est
 * pas validable par ce produit.
 */
function resolveMarkableResource(product, resourceType, resourceRef) {
  const adapter = resolveGatingProduct(product);
...
async function loadApprovedGatingLinks(db, product, resourceType, resourceRef) {
  const target = resolveMarkableResource(product, resourceType, resourceRef);
  if (!target) return [];
  const { adapter, rt, ref } = target;
  return db.queryAll(
    `SELECT ${adapter.questions.linkColumnsSql}
       FROM ${adapter.tables.links} l
      WHERE l.resource_type = ? AND l.resource_ref = ? AND l.status = 'approved' AND l.is_gating = 1
        AND ${adapter.questions.activeFilterSql}
...
async function loadApprovedGatingLinksForRefs(db, product, resourceType, refs = []) {
  const adapter = resolveGatingProduct(product);
  const rt = adapter ? normalizeResourceType(resourceType, adapter.resourceTypes) : null;
  if (!adapter || !rt || !adapter.markable.has(rt)) return new Map();
  const unique = [
    ...new Set(
  ...
  const rows = await db.queryAll(
    `SELECT l.resource_ref, ${adapter.questions.linkColumnsSql}
       FROM ${adapter.tables.links} l
...
function buildQuestionEntries(links, correctSet, adapter) {
  const seen = new Set();
  const questions = [];
  for (const link of links) {
    ...
    // GL : jeu de questions (`qcm` / `qcm_lore`) ; ForetMap : rien.
    adapter.decorateQuestionEntry(entry, link);
...
async function getChallengeState(
  db,
  {
    ...
    preload = null,
    /**
     * ForetMap : niveau de l'apprenant (`lib/pedago/learnerLevel.js`, `loadLearnerLevel`).
     * Les questions de niveau supérieur ne verrouillent pas la ressource ; si aucune n'est au
     * niveau, toutes restent posées (décision du 25/09/2026, Q1). `null` : aucun filtre.
     */
    learnerLevel = null,
  } = {},
) {
  const adapter = resolveGatingProduct(product);
  const p = adapter?.id ?? null;
  const rt = adapter ? normalizeResourceType(resourceType, adapter.resourceTypes) : null;
  const ref = normalizeResourceRef(resourceRef);
  if (!p || !rt || !ref) {
    return { ok: false, status: 400, error: 'Paramètres de ressource invalides' };
  }
  if (!adapter.markable.has(rt)) {
  ...
  if (adapter.capabilities.chapterGranularity && !resolvedChapterGranularity) {
    resolvedChapterGranularity = await adapter.resolveChapterGranularity(db, {
      resourceType: rt,
      resourceRef: ref,
      glAuth,
    });
  ...
  let links = preload?.links
    ? preload.links.get(ref) || []
    : await loadApprovedGatingLinks(db, p, rt, ref);
  let levelFallback;
  if (adapter.capabilities.levelFilter && learnerLevel) {
    const byLevel = adapter.filterLinksByLevel(links, learnerLevel);
    links = byLevel.links;
    levelFallback = byLevel.levelFallback;
  }
  ...
  let correctSet;
  // Lecteur : compte ForetMap (`userId`) ou couple GL (type, identifiant) ; 403 sinon.
  const learner = adapter.resolveLearner({ userId, glAuth });
  if (!learner.ok) {
    return { ok: false, status: learner.status, error: learner.error };
  }
  ...
  if (correctCodesPreload instanceof Set || Array.isArray(correctCodesPreload)) {
    ...
    if (adapter.capabilities.teamAnswers && teamGranularity && teamCodesPreload) {
    ...
    correctSet = new Set(await adapter.listCorrectCodes(db, { userId, reader }));
    // GL, granularité `team` : les bonnes réponses de l'équipe comptent en plus (union).
    if (adapter.capabilities.teamAnswers && teamGranularity) {
      for (const code of await adapter.listTeamCorrectCodes(db, glAuth)) {
        correctSet.add(code);
      }
    }
  }

  const questions = buildQuestionEntries(links, correctSet, adapter);
  ...
  return {
    ...
    cooldown,
    // ForetMap : `all_levels` quand aucune question n'était au niveau de l'élève (repli signalé).
    ...(levelFallback ? { level_fallback: levelFallback } : {}),
  };
}
...
async function assertGatingSatisfiedForAcknowledge(
  ...
  let links = await loadApprovedGatingLinks(db, product, resourceType, resourceRef);
  // Même jeu de questions que le défi : le filtre de niveau s'applique aussi à la décision.
  const adapter = resolveGatingProduct(product);
  if (adapter?.capabilities.levelFilter && learnerLevel) {
    links = adapter.filterLinksByLevel(links, learnerLevel).links;
  }
