...
const { filterGatingLinksByLevel } = require('./pedago/eligibility');
...
async function loadApprovedGatingLinks(db, product, resourceType, resourceRef) {
  ...
  return db.queryAll(
    `SELECT l.question_code, l.is_gating, l.weight, ${FM_QUESTION_NIVEAU}
...
async function loadApprovedGatingLinksForRefs(db, product, resourceType, refs = []) {
  ...
  const rows =
    p === 'gl'
      ...
      : await db.queryAll(
          `SELECT l.resource_ref, l.question_code, l.is_gating, l.weight, ${FM_QUESTION_NIVEAU}
             FROM resource_question_links l
            WHERE l.resource_type = ? AND l.resource_ref IN (${placeholders})
              AND l.status = 'approved' AND l.is_gating = 1
              AND ${FM_ACTIVE_QUESTION_EXISTS}
...
async function getChallengeState(
  ...
  let levelFallback;
  if (p !== 'gl' && learnerLevel) {
    const byLevel = filterGatingLinksByLevel(links, learnerLevel);
    links = byLevel.links;
...
async function assertGatingSatisfiedForAcknowledge(
  ...
  if (normalizeProduct(product) !== 'gl' && learnerLevel) {
    links = filterGatingLinksByLevel(links, learnerLevel).links;
  }
