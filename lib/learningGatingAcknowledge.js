...
async function loadApprovedGatingLinksForRefs(db, product, resourceType, refs = []) {
  ...
  const rows =
    p === 'gl'
      ? await db.queryAll(
          `SELECT resource_ref, question_code, question_dataset, is_gating, weight
             FROM gl_resource_question_links
            WHERE resource_type = ? AND resource_ref IN (${placeholders})
              AND status = 'approved' AND is_gating = 1
            ORDER BY weight DESC, question_code ASC`,
          [rt, ...unique],
        )
      : await db.queryAll(
          `SELECT resource_ref, question_code, is_gating, weight
             FROM resource_question_links
            WHERE resource_type = ? AND resource_ref IN (${placeholders})
              AND status = 'approved' AND is_gating = 1
            ORDER BY weight DESC, question_code ASC`,
...
async function getChallengeState(
  ...
  if (correctCodesPreload instanceof Set) {
    correctSet = correctCodesPreload;
  } else if (Array.isArray(correctCodesPreload)) {
    correctSet = new Set(correctCodesPreload.map((c) => normalizeQuestionCode(c)).filter(Boolean));
  } else if (p === 'gl') {
  ...
  const retryDays = clampCooldownDays(policy.retryCooldownDays, 0);
  const cooldown = preload?.cooldowns
    ? resourceCooldownStateFromRow(preload.cooldowns.get(ref), retryDays)
    : await getResourceCooldownState(db, {
        product: p,
        userId,
        reader,
        resourceType: rt,
  ...
  return {
    ...
    retry_cooldown_days: policy.retryCooldownDays,
