...
/**
 * Index SPA à servir : hors production, la page d'aide au déploiement ; en production,
 * l'entrée HTML du produit résolu (`distIndexByProduct`, construit depuis `lib/products.js`)
 * si elle existe dans `dist/`, sinon l'entrée ForetMap (`distSpaIndex`). `distGlIndex` reste
 * accepté pour compatibilité (équivaut à `distIndexByProduct.gl`).
 */
function resolveSpaIndexPath(req, options) {
  ...
  if (serveDist) {
    ...
    const byProduct = {
      ...(distGlIndex ? { gl: distGlIndex } : {}),
      ...(distIndexByProduct || {}),
    };
    const candidate = byProduct[product];
    indexPath = candidate && fs.existsSync(candidate) ? candidate : distSpaIndex;
  }
  return indexPath;
}
