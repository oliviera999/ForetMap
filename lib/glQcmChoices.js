...
function presentQuestion(questionRow, glossaryTerms = [], options = {}) {
  ...
  // Le payload JWT est seulement signé (pas chiffré) : tout client peut lire le JSON
  // en base64url. Ne jamais y mettre `correctChoiceId` — la bonne réponse est recalculée
  // côté serveur à la vérification via `reponse_correcte` + `choiceLetters`.
...
function verifyPresentationAnswer(presentationToken, questionCode, choiceId, options = {}) {
  ...
  // Toujours résoudre la bonne réponse côté serveur (jamais depuis le JWT, même legacy).
  const correctLetter = String(options.correctLetter || '')
    .trim()
    .toUpperCase();
  if (!CHOICE_LETTERS.includes(correctLetter)) {
    throw new Error('reponse_correcte invalide');
  }
  const correctChoiceId = choiceLetters.findIndex(
    (letter) => String(letter || '').toUpperCase() === correctLetter,
  );
  if (correctChoiceId < 0) {
    throw new Error('Réponse correcte absente des choix présentés');
  }
