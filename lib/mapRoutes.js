...
/**
 * Bornes des deux champs libres. Elles n'existaient pas : `description` et `step_text` étaient
 * passés tels quels à des colonnes `TEXT` (65 535 octets), donc une **500** SQL (`ER_DATA_TOO_LONG`)
 * en mode strict, ou une troncature silencieuse au milieu d'une phrase autrement
 * (`docs/AUDIT_PARCOURS_2026-09.md` §2.8). Un refus net vaut mieux : ce sont une phrase de
 * présentation et une phrase lue sur place, pas des articles.
 */
const ROUTE_DESCRIPTION_MAX = 2000;
const STEP_TEXT_MAX = 4000;

/** Un parcours plus long qu'une visite d'établissement n'a pas de sens ; borne de sûreté. */
const ROUTE_STEPS_MAX = 60;

/**
 * Bornes du rang d'affichage. `sort_order` est un `INT` signé : le champ « Ordre » de
 * l'éditeur est un `<input type="number">` sans butée, et un rang au-delà de 2 147 483 647
 * remontait en **500** (`ER_WARN_DATA_OUT_OF_RANGE`) au lieu d'un refus lisible
 * (`docs/AUDIT_PARCOURS_2026-09-17.md` §2.4).
 */
const ROUTE_SORT_ORDER_MIN = -2147483648;
const ROUTE_SORT_ORDER_MAX = 2147483647;

/**
 * Valide le rang d'affichage reçu d'un client.
 * @param {unknown} raw `undefined`, vide ou illisible = non fourni (l'appelant garde l'existant).
 * @returns {{ ok: true, value: number | undefined } | { ok: false, error: string }}
 */
function normalizeRouteSortOrder(raw) {
  if (raw === undefined || raw === null || String(raw).trim() === '') {
    return { ok: true, value: undefined };
  }
  const value = parseInt(raw, 10);
  if (!Number.isFinite(value)) return { ok: true, value: undefined };
  if (value < ROUTE_SORT_ORDER_MIN || value > ROUTE_SORT_ORDER_MAX) {
    return {
      ok: false,
      error: `Ordre hors bornes (${ROUTE_SORT_ORDER_MIN} à ${ROUTE_SORT_ORDER_MAX})`,
    };
  }
...
function slugifyRouteTitle(raw) {
  return (
    String(raw || '')
      ...
      // Après la troncature, sinon un titre très long laisse un slug terminé par un tiret.
      .replace(/^-+|-+$/g, '')
  );
}
...
function normalizeRouteSteps(raw) {
  ...
  for (const [index, step] of raw.entries()) {
    ...
    const stepText = step?.step_text == null ? null : String(step.step_text);
    if (stepText !== null && stepText.length > STEP_TEXT_MAX) {
      return {
        ok: false,
...
/**
 * Valide la description reçue d'un client.
 * @param {unknown} raw `undefined` = non fourni (le `PUT` conserve alors l'existant).
 * @returns {{ ok: true, value: string | null | undefined } | { ok: false, error: string }}
 */
function normalizeRouteDescription(raw) {
  if (raw === undefined) return { ok: true, value: undefined };
  if (raw === null) return { ok: true, value: null };
  const value = String(raw);
  if (value.length > ROUTE_DESCRIPTION_MAX) {
    return {
      ok: false,
...
module.exports = {
  ...
  STEP_TEXT_MAX,
  ROUTE_STEPS_MAX,
  ROUTE_SORT_ORDER_MIN,
  ROUTE_SORT_ORDER_MAX,
  STEP_TARGET_TYPES,
  slugifyRouteTitle,
  normalizeRouteSteps,
