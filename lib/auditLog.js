...
const { resolveActorFromReq } = require('./identity');
const logger = require('./logger');

/**
 * Identifiant `users` de l'acteur. Un joueur G&L est journalisé sous son compte lié ; un
 * acteur sans entrée `users` donne `null`.
 */
async function resolveCanonicalActorId(actorUserType, actorUserId) {
  if (!actorUserType || !actorUserId) return null;
  // Joueur G&L : ses identifiants sont des entiers `gl_players.id`, jamais des `users.id` ;
  // le compte canonique est celui qui lui est lié (unification des identités, migration 211).
  // Sans ce détour, chaque action de joueur était journalisée sans acteur canonique.
  if (String(actorUserType) === 'gl_player') {
    const player = await queryOne(
      'SELECT linked_foretmap_user_id FROM gl_players WHERE id = ? LIMIT 1',
      [String(actorUserId)],
    );
    return player?.linked_foretmap_user_id ? String(player.linked_foretmap_user_id) : null;
  }
  const existing = await queryOne('SELECT id FROM users WHERE id = ? LIMIT 1', [actorUserId]);
  return existing?.id ? String(existing.id) : null;
}

/** Complète le payload journalisé de l'identité contrôlée pendant un « voir comme ». */
function withImpersonationPayload(payload, actorFromReq) {
  if (!actorFromReq?.onBehalfOf) return payload || null;
  return { ...(payload || {}), on_behalf_of: actorFromReq.onBehalfOf };
}
...
async function logSecurityEvent(action, options = {}) {
  try {
    ...
    const payloadObject = withImpersonationPayload(options.payload, actorFromReq);
