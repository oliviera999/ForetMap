...
const { normalizeOriginStatus } = require('./plantOriginStatus');
const { normalizeTrophicRole } = require('./plantTrophicRole');
const { normalizeIucnStatus } = require('./plantIucnStatus');
const { normalizeTaxonRank } = require('./taxonRank');
const { normalizeToxicityLevel, normalizeHazardExposure } = require('./plantHazard');
const { normalizeHealthRisk } = require('./plantHealthRisk');
...
function syncNormalizedAndLegacyPlantFields(payload) {
  ...
  // Liste portée par lib/plantTrophicRole.js (ENUM `plants.trophic_role`, `detritivore`
  // depuis la migration 295) ; hors liste → null, comme avant.
  if (payload.trophic_role) payload.trophic_role = normalizeTrophicRole(payload.trophic_role);
  ...
  payload.origin_status = normalizeOriginStatus(payload.origin_status);
  payload.iucn_status = normalizeIucnStatus(payload.iucn_status);
  payload.taxon_rank = normalizeTaxonRank(payload.taxon_rank);

  // Danger (migration 251). `hazard_exposure` alimente un SET SQL : une valeur non
  // canonique y serait rejetée, la normalisation n'est donc pas optionnelle.
  payload.toxicity_level = normalizeToxicityLevel(payload.toxicity_level);
  payload.hazard_exposure = normalizeHazardExposure(payload.hazard_exposure);
  payload.hazard_reviewed = optionalTinyIntBool(payload.hazard_reviewed) ?? 0;

  // Risque sanitaire (migration 271) — même contrainte de SET SQL que `hazard_exposure`.
  payload.health_risk = normalizeHealthRisk(payload.health_risk);
