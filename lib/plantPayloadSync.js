...
function syncNormalizedAndLegacyPlantFields(payload) {
  ...
  if (!payload.taxon_kingdom && payload.group_1) payload.taxon_kingdom = optionalText(payload.group_1);
  if (!payload.taxon_group && payload.group_2) payload.taxon_group = optionalText(payload.group_2);
  if (!payload.taxon_family && payload.group_3) payload.taxon_family = optionalText(payload.group_3);
