...
const SETTINGS_REGISTRY = {
  ...
  'content.auth.guest_visit_cta': { scope: 'public', type: 'string', maxLength: 70, default: '🧭 Visiter sans compte' },
  ...
  /** Distance entre centres emoji et libellé sur la carte (× inv), zones SVG et repères. */
  'ui.map.emoji_label_center_gap': { scope: 'public', type: 'number', min: 6, max: 32, default: 14 },
  /** Échelle des emojis zones/repères (%), 100 = taille de référence. */
  'ui.map.overlay_emoji_size_percent': { scope: 'public', type: 'number', min: 70, max: 150, default: 100 },
  /** Échelle des libellés sous les repères (%). */
  'ui.map.overlay_label_size_percent': { scope: 'public', type: 'number', min: 70, max: 150, default: 100 },
  ...
  'tasks.student_max_active_assignments': { scope: 'teacher', type: 'number', min: 0, max: 99, default: 0 },
  ...
  'content.visit.mascot_dialog.defaults': { scope: 'public', type: 'string', maxLength: 12000, default: '{}' },
  'content.visit.mascot_dialog.catalog_overrides': { scope: 'public', type: 'string', maxLength: 24000, default: '{}' },
  'ui.visit.mascot.allowed_ids': { scope: 'public', type: 'string', maxLength: 500, default: DEFAULT_VISIT_MASCOT_ALLOWED_IDS },
  'ui.visit.mascot.default_id': { scope: 'public', type: 'string', maxLength: 80, default: DEFAULT_VISIT_MASCOT_ID },
  ...
  'content.about.purpose_body': { scope: 'public', type: 'string', maxLength: 500, default: 'ForetMap aide les n3beurs et les n3boss du Lycée Lyautey à organiser les activités de la forêt comestible: suivi des zones, de la biodiversité, des tâches et des observations.' },
  ...
  'content.help.hint_prefix': { scope: 'public', type: 'string', maxLength: 40, default: 'Astuce : ' },
  'content.help.panel_title_prefix': { scope: 'public', type: 'string', maxLength: 8, default: '💡' },
  'content.help.panel_close_cta': { scope: 'public', type: 'string', maxLength: 40, default: 'Fermer' },
  'content.help.panel_dismiss_cta': { scope: 'public', type: 'string', maxLength: 70, default: 'Ne plus afficher' },
  'content.help.map_quick_tip': { scope: 'public', type: 'string', maxLength: 180, default: 'Clique une zone ou un repère puis ouvre ? pour les actions guidées.' },
  'content.help.tasks_quick_tip': { scope: 'public', type: 'string', maxLength: 180, default: 'Filtre d abord par carte ou groupe, puis traite les retours en attente.' },
  'content.help.visit_quick_tip': { scope: 'public', type: 'string', maxLength: 180, default: 'Coche ce que tu vois déjà pour suivre ta progression sur la carte.' },
  ...
  'security.jwt_ttl_base_seconds': { scope: 'teacher', type: 'number', min: 900, max: 604800, default: 5400 },
  'security.jwt_ttl_elevated_seconds': { scope: 'teacher', type: 'number', min: 300, max: 604800, default: 5400 },
...
async function getSettings(audience = 'public') {
  ...
  if (nested?.ui?.visit?.mascot && Object.prototype.hasOwnProperty.call(scopedFlat, 'ui.visit.mascot.allowed_ids')) {
    nested.ui.visit.mascot.allowed_ids = parseVisitMascotAllowedIds(scopedFlat['ui.visit.mascot.allowed_ids']);
  }
...
async function getAuthJwtTtls() {
  ...
  const elKey = 'security.jwt_ttl_elevated_seconds';
  const baseMeta = SETTINGS_REGISTRY[baseKey];
  const elMeta = SETTINGS_REGISTRY[elKey];
  return {
    baseSeconds: flat[baseKey] ?? baseMeta.default,
    elevatedSeconds: flat[elKey] ?? elMeta.default,
  };
}
