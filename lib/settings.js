...
const SETTINGS_REGISTRY = {
  ...
  'ui.staff_plan.title': {
    ...
    type: 'string',
    maxLength: 120,
    default: 'Plan personnels — Lyautey',
  },
   ...
   * passe) avec la permission `staff_plan.access` : elle est toujours ouverte et ne se règle
   * pas ici. `disabled` (défaut) = pas de code du tout ; `code` = un code partagé est aussi
   * accepté, pour les personnels sans compte.
   */
   ...
   * les infos du fichier (défaut) ; `skip` ignore la ligne (ancien comportement).
   */
  ...
  /**
   * Si true (défaut) : le rattachement d'un compte à un groupe n3beur aligne aussitôt son
   * profil sur son nombre de tâches validées (montée seule). Si false, le compte reçoit
   * seulement le profil par défaut du groupe, et le palier se recale au premier recalcul
   * (bouton « Recalculer selon les tâches validées ») ou à la prochaine validation de tâche.
   */
  'rbac.progression_align_on_group_join': { scope: 'teacher', type: 'boolean', default: true },
