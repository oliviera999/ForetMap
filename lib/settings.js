...
const SETTINGS_REGISTRY = {
  ...
  // Liste vide = **toutes** les mascottes du registre sont proposées (catalogue + packs publiés).
...
/**
 * Liste d'ids autorisés : accepte séparateurs `,` `;` et retours ligne, dédoublonne,
 * ignore les entrées de forme invalide. Liste vide = aucune restriction.
 */
function parseVisitMascotAllowedIds(raw) {
  ...
  for (const id of candidates) {
    if (!isValidVisitMascotId(id)) continue;
...
/**
 * Invariant maintenu : **la mascotte par défaut est toujours proposée**. Si l'admin
 * restreint la liste sans y inclure le défaut, le défaut y est ajouté (plutôt que de
 * changer silencieusement la mascotte par défaut sous ses pieds).
 */
function normalizeVisitMascotSettingsFlat(flat) {
  ...
  const defaultId = isValidVisitMascotId(rawDefault) ? rawDefault : '';
  if (defaultId && allowedIds.length > 0 && !allowedIds.includes(defaultId)) {
    allowedIds.push(defaultId);
  }
  flat[allowedKey] = allowedIds.join(',');
