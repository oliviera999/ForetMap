...
// Chargé au premier export PDF : ~+26 Mo de RSS au boot sinon
// (audit docs/AUDIT_CHARGE_SERVEUR_2026-08.md, piste 1).
let PDFDocumentLazy = null;
function getPDFDocument() {
  if (!PDFDocumentLazy) PDFDocumentLazy = require('pdfkit');
  return PDFDocumentLazy;
}
...
/**
 * Styles de secours pour les fiches qui masquent du contenu en attendant un script
 * d’animation (classe `.reveal` → `.visible` via IntersectionObserver). L’aperçu app
 * sert le HTML **sans scripts** (sandbox + assainissement) : sans ce correctif, le
 * texte reste à `opacity: 0` alors que le téléchargement HTML (scripts actifs) l’affiche.
 * Source du motif : fiches `tutos/fiche-*-punk*.html`.
 */
const TUTORIAL_VIEW_NOSCRIPT_REVEAL_STYLE = `<style id="fm-tutorial-noscript-reveal">.reveal{opacity:1!important;transform:none!important}</style>`;
...
/**
 * Insère un fragment juste avant `</head>`, sinon avant `</body>`, sinon en suffixe.
 * @param {string} html
 * @param {string} fragment
 * @returns {string}
 */
function injectIntoTutorialHtml(html, fragment) {
  const s = String(html || '');
  if (!s.trim()) return s;
  const chunk = String(fragment || '');
  if (!chunk) return s;
  const inHead = s.replace(/<\/head\s*>/i, `${chunk}</head>`);
  if (inHead !== s) return inHead;
  const inBody = s.replace(/<\/body\s*>/i, `${chunk}</body>`);
  if (inBody !== s) return inBody;
  return `${s}${chunk}`;
}

/** Rend lisibles les blocs `.reveal` quand les scripts de fiche ne peuvent pas tourner. */
function injectTutorialViewNoScriptRevealStyle(html) {
  return injectIntoTutorialHtml(html, TUTORIAL_VIEW_NOSCRIPT_REVEAL_STYLE);
}

function injectTutorialViewIframeLinkScript(html) {
  return injectIntoTutorialHtml(html, TUTORIAL_VIEW_IFRAME_LINK_SCRIPT);
}
