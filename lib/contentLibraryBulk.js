...
// Chargé au premier import ZIP (audit charge serveur, piste 1 : rien au boot).
let AdmZipLazy = null;
function getAdmZip() {
...
const { parseQcmLoreWorkbook, applyQcmLoreImport } = require('./glQcmLoreImport');
...
const KIND_META = {
  ...
  qcm: { label: 'QCM', subTab: 'qcm-biomes' },
  qcm_lore: { label: 'QCM lore', subTab: 'qcm-lore' },
...
function extractZipEntries(archiveBuffer) {
  ...
  if (archiveBuffer.length > MAX_ARCHIVE_BYTES) {
    throw bulkError(
      `Archive trop volumineuse (max ${formatImportMaxFileLabel(MAX_ARCHIVE_BYTES)})`,
    );
  }
  let zip;
  try {
    zip = new (getAdmZip())(archiveBuffer);
  } catch (_) {
  ...
  for (const entry of entries) {
    ...
    if (seenBasenames.has(key)) {
      throw bulkError(
        `Archive ZIP ambiguë : plusieurs fichiers portent le nom ${fileName}. Renommez-les avant l'import.`,
      );
    }
    ...
    if (totalSize > MAX_DECOMPRESSED_BYTES) {
      throw bulkError(
        `Taille décompressée trop importante (max ${formatImportMaxFileLabel(MAX_DECOMPRESSED_BYTES)})`,
      );
    }
...
async function readWorkbookSheetNames(buffer) {
...
async function readSheetHeaderKeys(buffer, sheetName) {
...
async function classifyGlossaryKind(buffer, sheetNames) {
  if (!hasAnySheet(sheetNames, ['glossaire'])) return null;
  const headers = await readSheetHeaderKeys(buffer, 'glossaire');
...
async function classifyWorkbookKind(buffer) {
  ...
  if (
    hasAnySheet(sheetNames, ['chapitres']) &&
    hasAnySheet(sheetNames, ['reperes', 'zones_royaume'])
  ) {
    return 'chapters';
  }
  ...
  if (hasAnySheet(sheetNames, ['questions']) && hasAnySheet(sheetNames, ['categories'])) {
    const headers = await readSheetHeaderKeys(buffer, 'questions');
    if (headers.has('chapitre_slug') && !headers.has('biome_slug')) return 'qcm_lore';
    if (headers.has('biome_slug')) return 'qcm';
    return 'qcm';
  }
  ...
  if (hasAnySheet(sheetNames, ['glossaire'])) {
    return (await classifyGlossaryKind(buffer, sheetNames)) || 'unknown';
  }
...
async function classifyContentFile(fileName, buffer) {
  ...
  if (isSpreadsheetFile(name, buffer)) {
    if (buffer.length > MAX_FILE_BYTES) {
      return {
        kind: 'unknown',
        warnings: [`Fichier trop volumineux (max ${formatImportMaxFileLabel(MAX_FILE_BYTES)})`],
      };
    }
    const kind = await classifyWorkbookKind(buffer);
    if (kind === 'unknown') {
  ...
  try {
    ...
    return {
      kind: 'unknown',
      warnings: ['Type de fichier non reconnu (média ou XLSX catalogue attendu)'],
    };
  }
}
...
async function dryRunCatalogImport(deps, kind, buffer, options = {}) {
  ...
  switch (kind) {
    case 'species': {
      const { speciesRows, biomeRows } = await parseSpeciesWorkbook(buffer, parseOptions);
    ...
    case 'glossary': {
      const { glossaryRows } = await parseGlossaryWorkbook(buffer, parseOptions);
      const report = await applyGlossaryImport(deps, glossaryRows, { dryRun: true });
      return summarizeReport(report);
    }
    case 'lore_glossary': {
      const { glossaryRows } = await parseLoreGlossaryWorkbook(buffer, parseOptions);
      const report = await applyLoreGlossaryImport(deps, glossaryRows, { dryRun: true });
      return summarizeReport(report);
    }
    case 'spells': {
      const { spellRows, categoryRows } = await parseSpellsWorkbook(buffer, parseOptions);
    ...
    case 'qcm': {
      const { categoryRows, questionRows } = await parseQcmWorkbook(buffer, parseOptions);
      const report = await applyQcmImport(deps, categoryRows, questionRows, { dryRun: true });
      return summarizeReport(report);
    }
    case 'qcm_lore': {
      const { scopeRows, categoryRows, questionRows } = await parseQcmLoreWorkbook(
        buffer,
        parseOptions,
      );
      const report = await applyQcmLoreImport(deps, scopeRows, categoryRows, questionRows, {
        dryRun: true,
      });
      return summarizeReport(report);
    }
    case 'chapters': {
      const parsed = await parseChaptersWorkbook(buffer, parseOptions);
    ...
    case 'chapter_charte': {
      const { rows } = await parseChapterCharteWorkbook(buffer, parseOptions);
      const report = await applyChapterCharteImport(deps, rows, { dryRun: true });
      return summarizeReport(report);
    }
    case 'lore_feuillets': {
      const parsed = await parseFeuilletsWorkbook(buffer, parseOptions);
...
async function applyCatalogImport(deps, kind, buffer, options = {}) {
  ...
  switch (kind) {
    ...
    case 'qcm_lore': {
      ...
      return applyQcmLoreImport(deps, scopeRows, categoryRows, questionRows, { dryRun: false });
    }
    case 'chapters': {
      const parsed = await parseChaptersWorkbook(buffer, parseOptions);
      return withTransaction(async (tx) =>
...
async function analyzeFileEntry(deps, fileName, buffer) {
  const classification = await classifyContentFile(fileName, buffer);
...
function normalizeIncomingFilesJson(body = {}) {
  ...
  if (archive && (archive.fileDataBase64 || archive.fileData || archive.buffer)) {
    const buffer =
      archive.buffer || decodeBase64Payload(archive.fileDataBase64 || archive.fileData);
  ...
  return files.map((file, index) => {
    ...
    if (buffer.length > MAX_FILE_BYTES && isSpreadsheetFile(fileName, buffer)) {
      throw bulkError(
        `${fileName} : fichier trop volumineux (max ${formatImportMaxFileLabel(MAX_FILE_BYTES)})`,
      );
    }
...
function resolveIncomingFileList(uploadPayload = {}) {
  ...
  if (uploadPayload.transport === 'multipart') {
    ...
    const uploadedFiles = Array.isArray(uploadPayload.uploadedFiles)
      ? uploadPayload.uploadedFiles
      : [];
...
async function applyContentLibraryBulk(deps, uploadPayload = {}, options = {}) {
  ...
  const body =
    uploadPayload.transport === 'json' || isLegacyPayload
      ? isLegacyPayload
        ? uploadPayload
        : uploadPayload.body || {}
      : uploadPayload;
  ...
  if (uploadPayload.transport === 'multipart') {
    ...
    if (Array.isArray(uploadPayload.uploadedFiles) && uploadPayload.uploadedFiles.length > 0) {
      uploadedFileMap = new Map(
        uploadPayload.uploadedFiles.map((file) => [file.fileName, file.buffer]),
      );
    }
  } else if (body.archive && (body.archive.fileDataBase64 || body.archive.fileData)) {
    const extracted = extractZipEntries(
      decodeBase64Payload(body.archive.fileDataBase64 || body.archive.fileData),
    );
    archiveFileMap = new Map(extracted.map((file) => [file.fileName, file.buffer]));
  }
  ...
  for (const item of entriesInput) {
    ...
    try {
      ...
      if (kind === 'media') {
        ...
        KIND_META[kind]?.subTab ||
        ['chapter_charte', 'lore_feuillets', 'lore_glossary'].includes(kind)
      ) {
