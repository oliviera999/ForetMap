...
function buildEntryBase(fileName, buffer, classification) {
  ...
  return {
    ...
    error: null,
    preview: null,
    canApply: ['media', 'species', 'glossary', 'lore_glossary', 'spells', 'qcm', 'qcm_lore', 'chapters', 'chapter_charte', 'lore_feuillets'].includes(classification.kind),
  };
}
...
async function dryRunCatalogImport(deps, kind, buffer, options = {}) {
  ...
  switch (kind) {
    ...
    case 'qcm_lore': {
      const { scopeRows, categoryRows, questionRows } = await parseQcmLoreWorkbook(buffer, parseOptions);
      const report = await applyQcmLoreImport(deps, scopeRows, categoryRows, questionRows, { dryRun: true });
      return summarizeReport(report);
    }
    case 'chapters': {
...
async function applyContentLibraryBulk(deps, uploadPayload = {}, options = {}) {
  ...
  for (const item of entriesInput) {
    ...
    try {
      ...
      if (kind === 'media') {
        result.result = saveMediaFromBuffer(buffer, item.mimeType || null, fileName, { skipManifestSync: true, app: 'gl' });
        result.ok = true;
