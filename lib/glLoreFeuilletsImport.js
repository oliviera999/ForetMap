...
const FEUILLET_UPDATE_FIELDS_PROP = '__feuilletUpdateFields';
...
const { normalizeLoreBiomeSlug, LORE_BIOME_SLUG_ALIASES } = require('./glBiomesRegistry');
...
const FEUILLET_UPDATE_FIELDS = FEUILLET_INSERT_FIELDS.filter((field) => field !== 'feuillet_code');
...
// Résolution de feuille tolérante à la casse et aux accents : « Feuillets », « FEUILLETS »
// ou « feuillets » pointent tous vers la même feuille (normalizeImportHeader uniformise).
function resolveSheetName(wb, wanted) {
  const target = normalizeImportHeader(wanted);
  return (wb.sheetNames || []).find((name) => normalizeImportHeader(name) === target) || null;
}

function readSheetRows(wb, sheetName) {
  const resolved = resolveSheetName(wb, sheetName);
  return resolved ? wb.sheets[resolved] || [] : [];
}

function mapFeuilletRow(row = {}) {
  const canonicalFields = new Set(FEUILLET_HEADER_ALIASES.values());
  const out = {};
  const updateFields = new Set();
  for (const [key, value] of Object.entries(row)) {
    const normalized = normalizeImportHeader(key);
    const canonical =
      FEUILLET_HEADER_ALIASES.get(normalized) ||
      (canonicalFields.has(normalized) ? normalized : null);
    if (!canonical) continue;
    out[canonical] = value;
    if (normalizeOptionalString(value) !== null) updateFields.add(canonical);
  }
  attachFeuilletUpdateFields(out, updateFields);
  return out;
}

function getMappedUpdateFields(mapped = {}) {
  if (mapped[FEUILLET_UPDATE_FIELDS_PROP] instanceof Set) {
    return mapped[FEUILLET_UPDATE_FIELDS_PROP];
  }
  const updateFields = new Set();
  for (const [field, value] of Object.entries(mapped)) {
    if (field === FEUILLET_UPDATE_FIELDS_PROP) continue;
    if (normalizeOptionalString(value) !== null) updateFields.add(field);
  }
  return updateFields;
}

function attachFeuilletUpdateFields(target, updateFields) {
  Object.defineProperty(target, FEUILLET_UPDATE_FIELDS_PROP, {
    value: updateFields instanceof Set ? updateFields : new Set(),
    enumerable: false,
    configurable: true,
  });
  return target;
...
function cloneFeuilletPayload(payload, patch) {
  return attachFeuilletUpdateFields(
    { ...payload, ...patch },
    payload?.[FEUILLET_UPDATE_FIELDS_PROP],
  );
}

function shouldUpdateFeuilletField(payload, field) {
  const updateFields = payload?.[FEUILLET_UPDATE_FIELDS_PROP];
  // Compatibilité interne : un payload construit à la main garde l'ancien comportement complet.
  if (!(updateFields instanceof Set)) return true;
  return updateFields.has(field);
}

function buildFeuilletPayload(mapped = {}) {
  const updateFields = getMappedUpdateFields(mapped);
  ...
  const payload = attachFeuilletUpdateFields(
    {
      ...
      image_coupe_url: normalizeFeuilletImageUrl(
        mapped.image_coupe_url,
        'image_coupe_url',
        warnings,
      ),
...
async function resolveFeuilletsImportBody(body = {}) {
...
async function parseFeuilletsWorkbook(buffer, options = {}) {
  ...
  if (buffer.length > maxBytes) {
    throw new Error(`Fichier trop volumineux (max ${formatImportMaxFileLabel(maxBytes)})`);
  }
  ...
  const plateauRows = readSheetRows(wb, PLATEAUX_SHEET).map(buildPlateauPayload).filter(Boolean);
...
async function upsertFeuillet(deps, payload, knownBiomes, dryRun) {
  ...
  if (payload.biome_slug && knownBiomes && !knownBiomes.has(payload.biome_slug)) {
    // Tolérance « max flexibilité » : un biome hors-référentiel ne fait jamais échouer
    // l'import (FK gl_biomes nullable, ON DELETE SET NULL). Le feuillet est importé sans
    // biome et l'anomalie est signalée en avertissement (corrigeable a posteriori).
    warning = {
      code: payload.feuillet_code,
      warnings: [
        {
          field: 'biome_slug',
          warning: `Biome inconnu « ${payload.biome_slug} » ignoré (feuillet importé sans biome)`,
        },
      ],
    };
    payload = cloneFeuilletPayload(payload, { biome_slug: null });
  }
  if (dryRun) return { action: 'upsert', code: payload.feuillet_code, warning };
  const insertColumns = FEUILLET_INSERT_FIELDS.join(', ');
  const insertPlaceholders = FEUILLET_INSERT_FIELDS.map(() => '?').join(', ');
  const updateAssignments = FEUILLET_UPDATE_FIELDS.map(
    (field) => `${field} = IF(?, VALUES(${field}), ${field})`,
  ).join(',\n       ');
  await deps.execute(
    `INSERT INTO gl_lore_feuillets (
     ...
     ON DUPLICATE KEY UPDATE
       ...
       updated_at = NOW()`,
    [
      ...FEUILLET_INSERT_FIELDS.map((field) => payload[field]),
      ...FEUILLET_UPDATE_FIELDS.map((field) => (shouldUpdateFeuilletField(payload, field) ? 1 : 0)),
    ],
  );
  return { action: 'upsert', code: payload.feuillet_code, warning };
}

async function applyFeuilletsImport(
  deps,
  { feuilletRows, plateauRows, rowWarnings },
  options = {},
) {
  ...
  const report = {
    ...
    feuillets: {
      ...
      skipped: 0,
      errors: [],
      warnings: Array.isArray(rowWarnings) ? rowWarnings : [],
    },
  };
  ...
  for (const payload of feuilletRows || []) {
    try {
      const res = await upsertFeuillet(deps, payload, knownBiomes, dryRun);
      report.feuillets.upserted += 1;
      if (res && res.warning) report.feuillets.warnings.push(res.warning);
    } catch (err) {
...
async function buildFeuilletsTemplateWorkbook() {
  const headers = [...FEUILLET_HEADER_ALIASES.values()];
  return buildWorkbookBuffer([
    {
      name: FEUILLETS_SHEET,
      aoa: [
        headers,
        [
          'ex-feuillet',
...
async function buildFeuilletsExportWorkbook(rows) {
  const headers = [...FEUILLET_HEADER_ALIASES.values()];
  const data = [headers];
  ...
  return buildWorkbookBuffer([{ name: FEUILLETS_SHEET, aoa: data }]);
}
