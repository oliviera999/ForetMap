...
function buildFeuilletPayload(mapped = {}) {
  ...
  return {
    payload: {
      ...
      biome_slug: normalizeOptionalString(mapped.biome_slug),
...
function parseFeuilletsWorkbook(buffer) {
  ...
  for (let i = 0; i < rawRows.length; i += 1) {
    ...
    const { payload, errors } = buildFeuilletPayload(mapped);
    if (errors.length) {
  ...
  const plateauRows = readSheetRows(wb, PLATEAUX_SHEET)
    ...
    .filter(Boolean);
  return { feuilletRows, plateauRows, rowErrors };
}
...
async function upsertFeuillet(deps, payload, knownBiomes, dryRun) {
  ...
  await deps.execute(
    `INSERT INTO gl_lore_feuillets (
       ...
       references_scientifiques, lien_qcm_biome, signature, idee_cle, contexte,
       texte_accessible, texte, statut, created_at, updated_at
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
     ON DUPLICATE KEY UPDATE
...
async function applyFeuilletsImport(deps, { feuilletRows, plateauRows }, options = {}) {
  ...
  const report = {
    dryRun,
    plateaux: { upserted: 0 },
    feuillets: { upserted: 0, skipped: 0, errors: [] },
  };
