...
/** Nom passé à Pl@ntNet align : graine GBIF/Wikidata, hint formulaire, ou requête si forme scientifique. */
...
async function applyAutofillGapFill(merged, ctx) {
  let touched = applyPlantnetGapOverlay(merged, ctx.plantnetPack);
...
async function buildSpeciesAutofill(query, options = {}) {
  ...
  const primaryResults = (await Promise.all([
    fetchSourceSafe('wikipedia', () => fetchWikipediaSource(q, fetchOpts)),
    fetchSourceSafe('wikidata', () => fetchWikidataSource(q, fetchOpts)),
    fetchSourceSafe('gbif', () => fetchGbifSource(q, fetchOpts)),
  ])).filter(Boolean);
  const gbifRes = primaryResults.find((r) => r.source === 'gbif' && !r.error);
  const usageKey = gbifRes?.usageKey ?? null;
  const gbifTraitsPack = usageKey
  ...
  const secondaryResults = (await Promise.all([
    ...
    scientificSeed
      ? fetchSourceSafe('catalogue_of_life', () => fetchCatalogueOfLifeSource(scientificSeed, fetchOpts))
  ...
  const wikiHeuristicPack = buildWikipediaHeuristicSource(asOptionalText(wikiFrRes?.fields?.description));
  ...
  const [treflePack, openAiPack, plantnetPack] = await Promise.all([
    fetchSourceSafe('trefle', () => fetchTrefleSpeciesTraits(scientificSeed, fetchOpts)),
    fetchSourceSafe('openai', () => fetchOpenAiSpeciesTraits({
      query: q,
      scientificName: scientificSeed,
      partialContext: openAiPartialContext || asOptionalText(wikiFrRes?.fields?.description),
    }, fetchOpts)),
    fetchSourceSafe('plantnet', () => fetchPlantnetSpeciesTraits(plantnetAlignName, fetchOpts)),
  ]);
