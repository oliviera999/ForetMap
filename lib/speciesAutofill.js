...
const { fetchOpenAiSpeciesTraits } = require('./speciesAutofillOpenAi');
...
async function buildSpeciesAutofill(query, options = {}) {
  ...
  const [treflePack, openAiPack, plantnetPack] = await Promise.all([
    ...
    fetchSourceSafe('openai', () => fetchOpenAiSpeciesTraits({
      ...
      partialContext: asOptionalText(wikiFrRes?.fields?.description),
    }, fetchOpts)),
    fetchSourceSafe('plantnet', () => fetchPlantnetSpeciesTraits(scientificSeed, fetchOpts)),
  ]);
