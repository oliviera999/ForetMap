...
const { syncNormalizedAndLegacyPlantFields } = require('./plantPayloadSync');
const {
  OPENAI_ALLOWED_FIELD_KEYS,
  fetchOpenAiSpeciesTraits,
  fetchOpenAiSpeciesGapFill,
} = require('./speciesAutofillOpenAi');
...
/** @param {Set<string>|null|undefined} allowedSet null = toutes les sources reconnues (`SPECIES_AUTOFILL_SOURCE_IDS`) */
function shouldRunSource(id, allowedSet) {
  const sid = asTrimmedString(String(id)).toLowerCase();
  if (!SPECIES_AUTOFILL_SOURCE_IDS_SET.has(sid)) return false;
...
async function fetchWikipediaEnFallback(query, scientificAlt, options = {}) {
  const tries = uniqStrings([asTrimmedString(query), asTrimmedString(scientificAlt)]).filter(
    Boolean,
  );
  let lastErr = null;
...
async function fetchINaturalistSourceForQuery(searchQuery, options = {}) {
  ...
  const { fields: inatTraitFields, warnings: inatTraitWarnings } =
    extractINaturalistTraitFields(taxon);
  Object.assign(fields, inatTraitFields);
  fields.sources = taxonUrl;
  const photos = [];
  const photoUrl =
    asOptionalText(taxon?.default_photo?.url) ||
...
async function fetchINaturalistSource(searchQueries, options = {}) {
  ...
  // allSettled : une requête en échec (ex. nom scientifique) ne doit pas faire perdre
  // le résultat valide d'une autre (ex. vernaculaire). Promise.all rejetterait tout le lot.
  const settled = await Promise.allSettled(
    queries.map((sq) => fetchINaturalistSourceForQuery(sq, options)),
  );
  const merged = settled.map((r) => (r.status === 'fulfilled' ? r.value : null));
  const valid = merged.filter((r) => r && !r.error);
  if (valid.length === 0) return merged.find(Boolean) || null;
...
async function fetchGbifVernacularSource(usageKey, excludeLower, options = {}) {
  ...
  const exclude = new Set(
    (excludeLower || []).map((s) => asTrimmedString(s).toLowerCase()).filter(Boolean),
  );
  const french = [];
...
async function fetchGbifSource(query, options = {}) {
  ...
  if (family) {
    ...
    fields.taxon_family = family;
  }
  if (order) {
    fields.group_2 = order;
    fields.taxon_group = order;
  }
  if (kingdom) {
    fields.group_1 = kingdom;
    fields.taxon_kingdom = kingdom;
  }
  const usageKey = data?.usageKey != null ? Number(data.usageKey) : null;
  if (Number.isFinite(usageKey) && usageKey > 0) fields.gbif_key = String(usageKey);
...
async function fetchWikidataSource(query, options = {}) {
  ...
  async function loadScoredCandidate(candidate) {
    ...
    try {
      ...
      const isBiologicalTaxon = p31.some(
        (c) => asTrimmedString(c?.mainsnak?.datavalue?.value?.id) === 'Q16521',
      );
      const descFr = asOptionalText(entity?.descriptions?.fr?.value);
      const descEn = asOptionalText(entity?.descriptions?.en?.value);
      const candidateLabel =
        asOptionalText(entity?.labels?.fr?.value) ||
        asOptionalText(entity?.labels?.en?.value) ||
        '';
      let score = 0;
      if (p225) score += 0.7;
      ...
      if (
        ...
        q &&
        /^[A-Z][a-z]+ [a-z]+/.test(q) &&
        p225.toLowerCase().startsWith(q.toLowerCase().split(/\s+/)[0])
      ) {
        score += 0.08;
      }
      if (hasTaxonRank) score += 0.2;
      if (isBiologicalTaxon) score += 0.3;
      if (looksScientificTaxonDescription(descFr) || looksScientificTaxonDescription(descEn))
        score += 0.1;
...
async function fetchCatalogueOfLifeSource(scientificName, options = {}) {
  ...
  const kingdom =
    extractClassificationName(classification, 'kingdom') ||
    extractClassificationName(classification, 'domain');
  const fields = {
    scientific_name: scientific,
    sources: asOptionalText(
      `https://www.catalogueoflife.org/data/taxon/${encodeURIComponent(asTrimmedString(first?.id) || scientific)}`,
    ),
  };
  if (family) {
    fields.group_3 = family;
...
function mergeSources(results = []) {
  ...
  if (fields.sources) {
    ...
    fields.sources =
      uniqStrings(sourceSummaries.map((s) => s.source_url).filter(Boolean)).join('\n') || null;
  }
  ...
  const avgSourceConfidence =
    sourceSummaries.length > 0
      ...
      : 0;
  const fieldCoverageScore = Math.min(
    1,
    Object.keys(fields).filter((k) => k !== 'sources').length / 8,
  );
  const photoScore = photos.length > 0 ? Math.min(1, photos.length / 3) : 0;
  const confidence = clampConfidence(
    avgSourceConfidence * 0.55 + fieldCoverageScore * 0.35 + photoScore * 0.1,
  );
...
async function applyAutofillGapFill(merged, ctx) {
  ...
  let touched = false;
  const keysForOpenAi = computeEmptyGapKeys(merged.fields).filter((k) =>
    OPENAI_GAP_FIELD_KEYS.has(k),
  );
  ...
  const gapPack = await fetchOpenAiSpeciesGapFill(
    {
      ...
      allowGeneralKnowledge: ctx.allowGeneralKnowledge === true,
    },
    gapOpts,
  );
  if (gapPack) {
  ...
  if (touched) {
    merged.warnings = uniqStrings([
      ...(merged.warnings || []),
      'Complément automatique (trous) : champs complétés via OpenAI (passe ciblée) — vérifier avant publication.',
    ]);
  }
}
...
async function buildSpeciesAutofill(query, options = {}) {
  ...
  const gbifTraitsAllowed =
    shouldRunSource('gbif', sourcesAllowed) && shouldRunSource('gbif_traits', sourcesAllowed);
  const gbifTraitsPack =
    usageKey && gbifTraitsAllowed
      ? await fetchSourceSafe('gbif_traits', async () => {
          try {
            const enrich = await fetchGbifSpeciesEnrichment(
              usageKey,
              fetchJsonWithTimeout,
              fetchOpts,
            );
            const hasFields = Object.keys(enrich.fields || {}).some((k) =>
              asOptionalText(enrich.fields[k]),
            );
  ...
  if (shouldRunSource('inaturalist', sourcesAllowed)) {
    secondaryPromises.push(
      fetchSourceSafe('inaturalist', () => fetchINaturalistSource(inatQueries, fetchOpts)),
    );
  } else {
  ...
  if (scientificSeed && shouldRunSource('catalogue_of_life', sourcesAllowed)) {
    secondaryPromises.push(
      fetchSourceSafe('catalogue_of_life', () =>
        fetchCatalogueOfLifeSource(scientificSeed, fetchOpts),
      ),
    );
  } else {
    secondaryPromises.push(Promise.resolve(null));
  ...
  if (shouldRunSource('trefle', sourcesAllowed)) {
    extPromises.push(
      fetchSourceSafe('trefle', () => fetchTrefleSpeciesTraits(scientificSeed, fetchOpts)),
    );
  } else {
  ...
  const [treflePack, openAiPack] = await Promise.all(extPromises);
  ...
  syncNormalizedAndLegacyPlantFields(merged.fields);
  return {
    query: q,
