...
async function loadSideFeuilletsByPlayer(deps, tradeId) {
  const byTrade = await loadSideFeuilletsByTradeAndPlayer(deps, [tradeId]);
  return byTrade.get(Number(tradeId)) || new Map();
}

/**
 * Variante groupée pour une page d'échanges : une seule requête pour tous les
 * échanges de la page, indexée par échange puis par joueur.
 * @returns {Promise<Map<number, Map<number, object[]>>>}
 */
async function loadSideFeuilletsByTradeAndPlayer(deps, tradeIds) {
  const byTrade = new Map();
  if (!tradeIds.length) return byTrade;
  const rows = await deps.queryAll(
    `SELECT sf.trade_id, sf.player_id, sf.feuillet_code, f.titre
       FROM gl_market_trade_side_feuillets sf
       LEFT JOIN gl_lore_feuillets f ON f.feuillet_code = sf.feuillet_code
      WHERE sf.trade_id IN (${tradeIds.map(() => '?').join(', ')})
      ORDER BY f.ordre_voyage ASC, sf.feuillet_code ASC`,
    tradeIds,
  );
  for (const row of rows) {
    const tradeKey = Number(row.trade_id);
    if (!byTrade.has(tradeKey)) byTrade.set(tradeKey, new Map());
    const byPlayer = byTrade.get(tradeKey);
