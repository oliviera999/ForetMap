...
async function registerGlCooldownOnWrongIfGating(
  ...
  try {
    ...
    const perResource = await loadResourcePolicy(db, 'gl', rt, ref);
    const typePolicy = await loadTypePolicy(db, 'gl', rt);
    const chapterGranularity = await resolveGlChapterGranularity(db, {
      resourceType: rt,
      resourceRef: ref,
      glAuth,
    });
