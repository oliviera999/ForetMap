...
const FEUILLET_SELECT = `
  ...
  f.signature, f.idee_cle, f.contexte, f.texte_accessible, f.texte, f.statut
`;
...
function formatFeuilletRow(row, options = {}) {
  ...
  let displayText = preferAccessible && row.texte_accessible
...
async function findFeuilletsForZone(deps, { zoneId, zoneLabel, plateauNumber, biomeSlugs = [] }) {
  ...
  if (zoneLabel) {
    orParts.push('(f.zone_label = ? AND (f.plateau_number IS NULL OR f.plateau_number = ? OR ? = 0))');
...
async function upsertFeuilletState(deps, {
  gameId,
  teamId,
  feuilletCode,
  status,
  ...
  const now = new Date();
  const discoveredAt = status === 'discovered' && !existing?.discovered_at ? now : existing?.discovered_at || null;
  ...
  await deps.execute(
    `INSERT INTO gl_game_feuillet_states (
     ...
     ON DUPLICATE KEY UPDATE
       ...
       updated_at = NOW()`,
    [
      gameId, teamId, feuilletCode, status, effacementPct, unlockedVia, kingdomZoneId,
      discoveredAt, readAt, heldAt, effacedAt,
    ]
  );
}
