...
const FEUILLET_SELECT = `
  ...
  f.image_url, f.image_coupe_url, f.statut, f.offert_ouverture
`;
...
function formatFeuilletRow(row, options = {}) {
  ...
  return {
    ...
    vierge: !!row.vierge,
    offertOuverture: !!row.offert_ouverture,
...
async function loadPlayerFeuilletStates(deps, playerId) {
  // Deux sources unionnées :
  //  - les états des équipes auxquelles le joueur appartient (vue « en partie ») ;
  //  - sa possession propre (`gl_player_feuillet_states`), qui survit aux
  //    remaniements d'équipe et aux suppressions de partie, et porte aussi les
  //    feuillets reçus par échange.
  const rows = await deps.queryAll(
    `SELECT s.feuillet_code, s.status, s.effacement_pct,
     ...
     SELECT p.feuillet_code, p.status, p.effacement_pct,
            p.discovered_by_name, p.discovered_by_player_id, p.discovered_source
...
async function upsertFeuilletState(deps, options) {
  const {
    gameId,
    teamId,
    feuilletCode,
  ...
  const existing = await deps.queryOne(
    `SELECT status, discovered_at, read_at, held_at, effaced_at, effacement_pct
       FROM gl_game_feuillet_states
      WHERE game_id = ? AND team_id = ? AND feuillet_code = ? LIMIT 1`,
    [gameId, teamId, feuilletCode],
  );
  // `read` / `hold` ne fournissent pas `effacementPct` : un défaut à 0 réécrivait
  // silencieusement le pourcentage calculé à la découverte. Conséquence : ouvrir un
  // feuillet (POST …/read, y compris depuis le carnet ou la modale d'arrivée)
  // restaurait tout le texte « mangé par le Souffle », y compris un feuillet déjà
  // à 100 %. On ne pose 0 que s'il n'y a pas encore d'état, ou si l'appelant le
  // demande explicitement (remise d'ouverture / clôture, nouvelle découverte).
  const existingPct = Number(existing?.effacement_pct) || 0;
  const effacementPct =
    options.effacementPct == null ? existingPct : Number(options.effacementPct) || 0;
  const now = new Date();
  const discoveredAt =
  ...
  await snapshotTeamFeuilletPossession(deps, {
    gameId,
    teamId,
    feuilletCode,
    status,
...
/**
 * Matérialise la possession du feuillet pour chaque membre présent de l'équipe.
 *
 * Sans cette trace, la possession serait seulement *dérivée* de l'appartenance
 * à l'équipe : sortir un joueur d'une partie, le changer d'équipe ou supprimer
 * la partie lui retirerait rétroactivement des feuillets — y compris ceux qu'il
 * a lui-même découverts ou reçus contre des gemmes.
 *
 * La règle de partage est inchangée (une découverte profite à toute l'équipe) :
 * chaque membre reçoit simplement sa propre copie de l'état. Une copie plus
 * opaque n'écrase pas une possession personnelle déjà plus lisible.
 */
async function snapshotTeamFeuilletPossession(
  deps,
  {
    gameId,
    teamId,
  ...
  if (!isFeuilletFound(status)) return { players: 0 };

  const members = await deps.queryAll(
    'SELECT player_id FROM gl_team_members WHERE game_id = ? AND team_id = ?',
    [gameId, teamId],
  );
  for (const member of members) {
    await upsertPlayerFeuilletState(deps, {
  ...
  return { players: members.length };
}

/**
 * Fusionne une copie entrante avec la possession personnelle déjà stockée.
 *
 * Le carnet suit le joueur d'une partie à l'autre : on garde toujours le moins
 * effacé. Sans ça, un échange (ou une redécouverte plus « mangée ») vers une
 * équipe qui n'a pas encore le feuillet réécrivait `gl_player_feuillet_states`
 * et opacifiait une lecture déjà acquise — le même défaut que l'écrasement
 * d'équipe, corrigé dans `deliverFeuillets`, mais sur la trace durable.
 *
 * `acquired_via` n'est jamais rétrogradé : une découverte gagne sur un échange.
 *
 * @param {{ status?: string, effacement_pct?: number, acquired_via?: string }|null} existing
 * @param {{ status: string, effacementPct?: number, acquiredVia?: string }} incoming
 */
function mergePlayerFeuilletSnapshot(existing, incoming) {
  const incomingPct = Number(incoming.effacementPct) || 0;
  const incomingAcquiredVia = incoming.acquiredVia || 'decouverte';
  if (!existing || !isFeuilletFound(existing.status)) {
    return {
      status: incoming.status,
      effacementPct: incomingPct,
      acquiredVia: incomingAcquiredVia,
    };
  }
  const existingPct = Number(existing.effacement_pct) || 0;
  const incomingBetter = incomingPct < existingPct;
  const existingAcquiredVia = existing.acquired_via || 'decouverte';
  return {
    status: incomingBetter ? incoming.status : existing.status,
    effacementPct: incomingBetter ? incomingPct : existingPct,
    acquiredVia: incomingAcquiredVia === 'decouverte' ? 'decouverte' : existingAcquiredVia,
  };
}

/**
 * Possession d'un feuillet par un joueur (durable, indépendante des équipes).
 * `acquired_via` n'est jamais rétrogradé : un feuillet d'abord reçu par échange
 * puis retrouvé sur la carte devient une découverte, l'inverse est ignoré.
 * `effacement_pct` / `status` non plus : une copie plus opaque n'écrase pas
 * une lecture déjà acquise (même principe que `deliverFeuillets` côté équipe).
 */
async function upsertPlayerFeuilletState(
  deps,
  {
    playerId,
    feuilletCode,
  ...
  const existing = await deps.queryOne(
    `SELECT status, effacement_pct, acquired_via
       FROM gl_player_feuillet_states
      WHERE player_id = ? AND feuillet_code = ? LIMIT 1`,
    [playerId, feuilletCode],
  );
  const merged = mergePlayerFeuilletSnapshot(existing, { status, effacementPct, acquiredVia });
  await deps.execute(
    `INSERT INTO gl_player_feuillet_states (
       player_id, feuillet_code, status, effacement_pct, acquired_via,
       discovered_by_player_id, discovered_by_name, discovered_source, acquired_at, updated_at
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
     ON DUPLICATE KEY UPDATE
       -- Filet atomique : une course entre deux instantanés ne doit pas non plus
       -- opacifier une copie déjà plus lisible (LEAST + IF, pas VALUES seul).
       status = IF(VALUES(effacement_pct) < effacement_pct, VALUES(status), status),
       effacement_pct = LEAST(effacement_pct, VALUES(effacement_pct)),
       acquired_via = IF(VALUES(acquired_via) = 'decouverte', 'decouverte', acquired_via),
       -- Attribution posée une seule fois : le nom du découvreur d'origine
       -- voyage avec la copie et n'est jamais écrasé.
       discovered_by_player_id = COALESCE(discovered_by_player_id, VALUES(discovered_by_player_id)),
       discovered_by_name = COALESCE(discovered_by_name, VALUES(discovered_by_name)),
       discovered_source = COALESCE(discovered_source, VALUES(discovered_source)),
       updated_at = NOW()`,
...
module.exports = {
  ...
  upsertFeuilletState,
  snapshotTeamFeuilletPossession,
