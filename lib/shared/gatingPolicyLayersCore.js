...
const { clampCooldownHours, formatHoursLabel } = require('./cooldownDurationCore');
...
function clampRetryHours(value, fallback = 6) {
...
function siteLayerFromSettings(site = {}) {
  ...
  return {
    ...
    retryCooldownHours: clampRetryHours(hours, 6),
...
function sanitizePolicyPatch(body = {}, existing = {}) {
  ...
  for (const spec of POLICY_PATCH_FIELDS) {
    ...
    if (spec.column === 'cooldown_scope') {
      ...
      out[spec.column] = clampRetryHours(legacyDays ? Number(raw) * 24 : raw, 6);
    }
  }
