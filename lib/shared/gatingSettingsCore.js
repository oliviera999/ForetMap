...
const { DEFAULT_RETRY_COOLDOWN_HOURS } = require('./cooldownDurationCore');
...
/**
 * Severite du verrou (lot 2 de docs/AUDIT_VALIDATION_QUIZ_2026-09.md, choix 1) :
 *   - `advisory` : le verrou n'est pose que si le client envoie le contexte ressource avec la
 *     reponse (comportement historique) — rien n'empeche de reviser la meme question ailleurs ;
 *   - `flow`     : le contexte est grave dans le jeton de presentation ; une question presentee
 *     « pour la fiche » ne peut plus etre repondue hors contexte. Le Quiz libre reste libre ;
 *   - `strict`   : en plus, une question bloquante d'une ressource de ce type n'est presentable
 *     QUE dans le flux de validation (refus explicite hors flux, exclue du tirage libre).
 * Reglable en cascade site → type de ressource → fiche : les tutoriels peuvent etre stricts
 * et le glossaire souple.
 */
const LOCK_MODE_VALUES = Object.freeze(['advisory', 'flow', 'strict']);
/** Borne haute du delai de verrou : 365 jours, en heures. */
const MAX_RETRY_COOLDOWN_HOURS = 365 * 24;
...
const GATING_SETTING_DEFS = Object.freeze({
  ...
  // En HEURES depuis le lot 2 (il se reglait en jours, defaut 3 j ; puis 6 h). Les anciennes
  // cles `*.retry_cooldown_days` sont converties par la migration 213. Le defaut est
  // `DEFAULT_RETRY_COOLDOWN_HOURS` (1 h) : assez pour qu'une erreur coute quelque chose, assez
  // court pour qu'un eleve puisse rattraper la fiche dans la meme journee de cours.
  retryCooldownHours: {
    type: 'number',
    min: 0,
    max: MAX_RETRY_COOLDOWN_HOURS,
    default: DEFAULT_RETRY_COOLDOWN_HOURS,
    fmKey: 'learning.gating.retry_cooldown_hours',
    glKey: 'gating.retry_cooldown_hours',
  },
  lockMode: {
    type: 'enum',
    values: LOCK_MODE_VALUES,
    default: 'flow',
    fmKey: 'learning.gating.lock_mode',
    glKey: 'gating.lock_mode',
  },
  cooldownScope: {
    type: 'enum',
  ...
  requireLinkedTutorialsBeforeTaskDone: {
    type: 'boolean',
    default: false,
    fmKey: 'learning.gating.require_linked_tutorials_before_task_done',
    glKey: null,
  },
  granularity: {
    type: 'enum',
