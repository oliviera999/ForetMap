...
const { buildCanonicalChoices, CHOICE_LETTERS, presentQuestion } = require('../qcmChoices');
...
function createQuestionQuery({ select, catalogLabel }) {
  ...
  function buildPresentation(questionRow, glossaryTerms = [], options = {}) {
    if (!isPresentableQuestionRow(questionRow)) {
      throw new Error(presentableQuestionError(questionRow?.question_code));
    }
    return presentQuestion(questionRow, glossaryTerms, options);
  }
