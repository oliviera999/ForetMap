...
function sanitizeLinkInput(input = {}, options = {}) {
  ...
  // Un lien n'est bloquant que si on l'a demandé : une proposition, un import ou une
  // création sans le dire ne conditionne jamais (docs/AUDIT_VALIDATION_QUIZ_2026-09.md, B3).
  const rawGating = input.is_gating ?? input.isGating;
  const rawOrigin = input.origin;
  const value = {
    resource_type: resourceType,
    resource_ref: resourceRef,
    question_code: questionCode,
    is_gating: asBool(rawGating, false) ? 1 : 0,
  ...
  // Ce que l'appelant a EXPLICITEMENT fourni : une mise à jour d'un lien existant ne doit
  // réécrire ni le caractère bloquant ni l'origine quand le corps ne les mentionne pas.
  const provided = {
    is_gating: rawGating != null && rawGating !== '',
    origin: rawOrigin != null && String(rawOrigin).trim() !== '',
  };
  return { ok: true, value, provided };
}

/**
 * Resout la politique effective pour une ressource (delegue au module cascade).
 * @see gatingPolicyLayersCore.resolveEffectiveGatingPolicy
 */
function resolveEffectivePolicy(params = {}) {
  const layers = require('./gatingPolicyLayersCore');
  return layers.resolveEffectiveGatingPolicy({
    ...params,
    resourceType:
      params.resourceType ?? params.perResource?.resource_type ?? params.typePolicy?.resource_type,
  });
}
