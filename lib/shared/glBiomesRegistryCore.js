...
/* Source : src/shared/glBiomesRegistryCore.js — régénérer avec `npm run sync:shared-cores`. */
'use strict';

/**
 * Registre canonique des biomes G&L — source unique slugs + assets conventionnels.
 * Slugs assets alignés sur la médiathèque prod (noms GL_* uploadés).
 *
 * Noyau partagé client / serveur : le miroir CJS `lib/shared/glBiomesRegistryCore.js` est
 * généré par `scripts/sync-shared-cores.js` (audit du 13/09/2026, §4.1 — deux copies
 * manuelles divergeaient). Ne pas dupliquer : `src/gl/data/biomes.registry.js` et
 * `lib/glBiomesRegistry.js` ne font que réexporter.
 */
