...
/* Source : src/shared/glMarkerEventConfigCore.js — régénérer avec `npm run sync:shared-cores`. */
'use strict';
...
const MARKER_EFFECT_EVENT_TYPES = new Set([
  'event',
  'souffle',
  'trame',
  'challenge',
...
function hasEffectBranchData(branch) {
  return Boolean(
    branch &&
    (branch.label || branch.deltaPv || branch.deltaGems || branch.deltaMove || branch.passTurn),
  );
}
...
function normalizeLoreQuestionPool(input) {
  const src = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
  const chapitreMode =
    String(src.chapitreMode || 'chapter')
      .trim()
      .toLowerCase() === 'custom'
      ? 'custom'
      : 'chapter';
  return {
    ...
    tierLore: normalizeStringList(src.tierLore)
      .map((s) => s.toLowerCase())
      .filter((s) => s === 'cle' || s === 'recit'),
    ...
    selectedQuestionCodes: normalizeStringList(src.selectedQuestionCodes).map((c) =>
      c.toUpperCase(),
    ),
  };
}
...
function normalizeQuestionConfig(input) {
  ...
  const pool =
    set === 'lore' ? normalizeLoreQuestionPool(src.pool) : normalizeQuestionPool(src.pool);
...
function buildEffectsFromFlatImport(row = {}) {
  return normalizeMarkerEffects({
    ...
    gnome: {
      label: row.effet_gnome,
      deltaPv: row.dpv_gnome,
      deltaGems: row.dgem_gnome,
      deltaMove: row.dmvt_gnome,
    },
    unicorn: {
      label: row.effet_licorne,
      deltaPv: row.dpv_licorne,
      deltaGems: row.dgem_licorne,
      deltaMove: row.dmvt_licorne,
    },
  });
}
function mergeEventConfigWithImport(eventConfig, importExtras = {}) {
  ...
  const eventMeta = normalizeEventMeta({
    tonalite: importExtras.tonalite,
    rarete: importExtras.rarete,
  });
  let question = base.question || null;
  const categorie = String(
    importExtras.categorie_question || importExtras.qcm_categorie_slug || '',
  ).trim();
  ...
  if (categorie || niveau) {
    ...
    question = normalizeQuestionConfig({
      mode: question?.mode || 'random',
      fixedQuestionCode: question?.fixedQuestionCode || importExtras.qcm_question_code || null,
      pool,
    });
  }
  const out = {
    ...
    ...(question ? { question } : {}),
    ...(effectsFromImport
      ? { effects: effectsFromImport }
      : base.effects
        ? { effects: base.effects }
        : {}),
    ...(eventMeta ? { eventMeta } : base.eventMeta ? { eventMeta: base.eventMeta } : {}),
  };
  ...
  return fixedCode
    ? {
        ...
        question: {
          ...
          pool: fixedSet === 'lore' ? lorePool : pool,
        },
      }
...
function resolveMarkerEventConfig(marker) {
  if (!marker) return null;
  return (
    normalizeEventConfig(marker.event_config_json ?? marker.eventConfig ?? marker.event_config) ||
    migrateLegacyMarkerQcmConfig(marker)
  );
}
function isQuestionMarker(marker) {
  if (!marker) return false;
  if (
    MARKER_QUESTION_EVENT_TYPES.has(
      String(marker.event_type || '')
        .trim()
        .toLowerCase(),
    )
  )
...
function usesPeopleSpecificEffects(marker) {
  return MARKER_PEOPLE_EFFECT_TYPES.has(
    String(marker?.event_type || '')
      .trim()
      .toLowerCase(),
  );
...
function serializeEventConfig(eventConfig) {
  const n = normalizeEventConfig(eventConfig);
  return n ? JSON.stringify(n) : null;
}
function parseEventConfigJson(raw) {
  return raw ? normalizeEventConfig(raw) : null;
}
