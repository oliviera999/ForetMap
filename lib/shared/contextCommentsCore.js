...
/**
 * Totaux et dernier id par contexte — une seule requête GROUP BY pour une liste
 * (tâches, tutoriels…). Les contextes sans commentaire n'apparaissent pas dans la Map
 * (le client traite l'absence comme `{ total: 0, newestId: 0 }`).
 *
 * @param {string} contextType
 * @param {Array<string|number>} contextIds
 * @returns {Promise<Map<string, { total: number, newestId: number }>>}
 */
async function countContextCommentsByContextIds(contextType, contextIds = []) {
  ...
  const rows = await queryAll(
    `SELECT context_id, COUNT(*) AS total, MAX(id) AS newest_id
