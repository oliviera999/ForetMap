...
/**
 * Un gain net serait-il rogné par le plafond de jeu ?
 *
 * Sert aux transferts (Marché) : y appliquer le plafond en silence ferait
 * disparaître la monnaie de l'autre joueur. On refuse plutôt le mouvement.
 * Une dépense (delta ≤ 0) n'est jamais bloquée par le plafond.
 */
function wouldGainExceedCap(current, delta, max) {
  const d = parseVitalityDelta(delta);
  if (d <= 0) return false;
  const uncapped = applyDeltaWithCap(current, d, VITALITY_MAX);
  const capped = applyDeltaWithCap(current, d, max);
