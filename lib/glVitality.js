...
/**
 * Verrouille la ligne équipe pour sérialiser les effets one-shot (repères, feuillets,
 * dés, déplacement) et éviter les doubles applications concurrentes.
 */
async function lockGlTeamRow(tx, { gameId, teamId }) {
  const row = await tx.queryOne(
    'SELECT id FROM gl_teams WHERE id = ? AND game_id = ? LIMIT 1 FOR UPDATE',
    [teamId, gameId],
  );
  if (!row) {
...
async function applyPlayerVitalityDelta(tx, { playerId, healthDelta = 0, powerDelta = 0 }) {
  // FOR UPDATE : sérialise les read-modify-write concurrents (deux effets simultanés
  // sur le même joueur ne doivent pas écraser l'un l'autre avec une valeur absolue).
  const row = await tx.queryOne(
...
function resolveVitalityError(err) {
  ...
  if (err?.status === 404 && err?.message === 'TEAM_NOT_FOUND') {
    return { status: 404, error: 'Équipe introuvable dans cette partie' };
  }
