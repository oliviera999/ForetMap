 ...
 *
 * **Les métadonnées des images sont retirées à l'écriture** (`lib/imageMetadata.js`, lot F de
 * `docs/AUDIT_SECURITE_2026-09-22.md` — constat S7 : coordonnées GPS des photos d'élèves
 * servies publiquement). Le retrait est posé ici, au point de passage, et non sur les vingt
 * appelants. Un contenu qui n'est pas une image traitable est écrit tel quel.
 *
...
async function saveBase64ToDisk(relativePath, base64Data) {
  ...
  // Taille vérifiée sur le contenu **reçu**, avant nettoyage : c'est ce que l'appelant a
  // envoyé qui doit tenir dans la borne, pas ce qu'il en reste.
  assertUploadSize(buf.length);
  await fs.promises.writeFile(absolutePath, await stripImageMetadata(buf, { relativePath }));
}
...
async function writeBufferToDisk(relativePath, buffer) {
  ...
  await fs.promises.writeFile(absolutePath, await stripImageMetadata(buffer, { relativePath }));
}
