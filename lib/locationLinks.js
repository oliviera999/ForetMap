...
const LINKS_SELECT =
  'SELECT id, location_kind, location_id, label, url, audience_role_slugs, sort_order FROM location_links';
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    await db.execute(
      `INSERT INTO location_links
         (location_kind, location_id, label, url, audience_role_slugs, sort_order)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        kind,
        id,
