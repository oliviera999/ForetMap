 ...
 *  - `user.create`, `user.link`, `user.deactivate`, `user.reactivate`
...
} = require('../pedago/groupNiveauFromName');

const REPORT_LIST_CAP = 500;

/**
 * Niveau de la classe posé à la création d'un groupe miroir (décision du mainteneur du
 * 25/09/2026, question 5), **seulement sans ambiguïté** : même règle que la migration 301
 * (`autoCurriculumNiveauForGroup` : classes et unités, un seul niveau évoqué, voie précisée
 * pour la première et la terminale). Le nom de la cohorte fait foi ; son identifiant
 * (`26#601`) ne sert que si le nom ne dit rien du niveau. Un nom qui en évoque plusieurs, ou
 * un désaccord entre le nom et l'identifiant, ne pose rien.
 */
function cohortCurriculumNiveau(cohort, groupKind) {
  const kind = groupKind || 'class';
  const byName = autoCurriculumNiveauForGroup({ name: cohort?.name, kind });
  const byIdnumber = autoCurriculumNiveauForGroup({ name: cohort?.idnumber, kind });
  if (byName) return byIdnumber && byIdnumber !== byName ? null : byName;
  const nameSilent = suggestCurriculumNiveauFromName(cohort?.name).raison === 'aucun_indice';
  return nameSilent ? byIdnumber : null;
}
...
function buildPlan({ snapshot, decisions, local }) {
  ...
  for (const cohort of snapshot.cohorts) {
    ...
    // Le périmètre de désactivation ne compte que les cohortes réellement parcourues : une
    // cohorte ignorée (groupe hors synchronisation) ne doit pas faire passer ses membres pour
    // « partis » (CDG-21).
    ...
    if (!groupExists) {
      if (policy.role && !local.roleIdBySlug.has(String(policy.role))) {
        lists.alerts.push({
          code: 'policy_role_unknown',
          cohort: cohort.idnumber,
          message: `La politique ${policy.key} désigne un profil inconnu (${policy.role}) : le groupe ${cohort.idnumber} sera créé sans profil par défaut`,
        });
      }
      push({
        kind: 'group.ensure',
        ...
        payload: {
          ...
          policyKey: policy.key,
          curriculumNiveau: cohortCurriculumNiveau(cohort, policy.group_kind),
        },
      });
    } else {
      if (Number(eg.group_is_active) === 0) {
        // Groupe lié mais désactivé (annulation, désactivation manuelle) : réactivé, jamais
        // doublé (CDG-34).
        push({
          kind: 'group.ensure',
          cohort: cohort.idnumber,
    ...
    if (policy.gl_class) {
      ...
      } else if (Number(glClass.is_active) === 0) {
        push({
          kind: 'gl_class.ensure',
          cohort: cohort.idnumber,
          cohortId: cohort.id,
          payload: { name: glClass.name, existingClassId: Number(glClass.id), ...groupRef },
        });
      }
    }
      ...
      switch (decision.rule) {
        ...
        case 'name': {
          ...
          if (user && Number(user.is_active) !== 1) {
            ...
            if (decision.identity?.origin === 'created') {
              // Compte né de la synchronisation, désactivé après un départ (ou une annulation),
              // de nouveau membre d'une cohorte synchronisée : réactivé (CDG-22). Un compte
              // qu'un administrateur veut garder fermé se marque hors synchronisation.
              if (!reactivatedUserIds.has(userId)) {
                ...
                push({
                  ...
                  cohortId: cohort.id,
                  userId,
                  payload: { reason: 'back_in_cohort' },
                });
                lists.reactivations.push(view);
              }
            } else {
              lists.inactiveInCohort.push(view);
            }
          }
  ...
  for (const identity of local.identitiesByExternalId.values()) {
    ...
    // Limite connue : seules les cohortes déjà synchronisées sont consultées (la lecture Moodle
    // ne charge que les membres des cohortes du périmètre). Un élève passé dans une cohorte
    // jamais synchronisée est donc désactivé ici, puis réactivé (`user.reactivate`) dès que
    // sa nouvelle cohorte est synchronisée (CDG-22).
  ...
  const counts = {
    ...
    reactivations: lists.reactivations.length,
...
module.exports = { buildPlan, cohortCurriculumNiveau, REPORT_LIST_CAP };
