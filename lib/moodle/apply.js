 ...
 *    dépôt qui travaillent sur le pool (`recomputeUserRole`) sont rejoués **après**
...
// Classes et joueurs Gnomes & Licornes : accès regroupés dans l'adaptateur GL.
const game = require('./gameAdapter');
...
async function applyGroupEnsure(tx, { action, ctx, local, journal, issuer }) {
  ...
  if (payload.existingGroupId) {
    // Groupe lié mais désactivé (annulation d'une exécution, désactivation manuelle) : on le
    // réactive plutôt que d'en créer un second pour la même cohorte (CDG-34).
    await tx.execute('UPDATE `groups` SET is_active = 1, updated_at = NOW() WHERE id = ?', [
      String(payload.existingGroupId),
    ]);
    ctx.groupId = String(payload.existingGroupId);
    await journal.record(tx, {
      kind: 'group.ensure',
      targetType: 'group',
      targetId: ctx.groupId,
      before: { isActive: 0 },
      after: { groupId: ctx.groupId, reactivated: true, cohort: ctx.cohort.idnumber },
    });
    return;
  }
  const groupId = crypto.randomUUID();
  const slug = await uniqueGroupSlug(tx, `moodle-${payload.idnumber}`);
  // « n3beur » sans rôle explicite = palier d'entrée : depuis la migration 267, un groupe
  // n3beur est un groupe dont le profil par défaut est un palier n3beur.
  const roleSlug = payload.roleSlug || (payload.n3beur ? 'eleve_novice' : null);
  const defaultRoleId = roleSlug ? (local.roleIdBySlug.get(String(roleSlug)) ?? null) : null;
  // Niveau de la classe déduit du nom de la cohorte par le plan, seulement sans ambiguïté
  // (`cohortCurriculumNiveau`) ; revalidé ici, un plan relu depuis le journal pouvant dater.
  const curriculumNiveau = normalizeLearnerNiveau(payload.curriculumNiveau);
  await tx.execute(
    `INSERT INTO \`groups\`
       (id, slug, name, description, kind, parent_group_id, default_role_id, curriculum_niveau, is_active, created_by)
     VALUES (?, ?, ?, ?, ?, NULL, ?, ?, 1, NULL)`,
    [
      groupId,
      slug,
...
async function applyGlClassEnsure(tx, { action, ctx, journal }) {
  ...
  if (payload.existingClassId) {
    const classId = Number(payload.existingClassId);
    await game.reactivateClass(tx, classId);
    ctx.glClassId = classId;
    if (ctx.externalGroupId) {
      await tx.execute(
        'UPDATE external_groups SET gl_class_id = ?, updated_at = NOW() WHERE id = ?',
        [classId, ctx.externalGroupId],
      );
    }
    await journal.record(tx, {
      ...
      targetId: classId,
      before: { isActive: 0 },
      after: { glClassId: classId, reactivated: true, cohort: ctx.cohort.idnumber },
    });
    return;
  }
  ctx.glClassId = await game.createClass(tx, { name: payload.name, groupId: ctx.groupId });
  if (ctx.externalGroupId) {
    await tx.execute(
      'UPDATE external_groups SET gl_class_id = ?, updated_at = NOW() WHERE id = ?',
...
async function applyMemberAdd(tx, { action, ctx, state, journal }) {
  ...
  if (!existing) {
    await tx.execute(`INSERT INTO group_members (group_id, user_id, user_type) VALUES (?, ?, ?)`, [
      ctx.groupId,
      userId,
...
async function applyPlayerEnsure(tx, { action, ctx, state, journal, vitality }) {
  ...
  const existing = await game.findPlayerByUserId(tx, userId);
  ...
  const pseudo = await game.uniquePlayerPseudo(
    tx,
    user.pseudo || `${user.first_name}.${user.last_name}`.toLowerCase(),
  );
  const playerId = await game.createPlayer(tx, {
    classId,
...
async function applyPlayerMove(tx, { action, ctx, journal }) {
  ...
  const current = await game.findPlayer(tx, payload.playerId);
  if (!current) return;
  if (Number(current.class_id) === Number(toClassId)) return;
  await game.movePlayer(tx, payload.playerId, toClassId);
  await journal.record(tx, {
    kind: 'gl_player.move',
    targetType: 'gl_player',
...
/** Symétrique de `applyUserDeactivate`, dans la transaction de la cohorte où le compte réapparaît. */
async function applyUserReactivate(tx, { action, ctx, state, journal }) {
  const userId = String(action.userId);
  const before = await tx.queryOne('SELECT is_active FROM users WHERE id = ? LIMIT 1', [userId]);
  if (!before || Number(before.is_active) === 1) return;
  await tx.execute('UPDATE users SET is_active = 1, updated_at = NOW() WHERE id = ?', [userId]);
  ctx.touchedUserIds.add(userId);
  state.reactivated += 1;
  await journal.record(tx, {
    ...
    targetId: userId,
    before: { isActive: 0 },
    after: { isActive: 1, reason: action.payload?.reason || null, cohort: ctx.cohort.idnumber },
  });
}
  ...
  for (const action of orderActions(actions)) {
    switch (action.kind) {
      ...
      case 'user.reactivate':
        await applyUserReactivate(tx, { action, ctx, state, journal });
        break;
...
async function applyPlan({ runId, plan, snapshot, local, client, log = logger }) {
  ...
  const state = { createdUsers: new Map(), actionsApplied: 0, reactivated: 0 };
  const failedCohorts = [];
  const failedCohortIds = new Set();
  const touchedUserIds = new Set();
  const cohortResults = [];
  const vitality = await game.loadDefaultVitality();
  ...
  for (const cohort of snapshot.cohorts) {
    ...
    const reactivatedBefore = state.reactivated;
    try {
      ...
      state.reactivated = reactivatedBefore;
  ...
  for (const userId of touchedUserIds) {
    try {
      const r = await recomputeUserRole(userId);
