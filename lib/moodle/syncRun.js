 ...
 *  3. lecture Moodle, contrôles amont, rapprochement ;
...
async function runSync({
  ...
  try {
    ...
    const upstream = runUpstreamChecks({ members, emailDomains: settings.emailDomains });
    if (upstream.errors.length) {
      ...
      await closeRun(runId, {
        status: 'failed',
        totals: { upstreamErrors: upstream.errors.length },
        report,
        error: `Contrôles amont : ${upstream.errors.length} problème(s)`,
      });
      log.warn(
        { upstreamErrors: upstream.errors.length },
        'Exécution Moodle bloquée par les contrôles amont',
      );
    ...
    // 4–5. Plan et seuils.
    const plan = buildPlan({ snapshot, decisions, local, settings });
    const thresholds = evaluateThresholds({ plan, settings, local });
    let report = buildReport({ mode, scope, settings, snapshot, plan, thresholds });

    if (mode === 'dry_run') {
      await closeRun(runId, { status: 'succeeded', totals: plan.counts, report });
      log.info({ totals: plan.counts, blocked: thresholds.blocked }, 'Simulation Moodle terminée');
