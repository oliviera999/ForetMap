...
async function runPostChecks({ snapshot, log }) {
  ...
  try {
    const gl = await game.buildIdentityReport();
...
async function runSync({
  ...
  if (mode === 'apply') assertMoodleEnabled(settings);
  ...
  try {
    ...
    // Un membre déjà lié ne quitte pas le plan : l'écarter ferait croire au rapprochement
    // qu'il a quitté la cohorte (retrait de groupe + désactivation).
    const skippedIds = new Set(
      upstream.skips.map((s) => String(s.externalId)).filter((id) => !linkedExternalIds.has(id)),
    );
    ...
    async function attachTeamMirrors(dryRun, journal = null) {
    ...
    // Les écritures des miroirs d'équipes suivent le journal de l'exécution (annulables).
    await attachTeamMirrors(false, createJournal(runId, applied.journalCount || 0));
