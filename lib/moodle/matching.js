 ...
 *
 * Contrôles amont (8.3), bloquants pour toute l'exécution : `runUpstreamChecks`.
 */
...
/**
 * Contrôles amont (8.3). `members` : membres Moodle dédoublonnés (un par `id`).
 * @returns {{ errors: Array<{ code: string, message: string, externalId?: number|string }> }}
 */
function runUpstreamChecks({ members, emailDomains = [] }) {
  const errors = [];
  ...
  for (const member of members || []) {
    ...
    if (!email) {
      if (!isSuspended(member)) {
        errors.push({
          ...
          message: `Le membre Moodle ${member.id} (${member.username || '?'}) n’a pas d’e-mail`,
        });
      }
      continue;
    }
    if (domains.length && !domains.includes(emailDomain(email))) {
      errors.push({
        ...
        externalId: member.id,
        message: `E-mail hors des domaines autorisés pour le membre Moodle ${member.id} (${emailDomain(email)})`,
      });
    }
    if (byEmail.has(email)) {
      errors.push({
        ...
        externalId: member.id,
        message: `Deux membres Moodle portent le même e-mail (${byEmail.get(email)} et ${member.id})`,
      });
    } else {
