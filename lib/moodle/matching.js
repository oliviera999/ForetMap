 ...
 *  3. prénom + nom normalisés, **uniques des deux côtés** ; homonymes ⇒ en attente, de même
 *     qu'un e-mail présent des deux côtés mais différent ;
 ...
 * concernés sont laissés de côté ; le reste de l'exécution continue. Un membre **déjà
 * lié** n'est jamais écarté (e-mail manquant, domaine, doublon) : le plan le conserve.
 */
 ...
 * continue pour les autres. Un membre **déjà lié** n'est pas écarté (identité connue :
 * les appartenances restent ; un écart du plan serait lu comme un départ).
 *
 * @param {object} args
...
function runUpstreamChecks({ members, emailDomains = [], linkedExternalIds = null }) {
  ...
  for (const member of members || []) {
    ...
    const isLinked = Boolean(linked?.has(String(member.id)));
    if (!email) {
      if (!isSuspended(member) && !isLinked) {
        skips.push({
    ...
    if (domains.length && !domains.includes(emailDomain(email))) {
      // Déjà lié : on signale le problème sans l'écarter du plan (sinon retrait/désactivation).
      if (!isLinked) {
        skips.push({
    ...
    if (byEmail.has(email)) {
      ...
      if (!isLinked) {
        skips.push({
          ...
          externalId: member.id,
          message: `Deux membres Moodle portent le même e-mail (${otherId} et ${member.id}) — le compte non lié est laissé de côté`,
        });
      }
      // Le premier porteur n'est écarté que s'il n'est pas déjà lié.
      if (
        !linked?.has(String(otherId)) &&
  ...
  for (const member of members || []) {
    ...
    if (nameKey) {
      ...
      if (localCandidates.length === 1 && uniqueMoodleSide) {
        ...
        if (email && localEmail && localEmail !== email) {
          // Même nom, mais chacun a un e-mail et ils diffèrent : un humain tranche (CDG-47).
          decisions.set(externalId, {
            ...base,
            rule: 'pending',
