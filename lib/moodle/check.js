...
const ERROR_HINTS = Object.freeze({
  ...
  sitepolicynotagreed:
    'Le compte de service n’a pas accepté la politique du site : tant qu’elle ne l’est pas, Moodle ' +
    'refuse toutes ses fonctions Web Services, même correctement autorisées. Deux gestes : ouvrir ' +
    'une session Moodle avec ce compte et accepter la politique, ou, en administrateur, accepter en ' +
    'son nom (Administration du site → Utilisateurs → Confidentialité et politiques → Accords des ' +
    'utilisateurs). Un compte de service n’ayant pas vocation à se connecter, la seconde voie est ' +
    'la bonne.',
  invalidrecord:
...
const SITE_INFO_ACCESS_HINTS = Object.freeze({
  ...
  // La sonde a été *acceptée* puis refusée pour une autre raison (politique du site, capacité
  // Moodle…) : cela prouve aussi que l'autorisation Web Services passe. Ne pas dire « a répondu ».
  functionOther:
    `Le jeton s’authentifie et l’autorisation Web Services passe (${PROBE_FUNCTION} a été admise, ` +
    'puis refusée pour une autre raison — voir l’erreur qui suit) : c’est bien la fonction ' +
    'core_webservice_get_site_info qui manque au service externe. L’ajouter dans Moodle → ' +
    'Administration du site → Serveur → Services web → Services externes → Fonctions. Traiter ' +
    'aussi l’autre erreur : elle bloque les lectures.',
...
async function runMoodleCheck({ client, settings, chapters = [] }) {
  ...
  // 1. Jeton et site.
  let siteInfo = null;
  /** Refus déjà essuyé par la sonde : inutile de rejouer le même appel à l'étape 3. */
  let probeError = null;
  try {
    ...
    if (summary.errorcode === 'accessexception') {
      ...
      probeError = probe.ok ? null : probe.error || null;
      summary.hint =
        probe.scope === 'function' && !probe.ok
          ? SITE_INFO_ACCESS_HINTS.functionOther
          : SITE_INFO_ACCESS_HINTS[probe.scope];
  ...
  if (probeError) {
    // La sonde vient d'essuyer ce refus sur la même fonction : le reporter sans rappeler Moodle.
    report.errors.push({ step: 'cohorts', ...summarizeError(probeError) });
  } else if (mayCall('core_cohort_search_cohorts')) {
    try {
