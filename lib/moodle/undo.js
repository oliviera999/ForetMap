...
async function undoOne(tx, action, ctx) {
  ...
  switch (action.kind) {
    ...
    case 'group.ensure': {
      ...
      const linkedClass = await game.findClassByGroupId(tx, groupId);
    ...
    case 'gl_class.ensure': {
      ...
      if (after.reactivated) {
        await game.deactivateClass(tx, classId);
        return true;
      }
      if ((await game.countClassPlayers(tx, classId)) === 0) {
        await tx.execute(
          'UPDATE external_groups SET gl_class_id = NULL, updated_at = NOW() WHERE gl_class_id = ?',
          [classId],
        );
        await game.deleteClass(tx, classId);
      } else {
    ...
    case 'gl_player.ensure': {
      // Contributions de jeu déjà rattachées (FK RESTRICT) : désactivé au lieu d'être supprimé.
      await game.removePlayer(tx, Number(action.target_id));
      return true;
    }
    case 'gl_player.move': {
      await game.restorePlayerPlacement(tx, action.target_id, {
