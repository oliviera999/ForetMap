...
const { loadMoodleSettings, assertMoodleEnabled } = require('./settings');
const { createMoodleClientFromEnv } = require('./client');
const { httpError } = require('../shared/httpError');
// Parties, équipes, joueurs et classes Gnomes & Licornes : lus par l'adaptateur GL.
 ...
 */
/** Journalise une écriture sortante quand un journal d'exécution est fourni (annulable). */
async function recordOutbound(journal, entry) {
  if (journal) await journal.record(database, entry);
}

async function mirrorGameTeams({
  gameId,
  dryRun = true,
  client = null,
  settings = null,
  ...
  const game = await gameAdapter.findGame(gameId);
  ...
  if (!dryRun) assertMoodleEnabled(cfg);
  ...
  const cohort = await gameAdapter.cohortIdnumberForClass(game.class_id);
  if (!cohort) {
    report.notices.push('Classe G&L sans cohorte Moodle liée');
    return report;
  }
  const teams = await gameAdapter.listGameTeams(game.id);
  const members = await gameAdapter.listGameTeamMembers(game.id);
  ...
  if (!dryRun) {
    for (const w of wanted) {
      ...
      if (!group) {
        ...
        groups.push(group);
        await recordOutbound(journal, {
          kind: 'course_group.create',
          targetType: 'course_group',
          targetId: group.id,
          before: null,
          after: { courseId, name: w.team.name, idnumber: w.idnumber, teamId: w.team.id },
        });
        ...
        await recordOutbound(journal, {
          kind: 'course_group.rename',
          targetType: 'course_group',
          targetId: group.id,
          before: { name: group.name },
          after: { name: w.team.name, idnumber: w.idnumber },
        });
    ...
    if (toDelete.length) {
      ...
      for (const g of toDelete) {
        ...
        await recordOutbound(journal, {
          kind: 'course_group.delete',
          targetType: 'course_group',
          targetId: g.id,
          before: { courseId, name: g.name, idnumber: g.idnumber },
          after: null,
        });
      }
    }
    ...
    for (const w of wanted) {
      ...
      if (add.length) {
        ...
        for (const userid of add) {
          await recordOutbound(journal, {
            kind: 'course_group.member.add',
            targetType: 'course_group_member',
            targetId: `${gid}:${userid}`,
            before: null,
            after: { groupId: gid, moodleUserId: userid, team: w.team.name },
          });
        }
      }
      if (remove.length) {
        ...
        for (const userid of remove) {
          await recordOutbound(journal, {
            kind: 'course_group.member.remove',
            targetType: 'course_group_member',
            targetId: `${gid}:${userid}`,
            before: { groupId: gid, moodleUserId: userid, team: w.team.name },
            after: null,
          });
        }
      }
 ...
 */
async function mirrorForetmapGroup({
  groupId,
  courseId,
  dryRun = true,
  client = null,
  ...
  if (!moodle)
    throw httpError(503, 'Intégration Moodle non configurée', { code: 'MOODLE_NOT_CONFIGURED' });
  if (!dryRun) assertMoodleEnabled(settings || (await loadMoodleSettings()));
...
async function mirrorTeamsForCohorts({
  cohortIdnumbers,
  dryRun,
  client,
  settings,
  ...
  const games = await gameAdapter.listGamesForCohorts(cohortIdnumbers);
  const seenClasses = new Set();
  for (const row of games) {
    const game = await gameAdapter.findGameClass(row.id);
    if (!game || seenClasses.has(Number(game.class_id))) continue;
    seenClasses.add(Number(game.class_id));
    reports.push(await mirrorGameTeams({ gameId: game.id, dryRun, client, settings, journal }));
  }
  return reports;
}
