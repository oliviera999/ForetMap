...
const { normalizeCasterKind } = require('./glSpellOptions');
...
async function loadSpellsForChapterIds(db, chapterIds) {
  const map = new Map();
  const ids = [
    ...new Set(chapterIds.map((id) => Number(id)).filter((id) => Number.isFinite(id) && id > 0)),
  ];
  if (ids.length === 0) return map;
  const placeholders = ids.map(() => '?').join(', ');
  const rows = await db.queryAll(
    `SELECT cs.chapter_id, cs.spell_code, s.nom, s.emoji, s.category_slug, s.caster_kind,
            cs.order_index
  ...
  for (const row of rows) {
    ...
    map.get(chapterId).push({
      ...
      // Peuple autorisé : l'assistant de lancement s'en sert pour filtrer les équipes
      // sélectionnables sans re-solliciter la fiche complète du sort.
      caster_kind: normalizeCasterKind(row.caster_kind),
