...
async function spawnSingleRecurringTask(taskRow, today) {
  ...
  return withTransaction(async (tx) => {
    ...
    const dangerLevelRaw = row.danger_level != null ? String(row.danger_level).trim().toLowerCase() : "";
    const dangerLevel = ["safe", "potential_danger", "dangerous", "very_dangerous"].includes(dangerLevelRaw)
      ? dangerLevelRaw
      : null;
    const difficultyLevelRaw = row.difficulty_level != null ? String(row.difficulty_level).trim().toLowerCase() : "";
    const difficultyLevel = ["easy", "medium", "hard", "very_hard"].includes(difficultyLevelRaw)
      ? difficultyLevelRaw
      : null;
    const importanceLevelRaw = row.importance_level != null ? String(row.importance_level).trim().toLowerCase() : "";
    const importanceLevel = ["not_important", "low", "medium", "high", "absolute"].includes(importanceLevelRaw)
      ? importanceLevelRaw
      : null;
    const createdIso = new Date().toISOString();

    const livingBeingsClone =
      row.living_beings != null && String(row.living_beings).trim() !== ""
        ? String(row.living_beings).trim()
        : null;

    await tx.execute(
      `INSERT INTO tasks (
        id, title, description, map_id, project_id, zone_id, marker_id,
        start_date, due_date, required_students, completion_mode, danger_level, difficulty_level, importance_level, living_beings, status, recurrence, created_at, parent_task_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?, ?)`,
      [
        newId,
        row.title,
...
async function runRecurringTaskSpawnJob(options = {}) {
  ...
  if (!options.force) {
    ...
    if (!automationEnabled) {
      ...
      logger.info({ today, job: "recurring_tasks" }, "Job tâches récurrentes suspendu (réglage global)");
      return {
        skipped: true,
