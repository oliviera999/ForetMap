...
/** Marge de rangs explorés en arrière de l’estimation arithmétique (cf. ci-dessous). */
const OCCURRENCE_LOOKBACK = 12;
/** Nombre maximal de rangs sondés : borne dure du coût calendrier par tâche. */
const OCCURRENCE_MAX_PROBES = 60;

/** Date théorique de l’occurrence de rang k (aucun accrochage calendaire). */
function occurrenceDate(anchor, recurrence, k) {
  if (recurrence === 'weekly') return addDaysToDateString(anchor, 7 * k);
  if (recurrence === 'biweekly') return addDaysToDateString(anchor, 14 * k);
  if (recurrence === 'monthly') return addMonthsToDateString(anchor, k);
  return null;
}

function monthsBetweenUtc(fromStr, toStr) {
...
/**
 * Estimation arithmétique du premier rang k ≥ 1 dont la date théorique atteint `target`.
 * Évite d’itérer depuis une ancre vieille de plusieurs années : chaque sonde coûte des
 * requêtes calendrier (`isSchoolOpenDay` interroge la base jour par jour).
 */
function estimateOccurrenceIndex(anchor, recurrence, target) {
  if (recurrence === 'monthly') return Math.max(1, monthsBetweenUtc(anchor, target));
  const step = recurrence === 'weekly' ? 7 : 14;
  return Math.max(1, Math.ceil(daysBetweenUtc(anchor, target) / step));
}

/**
 * Ancre de récurrence de la série : la date qui porte le rythme.
 * `recurrence_anchor_date` (migration 258) d’abord, puis la date de départ, puis la date
 * de création — une tâche récurrente a donc toujours une ancre.
 */
function resolveRecurrenceAnchor(taskRow) {
  return (
    parseISODateOnly(taskRow.recurrence_anchor_date) ||
    parseISODateOnly(taskRow.start_date) ||
    parseISODateOnly(dateFromCreatedAt(taskRow.created_at))
  );
}
 ...
 *
 * Le calcul part de l’**ancre de la série**, jamais de l’occurrence précédente :
 * occurrence de rang k = ancre + k × période, sur la date **théorique**. L’accrochage au
 * jour ouvré scolaire ne sert qu’à écrire la ligne et **ne se réinjecte pas** dans le
 * calcul suivant — sans quoi une semaine de vacances déplace définitivement le rythme
 * (cf. `migrations/258_task_recurrence_anchor.sql` pour les chiffres).
 *
 * L’écart départ→échéance de la source est conservé, de sorte que l’échéance suit son
 * départ.
 *
 * Invariants : départ et échéance tombent un **jour ouvré scolaire** ; l’échéance n’est
 * **jamais déjà dépassée** (due >= today, rattrapage « une seule occurrence à jour ») et
 * est **strictement postérieure** à celle de la source — l’index unique
 * `(recurrence_series_id, due_date)` refuserait un doublon.
 *
 * @param {object} taskRow ligne `tasks` source
 * @param {object} [opts]
 * @param {(d: string) => Promise<string|null>} [opts.nextOpenDay]
 * @returns {Promise<{startDate: string, dueDate: string, index: number}|null>}
 */
async function computeNextOccurrenceWindow(taskRow, recurrence, today, opts = {}) {
  ...
  const anchor = resolveRecurrenceAnchor(taskRow);
  if (!anchor) return null;

  // Durée départ→échéance de la source. Le repli `created_at` sert ici uniquement à
  // mesurer cette durée (comportement d’origine) ; comme ancre, il est déjà couvert par
  // resolveRecurrenceAnchor.
  const spanBase = parseISODateOnly(taskRow.start_date) || dateFromCreatedAt(taskRow.created_at);
  const span = spanBase && dueSrc >= spanBase ? Math.max(0, daysBetweenUtc(spanBase, dueSrc)) : 0;

  const nextOpen = opts.nextOpenDay || (async (d) => nextSchoolOpenDay(d));

  // On repart quelques rangs en arrière de l’estimation : une date théorique tombant
  // pendant une coupure est repoussée après la réouverture et peut donc, à un rang plus
  // petit, satisfaire déjà `due >= today`. La marge couvre les grandes vacances.
  const estimated = estimateOccurrenceIndex(anchor, recurrence, todayStr);
  let k = Math.max(1, estimated - OCCURRENCE_LOOKBACK);

  for (let probe = 0; probe < OCCURRENCE_MAX_PROBES; probe += 1, k += 1) {
    const theoretical = occurrenceDate(anchor, recurrence, k);
    if (!theoretical) return null;
    const startDate = await nextOpen(theoretical);
    if (!startDate) return null;
    const dueDate = await nextOpen(addDaysToDateString(startDate, span));
    if (!dueDate) return null;
    if (dueDate >= todayStr && dueDate > dueSrc) return { startDate, dueDate, index: k };
  }
  return null;
}

/**
 * Résolution des jours ouvrés mémoïsée pour la durée d’une exécution du job.
 * `isSchoolOpenDay` interroge la base **jour par jour** et `nextSchoolOpenDay` peut en
 * scanner plusieurs dizaines pendant des vacances ; sans cache, chaque tâche candidate
 * repaie le même parcours. Le cache est local à l’exécution : aucun risque de servir un
 * calendrier périmé au lendemain.
 */
function createOpenDayResolver() {
  const cache = new Map();
  return async (dateStr) => {
    const key = parseISODateOnly(dateStr);
    if (!key) return null;
    if (cache.has(key)) return cache.get(key);
    const resolved = await nextSchoolOpenDay(key);
...
async function spawnSingleRecurringTask(taskRow, today, opts = {}) {
  ...
  return withTransaction(async (tx) => {
    ...
    // Ancre effective de la série : elle voyage sur le clone, et l'occurrence courante la
    // reçoit aussi si elle ne l'avait pas encore (série créée avant la migration 258).
    const anchorLocked = resolveRecurrenceAnchor(row);
    ...
    try {
      await tx.execute(
        `INSERT INTO tasks (
          ...
          status, recurrence, created_at, parent_task_id, recurrence_series_id, recurrence_anchor_date
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?, ?, ?, ?)`,
        [
          newId,
          row.title,
    ...
    if (anchorLocked && !parseISODateOnly(row.recurrence_anchor_date)) {
      await tx.execute(
        'UPDATE tasks SET recurrence_anchor_date = ? WHERE id = ? AND recurrence_anchor_date IS NULL',
        [anchorLocked, row.id],
      );
    }
    if (!row.recurrence_series_id) {
...
async function runRecurringTaskSpawnJob(options = {}) {
  ...
  // Un seul cache calendrier pour toute l'exécution : les séries partagent largement les
  // mêmes dates, et une coupure scolaire ne se parcourt ainsi qu'une fois.
  const spawnOptions = {
    ...options,
    nextOpenDay: options.nextOpenDay || createOpenDayResolver(),
  };

  for (const task of candidates) {
    try {
      const newId = await spawnSingleRecurringTask(task, today, spawnOptions);
      if (newId) {
        created.push(newId);
