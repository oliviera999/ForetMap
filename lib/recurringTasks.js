...
async function spawnSingleRecurringTask(taskRow, today, opts = {}) {
  ...
  return withTransaction(async (tx) => {
    ...
    const newDueLocked = await computeNextOccurrenceDue(dueLocked, recurrence, today, opts);
    if (!newDueLocked) return null;
    const newStartLocked = await computeCloneStartDate(row, newDueLocked, recurrence, opts);
