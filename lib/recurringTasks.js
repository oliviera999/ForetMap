  ...
  return withTransaction(async (tx) => {
    const row = await tx.queryOne(
      `SELECT * FROM tasks WHERE id = ?
         ...
         -- Archiver une tâche récurrente doit l'arrêter. Sans ce filtre, elle continuait
         -- d'engendrer une occurrence à chaque échéance : la ranger ne suffisait pas,
         -- il fallait aussi penser à retirer la récurrence.
    ...
    const importanceLevel = ['not_important', 'low', 'medium', 'high', 'absolute'].includes(
      ...
      ? importanceLevelRaw
      : null;
    const createdIso = nowIsoUtc();

    await tx.execute(
      `INSERT INTO tasks (
        id, title, description, map_id, project_id, zone_id, marker_id,
        start_date, due_date, required_students, completion_mode, danger_level, difficulty_level, importance_level, status, recurrence, created_at, parent_task_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?, ?)`,
      [
        newId,
        row.title,
  ...
  const candidates = await queryAll(
    `SELECT * FROM tasks
      WHERE recurrence IN ('weekly','biweekly','monthly')
        AND due_date IS NOT NULL AND TRIM(due_date) <> ''
        AND status = 'validated'
        -- Archiver une tâche récurrente doit l'arrêter (même filtre que la relecture
        -- verrouillée de spawnSingleRecurringTask, qui garde la course).
        AND archived_at IS NULL
        AND due_date <= ?
        AND (recurrence_spawned_for_due_date IS NULL OR recurrence_spawned_for_due_date <> due_date)`,
    [today],
