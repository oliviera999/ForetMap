...
function validateEnv() {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (!process.env.VISIT_COOKIE_SECRET || String(process.env.VISIT_COOKIE_SECRET).trim().length < 16) {
      throw new Error(
    ...
    if (hasGoogleClientId || hasGoogleClientSecret || hasGoogleRedirectUri) {
      ...
      if (!process.env.GOOGLE_OAUTH_ALLOWED_DOMAINS && !process.env.GOOGLE_OAUTH_ALLOWED_EMAILS) {
        logger.warn(
          'OAuth Google actif sans liste explicite : les domaines/emails par défaut seront utilisés. Définir GOOGLE_OAUTH_ALLOWED_DOMAINS/GOOGLE_OAUTH_ALLOWED_EMAILS en production.'
        );
      }
    }
  }
