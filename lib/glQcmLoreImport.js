...
function buildScopePayload(row = {}) {
  ...
  const plateau = plateauRaw === '' || plateauRaw == null
    ? null
    : Number(plateauRaw);
  return {
...
function validateQuestionPayload(payload, rowNumber, knownScopes, knownCategories) {
  ...
  if (!payload.chapitre_slug) {
    ...
    errors.push({ row: rowNumber, field: 'chapitre_slug', error: `chapitre_slug inconnu: ${payload.chapitre_slug}` });
  }
  ...
  if (!payload.reponse_correcte) {
    ...
    if (!asTrimmedString(payload[answerKey])) {
      errors.push({ row: rowNumber, field: 'reponse_correcte', error: 'reponse_correcte absente des choix non vides' });
    }
  }
...
function parseQcmLoreWorkbook(buffer, options = {}) {
...
function buildQcmLoreTemplateWorkbook() {
  const wb = XLSX.utils.book_new();
  const wsScope = XLSX.utils.aoa_to_sheet([SCOPE_TEMPLATE_HEADERS, SCOPE_TEMPLATE_SAMPLE_ROW]);
  const wsCat = XLSX.utils.aoa_to_sheet([CATEGORY_TEMPLATE_HEADERS, CATEGORY_TEMPLATE_SAMPLE_ROW]);
  const wsQ = XLSX.utils.aoa_to_sheet([QUESTION_TEMPLATE_HEADERS, QUESTION_TEMPLATE_SAMPLE_ROW]);
  XLSX.utils.book_append_sheet(wb, wsScope, SCOPES_SHEET);
  XLSX.utils.book_append_sheet(wb, wsCat, CATEGORIES_SHEET);
  XLSX.utils.book_append_sheet(wb, wsQ, QUESTIONS_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}

function buildQcmLoreExportWorkbook({ scopes, categories, questions }) {
  const wb = XLSX.utils.book_new();
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(scopeData), SCOPES_SHEET);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(catData), CATEGORIES_SHEET);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(qData), QUESTIONS_SHEET);
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
}
...
async function loadQcmLoreExportRows(deps, options = {}) {
  ...
  const scopeSlugsInExport = [...new Set(questions.map((q) => String(q.chapitre_slug)).filter(Boolean))];
