...
CREATE TABLE IF NOT EXISTS plants (
  ...
  taxon_rank VARCHAR(16) DEFAULT NULL COMMENT 'Rang : species|genus|family|clade|breed',
  ...
  origin_status ENUM('indigene','introduit','envahissant') DEFAULT NULL COMMENT 'Statut biogéographique pédagogique',
  iucn_status ENUM('EX','EW','CR','EN','VU','NT','LC','DD','NE') DEFAULT NULL COMMENT 'Statut Liste rouge UICN',
...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- migration 267 (roles est créée plus bas dans ce fichier).
  ...
  -- ÉCHAFAUDAGE — restriction de cartes individuelle retirée (migration 267) : seul le
...
CREATE TABLE IF NOT EXISTS `groups` (
  ...
  -- ÉCHAFAUDAGE — drapeau « accorde le statut n3beur » retiré (migration 267) : un groupe
...
CREATE TABLE IF NOT EXISTS group_members (
  ...
  user_id VARCHAR(64) NOT NULL,
  user_type VARCHAR(16) NOT NULL,
  -- ÉCHAFAUDAGE — « responsable / membre » retiré (migration 267) : le périmètre d'un
...
-- ---------------------------------------------------------------------------------------
-- Lien Moodle (migration 219, docs/AUDIT_MOODLE_IDENTITES_2026-09.md section 5).
-- Les tables qui référencent gl_classes / gl_teams (`external_groups`,
-- `external_group_members`, `sync_conflicts`) ne vivent que dans la migration, comme toutes
-- les tables gl_* ; celles-ci n'ont de dépendance que vers `users`.
-- ---------------------------------------------------------------------------------------
...
-- Suivi admin (migration 245) : activité légère + passage identifié multi-produits
