...
test.describe('Gnomes & Licornes — mascottes & équipes (Lot 2C)', () => {
  test('le MJ assigne deux mascottes différentes à deux équipes (refus de collision)', async ({ request }) => {
    const now = Date.now();
    ...
    const gameRow = await queryOne('SELECT id FROM gl_games WHERE name = ? LIMIT 1', [`Partie Mascots ${now}`]);
    ...
    const teamA = await queryOne('SELECT id FROM gl_teams WHERE game_id = ? AND name = ? LIMIT 1', [gameId, 'Equipe Mousse']);
    await execute(
      `INSERT INTO gl_teams (game_id, name, type, color, created_at, updated_at)
       VALUES (?, 'Equipe Aube', 'unicorn', '#fb7185', NOW(), NOW())`,
      [gameId]
    );
    const teamB = await queryOne('SELECT id FROM gl_teams WHERE game_id = ? AND name = ? LIMIT 1', [gameId, 'Equipe Aube']);
