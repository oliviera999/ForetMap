...
test.describe('GL MJ console flow', () => {
  ...
  test('MJ console : éditer une partie et changer de partie met à jour les équipes', async ({ page, request }) => {
    ...
    await request.post(`/api/gl/games/${secondGameId}/teams`, {
      headers: { Authorization: `Bearer ${seeded.adminToken}` },
      data: { name: 'Equipe Beta e2e', type: 'unicorn', color: '#a855f7' },
    ...
    const firstGameRow = page.locator('tr', { hasText: /Partie mj-console-ui/i });
    await firstGameRow.getByRole('button', { name: 'Ouvrir' }).click();
    await expect(page.locator('.gl-active-game-banner-title')).toContainText(/Partie mj-console-ui/i);
    ...
    const secondGameRow = page.locator('tr', { hasText: 'Partie secondaire e2e' });
    await secondGameRow.getByRole('button', { name: 'Ouvrir' }).click();
    await expect(page.locator('.gl-active-game-banner-title')).toHaveText('Partie secondaire e2e', { timeout: 10000 });

    await page.getByRole('button', { name: 'Équipes & effectifs' }).click();
    await expect(page.getByText('Equipe Beta e2e')).toBeVisible();
    await expect(page.getByText('Equipe A')).not.toBeVisible();
  });
});
