...
test.describe('Gnomes & Licornes game flow smoke', () => {
    ...
    try {
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(
          () => reject(new Error('timeout connexion socket joueur')),
          8000,
        );
        socket.once('connect', () => {
      ...
      const payloadPromise = new Promise((resolve, reject) => {
        const timeout = setTimeout(
          () => reject(new Error('timeout réception gl:game:event')),
          8000,
        );
  ...
  test('MJ active tours + narration : joueur reçoit turn_change puis narration', async ({
    request,
  }) => {
    const now = Date.now();
    const baseURL = process.env.E2E_BASE_URL || 'http://127.0.0.1:3000';
    ...
    try {
      ...
      const turnRes = await request.post(`/api/gl/games/${gameId}/turn/next`, {
        headers: adminHeaders,
      });
      ...
      const narrRes = await request.post(`/api/gl/games/${gameId}/events`, {
        ...
        data: {
          eventType: 'narration',
          teamId,
          payload: { text: 'Bienvenue dans la forêt magique !' },
        },
      });
