...
test('connexion élève via identifiant pseudo ou email', async ({ page }) => {
  ...
  await loginByIdentifier(page, profile.pseudo, profile.password, { waitForStudentRegister: true });
  await expect(page.locator('header')).toBeVisible();

  await logoutToAuth(page);
  await loginByIdentifier(page, profile.email, profile.password, { waitForStudentRegister: true });
  await expect(page.locator('header')).toBeVisible();
});
