...
describe('computeCanRequestAction', () => {
  ...
  test('jeu en tours : seulement au tour de son équipe', () => {
    ...
    expect(computeCanRequestAction({ ...base, gameplaySettings: turns, currentTeamId: 8 })).toBe(false);
    expect(computeCanRequestAction({ ...base, gameplaySettings: turns, currentTeamId: 7 })).toBe(true);
    expect(computeCanRequestAction({ ...base, gameplaySettings: turns, currentTeamId: null })).toBe(true);
  });
});
...
describe('computeCanSpellCast', () => {
  ...
  test('vitalité coupée ou partie absente/non live → refusé', () => {
    ...
    expect(computeCanSpellCast({ ...base, gameState: { game: { id: 3, status: 'draft' } } })).toBe(false);
  });
  ...
  test('spellCastMjOnly réserve le lancer au staff', () => {
    ...
    expect(computeCanSpellCast({ ...base, gameplaySettings: mjOnly, showStaffAdminUi: true })).toBe(true);
  });
...
describe('computePlayerVitality', () => {
  ...
  test('repli sur le profil GL, valeurs non numériques → 0', () => {
    expect(computePlayerVitality({ ...base, profile: { health_points: 3, power_points: null } }))
...
describe('findPlayerMascotId', () => {
  ...
  test('retourne la mascotte de l’équipe du joueur', () => {
    expect(findPlayerMascotId({ showsPlayerChrome: true, auth: { teamId: '1' }, teams })).toBe('gnome');
  });
  ...
  test('chrome staff, équipe absente ou sans mascotte → null', () => {
    ...
    expect(findPlayerMascotId({ showsPlayerChrome: true, auth: { teamId: 1 }, teams: null })).toBeNull();
