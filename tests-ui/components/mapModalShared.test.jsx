...
import { LocationTutorialPreviewList, TaskEnrollmentLegend } from '../../src/components/map/mapModalShared.jsx';

describe('LocationTutorialPreviewList', () => {
  test('état vide : message adapté zone / repère', () => {
    const { rerender } = render(<LocationTutorialPreviewList tutorials={[]} locationKind="zone" locationId="z1" />);
  ...
  test('affiche titre + résumé, et exclut le lieu courant des « autres zones »', () => {
    const tutorials = [{
      ...
      zones_linked: [{ id: 'z1', name: 'Verger' }, { id: 'z2', name: 'Potager' }],
      markers_linked: [],
    }];
    render(<LocationTutorialPreviewList tutorials={tutorials} locationKind="zone" locationId="z1" onOpenTutorialPreview={() => {}} />);
