...
import { PlantSummaryBadges, PlantEcosystemHumanLead } from '../../../src/components/biodiv/PlantSummaryBlocks.jsx';

describe('PlantSummaryBadges', () => {
  test('nutrition + température + pH → 3 pastilles', () => {
    render(<PlantSummaryBadges plant={{ nutrition: 'Autotrophe', ideal_temperature_c: '18', optimal_ph: '6.5' }} />);
  ...
  test('entrée « végétal » → préfère preferred_nutrients à nutrition', () => {
    render(<PlantSummaryBadges plant={{ group_1: 'Végétal', nutrition: 'Autotrophe', preferred_nutrients: 'Azote' }} />);
  ...
  test('au plus 3 pastilles', () => {
    const { container } = render(
      <PlantSummaryBadges plant={{ nutrition: 'x', ideal_temperature_c: '1', optimal_ph: '2', preferred_nutrients: 'y' }} />
    );
...
describe('PlantEcosystemHumanLead', () => {
  test('rôle + utilité → les deux blocs', () => {
    render(<PlantEcosystemHumanLead plant={{ ecosystem_role: 'Fixe l’azote', human_utility: 'Comestible' }} />);
  ...
  test('ni rôle ni utilité (ou « - ») → ne rend rien', () => {
    const { container } = render(<PlantEcosystemHumanLead plant={{ ecosystem_role: '-', human_utility: '' }} />);
    expect(container).toBeEmptyDOMElement();
  });
});
