...
import { BiodivLocationMapBlock, PlantLocationPreviewMaps } from '../../../src/components/biodiv/BiodivLocationMaps.jsx';
...
describe('BiodivLocationMapBlock', () => {
  test('aucune zone/repère traçable → ne rend rien', () => {
    const { container } = render(<BiodivLocationMapBlock mapId="foret" maps={MAPS} zones={[]} markers={[]} />);
    expect(container).toBeEmptyDOMElement();
  });
  test('repère valide → rend le label de carte + un point SVG', () => {
    const { container } = render(<BiodivLocationMapBlock mapId="foret" maps={MAPS} zones={[]} markers={[MARKER]} />);
  ...
  test('repère aux coordonnées non finies → ignoré (rien)', () => {
    const { container } = render(
      <BiodivLocationMapBlock mapId="foret" maps={MAPS} zones={[]} markers={[{ id: 2, x_pct: 'x', y_pct: null }]} />
    );
    expect(container).toBeEmptyDOMElement();
  });
