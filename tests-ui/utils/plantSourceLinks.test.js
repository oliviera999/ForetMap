...
describe('isLikelyDirectImageUrl', () => {
  ...
  test('URL http : fichier image direct ou Special:FilePath', () => {
    ...
    expect(
      isLikelyDirectImageUrl('https://commons.wikimedia.org/wiki/Special:FilePath/Foo.jpg'),
    ).toBe(true);
...
describe('parseCommonsFilePageFromUrl', () => {
  test('extrait le titre de fichier d’une page /wiki/File:', () => {
    expect(
      parseCommonsFilePageFromUrl('https://commons.wikimedia.org/wiki/File:Rosa_canina.jpg'),
    ).toBe('Rosa_canina.jpg');
    expect(parseCommonsFilePageFromUrl('https://www.commons.wikimedia.org/wiki/File:A.png')).toBe(
      'A.png',
    );
  });
  test('null hors Commons / hors page fichier / non-URL', () => {
    expect(parseCommonsFilePageFromUrl('https://x.fr/wiki/File:A.png')).toBe(null);
    expect(parseCommonsFilePageFromUrl('https://commons.wikimedia.org/wiki/Category:Foo')).toBe(
      null,
    );
...
describe('parseCommonsCategoryFromUrl', () => {
  test('extrait + décode le titre de catégorie', () => {
    expect(
      parseCommonsCategoryFromUrl('https://commons.wikimedia.org/wiki/Category:Rosa%20canina'),
    ).toBe('Category:Rosa canina');
