...
describe('formatAssigneeName', () => {
  ...
  test('identité visible : prénom + nom', () => {
    expect(
      formatAssigneeName({ student_first_name: 'Tom', student_last_name: 'Roy' }, student),
  ...
  test('détecte l’élève courant (par id)', () => {
    const r = formatAssigneeName(
      { student_id: '7', student_first_name: 'Léa', student_last_name: 'Martin' },
      student,
    );
    expect(r.isCurrentStudent).toBe(true);
  });
  test('anonymisé : "Toi" pour soi, "Participant" sinon', () => {
    expect(formatAssigneeName({ student_id: '7' }, student, false)).toEqual({
      fullName: 'Toi',
      isCurrentStudent: true,
    });
    expect(
      formatAssigneeName({ student_first_name: 'Tom', student_last_name: 'Roy' }, student, false),
...
describe('teacherCollectiveAssigneeLoadKey', () => {
  test('priorité id, puis student_id, puis nom, puis legacy', () => {
    ...
    expect(teacherCollectiveAssigneeLoadKey('5', { student_id: '3' })).toBe(
      '5_teacher_collective_done_sid:3',
    );
    expect(
      teacherCollectiveAssigneeLoadKey('5', {
