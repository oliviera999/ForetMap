...
import { isVegetalCatalogEntry, groupPlantLocationsByMap } from '../../src/utils/plantCatalogHelpers.js';
...
describe('groupPlantLocationsByMap', () => {
  test('groupe zones et repères par map_id', () => {
    const zones = [{ id: 1, map_id: 'a' }, { id: 2, map_id: 'b' }];
    const markers = [{ id: 9, map_id: 'a' }];
    const out = groupPlantLocationsByMap(zones, markers);
    expect(out.get('a')).toEqual({ zones: [{ id: 1, map_id: 'a' }], markers: [{ id: 9, map_id: 'a' }] });
    expect(out.get('b')).toEqual({ zones: [{ id: 2, map_id: 'b' }], markers: [] });
  });
  test('map_id absent/vide → repli sur « foret »', () => {
    const out = groupPlantLocationsByMap([{ id: 1 }, { id: 2, map_id: '  ' }], [{ id: 3, map_id: null }]);
