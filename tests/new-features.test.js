...
test('GET /api/visit/content expose map_lead_photo (première photo galerie carte)', async () => {
  ...
  assert.ok(String(vz.map_lead_photo.image_url).includes(`/uploads/zones/${zone.body.id}/`));
  ...
  assert.ok(String(vz.map_extra_photos[0].image_url).includes(`/uploads/zones/${zone.body.id}/`));
  ...
  assert.ok(String(vm.map_lead_photo.image_url).includes(`/uploads/markers/${marker.body.id}/`));
