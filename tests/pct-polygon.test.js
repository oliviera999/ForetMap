...
describe('pctPolygon', () => {
  it('normalise et borne les points %', async () => {
    const { normalizePctPoint, normalizePctPoints } = await import(pathToFileURL(join(__dirname, '../src/shared/pct-map/pctPolygon.js')).href);
  ...
  it('insère un sommet sur l’arête la plus proche', async () => {
    const { findNearestEdgeInsertion, insertPctPointAt } = await import(pathToFileURL(join(__dirname, '../src/shared/pct-map/pctPolygon.js')).href);
    const square = [
      { x: 0, y: 0 },
      { x: 10, y: 0 },
      { x: 10, y: 10 },
  ...
  it('refuse de retirer un sommet si le polygone aurait moins de 3 points', async () => {
    const { removePctPointAt } = await import(pathToFileURL(join(__dirname, '../src/shared/pct-map/pctPolygon.js')).href);
    const tri = [{ x: 0, y: 0 }, { x: 50, y: 0 }, { x: 25, y: 50 }];
    const next = removePctPointAt(tri, 1);
    assert.equal(next.length, 3);
  });

  it('translate un polygone', async () => {
    const { translatePctPoints } = await import(pathToFileURL(join(__dirname, '../src/shared/pct-map/pctPolygon.js')).href);
