...
async function createStudent(label) {
  ...
  await execute(
    "UPDATE users SET assigned_role_id = (SELECT id FROM roles WHERE slug = 'visiteur') WHERE id = ?",
    [id],
  );
  return id;
}
...
async function createN3beurGroup(label) {
  ...
  await execute(
    `INSERT INTO \`groups\` (id, slug, name, kind, default_role_id, is_active, created_at, updated_at)
     VALUES (?, ?, ?, 'class', (SELECT id FROM roles WHERE slug = 'eleve_novice' LIMIT 1), 1, NOW(), NOW())`,
    [id, slug, slug],
  );
  return id;
}
...
test('rattachement à un groupe n3beur : le profil par défaut du groupe est conféré, le palier mérité vient au recalcul', async () => {
  ...
  // Le rattachement ne fait que conférer le profil par défaut (« le plus élevé l'emporte »).
  ...
  // 12 tâches validées ≥ seuil chevronné (10) : le profil attribué est relevé.
...
test('rattachement : un palier attribué à la main au-dessus du compteur n’est pas perdu', async () => {
  ...
  await execute('UPDATE users SET assigned_role_id = ? WHERE id = ?', [chevronne.id, studentId]);
  await recomputeUserRole(studentId);
...
test('profil hors échelle n3beur jamais modifié par le recalcul', async () => {
  ...
  await execute('UPDATE users SET assigned_role_id = ? WHERE id = ?', [prof.id, studentId]);
  ...
  assert.strictEqual(result.changed, 0);
  // Profil effectif hors échelle : le compte n'est même plus candidat au recalcul.
  assert.strictEqual(result.results[0].reason, 'not_n3beur_member');
