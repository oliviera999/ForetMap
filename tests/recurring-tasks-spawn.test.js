...
async function getAdminAuthToken() {
  ...
  const requiredPermissions = ['tasks.manage', 'tasks.validate', 'teacher.access'];
...
test('Job recurrence : clone sans assignations et idempotence', async () => {
  ...
  assert.ok(
    sourceAfterValidate?.recurrence_template_zone_ids,
    'snapshot zones attendu après validation récurrente',
  );
  const templateZones = JSON.parse(sourceAfterValidate.recurrence_template_zone_ids);
  assert.ok(
    Array.isArray(templateZones) && templateZones.includes(zoneId),
    'zoneId dans le snapshot template',
  );
  ...
  assert.strictEqual(
    children1.length,
    1,
    `attendu 1 enfant pour cette tache, cree au total: ${r1.created.length}`,
  );
  ...
  const childZones = await queryAll(
...
test('Réglage global récurrence : désactive auto, force conserve le rattrapage manuel', async () => {
  ...
  try {
    ...
    await setSetting('tasks.recurring_automation_enabled', false, {
      userType: 'teacher',
      userId: 'test-recurring',
    });
    ...
    const childrenNoForce = await queryAll('SELECT id FROM tasks WHERE parent_task_id = ?', [
      taskIdNoForce,
    ]);
    ...
    const childrenForce = await queryAll('SELECT id FROM tasks WHERE parent_task_id = ?', [
      taskIdForce,
    ]);
    assert.strictEqual(childrenForce.length, 1);
    const childRow = await queryOne(
      'SELECT id, map_id, zone_id FROM tasks WHERE parent_task_id = ? LIMIT 1',
      [taskIdForce],
    );
    ...
    const cz = await queryAll('SELECT zone_id FROM task_zones WHERE task_id = ? ORDER BY zone_id', [
      childRow.id,
    ]);
    assert.deepStrictEqual(
      cz.map((r) => r.zone_id),
    ...
    await setSetting('tasks.recurring_automation_enabled', true, {
      userType: 'teacher',
      userId: 'test-recurring',
    });
