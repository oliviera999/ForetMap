...
test('glLegacyMediaUrl — détection et alias plateau / scène', async () => {
  ...
  const legacyPlateau =
    '/uploads/media-library/image/gl-plateau-1-tropiques-africains.jpg';
  assert.strictEqual(isLegacyGlMediaUrl(legacyPlateau), true);
  assert.strictEqual(
    resolveLegacyGlStableKey(legacyPlateau),
    'plateau-1_tropiques-africains',
  );

  const legacyScene = '/uploads/media-library/image/gl-scene-ch2-epreuve-grise.png';
  assert.strictEqual(
    resolveLegacyGlStableKey(legacyScene),
    'recit_02-chap2_la-grise-mediterranee',
  );
  ...
  assert.strictEqual(
    migrateStoryHeroToSceneRef(hero),
    '![Couverture](scene:1)\nTexte',
  );
});
