...
test('POST /api/quiz/questions/:code/answer refuse un jeton déjà utilisé', async () => {
  const draw = await request(app)
    .get('/api/quiz/draw?categorieSlug=vivant_classification&niveau=college')
    .expect(200);
  const code = draw.body.question_code;
...
test('POST /api/quiz/questions/:code/answer bloque le brute-force sur un seul jeton', async () => {
  const draw = await request(app)
    .get('/api/quiz/draw?categorieSlug=vivant_classification&niveau=college')
    .expect(200);
  const code = draw.body.question_code;
