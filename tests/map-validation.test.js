...
// O7 — vérifie SANS DB que les schémas zod de `routes/map.js` reproduisent exactement les
// gardes manuelles d'origine :
// - `PUT /markers/:id/photos/reorder` :
//     `const raw = req.body?.photo_ids ?? req.body?.ordered_ids; if (!Array.isArray(raw)) -> 400`
//     'Liste photo_ids (ou ordered_ids) requise' ;
// - `POST /markers/:id/photos` : `const { image_data } = req.body; if (!image_data) -> 400 'Image requise'`.
