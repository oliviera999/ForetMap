...
function runQuery(rawQuery) {
  ...
  validate({ query: statsScopeQuerySchema })(req, res, () => { nextCalled = true; });
...
test('group_id/subgroup_id : équivalence exacte avec String(x || \'\').trim(), jamais de 400', () => {
  for (const raw of EDGE_VALUES) {
    const query = {};
    if (raw !== undefined) { query.group_id = raw; query.subgroup_id = raw; }
    const { nextCalled, status, parsed } = runQuery(query);
    assert.strictEqual(nextCalled, true, `group_id=${JSON.stringify(raw)} ne doit jamais être rejeté`);
    assert.strictEqual(status, 200);
    const expected = legacy(query);
    assert.strictEqual(parsed.groupId, expected.groupId, `groupId pour ${JSON.stringify(raw)}`);
    assert.strictEqual(parsed.subgroupId, expected.subgroupId, `subgroupId pour ${JSON.stringify(raw)}`);
  }
});
...
test('map_id/project_id : transmis tels quels ou null si falsy, jamais de 400', () => {
  for (const raw of [undefined, '', 'm-1', '0', ' x ']) {
    const query = {};
    if (raw !== undefined) { query.map_id = raw; query.project_id = raw; }
    const { nextCalled, parsed } = runQuery(query);
    assert.strictEqual(nextCalled, true, `map_id=${JSON.stringify(raw)} ne doit jamais être rejeté`);
    const expected = legacy(query);
    assert.strictEqual(parsed.mapId, expected.mapId, `mapId pour ${JSON.stringify(raw)}`);
    assert.strictEqual(parsed.projectId, expected.projectId, `projectId pour ${JSON.stringify(raw)}`);
  }
});
