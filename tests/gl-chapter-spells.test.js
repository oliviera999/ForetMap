...
test('GET /api/gl/games/:id expose chapter_spells', async () => {
  ...
  const cls = await queryOne('SELECT id FROM gl_classes WHERE name = ? LIMIT 1', [`Classe Chapter Spells ${stamp}`]);
  ...
  const game = await queryOne('SELECT id FROM gl_games WHERE name = ? LIMIT 1', [`Partie spells ${stamp}`]);
