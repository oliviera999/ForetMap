...
if (String(process.env.FORETMAP_SNAPSHOT_GL || '').trim() !== '1') {
  test('snapshot GL: désactivé', { skip: 'Définir FORETMAP_SNAPSHOT_GL=1 pour activer ce test.' }, () => {});
} else {
  ...
  test('snapshot GL: permissions gl.* bootstrapées', async () => {
    const rows = await queryAll("SELECT `key` FROM permissions WHERE `key` LIKE 'gl.%' ORDER BY `key` ASC");
