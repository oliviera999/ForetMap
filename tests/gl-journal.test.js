...
test('presentJournalEvent mode joueur sans codes techniques', () => {
  const pres = presentJournalEvent(
    {
      eventType: 'action_request',
      payload: { actionType: 'explore' },
      actorType: 'team',
      actorId: '3',
      teamId: 2,
    },
    { teamsById: { 2: { id: 2, name: 'Les Gnomes' } }, forPlayer: true }
  );
  assert.strictEqual(pres.title, 'Demande des joueurs');
  assert.ok(pres.body.includes('explorer le plateau'));
  assert.ok(pres.technical == null);
  assert.ok(pres.body.includes('Les Gnomes'));
});
