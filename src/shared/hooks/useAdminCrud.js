...
/**
 * Squelette CRUD partagé des panneaux d'édition admin (ForetMap **et** GL) :
 * chargement de liste, ouverture d'une fiche, préparation d'une nouvelle fiche
 * via « next-code », persistance (POST/PUT) avec enregistrement automatique
 * débouncé, et états loading / error / info. Le rendu des champs et les filtres
 * restent dans chaque panneau.
 *
 * Extrait de `src/gl/hooks/useGlAdminCrud.js` (éprouvé sur les panneaux GL
 * espèces / sorts / glossaire). La **seule** spécificité produit était le
 * transport HTTP : elle est désormais injectée via `request`, ce qui rend le
 * hook utilisable côté ForetMap (`api`) comme côté GL (`apiGL`). Aucune autre
 ...
 * @param {(path: string, method?: string, body?: object) => Promise<object>} options.request — transport HTTP du produit (`api` / `apiGL`)
...
export function useAdminCrud({
  ...
  async function loadItem(code) {
    ...
    setError('');
    try {
      const data = await request(itemPath(code));
