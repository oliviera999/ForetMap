...
export function MapRouteBar({
  ...
  return (
    ...
    >
      {' '}
      <div className="map-route-bar__head">
        <div className="map-route-bar__titles">
          <p className="map-route-bar__route">{route.title}</p>
          {entry ? (
            <h2 className="map-route-bar__step">
              <span className="map-route__step-number" aria-hidden>
                {entry.number}
              </span>
              <span>{title}</span>
            </h2>
          ) : (
            <p className="map-route__empty">
              {route.description || 'Ce parcours n’a pas encore d’étape affichable.'}
            </p>
          )}
        </div>
        <button type="button" className="map-route-bar__quit" onClick={onExit}>
          Quitter
        </button>
      </div>
      {stepText ? (
        <p className={`map-route-bar__text${textExpanded ? ' is-expanded' : ''}`}>
          {longText && !textExpanded ? `${stepText.slice(0, 110).trim()}…` : stepText}
          {longText ? (
            <button
              type="button"
              className="map-route-bar__more"
      ...
      {distanceLabel ? (
        <p className="map-route-bar__hint">À {distanceLabel} à vol d’oiseau.</p>
      ) : (
        <p className="map-route-bar__hint">{canLocate ? hintLocate : hintManual}</p>
      )}
      <div className="map-route__actions">
        <Button variant="secondary" disabled={index <= 0} onClick={() => onGoToIndex(index - 1)}>
          Précédent
        </Button>
        <span className="map-route__counter" aria-live="polite">
