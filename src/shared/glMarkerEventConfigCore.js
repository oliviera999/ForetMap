...
const DEFAULT_QUESTION_CONFIG = Object.freeze({ mode: 'random', fixedQuestionCode: null, pool: { ...DEFAULT_QUESTION_POOL } });
...
function normalizeQuestionConfig(input) {
  ...
  return { mode, fixedQuestionCode: normalizeQuestionCode(src.fixedQuestionCode), pool: normalizeQuestionPool(src.pool) };
}
...
function defaultEventConfigForQuestion() {
  return { version: 2, question: { mode: 'random', fixedQuestionCode: null, pool: { ...DEFAULT_QUESTION_POOL } } };
}
...
function migrateLegacyMarkerQcmConfig(marker) {
  ...
  return fixedCode
    ? { version: 2, question: { mode: 'fixed', fixedQuestionCode: fixedCode, pool } }
    : { version: 2, question: { mode: 'random', fixedQuestionCode: null, pool } };
}
...
export {
  ...
  DEFAULT_QUESTION_POOL, DEFAULT_QUESTION_CONFIG, normalizeStringList, normalizeQuestionCode,
  normalizeQuestionPool, normalizeQuestionConfig, normalizeEffectDelta, normalizeEffectBranch,
  ...
  normalizeMarkerQuestionRetrigger, resolveBiomeSlugsForPool,
};
