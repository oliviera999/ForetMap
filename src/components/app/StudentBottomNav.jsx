  ...
  return (
    <nav className="bottom-nav">
      {canAccessStudentMapTasks && shouldUseDesktopSplit && (
        <button className={`nav-btn ${tab === 'maptasks' ? 'active' : ''}`} onClick={() => onTabChange('maptasks')}>
      ...
      {canAccessStudentMapTasks && (
        <button className={`nav-btn ${tab === 'map' ? 'active' : ''}`} onClick={() => onTabChange('map')}>
          <span className="nav-icon">🗺️</span> Carte
        </button>
      )}
      {canAccessStudentMapTasks && (
        mergeTasksTutoNav ? (
          <button className={`nav-btn ${tab === 'tasks' || tab === 'tuto' ? 'active' : ''}`} type="button" onClick={() => onTabChange('tasks')}>
            <span className="nav-icon">✅</span>
            Tâches&tuto{studentActiveAssignedTasksCount > 0 && ` (${studentActiveAssignedTasksCount})`}
          </button>
        ) : (
          <button className={`nav-btn ${tab === 'tasks' ? 'active' : ''}`} type="button" onClick={() => onTabChange('tasks')}>
            <span className="nav-icon">✅</span>
            {tutorialsModuleEnabled ? 'Tâches · tuto' : 'Tâches'}{studentActiveAssignedTasksCount > 0 && ` (${studentActiveAssignedTasksCount})`}
          </button>
        )
      )}
      <button className={`nav-btn ${tab === 'plants' ? 'active' : ''}`} onClick={() => onTabChange('plants')}>
        <span className="nav-icon">🌱</span> Biodiversité
      </button>
      {tutorialsModuleEnabled && !mergeTasksTutoNav && (
        <button className={`nav-btn ${tab === 'tuto' ? 'active' : ''}`} type="button" onClick={() => onTabChange('tuto')}>
          <span className="nav-icon">📘</span> Tuto
        </button>
      )}
      {canViewGeneralStats && (
        <button className={`nav-btn ${tab === 'stats' ? 'active' : ''}`} onClick={() => onTabChange('stats')}>
          <span className="nav-icon">📊</span> Stats
        </button>
      )}
      {observationsEnabled && (
        <button className={`nav-btn ${tab === 'notebook' ? 'active' : ''}`} onClick={() => onTabChange('notebook')}>
          <span className="nav-icon">📓</span> Carnet
        </button>
      )}
      {visitEnabled && (
        <button className={`nav-btn ${tab === 'visit' ? 'active' : ''}`} onClick={() => onTabChange('visit')}>
          <span className="nav-icon">🧭</span> Visite
        </button>
      )}
      {canAccessForum && (
        <button className={`nav-btn ${tab === 'forum' ? 'active' : ''}`} onClick={() => onTabChange('forum')}>
          <span className="nav-icon">💬</span> Forum
        </button>
      )}
      <button className={`nav-btn ${tab === 'about' ? 'active' : ''}`} onClick={() => onTabChange('about')}>
