...
import {
  ...
  resolveSettingLabel as resolveSettingLabelPure,
  scopeLabel,
} from '../utils/settingsAdmin.js';
...
function SettingsAdminView() {
  ...
  const resolveSettingLabel = (key) => resolveSettingLabelPure(key, roleTerms);

  const settingSections = useMemo(() => buildSettingSections(settings), [settings]);

  const filteredSettingSections = useMemo(
    () => filterSettingSections(settingSections, searchQuery, resolveSettingLabel),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [searchQuery, settingSections, roleTerms],
  );

  const filteredCount = useMemo(() => countSettingRows(filteredSettingSections), [filteredSettingSections]);
