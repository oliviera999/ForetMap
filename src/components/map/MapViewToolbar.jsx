      ...
      <div className="map-view-toolbar" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px',
        ...
        <div style={{ display: 'flex', gap: 3, background: 'var(--parchment)', borderRadius: 10, padding: 3 }}>
          {[['view', '🖐️ Nav'],
            ...(isTeacher && mode !== 'edit-points' ? [
              ['draw-zone', `🖊️ Zone${mode === 'draw-zone' && drawPointsCount > 0 ? ` (${drawPointsCount})` : ''}`],
              ['add-marker', '📍 Repère'],
            ] : [])
          ].map(([m, label]) => (
            <button key={m}
        ...
        {isTeacher && mode === 'draw-zone' && drawPointsCount > 0 && (
          <div style={{ display: 'flex', gap: 4 }}>
            {drawPointsCount >= 3 && <button className="btn btn-secondary btn-sm" onClick={onFinishZone}>✅ Terminer</button>}
            <button className="btn btn-ghost btn-sm" onClick={onUndoPoint}>↩ Undo</button>
            <button className="btn btn-danger btn-sm" onClick={onCancelDraw}>✕</button>
          </div>
        )}
        {mode === 'edit-points' && (
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <button type="button" className="btn btn-ghost btn-sm" disabled={!editCanUndo} onClick={onUndoEditPoints} title="Annuler la dernière modification (Ctrl+Z ou Cmd+Z)">↩ Annuler</button>
            <button className="btn btn-primary btn-sm" onClick={onSaveEditPoints}>💾 Sauver</button>
            <button className="btn btn-ghost btn-sm" onClick={onExitEditPoints}>✕</button>
          </div>
        )}
            ...
            {[
              ...
              <Tooltip key={label} text={tooltipText(helpEntry)}>
                <button onClick={() => {
                  ...
                  const ns = factor > 1 ? Math.min(txRef.current.s * factor, 6) : Math.max(txRef.current.s * factor, 0.15);
