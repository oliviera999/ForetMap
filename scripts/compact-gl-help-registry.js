#!/usr/bin/env node
/**
 * Compactage des bulles d'aide GL (`content.help`, table `gl_settings`).
 *
 * Pendant GL de `scripts/compact-help-registry.js`. Historiquement, enregistrer les
 * bulles d'aide écrivait en base l'objet **complet** : la ligne figeait alors tout le
 * corpus, et améliorer un texte dans `data/gl/help.default.json` n'avait plus aucun effet
 * à l'écran. Depuis le dégel (§11.2 de docs/MASCOT_NARRATEUR_OLU.md), l'écriture ne stocke
 * plus que la **surcharge** — mais une instance déjà en service conserve sa ligne dense
 * tant que personne n'enregistre.
 *
 * Ce script réécrit cette ligne en surcharge, à contenu affiché **identique** : il ne fait
 * que retirer ce qui est déjà égal aux défauts versionnés.
 *
 * Par défaut, dry-run (aucune écriture).
 *
 * Usage:
 *   node scripts/compact-gl-help-registry.js
 *   node scripts/compact-gl-help-registry.js --apply
 *   node scripts/compact-gl-help-registry.js --json
 */
...
async function main() {
  ...
  const before = normalizeGlHelpConfig(stored);
  const override = buildGlHelpOverride(stored);
  const after = normalizeGlHelpConfig(override);
  ...
  if (flags.apply) {
    await execute('UPDATE gl_settings SET value_json = ?, updated_at = NOW() WHERE `key` = ?', [
