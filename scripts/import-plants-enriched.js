...
function parseInsertRows(insertSql) {
  ...
  while (i < insertSql.length) {
    ...
    while (i < insertSql.length) {
      while (i < insertSql.length && /\s/.test(insertSql[i])) i++;
      if (insertSql[i] === ')') { i++; break; }
      if (insertSql[i] === ',') { i++; continue; }
      if (insertSql[i] === "'") {
        i++;
        let s = '';
        while (i < insertSql.length) {
          if (insertSql[i] === '\\') { s += insertSql[i + 1]; i += 2; continue; }
          if (insertSql[i] === "'") {
            if (insertSql[i + 1] === "'") { s += "'"; i += 2; continue; }
            i++; break;
          }
          s += insertSql[i++];
        }
      ...
      if (insertSql.substring(i, i + 4) === 'NULL') { row.push(null); i += 4; continue; }
      let n = '';
      while (i < insertSql.length && insertSql[i] !== ',' && insertSql[i] !== ')') n += insertSql[i++];
      const t = n.trim();
      if (t === '') row.push(null);
...
const ENRICH_IDX = {
  taxon_kingdom: 6, taxon_group: 7, taxon_family: 8, taxon_genus: 9, gbif_key: 10,
  habitat_type: 15, trophic_role: 19, is_ornamental: 20, life_cycle: 22,
  temp_min_c: 30, temp_max_c: 31, ph_min: 33, ph_max: 34, is_edible: 38,
};
...
async function run() {
  ...
  let updated = 0;
  try {
    const setClause = Object.keys(ENRICH_IDX).map((c) => c + '=?').join(', ');
    ...
    const [[{ c }]] = await conn.query('SELECT COUNT(*) c FROM plants WHERE taxon_kingdom IS NOT NULL');
