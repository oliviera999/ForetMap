...
| Marque affichée (nom logiciel / établissement)     | `lib/brand.js` (source unique, 6 variables `FORETMAP_BRAND_*`) ; consommée par `lib/products.js`, `lib/settings.js` et le plugin Vite `foretmap-brand-html` ; front : `src/shared/brand/brandNames.js`. **Ne jamais renommer les identifiants techniques homonymes** (id produit, `X-Foretmap-Product`, `/api/gl`, `FORETMAP_*`). Guide : `docs/reference/exploitation/marque-et-domaines.md` |
| Produits (host → entrée HTML, favicon, PWA)        | `lib/products.js` (registre `foret` / `gl` / `plan`), `lib/productResolver.js`, `lib/spaFallback.js`, `lib/pwaRoutes.js`, `scripts/build-pwa.js` ; garde d'accès `lib/accessGate.js`, journal `lib/auditLog.js`, compteur `lib/usage.js`                                                                                                                                                      |
| Utilitaires backend                                | `lib/` (`logger.js`, `routeLog.js`, `requestId.js`, `env.js`, `uploads.js`, `speciesAutofill*.js`, `glSettings.js`, `csp*.js`) ; helpers métier par domaine (`lib/tasks/`, `lib/*RouteHelpers.js`)                                                                                                                                                                                            |
...
| Documentation                                      | `docs/` — `API.md`, `EVOLUTION.md`, `LOCAL_DEV.md`, `EXPLOITATION.md`, `VERSIONING.md`, `DEPLOY_DIST_ARTIFACT.md`, `GL_*.md` ; **index des audits datés : `docs/audits/README.md`** ; stabilité / charge : `AUDIT_STABILITE_PERF_2026-09.md` (consolidé)                                                                                                                                      |
...
- **Licence** : dépôt **propriétaire** (`LICENSE`, tous droits réservés) ; `package.json` porte
  `"license": "UNLICENSED"` et `"private": true`. Toute dépendance ajoutée doit rester
  compatible avec une distribution propriétaire (cf. `.cursor/rules/foretmap-external-inspiration.mdc`).
- **Marque jamais en dur** : un nom de logiciel ou d'établissement affiché passe par
  `lib/brand.js` (serveur, build) ou `src/shared/brand/brandNames.js` (front). Écrire
  « ForêtMap » ou « Lycée Lyautey » dans une chaîne visible est une régression.
