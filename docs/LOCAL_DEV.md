...
**Pack mascotte `sprite_cut` (dev)** : format décrit dans **`docs/MASCOT_PACK.md`** ; validation **`npm run mascot:pack:validate -- docs/mascot-pack.example.json`** ; avec **`npm run dev`**, page outil **`/mascot-pack-tool.html`** (édition JSON, prévisualisation, export). Le build Vite produit aussi **`dist/mascot-pack-tool.html`**.
