...
**Pack mascotte `sprite_cut` (dev)** : format décrit dans **`docs/MASCOT_PACK.md`** ; validation **`npm run mascot:pack:validate -- docs/mascot-pack.example.json`** ; avec **`npm run dev:client`** (ou stack dev qui sert le bundle Vite), page autonome **`/mascot-pack-tool.html`** (édition JSON, prévisualisation, export). **Depuis l’onglet Visite** (compte prof, mode dev Vite uniquement) : bouton **« Boîte à outils pack mascotte (dev) »** sous l’aperçu mascotte — ouvre la même UI en modale. Le build produit aussi **`dist/mascot-pack-tool.html`**.
