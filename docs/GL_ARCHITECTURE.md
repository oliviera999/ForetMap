...
| Orchestration  | `lib/glTeamComposition.js`                                                                                                                                                                                                                     | `buildCompositionProposal` (chargement partie/classe/joueurs/historique, recette explicite ou déduite de `gl_classes.team_policy` — migration 217 —, verrous, épingles `pins`, rotation des peuples `buildPeopleStreaks`/`choosePeopleStart`, plancher de vitalité, avertissements, `explain` factuel, brassage avant/après), `applyComposition` (transaction : remplacement optionnel, INSERT équipes + membres, événement unique), `getClassMixingRate` |
