...
| Identité / groupes    | `lib/glGroupBridge.js`, `lib/glPlayerIdentity.js`, `lib/glIdentityReconcile.js`, `groups` + `group_members`, `gl_classes.foretmap_group_id` | **Identités unifiées** (migration 211) : chaque joueur GL a un compte `users` lié (FK `fk_gl_players_user`), qui porte mot de passe, e-mail, `google_sub`, `password_must_reset`, `token_epoch` ; `gl_players` ne garde que le gameplay. Classe GL ↔ groupe ForetMap miroir. Rapport / rattrapage : `GET | POST /api/gl/admin/players/reconcile` |
...
- Auth : `routes/gl/auth.js` (écran unique : `POST /login` avec `identifier`+mot de passe — joueur (pseudo de jeu, ou e-mail / pseudo du compte `users` lié ; mot de passe = celui du compte lié, `lib/glPlayerIdentity.js`, hash hérité adopté à sa première utilisation) puis MJ/Admin ForetMap ; OAuth Google `google/start` mode `auto` par défaut ; alias `staff/login` MJ-only (sous limiteur strict) ; joueur Google : `users.email` / `users.google_sub` du compte lié ; admins ForetMap RBAC `admin` → synchro auto `gl_admins` ; profil self-service `PATCH /api/gl/auth/me/profile` (e-mail écrit sur `users`), rattachement d'un compte élève `POST/DELETE /api/gl/auth/link-foretmap` (fusion / scission), changement mot de passe `POST /api/gl/auth/change-password` et staff `POST /api/gl/auth/staff/change-password` — tous deux incrémentent `users.token_epoch` (révocation) et renvoient un jeton neuf ; verrou par compte `lib/loginThrottle.js`)
...
| Profils (v2)   | `lib/gl/teamProfileAxes.js`                                                                                                                                                                                                                    | Six axes (savoir, exploration, échange, générosité, initiative, assiduité) : `loadProfileSignals` (une requête par table source existante) puis `normalizeAxes` pur — scores 0–1 relatifs à la classe, shrinkage k = 10, rôle dominant. Calcul à la volée, jamais persisté ni renvoyé ; recettes `mixed` / `roles` / `homogeneous`, réglage `gameplay.team_composition_profile_recipes_enabled`                                                                                               |
| Historique     | `lib/glTeamCompositionHistory.js`                                                                                                                                                                                                              | Paires de coéquipiers des parties passées de la classe (`gl_team_members ⋈ gl_games ⋈ gl_teams`, pondération `0.8^rang`, 12 parties max), taux de brassage (`computeMixingRate`)                                                                                                                                                                                                                                                                                                              |
| Verrous (v3)   | `lib/glClassPairingLocks.js`                                                                                                                                                                                                                   | Table `gl_class_pairing_locks` (migration 218) : paires ordonnées `together` / `apart` par classe, CRUD (`listPairingLocks`, `upsertPairingLock` idempotent + remplacement de type, `deletePairingLock` refus cross-classe), `loadLocksForEngine` (verrous hors pool ignorés et comptés). Staff uniquement                                                                                                                                                                                    |
| Orchestration  | `lib/glTeamComposition.js`                                                                                                                                                                                                                     | `buildCompositionProposal` (chargement partie/classe/joueurs/historique, recette explicite ou déduite de `gl_classes.team_policy` — migration 217 —, verrous, épingles `pins`, rotation des peuples `buildPeopleStreaks`/`choosePeopleStart`, plancher de vitalité, avertissements, `explain` factuel, brassage avant/après), `applyComposition` (transaction : `SELECT gl_games … FOR UPDATE` puis remplacement optionnel, INSERT équipes + membres, événement unique), `getClassMixingRate` |
| API            | `routes/gl/games/teams.js` — `POST .../teams/compose/preview`, `/apply`, `GET .../compose/mixing-rate` ; `routes/gl/admin.js` — `PUT /admin/classes/:id` (`teamPolicy`, `teamSizeDefault`), `GET/POST/DELETE /admin/classes/:id/pairing-locks` | `gl.team.manage` + `gl.players.manage`, partie `draft` uniquement ; codes détaillés dans `docs/API.md`                                                                                                                                                                                                                                                                                                                                                                                        |
| Journal        | `lib/glJournalPresent.js`                                                                                                                                                                                                                      | Événement `teams_composed` (« Le MJ a composé N équipes (recette …) »), sans score individuel                                                                                                                                                                                                                                                                                                                                                                                                 |
| Console MJ     | `src/gl/components/mj/GLTeamComposeDialog.jsx`, `GLTeamMixingRate.jsx`, `GLGameMasterConsoleTeams` ; `src/gl/components/admin/GLClassesPanel.jsx`                                                                                              | Bouton « Composer automatiquement » (désactivé hors `draft`), aperçu retouchable (sélecteur « déplacer vers… » par joueur = épingle, bouton épingle, renommage), section repliable « Contraintes de la classe » (verrous CRUD, épingles), peuple de la première équipe, indicateur de brassage en tête de l'onglet Équipes ; **Simuler / Pousser le miroir Moodle** ; politique + taille par défaut dans la fiche classe (admin)                                                              |
| Gabarits       | `lib/gl/teamTemplates.js`, réglage `gl.classes.team_templates`                                                                                                                                                                                 | Noms d’équipes par classe amorcés à la création de partie ; type gnome/licorne déduit du premier mot du nom                                                                                                                                                                                                                                                                                                                                                                                   |
| Miroirs Moodle | `lib/moodle/teamsMirror.js` ; `POST /api/gl/games/:id/teams/mirror` ; option `teams` de `POST /api/admin/integrations/moodle/runs` ; `POST …/moodle/mirrors` (sous-groupe ForetMap)                                                            | Groupes de cours `FM#…` uniquement (I-7) ; collision de nom hors `FM#` → arrêt ; joueur sans identité Moodle listé ; partie hors `draft` : pas de recomposition des affectations G&L. Spec : `docs/AUDIT_MOODLE_IDENTITES_2026-09.md` §10 ; contrat : `docs/API.md`                                                                                                                                                                                                                           |
...
- `GL_PROD_BASE_URL`
- `MOODLE_BASE_URL` / `MOODLE_WS_TOKEN` / `MOODLE_SYNC_ENABLED` (sync d’annuaire + miroirs)
- `LTI_*` + `GL_FRONTEND_ORIGIN` (entrée depuis un cours Moodle)
