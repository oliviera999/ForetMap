...
| O8 | Moyenne | Maint | ~338 try/catch dispersés ; `respondInternalError` redéfini en doublon | **Infra livrée** : `lib/asyncHandler.js` (catch sync+async → `next(err)` → handler central `server.js`) + test `tests/async-handler.test.js`. Rollout **incrémental** par route (préserver statut + corps d'erreur existants). Adopté sur `audit`/`zones`/`map`/`maps`/`observations`/`context-comments`/`stats`, **`media-library`** (les 4 routes : contrat 4xx `.status`/`.message` + 500 « Erreur serveur » reproduit par le handler central), puis **`forum`** (8 handlers migrés ; équivalence **prouvée** : diff after-strip-indentation = lignes wrapper/import uniquement, aucun corps modifié ; gate middleware `next(e)` inchangé), puis **`plants`** (les 4 handlers CRUD simples `GET /`/`POST /`/`PUT /:id`/`DELETE /:id` ; équivalence prouvée par diff after-strip-indentation). **Routes/handlers restants — rollout sélectif** : nombre de handlers passent un **label** à `logRouteError(e, req, 'label')` ou une **recovery custom** (ex. `plants` `autofill`/`plantnet-identify` → 502 spécifique ; `gl/*`) que le handler central ne reproduit pas → laissés en l'état pour préserver l'observabilité/le comportement ; les mappings spéciaux (`errno 1062`→409) se migrent avec le patron `rethrowSlugConflict` (cf. `groups` dans une PR dédiée). À reprendre route par route, jamais en masse mécanique (risque cross-boundary sur catches hétérogènes) | Moyen | wip |
