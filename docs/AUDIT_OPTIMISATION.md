...
| O4 | Haute | Sécu/Maint | `xlsx@0.18.5` — 2 CVE High via uploads | **Part 1 livrée** : adaptateur `lib/spreadsheet.js` (exceljs) + preuve d'équivalence xlsx sur les vrais fichiers ; **app principale migrée** (students/plants/tasks). **Part 2 (reste)** : 11 libs GL `gl*Import.js` (cascade async parse→exécuteurs→routes/tests) puis **retrait d'`xlsx`** pour fermer la CVE | Élevé | wip |
