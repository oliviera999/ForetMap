...
| O10 | Moyenne | Perf | Routes obèses (visit/tasks/games 2000+ l.) ; N+1 d'écriture (boucles INSERT) | Couche service par domaine ; INSERT multi-valeurs. **N+1 d'écriture convertis en INSERT multi-valeurs** (idiome `replaceTaskJoinRows` de `tasks.js`) : `groups.js` (`PUT /:id/members` — membres/managers/scopes carte+projet, + résolution des `user_type` en **1 requête** au lieu d'un SELECT par membre) ; `rbac.js` (duplication de profil — copie des `role_permissions` + résolution du catalogue en **1 requête**) ; `tasks.js` (`POST /:id/assign-group` — affectations en **1 requête**, sémantique `assigned`/`skipped`/créneaux préservée, vérifiée par simulation 8 cas). enfin **import plants** stratégie `replace_all` (`routes/plants.js` — INSERT multi-valeurs **par lots de 200**, 33 colonnes × 2000 lignes max dépasseraient la limite de placeholders d'une requête préparée ; aucune dépendance à `insertId` ; équivalence ordre+compte vérifiée par simulation). Équivalence relue (2 agents, 0 anomalie) ; couverts par `groups.test.js`/`rbac.test.js`/`tasks*`/`plants-import.test.js`. enfin **import students** : l'INSERT users garde sa récupération d'erreur par ligne (non batchable), mais l'**assignation de rôle** (`ensurePrimaryRole` par ligne = N requêtes) passe en **INSERT IGNORE multi-valeurs par lots** après la boucle (role_id résolus en 1 requête ; comptes UUID neufs → équivaut à `ensurePrimaryRole`, vérifié par simulation). Équivalence relue (2 agents, 0 anomalie) ; couverts par `groups.test.js`/`rbac.test.js`/`tasks*`/`plants-import.test.js`/`students-import.test.js` (CI). **Écartés volontairement** (récupération d'erreur **par ligne** `ER_DUP_ENTRY`→report+`continue`, incompatible avec un INSERT tout-ou-rien) : `gl/admin.js` (gl_players), **INSERT users** de students, et branches `upsert_name`/`insert_only` de l'import plants (dépendance à `insertId` pour la dédup intra-lot). **Volet « routes obèses » entamé (2026-06-12)** : 1er incrément prudent sur **`visit.js` 2367→1997 l (−370)** — 29 symboles **purs** (sans I/O ni req/res) déplacés en l'état vers `lib/visitContentHelpers.js` (13 : sérialisation médias/photos, normalisation points/coords/ids, `ratioPct`…) et `lib/visitMascotPackHelpers.js` (16 : templates/sérialisation/classification d'erreurs des packs mascotte) ; byte-identiques à l'original (vérifié par script), 0 référence orpheline, `git diff -w` = suppressions + imports seuls ; 30 tests no-DB `tests/visit-helpers.test.js`. **2e incrément : `tasks.js` 2135→1835 l (−300)** — 38 symboles purs byte-identiques (vérifié par script) vers `lib/taskRouteHelpers.js` (32 : niveaux danger/difficulté/importance, êtres vivants, image, dates/statuts, ids, `enrichTaskRow`…) et `lib/taskAuthzHelpers.js` (6 prédicats d'autorisation purs) ; le bloc helpers pré-handlers passe de ~946 à ~646 l ; `git diff == git diff -w` ; 36 tests no-DB `tests/tasks-helpers.test.js`. Pas d'unification avec `task-projects.js` (dédup → O9). **3e incrément : `tutorials.js` 975→782 l (−193)** — 20 symboles purs byte-identiques vers `lib/tutorialRouteHelpers.js` (validation cover URL, slugify, htmlToPlainText, `htmlToPdfBuffer` avec son import pdfkit, `toPublicTutorialRow`, script iframe… ; au passage `buildLinkedTaskLocationHint` découvert mort) ; 24 tests no-DB ; handlers/try-catch intouchés (exclusion O8 respectée). **`gl/games.js` (2024 l) examiné et écarté** : déjà conforme — sa logique pure vit dans ~25 modules `lib/gl*` importés, seules ~57 l isolables (< seuil 80) dont 22 déjà testées (schémas O7) ; le levier restant serait un découpage en sous-routeurs (autre refactor que le déplacement pur). **4e incrément (lot 7)** : **`rbac.js` 926→854 l** — 14 symboles purs (regex/limites/slugs réservés, normalisations, parsing de plafonds) → `lib/rbacRouteHelpers.js`, byte-identiques, aucun changement de logique sur cette route sécurité (15 tests, dont un documentant `Number(null)===0`) ; **`students.js` 748→529 l** — 25 symboles (constantes d'import, parsing CSV/classeur en mémoire, mapping/validation, `csvEscape`…) → `lib/studentRouteHelpers.js` (20 tests). **5e incrément (lot 8)** : **`plants.js` 844→511 l** — 31 symboles purs (colonnes/limites/alias d'import, parsing de liens et de nombres, validation de plages, helpers photo, parsing CSV/Google Sheet, `buildPlantPayload`/`validateImportPayloadRow`) → `lib/plantsRouteHelpers.js` byte-identiques ; handlers (recovery 502 autofill/plantnet) intouchés ; 26 tests no-DB. **`forum.js` examiné et écarté** : ~15 l de logique pure isolable (< seuil 80) — réactions déjà dans `lib/shared/reactionEmojiCore`, pagination dans `lib/shared/httpHelpers`. **6e incrément (lot 9, routes sécurité, déplacement pur byte-identique conservateur)** : **`auth.js` 1152→1053 l** — 17 symboles purs (regex pseudo, normalisations email/avatar, OAuth `splitDisplayName`/`isGoogleEmailAllowed`/builders de redirect, `validateProfileInput`, `exposeAuth`) → `lib/authRouteHelpers.js` (14 tests) ; tout ce qui touche bcrypt/JWT/cookies/`process.env` laissé dans la route ; **`gl/admin.js` 1059→1002 l** — 9 symboles purs (normalisations pseudo/password/slug, `buildGeneratedPassword`, Sets de réglages autorisés) → `lib/gl/adminRouteHelpers.js` (21 tests, schémas zod du lot 5 ré-exportés inchangés). **7e incrément (lot 10)** : **`gl/auth.js` 1284→1193 l** — 12 symboles purs (CSV→Set, perms de rôle GL, `exposeGlAuth`, helpers OAuth, prédicats d'impersonation staff) → `lib/gl/authRouteHelpers.js` byte-identiques, bcrypt/JWT/cookies laissés (17 tests) ; **`gl/lore.js` 903→857 l** — 7 symboles purs (`parseId`, normalisations code/slug, parseur CSV, `LORE_QUESTION_SELECT`, enrichisseur glossaire) → `lib/gl/loreRouteHelpers.js`, schémas zod intacts (16 tests). **8e incrément (lot 11)** : **`gl/chapters.js` 718→669 l** — 9 symboles purs (`normalizeSlug`, `clampPercent`, `parsePlateauNumber`, parsing de frame d'image, attacheurs thème/biomes/sorts) → `lib/gl/chaptersRouteHelpers.js` byte-identiques (22 tests) ; `readChapterFull` (DB) laissé. **2e passage `visit.js` (lot 12) : aucune extraction** — après le 1er passage (29 symboles), les ~22 symboles top-level restants sont tous à I/O (DB/fs/`process.env`) ou lisent l'env dans leur corps (signature de cookies) ; logique pure isolable < 60 l et non conforme au déplacement byte-identique → pas d'extraction forcée. **Bilan O10 routes obèses** : 12 routes allégées par déplacement pur testé (visit, tasks, tutorials, rbac, students, plants, auth, gl/admin, gl/auth, gl/lore, gl/chapters + helpers) ; `gl/games`/`forum`/2e passage `visit` examinés et écartés avec motif. **Couche service — 1er découpage en sous-routeur (lot 16, 2026-06-13)** : sous-domaine **spell-casts** de `gl/games.js` sorti vers `routes/gl/games/spell-casts.js` (`express.Router()` monté sans préfixe via `router.use(...)` en dernière position, chemins exposés identiques au caractère près — vérifié par script avant/après sur les 36 routes ; `handleSpellCastRoute`/`requireSpellCastPermission`/`parseId` recopiés localement, imports uniquement depuis `lib/`, **zéro import circulaire** `games.js`↔sous-routeur) → **`gl/games.js` 2024→1881 l**. Modèle réutilisable pour découper `visit.js`/`tasks.js`/le reste de `gl/games.js` par sous-domaines. **Lot 17 (2026-06-13) — modèle étendu** : **`visit.js` 1997→1316 l (−681)** — domaine **mascotte complet** (mascot-packs + mascot-assets + mascot-sprite-library, 13 routes) sorti vers `routes/visit/mascot.js` (les 9 helpers fs/pack exclusifs partent en entier → 0 duplication ; `nowIso`/`resolveVisitMapId`/`mapExists` courts recopiés ; 32 routes identiques prouvées, 0 import circulaire) ; **`tasks.js` 1810→1763 l (−47)** — sous-domaine **logs** (`/:id/logs*`, 3 routes) sorti vers `routes/tasks/logs.js` (`respondInternalError` local non requis ; 18 routes identiques prouvées). **Lot 18 (2026-06-14)** : **`tasks.js` 1763→1724 l** — sous-domaine **import** (`/import/template` + `/import`, 0 helper DB privé) → `routes/tasks/import.js` (les **assignations** examinées et écartées : trop couplées à `getTaskWithAssignments` & ~10 helpers DB privés partagés) ; **`gl/games.js` 1881→1707 l** — sous-domaine **qcm** (`POST /games/:id/qcm/answer` + helpers `resolveQcmAnswerContext`/`staffCanAnswerQcmForTeam`) → `routes/gl/games/qcm.js` (`getPlayerGameMembership` recopié localement byte-identique, conservé aussi dans games.js pour 6 autres routes ; 36 routes identiques). **Régression évitée (lot 17→18)** : un déplacement byte-identique **casse les chemins relatifs au fichier** — `listPublicMascotStaticAssets` (`path.join(__dirname,'..','public')`) a échoué en CI une fois sortie vers `routes/visit/mascot.js` (corrigé en `'..','..'`). Désormais : tout sous-routeur extrait est audité pour `__dirname`/`require` relatifs/`fs` (un niveau plus profond → `../` devient `../../`, deux niveaux → `../../../`). **Lot 19 (2026-06-14)** : **`visit.js` 1316→959 l** — sous-domaine **sync** (`/sync`, `/sync/options`, `/rebuild-from-map`) → `routes/visit/sync.js` (32 routes identiques ; `nowIso`/`resolveVisitMapId`/`mapExists` recopiés ; aucun chemin `__dirname`/fs) ; **`gl/games.js` 1707→1534 l** — **enabler** : les 5 helpers runtime à I/O partagés (`readGameState`, `getPlayerGameMembership`, `ensurePlayerInGameClass`, `recordVitalityChangeEvent`, `resolveRosterError`) extraits par déplacement pur byte-identique vers **`lib/gl/gamesRuntime.js`** (6 tests no-DB ; débloque le futur découpage des sous-domaines roster/vitalité/markers qui dépendaient de ces helpers). Reste : sortir les sous-domaines restants de `gl/games.js` (roster/vitalité/markers, désormais débloqués via `lib/gl/gamesRuntime`), `visit.js` (media/tutorials), `tasks.js` (assignations via extraction préalable de `getTaskWithAssignments` vers lib/) | Élevé/Faible | wip |
...
Vitest vert (1 674 tests UI, +5), lint sans erreur, require-smoke backend OK. Union complète `gl/games*` (36) inchangée.

- **O10** (`wip`) — `routes/gl/games.js` : sous-domaine **actions** (`POST /games/:id/actions`,
  `POST /games/:id/actions/:actionId/resolve`) extrait en sous-routeur `routes/gl/games/actions.js`.
  **Union complète inchangée (36)**.
- **O6** (`wip`) — `src/components/map-views.jsx` : `MapViewMarkerBubble` (bulle de repère sur la carte —
  bouton positionné %, épingle emoji/point, pastilles état-tâche & tutoriel, étiquette ; logique métier
  conservée dans `MapView` via props) extrait + test UI (5 cas). 2ᵉ extraction de ce fichier après `MapViewMascotOverlay`.
- **O5** (`wip`) — `src/App.jsx` : hook `useAuthMeHydration` extrait (hydratation session via `/api/auth/me`
  au montage + `mergeAuthMeResponse`). Aucun état déplacé, zéro orphelin (l'appel `/api/auth/me` de `fetchAll` conservé).
