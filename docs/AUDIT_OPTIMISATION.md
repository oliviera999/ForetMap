...
| O8 | Moyenne | Maint | ~338 try/catch dispersés ; `respondInternalError` redéfini en doublon | **Infra livrée** : `lib/asyncHandler.js` (catch sync+async → `next(err)` → handler central `server.js`) + test `tests/async-handler.test.js`. Rollout **incrémental** par route (préserver statut + corps d'erreur existants). Adopté sur `audit`/`zones`/`map`/`maps`/`observations`/`context-comments`/`stats`, puis **`media-library`** (les 4 routes : try/catch + `respondInternalError` supprimés ; contrat 4xx `.status`/`.message` + 500 « Erreur serveur » reproduit par le handler central) | Moyen | wip |
...
| O10 | Moyenne | Perf | Routes obèses (visit/tasks/games 2000+ l.) ; N+1 d'écriture (boucles INSERT) | Couche service par domaine ; INSERT multi-valeurs. **N+1 d'écriture convertis en INSERT multi-valeurs** (idiome `replaceTaskJoinRows` de `tasks.js`) : `groups.js` (`PUT /:id/members` — membres/managers/scopes carte+projet, + résolution des `user_type` en **1 requête** au lieu d'un SELECT par membre) ; `rbac.js` (duplication de profil — copie des `role_permissions` + résolution du catalogue en **1 requête**). Équivalence relue (2 agents, 0 anomalie) ; couverts par `groups.test.js`/`rbac.test.js`. Reste : `tasks.js` (task_assignments), `gl/admin.js` (gl_players), imports `students`/`plants` (2 étapes) ; couche service par domaine | Élevé/Faible | wip |
