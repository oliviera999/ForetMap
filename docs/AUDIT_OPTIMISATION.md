...
1. **`App.jsx` God component + prop-drilling massif** (37 `useState`, blocs de props dupliqués ×4)
   + refetch global toutes les 60 s. **Largement adressé** : O5 (~125 passes de props → contexts)
   et O2 (`TaskTileCard` `React.memo` + `taskTileProps` `useMemo`, tuiles stables au tick). Touche les 3 axes.
...
4. **RBAC recalculé depuis la DB à chaque requête authentifiée** (3-5 requêtes SQL).
   **Résolu (O3)** : `getRolePermissions(roleId)` caché (`lib/rbacCache.js`), invalidé au niveau
   couche données (compteur de version bumpé par `execute()` sur toute écriture RBAC).
...
| O2 | Haute | Perf | `taskTileProps` recréé chaque render + `TaskTileCard` non mémoïsé ⇒ re-render de toutes les tuiles par tick | **Fait** : `TaskTileCard = React.memo(...)` (+ test `tests-ui/components/TaskTileCard.test.jsx`) ; les ~11 handlers de `TasksView` (`assign`/`unassign`/`setTaskStatus`/`deleteTask`/`withLoad`/drag…) sont en `useCallback`, et `taskTileProps` (`tasks-views.jsx:972`) est un `useMemo` dont le tableau de deps liste toutes les valeurs volatiles et **omet correctement les setters `useState` (identité stable garantie par React)** → le memo est *effectif* : en régime stable (tick de refetch sans changement de `tasks`/`plants`), `taskTileProps` garde son identité et les tuiles mémoïsées ne se re-rendent pas | Faible→Moyen | done |
| O3 | Haute | Perf | RBAC : 3-5 requêtes DB par requête authentifiée, non caché | **Tenté puis reverté** : un cache TTL avec invalidation par hook (`setPrimaryRole`/routes rbac) s'est avéré à invalidation **incomplète** — des chemins mutent `roles`/`user_roles` en SQL direct (dédup `rbac.js`, tests) → permissions périmées (a cassé `api.test.js`). Re-tenter avec un **compteur de version RBAC global** inclus dans la clé de cache (incrémenté de façon centralisée à toute écriture des tables RBAC) **ou** un cache **request-scoped**. Sécurité-critique : à ne pas livrer sans preuve d'invalidation complète. **Maintenu `differe` (analyse 2026-06-10)** : (a) le cache **request-scoped** n'apporte ~rien — `hydrateAuthFromTokenClaims` (donc `buildAuthzPayload`) ne s'exécute **qu'une fois par requête** (un seul guard d'auth par route), pas de duplication intra-requête à collapser ; (b) le seul sous-ensemble **sûr** à cacher cross-request est `getRolePermissions(roleId)` (clé = `roleId`, partagé entre users, **4 chemins d'écriture seulement**, tous dans `rbac.js`/`lib/rbac.js`) — `getPrimaryRoleForUser` (user→rôle) garde de multiples writers (cause du revert) ; mais **11+ tests écrivent `role_permissions` en SQL direct puis lisent immédiatement** → un cache les casserait sauf bypass `NODE_ENV==='test'` (qui supprime toute vérification). **Fait (validé DB locale MariaDB)** : cache de `getRolePermissions(roleId)` dans `lib/rbacCache.js` (clé = roleId, partagé entre users) ; **invalidation au niveau couche données** — `database.js#execute`/`tx.execute` bumpent un **compteur de version global** dès qu'un SQL d'écriture touche une table RBAC (`role_permissions`/`user_roles`/`role_scopes`/`roles`/`permissions`). Cela résout la cause du revert (les écritures SQL directes des tests/dédup passent toutes par `execute()` → bump), sans bypass `NODE_ENV`. `getPrimaryRoleForUser` (user→rôle) **non caché** (multiples writers). Sur-invalidation = sans danger (rate le cache, jamais de périmé) ; anti-périmé si écriture concurrente pendant la lecture. **Preuve** : suite backend complète `1126→1131` (5 tests unitaires no-DB `tests/rbac-cache.test.js`) **0 échec** avec `--test-concurrency=1`, incl. `rbac.test.js`/`api.test.js`/`rbac-progression.test.js` (flux grant→read en SQL direct) | Moyen | done |
...
| O10 | Moyenne | Perf | Routes obèses (visit/tasks/games 2000+ l.) ; N+1 d'écriture (boucles INSERT) | Couche service par domaine ; INSERT multi-valeurs. **N+1 d'écriture convertis en INSERT multi-valeurs** (idiome `replaceTaskJoinRows` de `tasks.js`) : `groups.js` (`PUT /:id/members` — membres/managers/scopes carte+projet, + résolution des `user_type` en **1 requête** au lieu d'un SELECT par membre) ; `rbac.js` (duplication de profil — copie des `role_permissions` + résolution du catalogue en **1 requête**) ; `tasks.js` (`POST /:id/assign-group` — affectations en **1 requête**, sémantique `assigned`/`skipped`/créneaux préservée, vérifiée par simulation 8 cas). enfin **import plants** stratégie `replace_all` (`routes/plants.js` — INSERT multi-valeurs **par lots de 200**, 33 colonnes × 2000 lignes max dépasseraient la limite de placeholders d'une requête préparée ; aucune dépendance à `insertId` ; équivalence ordre+compte vérifiée par simulation). Équivalence relue (2 agents, 0 anomalie) ; couverts par `groups.test.js`/`rbac.test.js`/`tasks*`/`plants-import.test.js`. **Écartés volontairement** (récupération d'erreur **par ligne** `ER_DUP_ENTRY`→report+`continue`, incompatible avec un INSERT tout-ou-rien) : `gl/admin.js` (gl_players), import **students**, et branches `upsert_name`/`insert_only` de l'import plants (dépendance à `insertId` pour la dédup intra-lot). **Couche service (début)** : logique pure de `routes/tasks.js` extraite vers `lib/tasks/` — `taskFieldLevels.js` (niveaux danger/difficulté/importance : parsing client + mapping réponse, 5 tests) et `taskLivingBeings.js` (normalisation/sérialisation des êtres vivants, 4 tests). `tasks.js` 2119→2051 l. Puis `routes/visit.js` → `lib/visit/visitSerializers.js` (sérialiseurs purs : sanitizers de cible, médias de visite, photos de carte `serializeMapLeadPhoto`/`ExtraPhotos`, 7 tests) ; `visit.js` 2260→2202 l. Puis `routes/gl/games.js` → `lib/gl/glGamesHelpers.js` (purs : `parseId`/`parsePct`/`resolveRosterError`/`staffCanAnswerQcmForTeam` avec `hasGlPermission` injecté, 4 tests) ; `gl/games.js` 1997→1971 l. Équivalence validée DB locale (suite backend 0 échec ; `tasks*`/`visit*`/`gl-game*`/`gl-spell*` = 29+81+62). Reste : poursuivre la couche service (logique métier DB-bound des 3 routes obèses) | Élevé/Faible | wip |
| O11 | Moyenne | Perf bundle | Lazy ineffectif (`foretmap-views`/`stats-views`) ; markdown eager ; GL sans lazy ; sourcemap prod | Lazy renderers mascotte + GL ; **`foretmap-views`/`stats-views` rendus purement lazy** (`Toast`←shared, `PlantCatalogPreviewModal`/`StudentStats`/`StudentProfileEditor` en `lazy`) → `main` **431→315 Ko** (gzip 111→81) ; `sourcemap: false`. **Markdown rendu paresseux (fait)** : `src/utils/markdown.js` scindé → helpers purs (`applyMarkdown*`) restent légers, le moteur lourd (`marked`+`DOMPurify`+hooks, `renderMarkdownToSafeHtml`/`sanitizeRichHtml`) déplacé dans `src/utils/markdownRender.js`. `MarkdownContent` (lecture) et `RichTextEditor` (édition, via singleton + `renderReady`) le chargent en **dynamic import** ; importeurs GL en static (chunk `gl` séparé). Résultat : le chunk `markdown-*.js` (66 Ko) **ne figure plus dans le `modulepreload` eager** du bundle principal (chargé à la demande au 1er rendu markdown) | Moyen | done |
...
- **O2 résolu** : `TaskTileCard` est `React.memo` ; `taskTileProps` est un `useMemo` (deps
  volatiles listées, setters `useState` stables omis) spreadé dans ~16 `.map()` → les tuiles
  ne se re-rendent plus au tick de refetch tant que `tasks`/`plants` ne changent pas.
...
- `markdown` (66 KB, marked+DOMPurify) : **O11 résolu** — moteur lourd isolé dans
  `markdownRender.js`, chargé en `import()` dynamique par `MarkdownContent`/`RichTextEditor`
  ⇒ chunk retiré du `modulepreload` eager (chargé à la demande).
...
- RBAC : `buildAuthzPayload` (`lib/rbac.js:605`) appelé via `hydrateAuthFromTokenClaims`
  (`middleware/requireTeacher.js`) sur chaque requête authentifiée → 3-5 SELECT. **O3 : la lecture
  `getRolePermissions(roleId)` est désormais servie depuis `lib/rbacCache.js` (invalidation par
  version au niveau `execute()`), évitant la requête `role_permissions` à chaque requête.**
