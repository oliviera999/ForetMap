...
| B4 | Incoherence doc/code taches | Moyenne | [docs/API.md](c:/projets_code/ForetMap/docs/API.md), [routes/tasks.js](c:/projets_code/ForetMap/routes/tasks.js), [src/components/tasks-views.jsx](c:/projets_code/ForetMap/src/components/tasks-views.jsx) | Doc annonce `POST /api/tasks/:id/unassign` non-prof, mais route exige `requireTeacher`; le frontend eleve appelle cette route. | Aligner soit le code soit la doc et l'UI (decision produit necessaire). |
...
2. **Coherence fonctionnelle**: `B4` (unassign eleve), `B5`.
