...
| D   | **Arbitrages tranchés** (3 pôles + sous-nav ; icônes SVG lucide pour le chrome ; remplacement complet des dialogues natifs ; accordéons admin). **D-1 traité** : les 82 `window.confirm/alert/prompt` (recomptés — l'audit disait 54) migrés sur `AppDialogsProvider` (`useAppDialogs`), alerts → toasts, garde-fou `tests/native-dialogs-guard.test.js` ; vocabulaire unifié (Enregistrer, ↩ Annuler, 🚪 Déconnexion, `×`→`✕`, feuille de filtres carte alignée). **D-2 traité** : le chrome (barres d'outils, modales, en-têtes, boutons, badges d'état) passe sur des icônes SVG lucide via le point d'import unique `src/shared/icons.jsx` (fabrique `ui()`, `aria-hidden`, chunk Vite dédié) — les emojis restent réservés au contenu métier (emoji de plante/zone, palette, organismes du réseau trophique, mascottes). **D-3 traité** : accordéons `AdminSection` (persistance `localStorage`, ouverture forcée pendant la recherche) sur Paramètres admin, sections repliables de la fiche plante, bandeaux inline montés dans le `FloatingDock`, breakpoints canoniques documentés, projet Playwright `mobile-chromium` (390×844) + premier scénario carte/nav basse/feuille de filtres. **D-4 traité** : navigation prof en 3 pôles (Contenus/Suivi/Administration) avec sous-onglets et badge « à valider » — pôle actif dérivé de l'onglet, rien de nouveau persisté. |
