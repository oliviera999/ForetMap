# Extensions pré-saisie espèces (Trefle, OpenAI)
...
- **État actuel** : socle prêt (aucun appel HTTP tant que l’implémentation n’est pas complétée) ; retour `null` → pas d’impact sur la fusion.
...
- **Comportement** : requête `POST /v1/chat/completions` avec `response_format: json_object`, prompt limité au contexte (requête utilisateur, nom scientifique seed, extrait Wikipedia FR tronqué). Timeout court (plafond **8 s**, borné par le budget global de pré-saisie).
