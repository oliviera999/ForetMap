...
- **Contexte court** : si le résumé agrégé fait moins d’environ **420** caractères (ex. pré-saisie avec **OpenAI seul** ou peu de sources), le prompt bascule en mode **indicatif pédagogique** pour limiter les réponses JSON vides sur des noms courants (« aubergine », « basilic »…) : formulations générales autorisées, **sans** température/pH chiffrés inventés ; avertissement supplémentaire dans `warnings`.
...
- **Variables** :
  ...
  - optionnel **`PLANTNET_LANG`** (défaut **`fr`**, passé à la liste **`GET …/species`** pour les noms vernaculaires ; **non** envoyé sur **`species/align`** car l’API v2 le refuse) ;
...
- **Comportement** :
  1. `GET …/v2/projects/{project}/species/align` avec **`synonyms=true`**, **`authorship`** : nom accepté, **famille** → **`group_3`**, **genre** → **`group_4`**, **`scientific_name`** si le nom accepté diffère du libellé aligné ; prise en charge des payloads **`bestMatch`** hérités.
