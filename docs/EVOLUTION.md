...
| **Saturation HTTP / latence** liée au **long-polling** (nombreuses connexions simultanées) | **Option C** : hébergement ou frontal avec **WebSocket** correctement terminé ; réactiver via **`FORETMAP_SOCKETIO_ALLOW_WEBSOCKET=1`** (clients : `realtime.allow_websocket` dans settings/public et config GL) **après** validation proxy — défaut **off** (polling).                                                                                                                                                                                                                                                      |
| **Catalogue plantes très volumineux** (`GET /api/plants`)                                  | **Reporté (D4-A)** : garder la liste complète en un GET tant que le catalogue reste de l’ordre de la centaine (~150–200 fiches) ou qu’un signal perf n’apparaît pas ; projection liste + détail au clic seulement au-delà.                                                                                                                                                                                                                                                                                                   |
