...
| 10  | **Décisions structurantes ultérieures**, chacune sur son audit : identités unifiées (`users` couvrant GL, canonisation d'un acteur `gl_player`) — **livré** en septembre 2026, voir `AUDIT_COMPTES_2026-09.md` ; chaîne OAuth partagée (`googleOAuthShared` promu, tests avant) ; graphe de chemins et vrai routage (`PctPathsLayer`) ; étages ; mode sombre (tokens prêts au lot 3, palettes à dessiner) ; scission `src/foret/`.                                                                                                                                                                                                                                                  | tous                                               | 1, 3, 6   | à cadrer | élevé  |
