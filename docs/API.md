...
- Comportement:
  ...
  - avec un filtre **`sources`** restreint (sans GBIF/Wikidata/Wikipedia), **Pl@ntNet** et le **contexte OpenAI** s’appuient tout de même sur **`q`**, **`hint_name`** et **`hint_scientific`** : l’alignement Pl@ntNet accepte un **nom courant** ; le corps envoyé à OpenAI inclut le texte de recherche et les indices formulaire pour limiter les réponses vides lorsque les autres sources ne tournent pas ;
