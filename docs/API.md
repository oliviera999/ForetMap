...
| POST | `/api/gl/auth/admin/impersonate` | `{ userType: 'gl_player', userId }` | **Admin GL strict** (`userType: gl_admin`, `roleSlug: gl_admin`) : émet un JWT joueur avec `auth.impersonating=true` et `auth.impersonatedBy`. Refus si prise de contrôle déjà active, cible inactive/introuvable, ou token MJ. |
| POST | `/api/gl/auth/admin/impersonate/stop` | — | Termine la prise de contrôle GL et restaure la session admin GL d’origine. Requiert un JWT en mode impersonation. |
