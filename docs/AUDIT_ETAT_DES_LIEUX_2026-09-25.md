...
| 3   | `groups.curriculum_niveau` (mig. 290) | l'ouverture libre d'une fiche sans question active est **gardée** ; exposée par l'API (`inactive_gating_count`, `without_active_gating_count`) et ignorée par le générateur de questions — **l'écran de couverture ne l'affiche pas encore**                                                                                    | étape déduite ; remonte `parent_group_id`, **sans filtre `is_active`** sur les parents (`:214-216`)                                                    |
| 4   | `maps.pedago_level`                   | cible : une seule échelle côté apprenant (`cycle3` … `es_terminale`, plus `universite`), collège / lycée / université en regroupements d'affichage, la séance impose son niveau. **Fait** : résolveur serveur unique (`lib/pedago/learnerLevel.js`), filtre du quiz imposé par la séance. **Reste** : stockage unique (piste C) | entre dans le calcul du front                                                                                                                          |
...
| 17  | Quelle utilité ?                         | explication donnée ; **rien d'installé ni d'écrit** : ni `@vitest/coverage-v8`, ni contrôle des cycles d'import (script maison prévu, sans dépendance)                                                    |
