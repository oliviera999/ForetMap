...
**Bibliothèque sprites** : `framesBase` peut aussi être `/api/visit/mascot-sprite-library/assets/` (PNG partagés entre packs — une seule bibliothèque, sans notion de carte, voir **`docs/API.md`**). La forme historique `/api/visit/mascot-sprite-library/{mapId}/assets/` reste acceptée et servie.
...
- **Import souple — auto-déclaration des comportements personnalisés** : à l'import (`…/import` et `…/import/analyze`, **uniquement**), toute clé `stateFrames.<état>` **non canonique** et **non déclarée** dans `customStates` est **auto-déclarée** (option `autoDeclareCustomStates`, cf. `parseMascotPack`), à condition de respecter le format des clés (`^[a-z0-9]+(?:[-_][a-z0-9]+)*$`, ≤ 40 car.). Un libellé prof est dérivé de la clé (`idle_back` → « Idle back »). Cela permet d'importer **l'intégralité** d'un pack dont les états personnalisés n'ont pas été déclarés à la source (forme `stateFrames`), comme le fait déjà la forme `states[]`. Les états auto-déclarés sont remontés dans `autoDeclaredStates` et `warnings` (analyse + import). **Le studio (`POST`/`PUT` packs) garde la validation stricte** : une clé inconnue y reste une erreur (faute de frappe révélée).
