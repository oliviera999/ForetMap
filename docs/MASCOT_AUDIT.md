...
- **A** (reste à faire) : la carte éditeur ne charge **aucun** pack serveur (aucun fetch dans
  `map-views.jsx`). Pour un rendu des packs importés en vue d'édition : fetch des packs (ex.
  `GET /api/visit/:id` pour les publiés) → `buildVisitMascotCatalogExtrasFromContent` → passer
  `extraCatalogEntries` à `useMapViewMascot` (déjà supporté) **et** à `MapViewMascotOverlay` /
  `VisitMapMascotRenderer`. À valider en UI (non couvert ici).
