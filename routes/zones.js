...
router.put('/:id', requirePermission('zones.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
...
router.get('/:id/photos/:pid/data', requireAuth, asyncHandler(async (req, res) => {
...
router.delete('/:id/photos/:pid', requirePermission('zones.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
...
router.post('/', requirePermission('zones.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
...
router.delete('/:id', requirePermission('zones.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
  const zone = await queryOne('SELECT * FROM zones WHERE id = ?', [req.params.id]);
  if (!zone) return res.status(404).json({ error: 'Zone introuvable' });
