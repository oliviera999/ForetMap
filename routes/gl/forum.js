...
router.post('/threads/:id/posts', async (req, res) => {
  ...
  if (!thread || Number(thread.is_deleted)) return res.status(404).json({ error: 'Sujet introuvable' });
  ...
  const created = await queryOne('SELECT * FROM gl_forum_posts WHERE id = ? LIMIT 1', [result.insertId]);
  return res.status(201).json(created);
});
...
router.patch('/threads/:id/lock', async (req, res) => {
  ...
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'Identifiant invalide' });
  await execute('UPDATE gl_forum_threads SET is_locked = ?, updated_at = NOW() WHERE id = ?', [locked ? 1 : 0, id]);
...
router.delete('/posts/:id', async (req, res) => {
  ...
  const owns = post.author_user_type === req.glAuth.userType
    && String(post.author_user_id) === String(req.glAuth.userId);
  if (!owns && !canModerate(req.glAuth)) return res.status(403).json({ error: 'Permission insuffisante' });
