...
const spellCodeParamsSchema = z.unknown().superRefine((p, ctx) => {
  const code = String((p == null ? '' : p.code) || '')
    .trim()
    .toUpperCase();
...
router.get(
  '/spells',
  requireGlPermission('gl.read'),
  validate({ query: spellCodesQuerySchema }),
  asyncHandler(async (req, res) => {
    const spellCodes = parseSpellCodesFromQuery(req.query);
    if (spellCodes.length === 0) {
      return res
        .status(400)
        .json({ error: 'spellCodes requis (liste de codes séparés par des virgules)' });
    }
...
router.get(
  '/spells/:code',
  requireGlPermission('gl.read'),
  validate({ params: spellCodeParamsSchema }),
  asyncHandler(async (req, res) => {
...
router.get(
  '/admin/spells',
  requireGlPermission('gl.content.manage'),
  validate({ query: categorySlugQuerySchema }),
  asyncHandler(async (req, res) => {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const result = await upsertSpellRow({ queryAll, execute }, req.body || {}, {
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="foretmap-gl-modele-sortileges.xlsx"',
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="foretmap-gl-export-sortileges.xlsx"',
    );
    return res.send(buffer);
  }),
);
