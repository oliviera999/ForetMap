...
router.get(
  '/',
  requireGlAuth,
  validate({ query: glTutorialsListQuerySchema }),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!title || title.length < MIN_TITLE || title.length > MAX_TITLE) {
      return res
        .status(400)
        .json({ error: `Titre invalide (${MIN_TITLE}-${MAX_TITLE} caractères)` });
    }
    ...
    const created = await queryOne('SELECT * FROM gl_tutorials WHERE id = ? LIMIT 1', [
      result.insertId,
    ]);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const title = normalizeOptionalString(req.body?.title);
    const bodyMarkdown =
      req.body?.bodyMarkdown == null ? null : normalizeOptionalString(req.body.bodyMarkdown);
    const chapterId = req.body?.chapterId == null ? null : Number(req.body.chapterId);
    const markerId = req.body?.markerId == null ? null : Number(req.body.markerId);
