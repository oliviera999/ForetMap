...
router.get(
  '/species',
  requireGlPermission('gl.read'),
  validate({ query: biomeSlugQuerySchema }),
  asyncHandler(async (req, res) => {
    ...
    const biome = await queryOne('SELECT slug, nom FROM gl_biomes WHERE slug = ? LIMIT 1', [
      biomeSlug,
    ]);
...
function filterSpeciesAdminList(rows, { type, q }) {
  ...
  if (q) {
    ...
    items = items.filter((row) => {
      const hay =
        `${row.nom_commun} ${row.nom_scientifique || ''} ${row.species_code}`.toLowerCase();
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const result = await upsertSpeciesRow({ queryAll, execute }, req.body || {}, {
...
router.get(
  ...
  INNER JOIN gl_biomes b ON b.slug = s.biome_slug
    ...
    const total = await queryOne(`SELECT COUNT(*) AS total FROM gl_species WHERE statut = 'actif'`);
    return res.json({
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="foretmap-gl-modele-biocenose.xlsx"',
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename="foretmap-gl-export-biocenose.xlsx"',
    );
    return res.send(buffer);
  }),
);
...
router.get(
  '/admin/species/:code',
  requireGlPermission('gl.content.manage'),
  validate({ params: speciesCodeParamsSchema }),
  asyncHandler(async (req, res) => {
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    await execute('UPDATE gl_species SET statut = ?, updated_at = NOW() WHERE species_code = ?', [
      statut,
      code,
    ]);
    const species = await loadAdminSpeciesDetail(code);
