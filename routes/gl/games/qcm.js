...
router.post('/games/:id/qcm/answer', requireGlAuth, async (req, res) => {
  ...
  try {
    await withTransaction(async (tx) => {
      const consume = await consumePresentationJti(tx, {
        jti: verification.jti,
        gameId,
        teamId: teamIdForGame,
        questionCode,
      });
      if (consume === 'already_used') {
    ...
    if (err?.status === 409) {
      return res.status(409).json({ error: err.message || 'Présentation déjà utilisée' });
    }
    throw err;
  }

  // Après consommation réussie uniquement — un rejeu ne doit pas polluer le gating.
