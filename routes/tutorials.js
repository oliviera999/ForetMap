...
router.get('/me/read-ids', requireAuth, asyncHandler(async (req, res) => {
 ...
 */
router.post('/:id/acknowledge-read', requireAuth, asyncHandler(async (req, res) => {
 ...
 */
router.get('/:id/linked-tasks', asyncHandler(async (req, res) => {
  const tid = Number(req.params.id);
  if (!Number.isFinite(tid) || tid <= 0) {
    return res.status(400).json({ error: 'Identifiant de tutoriel invalide' });
  }
...
router.post('/', requirePermission('tutorials.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
...
router.put('/:id', requirePermission('tutorials.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
  if (!canManageTutorials(req)) return res.status(403).json({ error: 'Permission insuffisante' });
...
router.delete('/:id', requirePermission('tutorials.manage', { needsElevation: true }), asyncHandler(async (req, res) => {
  if (!canManageTutorials(req)) return res.status(403).json({ error: 'Permission insuffisante' });
...
router.get('/:id/download/html', asyncHandler(async (req, res) => {
